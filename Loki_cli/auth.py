...
is persisted in ~/.loki/auth.json with cross-process file locking.
...
from loki_cli.config import get_loki_home, get_config_path, read_raw_config
...
DEFAULT_NOUS_CLIENT_ID = "loki-cli"
...
SPOTIFY_DOCS_URL = "https://loki-agent.nousresearch.com/docs/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

OAUTH_OVER_SSH_DOCS_URL = "https://loki-agent.nousresearch.com/docs/guides/oauth-over-ssh"
...
def get_anthropic_key() -> str:
    ...
    ``~/.loki/.env`` so a deliberate key rotation isn't shadowed by a stale
...
def _resolve_api_key_provider_secret(
    ...
    for env_var in pconfig.api_key_env_vars:
        # Prefer ~/.loki/.env over os.environ so a deliberate key rotation
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    instead of prompting the operator to run ``loki auth``.
    """
    return (
...
def _format_nous_entitlement_auth_error(error: AuthError) -> str:
    try:
        from loki_cli.nous_account import (
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("LOKI_OAUTH_TRACE", "").strip().lower()
...
def _auth_file_path() -> Path:
    ...
    # Seat belt: if pytest is running and LOKI_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch LOKI_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".loki" / "auth.json").resolve(strict=False)
        try:
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom LOKI_HOME that is not a profile).
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.loki/auth.json`` even when LOKI_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_loki_root()`` for a profile-shaped LOKI_HOME can
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    global scope. Once the user runs ``loki auth login <provider>`` inside
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    ...
    Used by ``loki auth add`` OAuth paths that create credential-pool
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``loki auth add <provider>`` inside the profile, profile entries
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in Loki.
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `loki logout`.
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'loki model' for available providers, or run 'loki doctor' to diagnose config issues."
    ...
    # Auto-detect an OpenRouter credential added via `loki auth add openrouter`
    ...
    # `loki auth list` showing the credential while requests go out with no
    ...
    for pid, pconfig in PROVIDER_REGISTRY.items():
        ...
        for env_var in pconfig.api_key_env_vars:
            if has_usable_secret(os.getenv(env_var, "")):
                ...
                # OAuth but has a stale key in ~/.loki/.env isn't silently
...
def _nous_portal_env_override() -> Optional[str]:
    ...
    Mirrors ``_nous_inference_env_override()``: ``LOKI_PORTAL_BASE_URL`` /
    ``NOUS_PORTAL_BASE_URL`` are the documented dev/staging escape hatch for
    pointing Loki at a non-production Nous Portal (e.g. a hosted agent
    provisioned on nous-account-service's `staging` environment, which stamps
    ``LOKI_PORTAL_BASE_URL=https://portal.staging-nousresearch.com`` into
    ...
    return _optional_base_url(
        os.getenv("LOKI_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL")
    )
...
def _assert_nous_inference_jwt_usable(
    ...
    *,
    ...
    raise AuthError(
        ...
        f"({reason}). Re-authenticate with: loki auth add nous",
        provider="nous",
        code=reason,
...
def get_qwen_auth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        # and `loki model` walks users into a broken Qwen setup flow.
...
# =============================================================================
# Spotify auth — PKCE tokens stored in ~/.loki/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("LOKI_SPOTIFY_CLIENT_ID"),
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("LOKI_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("LOKI_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("LOKI_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `loki auth spotify` first.",
                provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.loki/.env, and return it.
    ...
    print("       App name:     anything (e.g. loki-agent)")
    ...
    # Persist so subsequent `loki auth spotify` runs skip the wizard.
    ...
    print()
    print("Saved LOKI_SPOTIFY_CLIENT_ID to ~/.loki/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "LOKI_SPOTIFY_CLIENT_ID is required".
    ...
    print()
    print("Open this URL to authorize Loki:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"Loki is waiting for the OAuth callback on {redirect_uri}")
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.loki/auth.json (not ~/.codex/)
#
# Loki maintains its own Codex OAuth session separate from the Codex CLI
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `loki auth` to authenticate.",
...
def _sync_codex_pool_entries(
    ...
    * ``device_code`` — the singleton-seeded entry written by the device-code
      OAuth flow when the user logged in via ``loki setup`` / the model
      picker.  Always synced with the fresh tokens.
    * ``manual:device_code`` — entries created by ``loki auth add openai-codex``
      ...
      ``loki auth add openai-codex`` and must not be overwritten by a
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    ...
    with _auth_store_lock():
        ...
        # ``loki auth add openai-codex`` created (which must not be
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    if response.status_code == 429:
        ...
        # "run loki auth" prompt (see issue #32790).
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Loki auth store automatically.
    """
    try:
        ...
        # Self-heal cross-store refresh_token rotation. Loki keeps its OWN
        ...
        # the Codex CLI (or another Loki process) rotates the shared token,
...
def resolve_codex_runtime_credentials(
    *,
    ...
    if data is None:
        ...
        if pool_token:
            base_url = (
                os.getenv("LOKI_CODEX_BASE_URL", "").strip().rstrip("/")
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.loki/auth.json
# =============================================================================
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "xAI OAuth state is missing tokens. Re-authenticate with `loki model`.",
            provider="xai-oauth",
    ...
    if not access_token:
        raise AuthError(
            "xAI OAuth state is missing access_token. Re-authenticate with `loki model`.",
            provider="xai-oauth",
    ...
    if not refresh_token:
        raise AuthError(
            "xAI OAuth state is missing refresh_token. Re-authenticate with `loki model`.",
            provider="xai-oauth",
...
def _write_through_xai_oauth_to_global_root(state: Dict[str, Any]) -> None:
    ...
    # ~/.loki/auth.json even when LOKI_HOME points at a profile path
...
def _xai_proactive_refresh_skew_seconds(access_token: str) -> int:
    ...
    resolution (chat turn, Imagine tool call, ``loki auth status``, …),
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.loki/auth.json``. A single MITM during initial
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    the user's SuperGrok subscription. ``XAI_BASE_URL`` / ``LOKI_XAI_BASE_URL``
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "xAI OAuth is missing refresh_token. Re-authenticate with `loki model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
    ...
    # written by an older Loki (or hand-edited) may carry a non-xAI
    ...
    # with a clear error so the user can re-run `loki model` to refetch.
    ...
    if response.status_code != 200:
        ...
        # on the allowlist for API access).  Re-running ``loki model``
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    base_url = _xai_validate_inference_base_url(
        os.getenv("LOKI_XAI_BASE_URL", "").strip().rstrip("/")
...
# so a new `loki --profile <name> auth add nous --type oauth` can one-tap
# import instead of running the full device-code flow every time.
#
# File lives at ${LOKI_SHARED_AUTH_DIR}/nous_auth.json, defaulting to
# ``<loki-root>/shared/nous_auth.json`` where ``<loki-root>`` is what
# ``get_default_loki_root()`` returns — ``~/.loki`` on Linux/macOS,
# ``%LOCALAPPDATA%\loki`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's LOKI_HOME so named profiles (which
# typically live under ``<loki-root>/profiles/<name>/``) all see the
# same file.
#
...
def _nous_shared_auth_dir() -> Path:
    ...
    Honors ``LOKI_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<loki-root>/shared/``, where ``<loki-root>`` is what
    :func:`loki_constants.get_default_loki_root` returns — so
    Linux/macOS classic installs land at ``~/.loki/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\loki\\shared\\``, and
    Docker / custom ``LOKI_HOME`` deployments at
    ``<LOKI_HOME>/shared/``. Sits outside any named profile so all
...
def _nous_shared_store_path() -> Path:
    ...
    # real user's Loki root, refuse rather than silently corrupt cross-profile
    # state. Tests must set LOKI_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from loki_constants import get_default_loki_root
        real_home_shared = (
            get_default_loki_root() / "shared" / NOUS_SHARED_STORE_FILENAME
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    try:
        ...
        # No LOKI_HOME yet (pre-setup): fall through without locking.
        yield
        return
...
def _refresh_access_token(
    *,
    ...
    # called POST /api/oauth/token with Loki's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, Loki's next refresh uses it, and the whole
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "Nous refresh tokens are single-use — only Loki may call the "
            "refresh endpoint. For health checks, use `loki auth status` "
            "instead.\n"
            "Re-authenticate with: loki auth add nous"
        )
        relogin = True
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Loki models — they're not reliable for agentic tool-calling
            if "loki" in mid.lower():
                continue
...
def resolve_nous_access_token(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Loki is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )

        # LOKI_PORTAL_BASE_URL / NOUS_PORTAL_BASE_URL is the trusted
...
def persist_nous_credentials(
    ...
    *,
    ...
    Historically ``loki auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``loki auth add nous --label <name>``).  It gets embedded in the
    ...
    # these credentials via `loki auth add nous --type oauth`. Best-
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("Loki is not logged into Nous Portal.",
        ...
        portal_base_url = (
            _optional_base_url(state.get("portal_base_url"))
            or os.getenv("LOKI_PORTAL_BASE_URL")
        ...
        # The trusted operator/deployment env override (LOKI_PORTAL_BASE_URL /
...
# ~350ms even on the failure path, and read-only UI surfaces (`loki tools`, status panels,
# subscription-feature checks) call it many times per render — `loki tools` → "All Platforms"
...
# `loki auth login/logout/add/remove` invalidate naturally on the next call.
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `loki auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `loki auth` and
    # `loki model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("LOKI_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("LOKI_COPILOT_ACP_ARGS", "").strip()
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``loki doctor`` runs the live
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set LOKI_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `loki logout` historically keyed off auth.json.active_provider only.
...
def _prompt_model_selection(
    ...
    """
    from loki_cli.models import _format_price_per_mtok
...
def login_command(args) -> None:
    ...
    print("Use 'loki auth' to manage credentials,")
    print("'loki model' to select a provider, or 'loki setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    # Check for existing Loki-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in Loki auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("LOKI_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
    ...
    # Save tokens to Loki auth store
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in Loki auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    # xAI credential re-enabled. ``loki auth remove xai-oauth`` leaves a
    # ``device_code`` suppression marker that otherwise stops the singleton
    # seed from re-creating the pool entry, so ``loki auth list`` would show
...
def build_minimax_oauth_token_provider() -> Callable[[], str]:
    ...
    def _provide() -> str:
        ...
        if not state or not state.get("access_token"):
            raise AuthError(
                "Not logged into MiniMax OAuth. Run `loki model` and select "
                "MiniMax (OAuth).",
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    diagnostic call sites like ``loki status`` that just want to know
...
def step_up_nous_billing_scope(
    *,
    ...
    targets the same deployment (incl. a preview via ``LOKI_PORTAL_BASE_URL`` set
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        try:
            ...
            if model_ids:
                ...
                if free_tier:
                    ...
                    # as free so users on older Loki builds still see
        ...
        if not selected_model:
            ...
            print("  Run `loki model` again to switch to Nous Portal.")
            return
