"""
Unified self-relaunch for Loki CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``loki sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``loki`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
def resolve_loki_bin() -> Optional[str]:
    """Find the loki entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("loki")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m loki_cli.main``.
    ...
    ``loki.exe`` on PATH or the ``sys.executable -m loki_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
...
def relaunch(
    ...
    *,
    ...
    relaunch contract wants: "run loki again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    is usually ``loki.exe`` (a Python console-script shim that wraps
    ``python -m loki_cli.main``) or a ``.cmd`` batch file, and both
    ...
    That's functionally equivalent — the user sees "loki exited, then
    new loki started" — just with two PIDs in play instead of one.
    """
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # cryptic.  Common causes: ``loki`` not on PATH yet (install
