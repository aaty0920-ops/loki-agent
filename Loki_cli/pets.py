...
def _cmd_list(args) -> int:
    ...
    if getattr(args, "installed", False):
        ...
        if not pets:
            _print("No pets installed. Try: loki pets install boba")
            return 0
...
def _cmd_install(args) -> int:
    ...
    if getattr(args, "select", False) or not _has_active_pet():
        ...
        _print(f"  Make it active with: loki pets select {slug}")
    return 0
...
def _cmd_doctor(args) -> int:
    ...
    ok = True
    if not pets:
        _print("  → no pets installed. Run: loki pets install boba")
        ok = False
    elif active is None:
        _print("  → active pet unresolved. Run: loki pets select <slug>")
        ok = False
    elif not enabled:
        _print("  → pet display is disabled. Run: loki pets select " + active.slug)
