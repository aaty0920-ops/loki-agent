...
def build_models_payload(
    ...
    *,
    ...
    - ``pricing``: enrich each row with formatted per-model pricing and,
      ...
      mirroring the ``loki model`` CLI picker. Adds network calls
    ...
    try:
        from loki_cli.providers import is_routing_aggregator as _is_routing_aggregator
    except Exception:
...
def _append_unconfigured_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    from loki_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
...
def _moa_provider_row(current_provider: str = "") -> dict | None:
    ...
    picker path (:func:`loki_cli.model_switch.list_picker_providers`) so the
