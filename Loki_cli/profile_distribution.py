...
A distribution is a Loki profile published as a git repository (or
...
* ``loki profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``loki skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``loki profile``, not a parallel tree):

    loki profile install <source> [--name N] [--alias] [--force] [--yes]
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            loki_requires=str(data.get("loki_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.loki_requires:
            out["loki_requires"] = self.loki_requires
        if self.author:
...
def check_loki_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Loki {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``loki profile update`` can
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Loki distribution."
        )

    # Version check up-front so we fail fast
    check_loki_requires(manifest.loki_requires, loki_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.loki).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
    ...
    with tempfile.TemporaryDirectory(prefix="loki_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `loki profile update` to upgrade in place, "
