"""``loki portal`` — the human-readable entry point for Nous Portal.

Running ``loki portal`` with no subcommand performs the one-shot Portal
...
``loki auth add nous --type oauth`` (which still works), is identical to
``loki setup --portal``, and runs the same Nous flow as the first-time quick
setup.
...
already in ``loki auth`` or ``loki tools``. It's the onboarding + discovery
...
DOCS_URL = "https://loki-agent.nousresearch.com/docs/user-guide/features/tool-gateway"
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `loki portal`.", Colors.YELLOW))
        print()
...
def _cmd_login(args) -> int:
    ...
    This is the human-readable front door for `loki auth add nous --type
    oauth`. It reuses the exact wiring behind `loki setup --portal` (which in
...
def portal_command(args) -> int:
    ...
    if sub in {None, "", "login"}:
        # Default to the one-shot onboarding — `loki portal` is the
        # human-readable alias for `loki auth add nous --type oauth` /
        # `loki setup --portal`.
...
def add_parser(subparsers) -> None:
    ...
    portal_parser = subparsers.add_parser(
        ...
        description=(
            "Run `loki portal` with no subcommand to log in to Nous Portal "
            ...
            "nous --type oauth`, identical to `loki setup --portal`). "
