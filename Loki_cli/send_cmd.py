"""CLI subcommand: ``loki send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"loki send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"loki send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"loki send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
...
def _load_loki_env() -> None:
    """Populate ``os.environ`` from ``~/.loki/.env`` AND bridge top-level
    ...
    startup that ``loki send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.loki/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.loki/config.yaml`` into
    ...
    intentionally reimplement the minimum needed here so ``loki send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.loki/.env and ~/.loki/config.yaml into os.environ so the
    ...
    # Import lazily so `loki send --help` stays fast and does not pull in
