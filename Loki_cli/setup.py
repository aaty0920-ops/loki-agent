"""
Interactive setup wizard for Loki Agent.
...
_DOCS_BASE = "https://loki-agent.nousresearch.com/docs"
...
# display_loki_home imported lazily at call sites (stale-module safety during loki update)
...
from loki_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from loki_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print_info("Configure Loki using environment variables or config commands:")
    print_info("  loki config set model.provider custom")
    print_info("  loki config set model.base_url http://localhost:8080/v1")
    print_info("  loki config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'loki setup' in an interactive terminal to use the full wizard.")
    print()
...
def is_noninteractive() -> bool:
    ...
    ``LOKI_NONINTERACTIVE=1`` (see ``loki_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("LOKI_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``LOKI_NONINTERACTIVE=1`` or a closed/redirected
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'loki setup')")
...
def _print_setup_summary(config: dict, loki_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'loki setup' to configure"))
    ...
    # Video generation — opt-in via `loki tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from loki_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'loki setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'loki setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'loki setup terminal'"))
    ...
    # Spotify (OAuth via loki auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'loki setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {loki_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('loki setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('loki setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('loki setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('loki setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('loki setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('loki config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('loki config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('loki config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('loki', Colors.GREEN)}              Start chatting")
    print(f"   {color('loki gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('loki doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `loki tools` and `loki setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``loki model``)
    ...
    provider added to ``loki model`` is automatically available here.
    ...
    # Delegate to the shared loki model flow — handles provider picker,
    ...
    # on demand via `loki auth add`, `loki setup` vision, and
    # `loki setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``loki model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.loki/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Loki auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via loki setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Loki runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `loki setup terminal` / config.yaml.
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into LOKI_MAX_ITERATIONS at startup. We no longer write
    ...
    print_info("  Run `loki setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # LOKI_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.loki/logs/tool_calls.log (gateway only)")
...
def _profile_name_from_loki_home(loki_home) -> str | None:
    ...
    if loki_home.parent.name == "profiles":
        return loki_home.name
    return None
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Loki delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Loki to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from loki_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://loki-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://loki-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Loki from anywhere.")
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     loki config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_loki_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd and os.geteuid() == 0:  # windows-footgun: ok — guarded by supports_systemd (Linux only)
                    print_info("  Or as a boot-time service: loki gateway install --system")
                print_info("  Or run in foreground:  loki gateway")
        else:
            ...
            if is_container():
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   loki gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `loki setup tools` and `loki tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``loki_cli.auth`` — lists every supported
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from loki_cli.config
...
def _load_openclaw_migration_module():
    """Load the openclaw_to_loki migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_loki", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Loki.
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Loki config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Loki.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Loki's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(loki_home: Path) -> bool:
    ...
    print_info("Loki can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: loki claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: loki claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Loki configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Loki config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Loki (use loki claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``loki setup --portal`` and ``loki portal``. This is the
    ...
    Loki session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``loki setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``loki model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``loki portal`` always offers a model picker,
    ...
    print(color("│     ⚕ Loki Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `loki portal` == quick setup's Nous step.
    try:
        from loki_cli.main import _model_flow_nous
        ...
        logger.debug("_model_flow_nous error during `loki portal`: %s", exc)
        print()
    ...
    print_info("  Run `loki portal info` to inspect routing.")
    print_info("  Run `loki` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      loki setup           — full or quick (auto-detected)
      loki setup model     — just model/provider
      loki setup tts       — just text-to-speech
      loki setup terminal  — just terminal backend
      loki setup gateway   — just messaging platforms
      loki setup tools     — just tool configuration
      loki setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Loki Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Loki Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Loki Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, loki_home)
            return
        ...
        print_info("")
        print_info("Tip: jump straight to a section with 'loki setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(loki_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Loki?",
            [
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, loki_home, is_existing)
            return
        if setup_mode == 2:
            _run_blank_slate_setup(config, loki_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {loki_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'loki config edit'")
    ...
    # Tune later with `loki setup agent`.
    if not is_existing:
...
def _run_first_time_quick_setup(config: dict, loki_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``loki setup <section>``
    or switch providers with ``loki model``.
    """
    from loki_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'loki setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    loki setup")
    if gateway_choice != 0:
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``loki tools`` (which
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``loki setup agent`` /
    ``loki config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, loki_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `loki update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        loki tools")
        print_info("  Seed skills:         loki skills opt-in --sync")
        print_info("  Add MCP servers:     loki mcp add")
        print_info("  Enable plugins:      loki plugins")
        print_info("  Tune agent settings: loki setup agent")
        print()
        _print_setup_summary(config, loki_home)
        return
...
def _blank_slate_walkthrough(config: dict, loki_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`loki update` runs from re-injecting them. Opt back in any")
            print_info("time with `loki skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `loki plugins list` / `loki plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `loki plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `loki mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `loki mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   loki tools")
    print_info("  Seed skills:         loki skills opt-in --sync")
    print_info("  Add MCP servers:     loki mcp add")
    print_info("  Tune agent settings: loki setup agent")
    print()
...
def _run_quick_setup(config: dict, loki_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'loki setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Loki to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'loki setup gateway'.")
