...
A system-level directory (default ``/etc/loki``, root-owned and not
user-writable) supplies ``config.yaml`` and ``.env`` values that WIN over the
user's ``~/.loki/config.yaml`` and ``~/.loki/.env`` on a per-leaf-key basis.

This is DISTINCT from ``loki_cli.config.is_managed()`` / ``LOKI_MANAGED``,
...
def _under_pytest() -> bool:
    ...
    Used to ignore the system default ``/etc/loki`` during tests so a real
    managed scope on a developer/CI box can't leak policy into the suite. Tests
    that exercise managed scope set ``LOKI_MANAGED_DIR`` explicitly, which is
...
def get_managed_dir() -> Optional[Path]:
    ...
    Resolution (highest priority first):
      1. ``$LOKI_MANAGED_DIR`` — deployment/bootstrap path override (IT-only;
         never persisted to any .env). Honored only when set to a non-empty value
         AND the directory exists.
      2. ``/etc/loki`` — POSIX default, when it exists. Ignored under pytest so
...
def apply_managed_overlay(config: dict) -> dict:
    ...
    (rather than going through loki_cli.config.load_config) to honor managed
    scope. Mirrors loki_cli.config._load_config_impl's managed merge exactly:
