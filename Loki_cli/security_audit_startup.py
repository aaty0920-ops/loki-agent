...
3. Running inside a container with no persistent volume mount over the
   LOKI_HOME data dir (state is ephemeral — lost on container restart).
...
def _running_as_root() -> Optional[str]:
    ...
    return (
        ...
        "full host compromise. Run Loki as an unprivileged user (or in a "
...
def _container_no_volume_mount(loki_home: Optional[Path]) -> Optional[str]:
    if not _in_container():
        return None
    home = loki_home or Path(
        os.environ.get("LOKI_HOME", os.path.expanduser("~/.loki"))
    )
    try:
...
def run_security_audit(
    *, loki_home: Optional[Path] = None, config: Optional[dict] = None
) -> list[str]:
    ...
    for check in (
        ...
        try:
            ...
            continue
    try:
        r = _container_no_volume_mount(loki_home)
        if r:
            findings.append(r)
    except Exception:
        pass
