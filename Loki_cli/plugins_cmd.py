...
Plugins are installed from Git repositories into ``~/.loki/plugins/``.
...
def _resolve_git_executable() -> Optional[str]:
    ...
    Matches other Loki subprocess resolution: :func:`shutil.which` first,
...
def _missing_requires_env_names(manifest: dict) -> list[str]:
    """Return declared ``requires_env`` names that are unset in ``~/.loki/.env``."""
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from loki_cli.config import get_env_value, save_env_value  # noqa: F811
...
def _install_plugin_core(identifier: str, *, force: bool) -> tuple[Path, dict, str]:
    """Clone Git plugin into ``~/.loki/plugins``.
...
def cmd_install(
    ...
    if not (target / "plugin.yaml").exists() and not (target / "plugin.yml").exists() and not (
        ...
        console.print(
            ...
            f"or __init__.py. It may not be a valid Loki plugin.",
        )
...
def _resolve_plugin_key(name: str) -> Optional[str]:
    ...
    This is the single normalization point so ``loki plugins enable`` /
...
def cmd_enable(name: str, allow_tool_override: Optional[bool] = None) -> None:
    ...
    # Loki core and are trusted; every other source needs operator opt-in.
    if source == "bundled":
        return
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    plugin-registered engines (third-party engines installed as Loki
