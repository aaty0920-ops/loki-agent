...
def _print_aiohttp_missing() -> None:
    print(
        "loki proxy requires aiohttp. Install one of:\n"
        "  pip install 'loki-agent[messaging]'\n"
        "  pip install aiohttp",
        file=sys.stderr,
    )
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``loki proxy <subcommand>``."""
    ...
    print(
        ...
        "\n"
        "Subcommands:\n"
        "  loki proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
