...
# xAI's public API is OpenAI-compatible for the endpoints Loki commonly
# uses. The Responses endpoint is included because Loki' native xAI runtime
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`loki auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`loki auth reset xai-oauth` or re-authenticate with "
                    "`loki auth add xai-oauth --type oauth`."
                )
