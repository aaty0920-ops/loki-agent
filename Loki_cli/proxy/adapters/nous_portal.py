...
Reads the user's Nous OAuth state from ``~/.loki/auth.json`` through the
...
class NousPortalAdapter(UpstreamAdapter):
    ...
    def _get_credential(
        ...
        *,
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Nous Portal. Run `loki auth add nous` first."
                )
    ...
    # to loki_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
