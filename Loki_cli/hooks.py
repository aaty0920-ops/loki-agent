...
Usage::
    ...
    loki hooks list
    loki hooks test <event> [--for-tool X] [--payload-file F]
    loki hooks revoke <command>
    loki hooks doctor

Consent records live under ``~/.loki/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.loki/config.yaml``
...
# stdin a script sees under `loki hooks test` and `loki hooks doctor`
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    # is already allowlisted.  Otherwise `loki hooks doctor` would execute
