...
goal satisfied by the assistant's last response?". If not, Loki feeds a
...
- When a real user message arrives mid-loop it preempts the continuation
  ...
  will say ``done``).
- This module has zero hard dependency on ``cli.LokiCLI`` or the gateway
...
def _get_session_db() -> Optional[Any]:
    """Return a SessionDB instance for the current LOKI_HOME.

    SessionDB has no built-in singleton, but opening a new connection per
    /goal call would thrash the file. We cache one instance per
    ``loki_home`` path so profile switches still pick up the right DB.
...
def run_kanban_goal_loop(
    *,
    ...
    (``loki -p <profile> chat -q "work kanban task <id>"``). The worker's
