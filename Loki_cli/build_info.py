...
``loki_cli/dump.py`` and ``loki_cli/banner.py``).  That doesn't work inside
...
To make ``loki dump`` and the startup banner identify the exact commit the
image was built from, the Docker build writes the build-time ``$LOKI_GIT_SHA``
arg into ``<project_root>/.loki_build_sha``.  This module is the single
...
- Returns ``None`` when the file is absent.  Source installs and dev images
  built without the ``LOKI_GIT_SHA`` build-arg fall through to live-git
...
def get_build_sha(short: int = 8) -> Optional[str]:
    ...
    Reads ``<project_root>/.loki_build_sha`` if present.  The file is
    written by the Dockerfile's ``LOKI_GIT_SHA`` build-arg and contains
