...
store at ``~/.loki/checkpoints/``.  Actions:

    loki checkpoints               # same as `status`
    loki checkpoints status        # total size, project count, breakdown
    loki checkpoints list          # per-project checkpoint counts + workdir
    loki checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    loki checkpoints clear [-f]    # nuke the entire base (asks first)
    loki checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    loki checkpoints
    loki checkpoints prune --retention-days 3 --max-size-mb 200
    loki checkpoints clear -f
