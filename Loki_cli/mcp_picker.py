...
``loki mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `loki plugin` picker UX: arrow keys to navigate, ENTER on a row
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `loki mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: loki mcp install <name>    Picker: loki mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Loki is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Loki — run `loki update` "
...
def run_picker() -> None:
    """`loki mcp picker` (and default `loki mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    ...
    if entry is None:
        print(color(
            ...
            "Run `loki mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
