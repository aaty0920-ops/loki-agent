...
on Japanese locales, etc.).  Loki's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``loki_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Loki is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``LOKI_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("LOKI_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Loki-managed Git install directories so
    ...
    # already-running shells don't see the change — which means loki
    ...
    # install without Loki-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Loki's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Loki's
    ``loki config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Loki (User env var in Windows Settings, or
    export in a PowerShell profile) and Loki picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Loki-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\loki\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``loki`` from right after install) retain their old PATH.

    Any subprocess Loki spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Loki-managed tool directories to our
    ...
    candidate_dirs = [
        ...
        # Loki venv Scripts directory — host of the loki.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes loki, but harmless to include.
