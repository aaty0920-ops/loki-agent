...
the existing module-level functions in loki_cli.gateway and
loki_cli.gateway_windows directly. This protocol is a thin facade
...
``loki gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``loki gateway start/stop/restart``).
    """
    ...
    # `loki gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged loki
    ...
    detection always fails. Since every Loki runtime call inside the
    container drops to loki via ``s6-setuidgid``, that silent failure
...
# in ``loki_cli.gateway`` (systemd/launchd) and ``loki_cli.gateway_windows``
...
# active (set via the ``loki -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in loki_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from loki_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from loki_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from loki_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from loki_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in loki_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from loki_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from loki_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from loki_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from loki_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``loki_cli.gateway_windows`` (Scheduled Task /
...
#
# Per-profile gateways are registered dynamically when `loki profile create`
# runs inside the container (Phase 4). Static services (main-loki, dashboard)
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's LOKI_HOME blindly;
    derive the shared profile root from the current LOKI_HOME and map the
...
# UID/GID of the in-image ``loki`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the loki user.
    ...
    (the ``loki`` user — UID 10000 — running every Loki runtime
    ...
    is dispatched as the loki user.
    ...
    with loki ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           loki:loki, 0755 (parent must already exist)
    ``svc_dir/event/``                     loki:loki, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 loki:loki, 0755
    ``svc_dir/supervise/event/``           loki:loki, 03730
    ``svc_dir/supervise/control``          loki:loki, 0660    (FIFO)
    ...
    slot's supervise/ was loki-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        try:
            ...
            # Running as the loki user already — directory is loki-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _LOKI_UID, _LOKI_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is loki-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _LOKI_UID, _LOKI_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``loki -p typo gateway start`` when no profile
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-loki, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources LOKI_HOME (and any extra env) via with-contenv —
             so e.g. ``-e LOKI_HOME=/data/loki`` is honored at run
          ...
          4. Drops to the loki user and exec's
             ``loki -p <profile> gateway run`` (or just ``loki
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``loki gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root LOKI_HOME profile" (the implicit profile that exists at
        the top of $LOKI_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``loki_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$LOKI_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``LOKI_HOME``,
        ...
        allocator in ``loki_cli.profiles._allocate_gateway_port``).
        ...
        # profile's LOKI_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``loki gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-LOKI_HOME PID lock first;
        ...
        # gateway/run.py) so s6 always wins. The LOKI_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own LOKI_HOME and s6 guarantees a
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${LOKI_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the LOKI_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e LOKI_HOME=/data/loki``
        gets its logs under /data/loki/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``loki logs`` reads and what
        ...
        return (
            ...
            # later then runs its log service as loki and crash-loops on
            ...
            f'chown loki:loki "$LOKI_HOME/logs/gateways" 2>/dev/null || true\n'
            f'chown -R loki:loki "$log_dir" 2>/dev/null || true\n'
            f'rm -f "$log_dir/lock"\n'
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``loki gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``loki -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with loki ownership
            ...
            # the loki user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is loki-owned (see ``_seed_supervise_skeleton``), and on
