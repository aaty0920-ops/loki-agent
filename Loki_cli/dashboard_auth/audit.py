...
from ``loki_constants`` or other loki_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$LOKI_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``loki_constants.get_loki_home`` semantics: env var wins,
    else ``~/.loki``. A local copy avoids an import cycle with the
    middleware which lives below ``loki_cli``.
    """
