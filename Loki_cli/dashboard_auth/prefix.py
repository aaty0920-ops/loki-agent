...
prefix (e.g. ``mission-control.tilos.com/loki/*`` -> dashboard on
:9119), injecting ``X-Forwarded-Prefix: /loki`` so the backend can
reconstruct prefixed URLs (Location: headers, OAuth redirect_uri,
cookie Path attributes, SPA asset URLs).

This module is also the home of the ``LOKI_DASHBOARD_PUBLIC_URL`` /
...
def _warn_if_malformed(source: str, raw: str) -> None:
    ...
    missing scheme (``loki.example.com`` instead of
    ``https://loki.example.com``) — the single most common cause of
    "I set LOKI_DASHBOARD_PUBLIC_URL but the OAuth callback is still
    ...
    _log.warning(
        ...
        source,
        cleaned,
        cleaned.split("://")[-1] or "loki.example.com",
    )
...
def normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/loki"`` (no trailing slash) or ``""``
...
def resolve_public_url() -> str:
    ...
    Precedence (mirrors ``dashboard.oauth.client_id``):
      ...
      1. ``LOKI_DASHBOARD_PUBLIC_URL`` env var (when non-empty after
