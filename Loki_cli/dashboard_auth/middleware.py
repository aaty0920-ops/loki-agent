...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /loki``, the
    ``login_url`` is prefixed (``/loki/login?next=...``) so the
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["loki.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # sends only ``loki_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if session is None:
        ...
        from loki_cli.dashboard_auth.cookies import clear_session_cookies
        from loki_cli.dashboard_auth.prefix import prefix_from_request
