...
Three cookies in play:
  ...
  - loki_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
setter wrote ``__Secure-loki_session_at`` would never find the value.
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /loki`` we want ``Path=/loki`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    issues a 24h rotating refresh token (loki #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/loki``)
