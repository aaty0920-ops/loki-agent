...
def _has_loki_agent_browser() -> bool:
    ...
    # install.sh installs globally into $LOKI_HOME/node/bin/ via npm -g --prefix
...
def ensure_dependency(
    ...
    if shell == "powershell":
        ...
        cmd = [
            ...
            "-File", str(script),
            "-Ensure", dep,
            "-LokiHome", str(get_loki_home()),
        ]
    else:
        cmd = ["bash", str(script), "--ensure", dep]

    run_env = loki_subprocess_env(inherit_credentials=False)
