...
- loki chat          - Interactive chat (same as ./loki)
- loki gateway       - Run gateway in foreground
- loki gateway start - Start gateway service
...
def _ensure_utf8():
    ...
    can even start — e.g. `loki setup` on a fresh Pi.
    ...
    loki_cli/stdio.py::configure_windows_stdio() runs later from the entry
