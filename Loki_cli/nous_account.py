...
def format_nous_portal_entitlement_message(
    ...
    *,
    ...
    if account_info is None:
        return (
            f"Loki could not verify your Nous Portal entitlement, so {capability} "
            f"is unavailable. Run `loki model` to refresh your login, or check "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Nous inference credentials are configured, but Loki cannot verify "
                f"your Nous Portal paid access for {capability}. Log in with "
                f"`loki model` to enable Portal-managed features. Billing and "
                f"credits are managed at {billing_url}."
            )
        return (
            f"Log in to Nous Portal to use {capability}: run `loki model`. "
            f"Billing and credits are managed at {billing_url}."
        )

    if account_info.paid_service_access is None:
        detail = (
            f"Loki could not verify your Nous Portal paid access, so {capability} "
            f"is unavailable."
        )
    ...
    if reason == "account_missing":
        return (
            f"Loki could not find a Nous Portal account or organisation for this "
            f"login, so {capability} is unavailable. Run `loki model` to "
...
def _fresh_account_info(
    *,
    ...
    try:
        from loki_cli.auth import get_provider_auth_state, resolve_nous_access_token
