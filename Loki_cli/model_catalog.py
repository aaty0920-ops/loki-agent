...
The Loki docs site hosts a JSON manifest of curated models for providers
...
1. ``get_catalog()`` — returns a parsed manifest dict.
   ...
   - Reads disk cache at ``~/.loki/cache/model_catalog.json``.
...
DEFAULT_CATALOG_URL = (
    "https://loki-agent.nousresearch.com/docs/api/model-catalog.json"
)
...
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = (
    "https://raw.githubusercontent.com/NousResearch/loki-agent/main/website/static/api/model-catalog.json",
)
DEFAULT_TTL_HOURS = 1
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``loki update`` pulls the latest repo, so the freshly-pulled
    ...
    to ``~/.loki/cache/model_catalog.json`` via the same atomic writer the
