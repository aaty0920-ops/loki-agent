...
This module backs ``loki console`` and is intentionally narrower than the
full Loki CLI. It exposes a curated set of native adapters that can later be
...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (
        prev.startswith("Run 'loki doctor'")
        and last.startswith("Run 'loki setup'")
    ):
        return text.rstrip()
...
def _clean_summary(text: str | None) -> str:
    ...
    if not summary:
        return ""
    if summary.startswith("Run `loki "):
        return ""
    return summary
...
# dashboard opens a fresh LokiConsoleEngine per /api/console connection, so
...
def _dispatch_extracted_subcommand(
    *,
    ...
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module("loki_cli.main")
    builder = getattr(module, builder_name)
...
def _extracted_handler(
    ...
    main_handler_name: str,
    namespace_update: Callable[[argparse.Namespace, ConsoleContext], None] | None = None,
) -> Callable[["LokiConsoleEngine", list[str]], str]:
...
def _register_command_family(
    ...
    *,
    ...
    handler_factory: Callable[[Sequence[str]], Callable[["LokiConsoleEngine", list[str]], str]],
    ...
    for child_path in paths:
        ...
        command_summary = summary or (summaries or {}).get(full_path) or f"Run `loki {usage}`."
        engine.register(
            full_path,
            usage,
            command_summary,
            ...
            confirmation=confirmation or f"Run `loki {usage}`?",
...
class LokiConsoleEngine:
    ...
    def _register_defaults(self) -> None:
        self.register(("status",), "status", "Show Loki component status.", _status, contexts=ALL_CONTEXTS)
        self.register(("doctor",), "doctor", "Run diagnostics without auto-fix.", _doctor, contexts=ALL_CONTEXTS)
        self.register(("logs",), "logs [name] [-n N]", "Show recent Loki logs.", _logs, contexts=ALL_CONTEXTS)
        ...
        self.register(
            ...
            _config_set,
            mutating=True,
            confirmation="Update Loki configuration?",
    ...
    def _register_broad_cli_surface(self) -> None:
        """Register non-admin CLI commands that are safe for Loki Console."""

        extracted = {
            "version": (
                "loki_cli.subcommands.version",
            ...
            "prompt-size": (
                "loki_cli.subcommands.prompt_size",
            ...
            "import": (
                "loki_cli.subcommands.import_cmd",
        ...
        self.register(
            ...
            _config_migrate,
            mutating=True,
            confirmation="Update Loki configuration with missing defaults?",
        )
        self.register(
            ("sessions", "export"),
        ...
        self.register(
            ...
            _adder_handler("send", (), "loki_cli.send_cmd", "register_send_subparser"),
            mutating=True,
        ...
        _register_command_family(
            ...
            root="portal",
            paths=portal_paths,
            summaries=_adder_summaries("loki_cli.portal_cli", "add_parser"),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("loki_cli.projects_cmd", "build_parser"),
            mutating=[
                ("create",),
                ("add-folder",),
                ("remove-folder",),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("loki_cli.kanban", "build_parser"),
            mutating=[
                ("init",),
                ("boards", "create"),
                ("boards", "rm"),
    ...
    def _resolve_command(self, tokens: Sequence[str]) -> tuple[ConsoleCommand, list[str]]:
        ...
        for size in range(min(len(tokens), 3), 0, -1):
            ...
            if command:
                if self.context not in command.contexts:
                    raise ConsoleCommandError(
                        f"`loki {command.usage}` is not available in "
                        f"{self.context} Loki Console."
                    )
    ...
    def _rejection_for(self, tokens: Sequence[str]) -> str:
        ...
        if tuple(tokens[:2]) in {("sessions", "delete"), ("sessions", "prune")}:
            return "`sessions delete` and `sessions prune` are not available in Loki Console."
        return ""
...
def _enforce_hosted_line_policy(path: tuple[str, ...], args: Sequence[str]) -> None:
    if path == ("config", "set"):
        ...
        if key and not _hosted_config_key_allowed(key):
            raise ConsoleCommandError(
                f"`config set {key}` is not available in hosted Loki Console. "
    ...
    if path == ("mcp", "add"):
        if _flag_present(args, "--command") or _flag_present(args, "--args"):
            raise ConsoleCommandError(
                "Hosted Loki Console does not add stdio MCP servers. "
                "Use catalog install or an HTTP/SSE URL."
            )
        if _flag_present(args, "--preset"):
            raise ConsoleCommandError(
                "Hosted Loki Console does not add MCP presets directly. "
                "Use `mcp install <catalog-name>`."
            )
        url = _flag_value(args, "--url")
        if not url:
            raise ConsoleCommandError(
                "Hosted Loki Console requires `mcp add` to use --url with "
                "an HTTP/SSE endpoint."
            )
        scheme = urlparse(url).scheme.lower()
        if scheme not in {"http", "https"}:
            raise ConsoleCommandError(
                "Hosted Loki Console only accepts http:// or https:// MCP URLs."
            )
        return
...
def _apply_confirmed_defaults(args: argparse.Namespace, context: ConsoleContext) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError("auth add --type api-key requires --api-key in Loki Console.")
...
def _doctor(_engine: LokiConsoleEngine, args: list[str]) -> str:
...
def _config_show(_engine: LokiConsoleEngine, args: list[str]) -> str:
...
def _config_migrate(_engine: LokiConsoleEngine, args: list[str]) -> str:
...
def _sessions_export(_engine: LokiConsoleEngine, args: list[str]) -> str:
...
def _sessions_rename(_engine: LokiConsoleEngine, args: list[str]) -> str:
...
def _sessions_optimize(_engine: LokiConsoleEngine, args: list[str]) -> str:
...
def _sessions_repair(_engine: LokiConsoleEngine, args: list[str]) -> str:
    ...
    def _run() -> None:
        from loki_state import DEFAULT_DB_PATH, _db_opens_cleanly, repair_state_db_schema
...
def _profile_status(_engine: LokiConsoleEngine, args: list[str]) -> str:
    ...
    return _dispatch_extracted_subcommand(
        ...
        fixed=(),
        args=(),
        module_name="loki_cli.subcommands.profile",
...
def _cron_status(_engine: LokiConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "cron status")
    from loki_cli.cron import cron_status
...
def _cron_pause(_engine: LokiConsoleEngine, args: list[str]) -> str:
    ...
    try:
        job = pause_job(args[0], reason="paused from loki console")
...
def run_console_repl(
    *,
    ...
    engine = LokiConsoleEngine()
    if interactive:
        print("Loki Console. Type `help` for commands, `exit` to quit.", file=stdout)

    while True:
        if interactive:
            print("loki> ", end="", file=stdout, flush=True)
        line = stdin.readline()
        if line == "":
            if interactive:
                print(file=stdout)
