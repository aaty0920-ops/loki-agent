"""
Doctor command for loki CLI.

Diagnoses issues with Loki Agent setup.
"""
...
from loki_cli.config import get_project_root, get_loki_home, get_env_path
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from loki_cli.auth import get_minimax_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches loki_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``loki --version`` reporting a stale version while
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != loki_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'loki update', or set "
            "loki_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-loki and dashboard static services are up
    ...
    for static in ("main-loki", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `loki profile create <name>`")
        return
...
def managed_scope_check() -> None:
    ...
    resolved from the LOKI_MANAGED_DIR override (rather than the system
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `loki`.
    os.environ.setdefault("LOKI_INTERACTIVE", "1")

    # Handle `loki doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.loki/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 Loki Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`loki doctor --ack {hit.advisory.id}`."
                )
    ...
    # Detect drift between pyproject.toml and loki_cli/__init__.py versions
    ...
    managed_scope_check()
    # Check ~/.loki/.env (primary location for user config)
    ...
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'loki setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                issues.append("Run 'loki setup' to create .env")
    
    # Check ~/.loki/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from loki_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'loki config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            (
                                ...
                                f"Run 'loki setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'loki config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
    ...
    if config_path.exists():
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'loki setup' to migrate config")
                else:
                    issues.append("Run 'loki doctor --fix' or 'loki setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'loki doctor --fix'")
        except Exception:
            pass

        # Detect stale LOKI_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives LOKI_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            ...
            if drift:
                check_warn(
                    f"LOKI_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `loki setup` run)",
                )
                if should_fix:
                    if remove_env_value("LOKI_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale LOKI_MAX_ITERATIONS from .env "
                        ...
                        manual_issues.append(
                            "Manually delete the LOKI_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale LOKI_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'loki doctor --fix'"
                    )
        except Exception:
            pass
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Loki' own device-code flow — the Codex CLI is
    ...
    for subdir_name in expected_subdirs:
        subdir_path = loki_home / subdir_name
    ...
    if soul_path.exists():
        ...
        if should_fix:
            ...
            soul_path.write_text(
                ...
                "You are Loki, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    if state_db_path.exists():
        try:
            ...
            from loki_state import _db_opens_cleanly, repair_state_db_schema
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'loki doctor --fix' "
                        "(or 'loki sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
            from loki_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'loki doctor --fix' "
                        "(or 'loki sessions repair') to recover hidden sessions"
                    )
            else:
    ...
    wal_path = loki_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'loki doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "loki"
        ...
        _cmd_link = _cmd_link_dir / "loki"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(loki not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/loki → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/loki points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/loki → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/loki — run 'loki doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/loki exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/loki not found",
                    "(loki command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/loki → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/loki symlink — run 'loki doctor --fix'")
    ...
    if _safe_which("node"):
        ...
        if agent_browser_path.exists():
            ...
            # `loki update` wiped node_modules (issue #48521).
            check_warn(
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `loki doctor` invocation.
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable LOKI_HOME mirror
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Loki one.
                        check_info(
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``loki doctor``.
    ...
    # never the legitimate source for `loki doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'loki setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: loki skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = LOKI_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: loki memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: loki memory setup",
                    "No Honcho API key — run 'loki memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run loki memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: loki memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: loki memory setup")
        except Exception as _e:
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "loki -p" in content:
                            _m = _re.search(r"loki -p (\S+)", content)
