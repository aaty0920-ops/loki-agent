"""
Loki Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``loki desktop`` / ``loki gui``)
     Built inside the agent checkout under ``$LOKI_HOME/loki-agent/``:
       ...
       - ``$LOKI_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Loki.app`` or ``~/Applications/Loki.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Loki`` (NSIS per-user)
...
the app name ("Loki"), separate from ``$LOKI_HOME``:
  - macOS:   ``~/Library/Application Support/Loki``
  - Windows: ``%APPDATA%\\Loki``
  - Linux:   ``$XDG_CONFIG_HOME/Loki`` (default ``~/.config/Loki``)
...
``loki uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def source_built_gui_artifacts(loki_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``loki desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``loki-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(loki_home)
    ...
    return [
        ...
        loki_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Loki.app"),
            home / "Applications" / "Loki.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Loki).
            local_base / "Programs" / "Loki",
            # Older / alternate layout some builds used.
            local_base / "loki-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Loki")
    else:
        ...
        paths += [
            data_base / "applications" / "loki.desktop",
            data_base / "applications" / "Loki.desktop",
        ]
    return paths
...
def agent_is_installed(loki_home: Path) -> bool:
    """Return True when a usable Python agent install exists under LOKI_HOME.
...
def gui_is_installed(loki_home: Path) -> bool:
    ...
    for p in source_built_gui_artifacts(loki_home):
        if p.exists():
            return True
...
def uninstall_gui(loki_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``loki-agent/loki_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$LOKI_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove loki' or "
            "'sudo dnf remove loki'). AppImage builds are a single file you "
