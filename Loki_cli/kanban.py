"""CLI for the Loki Kanban board — ``loki kanban …`` subcommand.

Exposes the full Kanban command surface documented in the design spec
(``docs/loki-kanban-v1-spec.pdf``).  All DB work is delegated to
...
def _check_dispatcher_presence() -> tuple[bool, str]:
    ...
    - ``running=True``: a gateway is alive for this LOKI_HOME and its
    ...
    Used by ``loki kanban create`` (and callers) to warn when a task
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description=(
            "Durable SQLite-backed task board shared across Loki profiles. "
            "Tasks are claimed atomically, can depend on other tasks, and "
            "are executed by a named profile in an isolated workspace. "
            "See https://loki-agent.nousresearch.com/docs/user-guide/features/kanban "
    ...
    # LOKI_KANBAN_BOARD env var, then the persisted current-board
    ...
    kanban_parser.add_argument(
        ...
        help=(
            ...
            "(set via `loki kanban boards switch <slug>` or the "
            "LOKI_KANBAN_BOARD env var). Use `loki kanban boards list` "
            "to see all boards."
        ),
    )
    ...
    p_list.add_argument("--mine", action="store_true",
                        help="Filter by $LOKI_PROFILE as assignee")
    ...
    p_comment.add_argument("--author", default=None,
                           help="Author name (default: $LOKI_PROFILE or 'user')")
    ...
    p_asg = sub.add_parser(
        ...
        help="List known profiles + per-profile task counts "
             "(union of ~/.loki/profiles/ and current assignees on the board)",
    )
    ...
    p_specify.add_argument(
        ...
        help="Author name recorded on the audit comment "
             "(default: $LOKI_PROFILE or 'specifier')",
    )
    p_specify.add_argument(
        "--json",
        action="store_true",
    ...
    p_decompose.add_argument(
        ...
        default=None,
        help="Author name recorded on the audit comment "
             "(default: $LOKI_PROFILE or 'decomposer')",
    )
    p_decompose.add_argument(
        "--json",
        action="store_true",
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``loki kanban …`` argparse dispatch.
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: loki kanban <action> [options]\n"
                "Run 'loki kanban --help' for the full list of actions.",
                file=sys.stderr,
            )
        return 0
    ...
    # env-var pin for the duration of this call. Using LOKI_KANBAN_BOARD
    ...
    # LOKI_HOME. Previously only `init` and `daemon` triggered
...
# ---------------------------------------------------------------------------
# Boards management (loki kanban boards …)
# ---------------------------------------------------------------------------

def _dispatch_boards(args: argparse.Namespace) -> int:
    """Handle ``loki kanban boards <action>``.
    ...
    DB, so a fresh LOKI_HOME that has never called ``kanban init``
...
def _cmd_boards_rm(args: argparse.Namespace) -> int:
    # When the user runs `loki kanban boards delete <slug>` (alias), the
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    if profiles:
        ...
        print("Create one with `loki -p <name> setup` before assigning tasks.")
    print()
    print("Next step: start the gateway so ready tasks actually get picked up.")
    print("  loki gateway start")
    print()
    print(
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    if task.result:
        ...
        # Surface it at top-level so a glance at ``loki kanban show <id>``
...
def _cmd_daemon(args: argparse.Namespace) -> int:
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                print(
                    ...
                    f"`loki kanban list --status ready` / "
                    f"`loki kanban list --status blocked` for recent "
    ...
    def _ready_queue_nonempty() -> bool:
        ...
        task whose assignee maps to a real Loki profile (i.e. one the
