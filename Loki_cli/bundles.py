...
Mirrors the structure of ``loki_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `loki bundles list | grep …` doesn't
...
def register_cli(subparser) -> None:
    """Build the ``loki bundles`` argparse tree.

    Called from ``loki_cli/main.py`` where it owns the top-level
