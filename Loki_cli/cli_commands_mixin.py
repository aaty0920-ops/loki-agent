...
``cli.py``'s ``LokiCLI`` class. ``LokiCLI`` inherits ``CLICommandsMixin`` so
...
class CLICommandsMixin:
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: loki --checkpoints")
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Loki config/state.
    ...
    def _handle_journey_command(self, cmd_original: str) -> None:
        """Handle /journey — the learning timeline (see `loki journey`).
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run loki chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in loki_cli/colors.py
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `loki gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                ...
                return
            _cprint("  Tip:   Use /history or `loki sessions list` to find sessions.")
            return
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `loki sessions list` to see available sessions.")
            return
    ...
    def _handle_pet_command(self, cmd: str):
        ...
        from loki_cli.pets import _set_active, _set_enabled, print_pet_gallery, set_pet_scale, toggle_pet_display
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to loki_cli.curator so the CLI and the `loki curator`
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to loki_cli.skills_hub."""
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``loki bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: loki bundles create "
        ...
        _cprint(
            ...
            f"Manage with `loki bundles`.{_RST}"
        )
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"Loki keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_goal_draft(self, objective: str) -> None:
        ...
        from cli import _DIM, _RST, _cprint
        from loki_cli.goals import draft_contract
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_loki_home()}/skins/\n")
            return
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Loki is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Loki is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while Loki is busy."
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from loki_cli.models import _is_anthropic_fast_model
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update Loki Agent to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``loki update`` so the user sees update output directly and gets
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Loki Agent"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="⚕  Update Loki Agent",
            detail="This will exit the current session and run `loki update`.",
            choices=choices,
        )
        if raw is None:
