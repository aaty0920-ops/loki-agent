"""``loki slack ...`` CLI subcommands.

Today only ``loki slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ loki slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Loki. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Loki deployment — users can tweak them in the Slack UI after
    pasting.
    ...
    ``/new``, etc. work inline.
    """
    from loki_cli.commands import slack_app_manifest
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``loki_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$LOKI_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Loki")
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("LOKI_HOME") or str(Path.home() / ".loki")) / "slack-manifest.json"
        else:
