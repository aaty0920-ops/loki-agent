"""
loki fallback — manage the fallback provider chain.
...
Subcommands:
  loki fallback [list]   Show the current fallback chain (default when no subcommand)
  loki fallback add      Pick provider + model via the same picker as `loki model`,
                           then append the selection to the chain
  loki fallback remove   Pick an entry to delete from the chain
  loki fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.loki/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        ...
        # `loki model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    print("  Docs: https://loki-agent.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    ...
    print("  `loki model` — select the provider + model you want as a fallback.")
    print()
