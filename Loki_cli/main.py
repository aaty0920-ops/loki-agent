...
Usage:
    ...
    loki chat                # Interactive chat
    loki gateway             # Run gateway in foreground
    loki gateway start       # Start gateway as service
    loki gateway stop        # Stop gateway service
    loki gateway status      # Show gateway status
    loki gateway install     # Install gateway service
    loki gateway uninstall   # Uninstall gateway service
    loki setup               # Interactive setup wizard
    loki logout              # Clear stored authentication
    loki status              # Show status of all components
    loki cron                # Manage cron jobs
    loki cron list           # List cron jobs
    loki cron status         # Check if cron scheduler is running
    loki doctor              # Check configuration and dependencies
    loki honcho setup                    # Configure Honcho AI memory integration
    loki honcho status                   # Show Honcho config and connection status
    loki honcho sessions                 # List directory → session name mappings
    loki honcho map <name>               # Map current directory to a session name
    loki honcho peer                     # Show peer names and dialectic settings
    loki honcho peer --user NAME         # Set user peer name
    loki honcho peer --ai NAME           # Set AI peer name
    loki honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    loki honcho mode                     # Show current memory mode
    loki honcho mode [hybrid|honcho|local]  # Set memory mode
    loki honcho tokens                   # Show token budget settings
    loki honcho tokens --context N       # Set session.context() token cap
    loki honcho tokens --dialectic N     # Set dialectic result char cap
    loki honcho identity                 # Show AI peer identity representation
    loki honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    loki honcho migrate                  # Step-by-step migration guide: OpenClaw native → Loki + Honcho
    loki version             Show version
    loki update              Update to latest version
    loki uninstall           Uninstall Loki Agent
    loki acp                 Run as an ACP server for editor integration
    loki sessions browse     Interactive session picker with search

    loki claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: loki_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``loki_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``loki update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``loki_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, loki crashes on import and the user can't run
# ``loki update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
def _set_process_title() -> None:
    """Set the process title to 'loki' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``loki tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``loki.exe``).
    """
    ...
    try:
        ...
        # Windows: the .exe name is already ``loki.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* loki_cli.config is importable. Mirrors the explicit
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``LOKI_TUI=1``, then ``display.interface`` in config.
    """
    if argv is None:
        argv = sys.argv[1:]
    if "--cli" in argv:
...
# `entry.tsx`; this is just the earlier cousin. ``LOKI_TUI_NO_EARLY_DISABLE``
...
def _suppress_mouse_residue_early() -> None:
    ...
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`loki --tui … >log`, CI capture):
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``loki --version`` before config/logging imports on Termux."""
...
from loki_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
...
from loki_cli.subcommands.postinstall import build_postinstall_parser
...
from loki_cli.subcommands.version import build_version_parser
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (loki tools, loki setup, loki model) use
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any loki module import.
#
# Many modules cache LOKI_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("LOKI_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.loki/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        ...
        candidate = home / ".loki" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`loki chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors loki_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If LOKI_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.loki/profiles/coder or
    # /opt/data/profiles/coder).  If LOKI_HOME points to the loki root
    # instead (e.g. systemd hardcodes LOKI_HOME=/root/.loki), we must
    # still read active_profile — the user may have switched profiles via
    # `loki profile use` and the gateway should honour that choice.
    # See issue #22502.
    loki_home_env = os.environ.get("LOKI_HOME", "")
    if profile_name is None and loki_home_env:
        if Path(loki_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the loki root.
    #
    # EXCEPTION: a supervised s6 gateway child (exported by the container
    # run-script as LOKI_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # reserved ``gateway-default`` slot runs bare ``loki gateway run`` to mean
    # "the root LOKI_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("LOKI_S6_SUPERVISED_CHILD"):
        try:
    ...
    # 3. If we found a profile, resolve and set LOKI_HOME
...
load_loki_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → LOKI_REDACT_SECRETS env
# var BEFORE loki_logging imports agent.redact (which snapshots the flag at
...
# Initialize centralized file logging early — all `loki` subcommands
...
# Provider model-selection wizard flows extracted to loki_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (loki_cli.main._model_flow_*) keep resolving unchanged.
...
def _has_any_provider_configured() -> bool:
    ...
    from loki_cli.config import get_env_path, get_loki_home, load_config
    from loki_cli.auth import get_auth_status

    # Determine whether Loki itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_loki_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    auth_file = get_loki_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Loki has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Loki to use their tokens.
    if _has_loki_config:
        try:
...
def _print_tui_exit_summary(
    ...
    print(f"  loki --tui --resume {target}")
    if title:
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @loki/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@loki" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``LOKI_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
...
def _ensure_tui_node() -> None:
    ...
    ``LOKI_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    ...
    extras.extend([Path(loki_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``loki update`` (HEAD stays
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    if tui_dev:
        # Keep the local @loki/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @loki/ink resolves through
        # packages/loki-ink/dist/entry-exports.js. If that dist bundle is
...
def _launch_tui(
    ...
    env["LOKI_PYTHON_SRC_ROOT"] = os.environ.get(
        "LOKI_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("LOKI_PYTHON", sys.executable)
    env.setdefault("LOKI_CWD", os.getcwd())
    ...
    if model:
        ...
        env["LOKI_INFERENCE_MODEL"] = model
    if provider:
        env["LOKI_TUI_PROVIDER"] = provider
        env["LOKI_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["LOKI_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["LOKI_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["LOKI_TUI_SKILLS"] = value
    if query:
        env["LOKI_TUI_QUERY"] = query
    if image:
        env["LOKI_TUI_IMAGE"] = image
    if checkpoints:
        env["LOKI_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["LOKI_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["LOKI_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["LOKI_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["LOKI_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["LOKI_ACCEPT_HOOKS"] = "1"
    ...
    # LOKI_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `loki --tui` try to
    ...
    # still set LOKI_TUI_RESUME themselves.
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `loki update` so
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``LOKI_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``loki kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``loki kanban boards switch`` from another session can flip the file
...
def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.loki/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Loki — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Loki still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      2. ``--tui`` flag / ``LOKI_TUI=1`` → always TUI
...
def cmd_chat(args):
    ...
    # setup (config, rules files, plugins, MCP servers) or from Loki itself.
    # Implemented as a superset of --ignore-user-config + --ignore-rules plus
    # plugin/MCP discovery suppression (LOKI_SAFE_MODE is checked by
    # loki_cli/plugins.py and tools/mcp_tool.py).
    ...
    # user's ~/.loki/config.yaml and return built-in defaults. Set BEFORE
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Loki?")
        print()
    ...
    # has no creds.json.  Every subsequent `loki gateway` then paid a 30s
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: loki gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `loki gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  loki gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Loki Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  loki gateway")
        ...
        print()
        print("  Or install as a service: loki gateway install")
    else:
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``loki whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``loki_cli/setup_whatsapp_cloud.py``.
    """
...
def cmd_postinstall(args):
    ...
    from loki_cli.config import stamp_install_method
    from loki_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ Loki post-install bootstrap")
    print()
...
def select_provider_and_model(args=None):
    ...
    effective_provider = (
        config_provider or os.getenv("LOKI_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'loki model' for "
    ...
    # PROVIDER_GROUPS in loki_cli/models.py). A multi-member group shows one
    ...
    # a leftover OPENAI_BASE_URL in ~/.loki/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.loki/.env if the active provider is not 'custom'.
...
#
# Loki uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `loki model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `loki model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`loki_cli.plugins.PluginContext.register_auxiliary_task`) appear
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Loki only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``loki model`` flow, then route aux tasks to them here.
    """
    from loki_cli.config import load_config
...
def _aux_flow_provider_model(
    ...
    from loki_cli.models import get_pricing_for_provider
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Loki URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
# downstream call sites read `loki_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `loki` invocation — including
# fast paths like `loki --version` and slash-command dispatch that never
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``loki setup`` / ``loki model``.
    ...
    recover from a malformed paste without editing ``~/.loki/.env`` by hand.
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `loki setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(
                f"    Loki will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Loki CLI with a provider."""
    from loki_cli.auth import login_command
...
def cmd_auth(args):
    ...
    from loki_cli.auth_commands import auth_command
...
def cmd_webhook(args):
    ...
    from loki_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``loki slack <subcommand>``. Currently supports:
...
def cmd_project(args):
    ...
    from loki_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from loki_cli.hooks import hooks_command
...
def cmd_backup(args):
    ...
    if getattr(args, "quick", False):
        from loki_cli.backup import run_quick_backup
...
# Critical files that every ``loki`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``loki update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "loki_cli/main.py",
    "loki_cli/config.py",
    "loki_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``loki`` startup; if any of them
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``loki update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``loki_cli/web_dist/`` (per vite.config.ts
    outDir: "../loki_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "loki_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``loki`` launcher present but ``loki_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the loki venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — loki venv python3, no nix-shell overhead
    ...
    # a loki venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``loki update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``loki web``).
    ...
    # build path usable on a stock `py -m loki_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (loki web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``loki update`` can unconditionally call ``loki desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``loki desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $LOKI_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $LOKI_HOME."""
    from loki_constants import get_loki_home
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``loki update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        ...
        candidates = [
            release_dir / "win-unpacked" / "Loki.exe",
            release_dir / "win-ia32-unpacked" / "Loki.exe",
            release_dir / "win-arm64-unpacked" / "Loki.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "loki",
            release_dir / "linux-unpacked" / "Loki",
            release_dir / "linux-arm64-unpacked" / "loki",
            release_dir / "linux-arm64-unpacked" / "Loki",
        ]
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Loki'`` desktop build failure: a corrupt zip in the
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Loki.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Loki.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Loki.exe: Access is
    ...
    an unrelated "Loki" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Loki is damaged and can't be opened." The
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``loki desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_launch_options() -> tuple[list[str], str]:
    ...
    (normalized for the LOKI_DESKTOP_DISABLE_GPU env var the Electron app
...
def cmd_gui(args: argparse.Namespace):
    ...
    if getattr(args, "fake_boot", False):
        env["LOKI_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["LOKI_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "loki_root", None):
        env["LOKI_DESKTOP_LOKI_ROOT"] = str(Path(args.loki_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["LOKI_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    ...
    # `LOKI_DESKTOP_DISABLE_GPU=... loki desktop` keeps working.
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if not source_mode:
                ...
                # holds Loki.exe locked on Windows, so the pack can't replace
            ...
            if (
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Loki.exe
            ...
            if not source_mode:
                ...
                # an in-place self-update (otherwise macOS reports "Loki is
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``loki dashboard`` processes other than ourselves.

    ``loki dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``loki update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Loki Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``loki serve`` as
    ...
    environment variable ``LOKI_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        "loki dashboard",
        "loki_cli.main dashboard",
        "loki_cli/main.py dashboard",
        # The headless backend (`loki serve`) is the same long-lived server
        # under a different command name — the desktop app spawns it. Reap it
        # on update for the same frontend/backend-mismatch reason.
        "loki serve",
        "loki_cli.main serve",
        "loki_cli/main.py serve",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "loki.*dashboard"`) keeps us consistent
            # with `loki_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `loki update`.
    ...
    print(
        "  Docs:         https://loki-agent.nousresearch.com/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``loki update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``loki update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: loki curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``loki dashboard`` processes.

    Called at the end of ``loki update`` (default ``reason``) and also
    from ``loki dashboard --stop`` (which overrides ``reason``).  The
    ...
    """
    # When the Loki Desktop Electron app spawns this dashboard as a
    # backend child, it sets LOKI_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("LOKI_DESKTOP_CHILD_PID")
    if raw_pid:
...
def _update_via_zip(args):
    """Update Loki Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        sys.exit(1)
    zip_url = (
        f"https://github.com/NousResearch/loki-agent/archive/refs/heads/{branch}.zip"
    )
    ...
    try:
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for loki-agent itself
        ...
        # GitHub ZIPs extract to loki-agent-<branch>/
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: loki skills list-modified  "
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "loki-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes loki completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Loki couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Loki couldn't drop the saved stash entry."
            )
...
def _discard_stashed_changes(
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Loki couldn't drop "
...
# =========================================================================
# Fork detection and upstream management for `loki update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/loki-agent.git",
    "git@github.com:NousResearch/loki-agent.git",
    "https://github.com/NousResearch/loki-agent",
    "git@github.com:NousResearch/loki-agent",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/loki-agent.git"
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print("  This means you may miss updates from NousResearch/loki-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NousResearch/loki-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NousResearch/loki-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``loki update``, every profile is now current.
    """
    homes = []
...
# Install-scoped breadcrumb dropped right before ``loki update`` mutates the
...
# survives and the next ``loki`` launch finishes the install instead of
# limping along on a half-built venv (e.g. pip wiped, a core dep like Pillow
# never landed).  Lives next to the venv (not under $LOKI_HOME) because the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``loki update`` left half-done.
    ...
    protocol stream (``loki acp`` speaks JSON-RPC on stdout) must never get
    ...
    # Windows self-lock guard: if loki.exe is the launcher that spawned
    ...
    if _is_windows():
        ...
        if scripts_dir is not None:
            shims = _loki_exe_shims(scripts_dir)
            if shims:
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``loki update`` is still progressing even if pip/uv itself is silent.
    """
...
def _loki_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    names = set(_load_console_script_names()) or {"loki", "loki-agent", "loki-acp"}
...
def _detect_concurrent_loki_instances(
    ...
    The Loki Desktop Electron app spawns ``loki.EXE`` as a backend child,
    so during ``loki update`` the user-invoked process and the desktop's
    ...
    shims (``loki.exe`` / ``loki-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``loki update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``loki.exe``) is a separate process from the Python
    ...
    # setuptools-generated loki.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``loki update``
    ...
    #      loki.exe sitting *under* a non-Loki parent (e.g. a Loki
...
def _format_concurrent_instances_message(
    ...
    lines = ["✗ Another loki.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Loki Desktop, exit any open `loki` REPLs, and")
    lines.append("  stop the gateway (`loki gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `loki update --force` if you've already")
...
def _quarantine_running_loki_exe(
    ...
    """Pre-empt Windows file lock on the running ``loki.exe``.
    ...
    when ``loki update`` runs, ``loki.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``loki.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next loki invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Loki Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Loki Desktop / gateway / REPL) and pointing to ``--force``.
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_loki_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    shims, and on Windows the live ``loki.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``loki`` drops off PATH. This wraps
    ...
    moved: list[tuple[Path, Path]] = []
    if scripts_dir is not None:
        moved = _quarantine_running_loki_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``loki.exe.old.*`` left by prior updates.

    Called early on every loki invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``loki update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `loki update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``loki.exe`` / ``loki-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_loki_exe`` for the rationale.
    """
    ...
    # hours later inside ``loki update``'s desktop-rebuild or skill-sync
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    On Windows, ``uv pip install -e .`` can register ``loki.exe`` in the
    wheel RECORD while the file never lands on disk — typically when the live
    ``loki.exe`` shim is locked during ``loki update``, or when uv/distlib
    skips a launcher write. The symptom is ``loki-agent.exe`` and
    ``loki-acp.exe`` present but ``loki.exe`` missing, so ``loki`` drops
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m loki_cli.main <command>"
        )
    else:
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``loki update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``loki.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``loki`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `loki update` output above.")
        return
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$LOKI_HOME/bin/uv``, but on Termux the official
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `loki update`, so we install root-only first
    ...
    # hung. The chatty npm-deprecation noise during `loki update` comes from
...
class _UpdateOutputStream:
    """Stream wrapper used during ``loki update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.loki/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``loki update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``loki update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.loki/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``loki update --gateway``) the update is already
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.loki/logs/update.log`` only, never the terminal.

    During ``loki update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``loki update --check``: fetch and report without installing.
    ...
    if method == "docker":
        ...
        # same long-form ``docker pull`` guidance ``loki update`` (apply
    ...
    if method == "pip":
        from loki_cli.config import recommended_update_command
        from loki_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``loki update`` without requiring a reinstall.  The
    ...
    ``loki`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/loki.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v loki'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/loki, code at /usr/local/lib/loki-agent).
    ...
    path_comment = (
        "# Loki Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of LOKI_HOME before running the update.
    ...
    LOKI_HOME directories.  The ``--backup`` flag on ``loki update``
    ...
    # ``loki_cli/config.py``). Fall back to false if the key is missing
    # so the default behaviour matches the shipped config: zipping a large
    # LOKI_HOME can add minutes to every update. Users who want the
    ...
    # Render path using display_loki_home so the user sees ~/.loki/...
    try:
        from loki_constants import get_loki_home, display_loki_home

        home = get_loki_home()
        try:
            display_path = f"{display_loki_home()}/{out_path.relative_to(home)}"
        except ValueError:
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``loki update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``loki update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run loki from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.loki-bootstrap-complete`,
        ...
        managed_markers = (
            PROJECT_ROOT / ".loki-bootstrap-complete",
            _update_marker_path(),
        )
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The loki.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m loki_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``loki
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines = [
        "✗ Other Loki processes are running from this install's venv:",
    ]
    for pid, name, cmdline in matches[:6]:
        hint = ""
        low = cmdline.lower()
        if "serve" in low or "dashboard" in low:
            hint = "  ← Loki Desktop backend (close the desktop app)"
        elif "gateway" in low:
            hint = "  ← gateway"
    ...
    lines.append("    loki update")
    lines.append("  (or use `loki update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    loki.exe concurrent-instance guard does not see them. They still import
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m loki_cli.main
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``loki gateway start`` uses (``gateway_windows._spawn_detached``).
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``loki update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update Loki Agent to the latest version.
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `loki update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Loki via pip (for PyPI installs)."""
    from loki_cli import __version__
    from loki_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "loki-agent"]
        ...
        cmd = [pipx, "upgrade", "loki-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "loki-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "loki-agent"]
    ...
    print("✓ Update complete! Restart loki to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # On Windows, abort early if another loki.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_loki_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `loki serve` backend) will keep .pyd
    ...
    # the loki.exe shim guard above, but its lock probe only checks the shim
    ...
    try:
        ...
        # though `git pull` can't touch $LOKI_HOME, this is cheap
        ...
        # orphan merge-conflict markers in loki_cli/config.py bricked
        # every user who ran ``loki update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``loki
        ...
        # the old bytecode (e.g. get_loki_home added to loki_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # the marker survives and the next ``loki`` launch finishes the
        ...
        # build.  ``loki desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``loki desktop`` shouldn't be forced into a full
        # Electron build by ``loki update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        ...
        if (desktop_dir / "package.json").exists() and find_node_executable("npm") and has_desktop_app:
            ...
            _desktop_build_cmd = [sys.executable, "-m", "loki_cli.main", "desktop", "--build-only"]
            ...
            if build_result.returncode != 0:
                build_result = _run_logged_subprocess(_desktop_build_cmd, cwd=PROJECT_ROOT)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `loki desktop` to retry)")
        ...
        # current — so copy it straight over ~/.loki/cache/model_catalog.json
        ...
        # modules in this process.  Reload loki_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_loki_home() added since the last release.
        try:
            import importlib
            import loki_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit LOKI_HOME so
        # it is not affected by sync_skills()'s module-level LOKI_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's LOKI_HOME env var points at the default or a named profile.
        try:
        ...
        # check `loki curator status`. Self-stamps after printing so it
        ...
        # don't have it. Tying the refresh to ``loki update`` gives users a
        ...
        # When running as ``loki update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_loki_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... loki-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... loki-gateway*)
            ...
            # Discover all loki-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. loki-gateway.service or loki-gateway-coder.service
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    if unmapped_count > 1:
                        print(
                            "    (or: loki -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Loki gateway unit files are still installed.
        # When both loki.service (from a pre-rename install) and the
        # current loki-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `loki update` surfaces the issue until the user migrates.
        try:
            from loki_cli.gateway import (
                has_legacy_loki_units,
                _find_legacy_loki_units,
                supports_systemd_services,
            )
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``loki -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → loki -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.loki)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  loki profile alias {name} --name <custom>"
                    )
        ...
        if alias_name:
            ...
            print(f"Alias:   {alias_name} → loki -p {name}  ({wrapper})")
        print()
        ...
        from loki_cli.profiles import profile_exists, validate_alias_name
        ...
        from loki_cli.profiles import rename_profile
        ...
        from loki_cli.profiles import import_profile
        ...
        try:
            ...
            if plan.has_cron:
                print(
                    ...
                    f"loki -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        if data.get("loki_requires"):
            print(f"Requires:     Loki {data['loki_requires']}")
        if data.get("source"):
...
def _report_dashboard_status() -> int:
    """Print ``loki dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``loki dashboard --status``
    ...
    print(f"{len(pids)} loki dashboard process(es) running:")
    for pid in pids:
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``loki dashboard register`` for OAuth.
    ...
    print("    [2] OAuth via Nous Portal (run `loki dashboard register`)")
    print("    [3] Cancel")
    print()
    ...
    if choice == "2":
        ...
        print(
            ...
            "  Docs: https://loki-agent.nousresearch.com/docs/"
...
def cmd_dashboard(args):
    ...
    # launches the dashboard (`worker dashboard` → LOKI_HOME points into
    ...
    if (
        ...
        # LOKI_HOME.  We must resolve the root explicitly instead of just
        # dropping LOKI_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV LOKI_HOME=/opt/data`), so an unset
        # LOKI_HOME falls back to $HOME/.loki = /opt/data/.loki — an
        ...
        # update-button guard also misfires).  get_default_loki_root()
        # returns the root for both layouts: ~/.loki for a standard install
        ...
        try:
            ...
            # behaviour (drop LOKI_HOME) rather than block the reroute.
    ...
    if "LOKI_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
...
def cmd_dashboard_register(args):
    ...
    from loki_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from loki_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from loki_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from loki_cli.prompt_size import cmd_prompt_size as _impl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from loki_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``loki -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``loki_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``loki -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``loki`` or only flags → defaults to ``chat``.
        return False
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("LOKI_TUI") == "1")
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `loki --tui` is the hot path on phones. The full parser setup imports
...
def cmd_plugins(args):
    from loki_cli.plugins_cmd import plugins_command
...
def cmd_claw(args):
    from loki_cli.claw import claw_command
...
def main():
    ...
    # Cosmetic: make the process show up as 'loki' instead of 'python3.11'
    ...
    # Sweep stale ``loki.exe.old.*`` quarantine files left by previous
    # ``loki update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_loki_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # Self-heal a venv left half-built by an interrupted ``loki update``
    ...
    # ``loki skills install update``) merely defers recovery one launch;
    # under-matching (missing ``loki -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in loki_cli/subcommands/model.py)
    # =========================================================================
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://loki-agent.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.loki/.env.  Currently supports "
            "Bitwarden Secrets Manager.  See: "
            "https://loki-agent.nousresearch.com/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in loki_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in loki_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in loki_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in loki_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    # =========================================================================
    # slack command  (parser built in loki_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    # login command  (parser built in loki_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in loki_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in loki_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in loki_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in loki_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in loki_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    # hooks command  (parser built in loki_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in loki_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in loki_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in loki_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in loki_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in loki_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.loki/checkpoints/",
    ...
    # =========================================================================
    # import command  (parser built in loki_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in loki_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # console command  (parser built in loki_cli/subcommands/console.py)
    # =========================================================================
    build_console_parser(subparsers, cmd_console=cmd_console)

    # =========================================================================
    # pairing command  (parser built in loki_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in loki_cli/subcommands/skills.py)
    # =========================================================================
    ...
    # =========================================================================
    # plugins command  (parser built in loki_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``loki --help``, ``loki version``, ``loki logs``,
    ...
    # =========================================================================
    # memory command  (parser built in loki_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in loki_cli/subcommands/tools.py)
    # =========================================================================
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            import shutil
            import subprocess
            from loki_cli.tools_config import _cua_driver_cmd
            # Honor LOKI_CUA_DRIVER_CMD for local-build testing — same
    ...
    # =========================================================================
    # mcp command  (parser built in loki_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch loki --resume <id> by replacing the current process
    ...
    # =========================================================================
    # insights command  (parser built in loki_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in loki_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in loki_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in loki_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in loki_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in loki_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in loki_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in loki_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Loki-Setup.exe success screen tells users to
    # run `loki desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in loki_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in loki_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in loki_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``loki -c Pokemon Agent Dev`` → ``loki -c 'Pokemon Agent Dev'``
    ...
    # fails (e.g. 'loki -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (loki hooks list, cron
