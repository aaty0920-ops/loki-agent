...
Implements ``loki mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.loki/config.yaml under the ``mcp_servers`` key.
"""
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from loki_cli.cli_output import prompt as _shared_prompt
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.loki/.env`` into ``os.environ`` and recursively interpolate any
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``loki mcp login`` to distinguish a genuine authentication
...
def cmd_mcp_add(args):
    ...
    # loki_cli/main.py for why the dest is renamed.
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(f"Saved '{name}' to {display_loki_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `loki mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    landed before reporting success. Shared by ``loki mcp login`` and
    ``loki mcp reauth`` so both behave identically for a single server.
    """
...
def cmd_mcp_reauth(args):
    ...
    ``loki mcp reauth <name>`` re-auths a single server (same as ``login``).
    ``loki mcp reauth --all`` discovers every ``auth: oauth`` server in
...
def mcp_command(args):
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `loki plugin`.
        from loki_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("loki mcp                                    Open the catalog picker (default)")
        _info("loki mcp catalog                            List Nous-approved MCPs")
        _info("loki mcp install <name>                     Install a catalog MCP")
        _info("loki mcp serve                              Run as MCP server")
        _info("loki mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("loki mcp add <name> --command <cmd>         Add a stdio server")
        _info("loki mcp add <name> --preset <preset>       Add from a known preset")
        _info("loki mcp remove <name>                      Remove a server")
        _info("loki mcp list                               List configured servers")
        _info("loki mcp test <name>                        Test connection")
        _info("loki mcp configure <name>                   Toggle tools")
        _info("loki mcp login <name>                       Re-authenticate OAuth")
        _info("loki mcp reauth <name> | --all              Re-auth one or all OAuth servers")
        print()
