...
Each profile is a fully independent LOKI_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.loki/profiles/<name>/`` by default.

The "default" profile is ``~/.loki`` itself — backward compatible,
zero migration needed.

Usage::

    loki profile create coder          # fresh profile + bundled skills
    loki profile create coder --clone  # also copy config, .env, SOUL.md, skills
    loki profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    loki -p coder chat                 # or via flag
    loki profile use coder             # set as sticky default
    loki profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See loki_constants.get_subprocess_home().
    "home",
]
...
# Marker file written by `loki profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `loki
...
# `loki skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Loki infrastructure directories that only the default profile
         (``~/.loki``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_loki_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.loki) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "loki-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "loki", "default", "test", "tmp", "root", "sudo",
})
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the loki root, NOT to the current LOKI_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where LOKI_HOME points outside
    ``~/.loki``, profiles live under ``LOKI_HOME/profiles/`` so
...
def _get_default_loki_home() -> Path:
    """Return the default (pre-profile) LOKI_HOME path.

    In standard deployments this is ``~/.loki``.
    In Docker/custom deployments where LOKI_HOME is outside ``~/.loki``
    (e.g. ``/opt/data``), returns LOKI_HOME directly.
    """
...
def validate_profile_name(name: str) -> None:
    ...
    Also rejects names in :data:`_RESERVED_NAMES` (``loki``, ``test``,
    ``tmp``, ``root``, ``sudo``) that would create confusing on-disk
    collisions (a ``loki`` profile inside ``~/.loki/``) or get refused
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, loki subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Loki. If we
    ...
    try:
        ...
        # not be migrated. The next `loki doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``loki -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``loki profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``loki -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``loki profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``LOKI_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Loki config
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, loki_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own LOKI_HOME.
    ...
    The returned ``loki_home`` is the path to pass to
    ``set_loki_home_override`` when scoping a turn to that profile.
    """
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``loki update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.loki)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.loki/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Keys pages, `loki -p <name> auth add`) had no file until first
    ...
    # Write the opt-out marker so seed_profile_skills() and `loki update`'s
    ...
    # Cloned configs can be older than the running Loki (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `loki profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `loki -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches LOKI_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``loki profile create
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Loki *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Loki backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``LOKI_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `loki -p <canon> profile
    ...
    loki_markers = ("loki_cli.main", "loki-gateway", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Loki process: either an entrypoint marker in argv, or
            # a resolved executable named `loki`.
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_loki = (
                any(marker in joined for marker in loki_markers)
                or exe_name == "loki"
                or exe_name.startswith("loki")
            )
            if not is_loki:
                continue
            ...
            # ...or by LOKI_HOME env pointing at this profile dir.
            if not bound:
                try:
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.loki).\n"
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_loki_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``LOKI_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from loki_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set LOKI_HOME so _profile_suffix resolves correctly
    ...
    try:
        ...
        elif "LOKI_HOME" in os.environ:
            del os.environ["LOKI_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.loki/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: loki profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from LOKI_HOME.

    Returns ``"default"`` if LOKI_HOME is not set or points to ``~/.loki``.
    Returns the profile name if LOKI_HOME points into ``~/.loki/profiles/<name>``.
    Returns ``"custom"`` if LOKI_HOME is set to an unrecognized path.
    """
    from loki_constants import get_loki_home
    loki_home = get_loki_home()
    resolved = loki_home.resolve()

    default_resolved = _get_default_loki_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.loki itself — its parent is ~/ and its
        # directory name is ".loki", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    # dir.  Importing as "default" would target ~/.loki itself — disallow
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    candidates = [
        new_dir / "honcho.json",
        _get_default_loki_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("loki_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a LOKI_HOME path string.

    Called early in the CLI entry point, before any loki modules
    are imported, to set the LOKI_HOME environment variable.
    """
