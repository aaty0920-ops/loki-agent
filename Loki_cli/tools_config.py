...
`loki tools` and `loki setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.loki/config.yaml under
...
from loki_cli.nous_subscription import (
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from loki_cli.nous_account import format_nous_portal_entitlement_message
...
from loki_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _LOKI_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `loki tools` → Video Generation, which walks
...
# `loki tools` → X (Twitter) Search setup walks users through credential
...
# Platform-scoped toolsets: only appear in the `loki tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``loki tools`` → "reconfigure existing" would
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``loki tools`` checklist actually offers
    for ``platform``.
    ...
    so ``loki tools`` never claims to add or remove a toolset the user was
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Loki routes X searches through xAI's built-in x_search "
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set LOKI_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "LOKI_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "LOKI_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "LOKI_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "LOKI_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "LOKI_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
# provider+model picker like `loki model` — NOT this single-key prompt, so
...
def _cua_driver_env() -> dict:
    """cua-driver child env with the Loki telemetry policy applied.
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``loki update`` and
      by ``loki computer-use install --upgrade``.
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `loki update` calls this
...
def _run_cua_driver_installer(label: str = "Installing", verbose: bool = True) -> bool:
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    driver_cmd = _cua_driver_cmd()
    try:
        # When not verbose (e.g. `loki update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `loki update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.loki/logs/update.log — write straight to it so
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_loki_home()}/loki-agent && npm install --workspaces=false")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in loki-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/nousresearch/loki-agent:latest"
            )
            return
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.loki/config.yaml")
        ...
        # Run the full `loki auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.loki/.env), then continues straight into PKCE. If they
        ...
        try:
            ...
            _print_info("    Run manually: loki auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `loki auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: loki auth spotify")
        except Exception as exc:
        ...
        try:
            ...
            _print_info("    Run manually: loki plugins enable observability/langfuse")
        _print_info("    Restart Loki for tracing to take effect.")
        _print_info("    Verify: loki plugins list")
        ...
        try:
            ...
            _print_info("    Run later: loki auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: loki auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: loki auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``loki tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``loki tools post-setup <key>`` — non-interactive post-setup runner.
...
def _get_platform_tools(
    ...
    *,
    ...
    # "loki-cli" (which include all _LOKI_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[loki-cli, spotify]`` after enabling Spotify via ``loki
        ...
        # (e.g. "loki-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``loki tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `loki tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"loki-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("loki-")}
    ...
    # shipped as a bundled plugin but user must opt in via `loki tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `loki tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by loki tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # is invalid (e.g. a migration or hand-edit left `loki` instead of
    # `loki-cli`), resolve_toolset() returns [] for each and the platform ends
    ...
    # at runtime, not only during the next `loki update`/`loki doctor`.
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (loki-cli, loki-telegram, etc.)
    ...
    # Opening `loki tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `loki tools` gets a silent no-op
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `loki model`).
    if managed_feature:
        from loki_cli.nous_subscription import (
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``loki model`` — or point at a
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `loki model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `loki tools` and `loki setup tools`.
    ...
    print(color("  Guide: https://loki-agent.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, loki mcp
