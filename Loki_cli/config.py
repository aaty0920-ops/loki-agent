"""
Configuration management for Loki Agent.

Config files are stored in ~/.loki/ for easy access:
- ~/.loki/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.loki/.env         - API keys and secrets

This module provides:
- loki config          - Show current configuration
- loki config edit     - Open config in editor
- loki config set      - Set a specific value
- loki config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``loki config set`` (which rewrites
    ...
    ``config.yaml`` in place: loki never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.loki/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``loki logs``
...
#   the next ``subprocess.run([...])`` Loki makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Loki itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``loki update``, the TUI build.
...
#   on every plugin install / ``loki update``.
...
# * ``LOKI_HOME`` / ``LOKI_PROFILE`` / ``LOKI_CONFIG`` /
#   ``LOKI_ENV`` — Loki runtime location flags. Writing these into
...
#
# IMPORTANT: ``LOKI_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (LOKI_LANGFUSE_PUBLIC_KEY,
# LOKI_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Loki runtime location — never via dashboard env writer.
    # NOT a LOKI_* blanket: integration credentials (LOKI_GEMINI_*,
    # LOKI_LANGFUSE_*, LOKI_SPOTIFY_*, ...) ARE allowed.
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Loki runtime location "
            "(LOKI_HOME, LOKI_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "LOKI_TOOL_PROGRESS", "LOKI_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `loki plugins enable
    # observability/langfuse` or `loki tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "LOKI_LANGFUSE_ENV",
    "LOKI_LANGFUSE_RELEASE",
    "LOKI_LANGFUSE_SAMPLE_RATE",
    "LOKI_LANGFUSE_MAX_CHARS",
    "LOKI_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("LOKI_MANAGED", "").strip()
    if raw:
...
def is_managed() -> bool:
    ...
    Two signals: the LOKI_MANAGED env var (set by the systemd service),
    or a .managed marker file in LOKI_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade loki-agent"
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    installs, ``/opt/loki`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$LOKI_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Loki was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$LOKI_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. LOKI_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.loki``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$LOKI_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.loki:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$LOKI_HOME``, a
    ...
    ``docker`` and ``loki update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/loki-agent`` image bakes a ``docker``
        stamp into ``/opt/loki`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $LOKI_HOME.
    try:
    ...
    #    host install whose shared $LOKI_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``loki update``.
    try:
        method = (
            (get_loki_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``loki_constants.is_container`` (import-safe)."""
    try:
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Loki lives in a ``uv tool`` layout.

    ``uv tool install loki-agent`` places the install at
    ``.../uv/tools/loki-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``loki-agent`` happens to be uv-tool-installed on the machine while
    the *active* Loki is a regular pip/venv install, causing
    ``loki update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/loki-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade loki-agent"
    if method == "docker":
        return "docker pull nousresearch/loki-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade loki-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade loki-agent"
        return "pip install --upgrade loki-agent"
    return "loki update"
...
# Long-form text for ``loki update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# loki_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Loki, it doesn't update
...
Loki Agent runs as a published image (nousresearch/loki-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/loki-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate loki-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/loki-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/loki-agent/tags
...
def format_managed_message(action: str = "modify this Loki installation") -> str:
    ...
    raw = os.getenv("LOKI_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Loki installation is managed by NixOS "
            f"(LOKI_MANAGED={env_hint}).\n"
            "Edit services.loki-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Loki installation is managed by Homebrew "
            f"(LOKI_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade loki-agent"
        )
...
def get_container_exec_info() -> Optional[dict]:
    ...
    Returns a dict with keys: backend, container_name, exec_user, loki_bin
    or None if container mode is not active, we're already inside the
    container, or LOKI_DEV=1 is set.
    ...
    container_mode_file = get_loki_home() / ".container-mode"
...
from loki_constants import get_loki_home  # noqa: F811,E402
...
def _resolve_loki_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the LOKI_UID / LOKI_GID env vars set by Docker deployments.

    Docker containers running Loki commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level LOKI_HOME once, but
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("LOKI_UID", "").strip()
    gid_str = os.environ.get("LOKI_GID", "").strip()
    try:
...
def _chown_to_loki_uid(path) -> None:
    """Chown ``path`` to ``LOKI_UID:LOKI_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_loki_home` on Docker deployments.
    See #34107.
    """
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the loki group can
    share state with the gateway service.

    The mode can be overridden via the LOKI_HOME_MODE environment variable
    (e.g. LOKI_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse LOKI_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``LOKI_UID``/``LOKI_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("LOKI_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Loki runs in a container with volume-mounted config files, forcing
...
def _secure_file(path):
    ...
    permissions.  Set LOKI_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into LOKI_HOME, upgrading legacy empty templates.
...
def ensure_loki_home():
    """Ensure ~/.loki directory structure exists with secure permissions.
    ...
    """
    home = get_loki_home()
    # Named profiles must be created explicitly (e.g. ``loki profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old LOKI_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_loki_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_loki_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"LOKI_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Loki-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Loki
        ...
        # identity slot (SOUL.md). Empty by default. The LOKI_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Loki adds a coding operating brief
    ...
    "terminal": {
        ...
        #             LOKI_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force LOKI_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Loki auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Loki sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Loki
        ...
        # bundled Loki image, which drops to the `loki` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Loki sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Loki should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``loki chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.loki/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: loki sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .loki.md, or .cursorrules before Loki applies
    ...
    # single read_file page exceeds these limits, Loki truncates the
    ...
    # See tools/kanban_tools.py and loki_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``loki dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``loki kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``loki kanban decompose`` and the kanban
        ...
        # ``loki profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `loki model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `loki` (and `loki chat`) launches by default:
        ...
        # REPL and `--tui` (or LOKI_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `loki --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `loki -c` muscle memory.  Default off so existing
        # users aren't surprised.  LOKI_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `loki --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/loki`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `loki pets`. Disabled until
        ...
        "pet": {
            ...
            # get_loki_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``LOKI_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``LOKI_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # ``LOKI_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``LOKI_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/loki``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Loki appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.loki/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.loki/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.loki/cache/delegation/
    ...
    # response. If not, Loki feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Loki auto-pauses the goal and
    ...
    "moa": {
        ...
        # <loki_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.loki/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${LOKI_SKILL_DIR} and ${LOKI_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `loki curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `loki curator run --consolidate` overrides
        ...
        # normally restored on every `loki update`, so pruning them only
        ...
        # skipped), snapshot ~/.loki/skills/ into
        # ~/.loki/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `loki curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for loki-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Loki Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (LOKI_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Loki' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.loki/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or LOKI_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `loki doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``loki_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Loki to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $LOKI_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via LOKI_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `loki -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `loki` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `loki kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     loki-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Loki tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.loki/logs/.
    ...
    # update model picker lists without shipping a loki-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://loki-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `loki model` invocation; network failures
    ...
    "gateway": {
        ...
        # internal LOKI_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # the LOKI_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.loki.gateway` that defenses 1-2 don't
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.loki/.env,
        ...
        # behavior: files must live under the Loki cache, under
        ...
        # user. Bridged to LOKI_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Loki
        # cache (~/.loki/cache/{documents,images,audio,video,screenshots})
        ...
        # to LOKI_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # LOKI_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to LOKI_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.loki/state.db.
    ...
    "sessions": {
        ...
        # default of ``loki sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.loki/sessions/session_{sid}.json`` on every turn
    ...
    # ``loki update`` behaviour.
    "updates": {
        # Run a full ``loki backup``-style zip of LOKI_HOME before every
        # ``loki update``.  Backups land in ``<LOKI_HOME>/backups/`` and
        # can be restored with ``loki import <path>``.  Off by default:
        # zipping a large LOKI_HOME (sessions DB, caches, skills) can add
        # minutes to every update.  The #48200 incident — a ``loki update
        ...
        "backup_keep": 5,
        # What `loki update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<LOKI_HOME>/lsp/bin/`` on first use.
    ...
    # `loki tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.loki/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.loki/.env (or your shell) and never in config.yaml.
            ...
            # ~/.loki/bin/ on first use.  When False you must install
    ...
    "computer_use": {
        ...
        # by default upstream. Loki disables it for our users unless they
        # explicitly opt in here. When false (default), Loki sets
    ...
    # Loki Desktop (Electron app) launch options. These only affect
    # `loki desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the LOKI_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Loki mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'loki model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Loki auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Loki, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @loki:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. LOKI_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #loki)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: loki-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'loki-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Loki API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Loki API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Loki API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # LOKI_TOOL_PROGRESS and LOKI_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.loki.config`` entries, then checks
    ...
    try:
        ...
        # should never break `loki update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Loki versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``loki_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("LOKI_HOME", "~/.loki")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "loki update / loki -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("LOKI_TOOL_PROGRESS")
            old_mode = get_env_value("LOKI_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from LOKI_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from LOKI_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("LOKI_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from LOKI_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$LOKI_HOME/plugins/`` for currently installed user plugins.
    ...
    # settings in their `config.yaml`, and `loki curator status` has no
    ...
    #   3. Creates `~/.loki/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_loki_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_loki_home() / "logs" / "curator"
            ...
            if added_curator:
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_curator)}) — edit via `loki config set`"
                    )
            if added_aux:
                results["config_added"].append(
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_aux)}) — edit via `loki config set`"
                    )
    ...
    # behavior set it to true explicitly via `loki config set`.
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from loki_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    # `loki update`, but only the version bump is persisted.
    ...
    # metadata.loki.config in their SKILL.md frontmatter.
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``loki config set``
    ...
    backends) — while display paths (``loki status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _loki_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.loki/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.loki/config.yaml.
    ...
    (which change ``LOKI_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/loki/config.yaml invalidates the
...
#   nous         (OAuth — loki auth) — Nous Portal
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Loki writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.loki/.env.
    ...
    interactive menu render (`loki tools`, `loki setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `loki tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Loki env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.loki/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.loki/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.loki/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.loki/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Loki — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Loki vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.loki/.env or environment."""
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.loki/.env`` over ``os.environ``.

    Used for Loki-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    # Warn on stale LOKI_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("LOKI_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale LOKI_MAX_ITERATIONS={_env_ghost} "
                f"(run 'loki doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
...
def set_config_value(key: str, value: str):
    ...
    # so a fresh `loki config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `loki config set model.api_key cfut_...` routes to config.yaml
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: loki config set <key> <value>")
            print()
            print("Examples:")
            print("  loki config set model anthropic/claude-sonnet-4")
            print("  loki config set terminal.backend docker")
            print("  loki config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        print("  loki config           Show current configuration")
        print("  loki config edit      Open config in editor")
        print("  loki config set <key> <value>   Set a config value")
        print("  loki config check     Check for missing/outdated config")
        print("  loki config migrate   Update config with new options")
        print("  loki config path      Show config file path")
        print("  loki config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``loki config`` UI so users
