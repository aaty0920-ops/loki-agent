...
- Keep data: Remove code but keep ~/.loki/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing loki-agent or loki PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Loki Agent" comment and following line
                ...
                # Remove any PATH line containing loki
...
def remove_wrapper_script():
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains loki_cli reference)
                content = wrapper.read_text()
                if 'loki_cli' in content or 'loki-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(loki_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``loki`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Loki home's ``node``
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``loki gateway run`` processes
...
#
#   1. Sets User-scope env vars ``LOKI_HOME`` and ``LOKI_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\loki\git\cmd``, ``%LOCALAPPDATA%\loki\git\bin``,
#      ``%LOCALAPPDATA%\loki\git\usr\bin``, ``%LOCALAPPDATA%\loki\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\loki\git\`` and Node to
#      ``%LOCALAPPDATA%\loki\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``loki dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\loki\gateway-service\`` and sometimes
...
def _loki_path_markers(loki_home: Path) -> list[str]:
    """Path-entry substrings that identify Loki-owned User-PATH entries."""
    root = str(loki_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare loki-agent install dir.
    markers = [root + "\\loki-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if LOKI_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "loki".  We don't want to catch
    # unrelated entries like "cloki-foo" or "ephermeral", so we look for
    # backslash-loki as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(loki_home: Path) -> list[str]:
    """Strip Loki-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _loki_path_markers(loki_home)
            kept: list[str] = []
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its LOKI_HOME directory.

    We shell out to ``loki -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current LOKI_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m loki_cli.main` so we don't depend on a `loki`
    #    wrapper that may be half-removed mid-uninstall.
    loki_invocation = [_sys.executable, "-m", "loki_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                loki_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
...
def run_gui_uninstall(args):
    ...
    Mirrors ``loki uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$LOKI_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from loki_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    print(color("│         ⚕ Loki Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.loki/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.loki/ for future reinstall
    """
    project_root = get_project_root()
    loki_home = get_loki_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie LOKI_HOMEs
    ...
    # full wipe (code + ~/.loki data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Loki Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    # their alias wrappers, and wiping their LOKI_HOME dirs. Otherwise
...
def _perform_uninstall(
    *,
    ...
    ``loki`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$LOKI_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in loki_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\loki\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(loki_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Loki-owned PATH entries in User environment")

        log_info("Removing LOKI_HOME / LOKI_GIT_BASH_PATH User env vars...")
        removed_env = remove_loki_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Loki-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing loki command...")
    ...
    #     (only when they still point into this Loki home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Loki-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(loki_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Loki-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(loki_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE LOKI_HOME and
    ...
    try:
        ...
        gui_removed = uninstall_gui(loki_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.loki/, just remove the loki-agent subdir
            if loki_home in project_root.parents or project_root.parent == loki_home:
    ...
    #     under LOKI_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(loki_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(loki_home)
    ...
    if full_uninstall:
        ...
        #     alias wrapper. The profile LOKI_HOME dirs live under
        ...
        try:
            if loki_home.exists():
                shutil.rmtree(loki_home)
                log_success(f"Removed {loki_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {loki_home}: {e}")
            log_info("You may need to manually remove it")
    else:
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://loki-agent.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://loki-agent.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m loki_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import loki_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``loki_constants`` + ``loki_cli.colors``
    (and lazily ``loki_cli.gui_uninstall``), so it runs fine under a bare
