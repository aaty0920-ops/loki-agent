...
Toggles `model.openai_runtime` between "auto" (= chat_completions, Loki'
...
The actual runtime resolution happens in loki_cli.runtime_provider's
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in {"off", "default", "disable", "loki"}:
        return "auto", []
...
def apply(
    ...
    *,
    ...
    # the world state (managed block in ~/.codex/config.toml, loki-tools
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate Loki' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into Loki for
        ...
        try:
            ...
            # Tools/MCP servers (excluding the loki-tools callback,
            # which is internal plumbing — surface separately).
            user_servers = [
                s for s in mig_report.migrated if s != "loki-tools"
            ]
            if user_servers:
                msg_lines.append(
            ...
            # Permissions + Loki tool callback are always-on production
