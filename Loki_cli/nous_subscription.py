...
# coverage category (loki_cli.nous_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `loki tools` picker scope its entitlement gate to the selected backend, so a
...
def get_nous_subscription_features(
    ...
    *,
    ...
    # Tool Gateway via `loki model`, so direct credentials should NOT
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("LOKI_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("LOKI_LOCAL_STT_COMMAND"))
...
# ---------------------------------------------------------------------------
# Inline Nous Portal login for the Tool Gateway picker (`loki tools`)
# ---------------------------------------------------------------------------
...
def ensure_nous_portal_access(
    *,
    ...
    Used by ``loki tools`` when a user selects a Nous-managed Tool Gateway
    backend (e.g. "Firecrawl (Nous Portal)").  Unlike ``loki model``'s Nous
    login, this:
