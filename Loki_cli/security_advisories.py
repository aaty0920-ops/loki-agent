...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``loki doctor --ack <id>``; the ack is persisted to
...
1. ``loki doctor`` (and ``loki doctor --ack <id>``)
...
run in environments where the rest of Loki failed to import.
"""
...
def _installed_version(pkg_name: str) -> Optional[str]:
    ...
    except ImportError:  # py<3.8 — Loki requires 3.10+ but defensive.
        return None
    try:
...
# ``~/.loki/cache/advisory_banner_seen`` (a single line per advisory ID:
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active "
            ...
            f"Run `loki doctor` on the gateway host for details.")
