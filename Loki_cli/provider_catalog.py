...
The provider list shown by ``loki model`` (CLI/TUI) and the desktop Settings
→ Providers tabs (Accounts + API keys) **must be the same set**.  Historically
they were not: the CLI picker read :data:`loki_cli.models.CANONICAL_PROVIDERS`
...
the same universe ``loki model`` renders (``CANONICAL_PROVIDERS``), joining:

* ``auth_type`` / ``api_key_env_vars`` / ``base_url_env_var`` from
  :data:`loki_cli.auth.PROVIDER_REGISTRY` (credential truth), and
...
``CANONICAL_PROVIDERS`` universe, i.e. exactly what ``loki model`` shows.
"""
...
# loki_cli.auth.PROVIDER_REGISTRY and providers.base.ProviderProfile.
...
def provider_catalog() -> list[ProviderDescriptor]:
    """Return one descriptor per provider in the ``loki model`` universe.
    ...
    # Loki overlays carry auth_type for providers that have no registry/profile
    ...
    for order, entry in enumerate(CANONICAL_PROVIDERS):
        ...
        # Loki overlay (e.g. moa → "virtual"), then api_key.
        auth_type = (
