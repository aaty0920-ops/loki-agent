...
``<root>`` is the **shared Loki root** (the parent of any active
...
``loki -p <profile>`` joins the same board as the dispatcher that
...
the profile's Loki home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``LOKI_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``LOKI_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``loki kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.loki``. In Docker / custom
deployments where ``LOKI_HOME`` points outside ``~/.loki`` (e.g.
``/opt/loki``), ``<root>`` is ``LOKI_HOME``. Legacy env-var
overrides still work:

* ``LOKI_KANBAN_DB`` — pin the database file path directly.
* ``LOKI_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``LOKI_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``LOKI_KANBAN_DB``,
``LOKI_KANBAN_WORKSPACES_ROOT``, and ``LOKI_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/loki-kanban-v1-spec.pdf`` for the full
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    ``profile_name`` is resolved from the active LOKI_HOME so dispatcher- and
    worker-side hooks both carry the right profile without the caller plumbing
    it through.
    """
    try:
        from loki_cli.plugins import invoke_hook
...
# ``LOKI_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``LOKI_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("LOKI_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``LOKI_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("LOKI_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``LOKI_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "loki_kanban_current_board_override",
    default=None,
)
...
# enough that kebab-case names like ``atm10-server`` or ``loki-agent``
...
def kanban_home() -> Path:
    """Return the shared Loki root that anchors the kanban board.

    Resolution order:

    1. ``LOKI_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_loki_root()``, which already returns ``<root>``
       when ``LOKI_HOME`` is ``<root>/profiles/<name>``, and returns
       ``LOKI_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``LOKI_HOME`` would silently fork the board per profile,
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``loki kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``LOKI_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``loki kanban boards
    ...
    env = os.environ.get("LOKI_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``loki kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``LOKI_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("LOKI_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``LOKI_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``LOKI_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``loki kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``LOKI_SESSION_ID``. NULL for
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("LOKI_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``loki gateway run --replace`` /
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Loki treats as user-controlled-but-trusted on the
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``LOKI_KANBAN_DB`` env → ``LOKI_KANBAN_BOARD`` env →
    ...
    if resolved in _INITIALIZED_PATHS:
        ...
        try:
            ...
            with _INIT_LOCK:
                from loki_state import apply_wal_with_fallback
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See loki_state._WAL_INCOMPAT_MARKERS for detection logic.
                from loki_state import apply_wal_with_fallback
                apply_wal_with_fallback(conn, db_label=f"kanban.db ({path.name})")
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``loki kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``LOKI_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def create_task(
    ...
    *,
    ...
    each name to ``loki --skills ...``. Use this to pin a task to a
    ...
    # `loki --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `loki kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``loki kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``loki kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``loki kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``loki kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``LOKI_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Loki' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Loki-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``loki kanban show <id>`` and the dashboard.
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the Loki checkout). If no anchor is configured
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, Loki treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, Loki creates/reuses that linked worktree. With
      no ``workspace_path``, Loki anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, Loki uses
      ``wt/<task-id>``.
...
# without thrashing. Overridable via ``LOKI_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Loki
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``loki kanban tail``. The next dispatch
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``loki kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Loki profile.
    ...
    if not rows:
        return False
    try:
        from loki_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def _dispatch_once_locked(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from loki_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Loki profile.
        # `_default_spawn` invokes ``loki -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Loki
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `loki kanban tail` — without
...
def _module_loki_argv() -> list[str]:
    """Return the interpreter-bound Loki CLI invocation."""
    # ``loki_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``loki`` package — there is no
    # ``loki`` package to import.
...
def _loki_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Loki executable path.
...
def _resolve_loki_argv() -> list[str]:
    """Resolve the ``loki`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$LOKI_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("loki")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\loki.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m loki_cli.main`` — fallback for setups where
       Loki is launched from a venv and the ``loki`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_loki_bin`` for the same reason. Kept
    local (not imported from gateway) because ``loki_cli`` sits below
    ...
    loki_bin = _safe_which_no_cwd("loki") if _IS_WINDOWS else shutil.which("loki")
    if loki_bin:
...
def _resolve_worker_cli_toolsets(loki_home: Optional[str]) -> Optional[list[str]]:
    ...
    task-scoped kanban lifecycle tools when ``LOKI_KANBAN_TASK`` is set.
    """
    if not loki_home:
        return None
    try:
...
def _default_spawn(
    ...
    *,
    ...
    ``LOKI_KANBAN_DB`` / ``LOKI_KANBAN_BOARD`` / workspaces_root env
    ...
    # env, and when the child process starts `loki -p <name>` the
    # _apply_profile_override() runs *before* loki_constants is imported.
    # If LOKI_HOME is absent from the child's env, get_loki_home() falls
    # back to Path.home() / ".loki" (the DEFAULT profile root), ignoring the
    ...
    try:
        ...
        # _apply_profile_override() via LOKI_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # LOKI_HOME never had profiles created.
        pass
    if task.tenant:
    ...
    # that even when the worker activates a profile (`loki -p <name>`
    # rewrites LOKI_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_loki_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["LOKI_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["LOKI_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["LOKI_KANBAN_BOARD"] = resolved_board
    # LOKI_PROFILE is the author the kanban_comment tool defaults to.
    # `loki -p <assignee>` activates the profile, but the env var is
    ...
    cmd = [
        *_resolve_loki_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped LOKI_HOME above,
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("LOKI_HOME"))
    if worker_toolsets:
    ...
    # `loki kanban log` on a specific board reads its own file and
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``loki kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled LOKI_PROFILE values like "loki-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Loki root exists

    Reads profile paths directly so this module has no import dependency on
    ``loki_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from loki_constants import get_default_loki_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``loki kanban assignees`` for the terminal.
