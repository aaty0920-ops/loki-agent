...
Scans three surfaces a Loki user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Loki venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.loki/plugins``
...
def _discover_plugins(loki_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.loki/plugins``.
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, loki_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(loki_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, loki_home=home
    )
    if total == 0:
