...
Add, remove, or reorder entries here — both `loki setup` and
`loki` provider-selection will pick up the change automatically.
"""
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `loki model`
    flow without maintaining a separate static list.
    """
    from loki_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $LOKI_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Loki-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $LOKI_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Loki refreshes the cache from
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Loki that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Loki that doesn't ship
...
def check_nous_free_tier(*, force_fresh: bool = False) -> bool:
    ...
    try:
        from loki_cli.nous_account import get_nous_portal_account_info
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_loki_home() / "cache" / "nous_recommended_cache.json"
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    (``$LOKI_HOME/cache/nous_recommended_cache.json``) as last-known-good.
...
# this list:  loki model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `loki model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `loki model` TUI
...
#
# Some vendors expose several Loki provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `loki model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``loki model``, the
...
# path. The *interactive* default (GUI onboarding / ``loki model``) uses the
# richer free/paid-tier-aware resolver — see ``get_recommended_default_model``
# in loki_cli/web_server.py and ``partition_nous_models_by_tier`` — which can
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``loki auth add openai-codex`` without
    ``loki model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``loki model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    loki-agent is tool-calling-first — every provider path assumes the model
    ...
    *,
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — loki-agent
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from loki_cli.model_catalog import get_curated_nous_models
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    ...
    """
    try:
        from loki_cli.auth import resolve_nous_runtime_credentials
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``loki model``, ``/model``, etc.).
    """
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Loki' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``loki_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``loki auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.loki/.env``.
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Loki release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Loki release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        try:
            from loki_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
        ...
        # added Portal models still surface without a Loki release.
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``loki model`` shows.
...
#
# Cache strategy:
#   - One JSON file at $LOKI_HOME/provider_models_cache.json
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$LOKI_HOME/auth.json`` and external
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _LOKI_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Loki' default Claude IDs elsewhere use
    ...
    *,
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `loki moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Loki will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
    ...
    if api_models is not None:
        ...
        if requested_for_lookup in set(api_models):
            ...
            # fetch) so we only accept models Loki actually ships.  (#46850)
