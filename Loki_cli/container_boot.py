...
``$LOKI_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-loki-setup
(the stage2 hook) has chowned the volume and seeded $LOKI_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$LOKI_HOME``,
    not under ``profiles/``). The dispatcher in ``loki_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``loki gateway start`` (no ``-p``) targets. Without it,
    bare ``loki gateway start`` inside the container would land on
    ...
    ``$LOKI_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        loki_home: The container's LOKI_HOME (typically /opt/data).
            Profiles live under ``<loki_home>/profiles/<name>/``;
            the default profile lives at ``<loki_home>`` itself.
    ...
    # ``loki gateway start`` (no ``-p``) has somewhere to land;
    ...
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `loki profile create` (config.yaml
            # is not — that comes later via `loki setup`). Use it as the
            ...
            # via ``loki -p default-named gateway start`` if they
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    ...
    * **s6-overlay v2 / tini:** PID 1 argv is
      ``/init /opt/loki/docker/main-wrapper.sh <subcommand> [args...]``.
    * **s6-overlay v3:** PID 1 is ``s6-svscan`` and the command lives on the
      rc.init-launched process as ``/bin/sh -e
      /run/s6/basedir/scripts/rc.init top /opt/loki/docker/main-wrapper.sh
    ...
    ``loki`` invocations carry no wrapper, so fall back to peeling a bare
    ``init`` prefix. The wrapper re-execs ``loki <subcommand>``, so an
    explicit leading ``loki`` is peeled too. Shared by the legacy-gateway
    ...
    # The wrapper re-execs `loki <subcommand>`; peel an explicit loki.
    if args and Path(args[0]).name == "loki":
        args = args[1:]
    return args
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``loki dashboard ...``) never spawns or
    ...
    the gateway and dashboard containers share a bind-mounted LOKI_HOME,
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `loki -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit loki
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the loki user) won't hit EACCES. See
...
def _write_reconcile_log(
    loki_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $LOKI_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    # bind-mounted LOKI_HOME, both race to flock() the same s6-log lock
    ...
    actions = reconcile_profile_gateways(
        loki_home=loki_home, scandir=scandir,
    )
    for a in actions:
        print(
