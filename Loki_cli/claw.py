...
Usage:
    loki claw migrate              # Preview then migrate (always shows preview first)
    loki claw migrate --dry-run    # Preview only, no changes
    loki claw migrate --yes        # Skip confirmation prompt
    loki claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    loki claw migrate --no-backup  # Skip pre-migration snapshot
    loki claw cleanup              # Archive leftover OpenClaw directories
    loki claw cleanup --dry-run    # Preview what would be archived
"""
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a Loki gateway is running with connected platforms.
    ...
    print()
    print_error(
        "Loki gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
    ...
    print_info("Recommendation: stop the gateway first with 'loki gateway stop'.")
    print()
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Loki")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'loki claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → Loki migration."""
    ...
    # migrate-loki posture (two-phase: run once without secrets, rerun with
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: loki claw migrate --source /path/to/.openclaw")
        return
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: loki claw migrate --yes")
            return
        if not prompt_yes_no("Proceed with migration?", default=True):
            print_info("Migration cancelled.")
            return

    # ── Phase 2b: Pre-apply backup of the Loki home ─────────
    # Delegates to loki_cli.backup.create_pre_migration_backup(), which
    # shares implementation with the pre-update backup (same exclusion
    # rules, same SQLite safe-copy, zip format) so the archive is
    # restorable with `loki import`.  Mirrors OpenClaw's
    ...
    if not no_backup:
        try:
            from loki_cli.backup import create_pre_migration_backup, _format_size
            backup_archive = create_pre_migration_backup(loki_home=loki_home)
            if backup_archive:
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: loki claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: loki claw cleanup --yes")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  loki claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  loki claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  loki config set OPENROUTER_API_KEY sk-or-v1-...")
