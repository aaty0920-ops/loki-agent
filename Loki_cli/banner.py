...
Pure display functions with no LokiCLI state dependency.
"""
...
# (e.g. nix-built loki — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/NousResearch/loki-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/nousresearch/loki-agent"
...
def _fetch_pypi_latest(package: str = "loki-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    ...
    Two paths: if ``LOKI_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = loki_home / ".update_check"
    embedded_rev = os.environ.get("LOKI_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # LOKI_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `loki update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/loki/update/check`
    ...
    if embedded_rev:
        ...
        # $LOKI_HOME/loki-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = loki_home / "loki-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Loki git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$LOKI_HOME/loki-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``loki_cli/build_info.py``) and return it as a frozen
...
_RELEASE_URL_BASE = "https://github.com/NousResearch/loki-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Loki checkout. Cached per-process. Release URL always points at the
    canonical NousResearch/loki-agent repo (forks don't get a link).
    """
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else LOKI_CADUCEUS
    except Exception:
        _bskin = None
        _hero = LOKI_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    try:
        ...
        if behind is not None and behind != 0:
            ...
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built loki; we know an update
    ...
    # Pip-install warning — `pip install loki-agent` is not the supported
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else LOKI_AGENT_LOGO
        console.print(_logo)
        console.print()
