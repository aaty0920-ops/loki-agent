...
convention. State lives in the per-profile ``$LOKI_HOME/projects.db`` store
(see :mod:`loki_cli.projects_db`).
...
def projects_command(args: argparse.Namespace) -> int:
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: loki project <action> [options]\n"
                "Run 'loki project --help' for the full list.",
                file=sys.stderr,
            )
        return 0
...
def _cmd_list(args: argparse.Namespace) -> int:
    ...
    if not projs:
        print("No projects yet. Create one with `loki project create <name>`.")
        return 0
    for p in projs:
...
def _cmd_set_primary(args, conn, proj) -> int:
    if not pdb.set_primary(conn, proj.id, args.path):
        print(
            ...
            f"add it first with `loki project add-folder`.",
            file=sys.stderr,
        )
        return 1
