...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with loki-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.loki/plugins/<name>/``
3. **Project plugins** – ``./.loki/plugins/<name>/`` (opt-in via
   ``LOKI_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``loki_agent.plugins``
   entry-point group.
...
from loki_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE


def get_bundled_plugins_dir() -> Path:
    """Locate the bundled ``plugins/`` directory.

    Honours ``LOKI_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
...
# ---------------------------------------------------------------------------
#
# Set ``LOKI_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.loki/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("LOKI_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When LOKI_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Loki loggers.
    """
    ...
    logger.debug(
        "LOKI_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. Loki' shipped guidance lives in the evidence-based
    ...
    # Kanban task lifecycle hooks. Fired by loki_cli.kanban_db when a task
    ...
    # separate `loki -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `loki kanban dispatch`),
...
class PluginManifest:
    ...
    #              in ~/.loki/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``loki plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        """Return the active Loki profile name (e.g. ``"default"``).

        Derived from ``LOKI_HOME`` via
        :func:`loki_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.loki/profiles/<name>``, or ``"custom"`` when
        ``LOKI_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Loki core) are trusted by default —
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``loki honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``loki <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`loki_cli.dashboard_auth.DashboardAuthProvider`. Used by
    ...
    def register_slack_action_handler(
        ...
        Loki' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``loki model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: loki_cli.main imports plugins indirectly
        from loki_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.loki/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        if self._discovered and not force:
            return
        # Safe mode (--safe-mode / LOKI_SAFE_MODE=1): troubleshooting run
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to loki_cli/. Two layouts are
        ...
        # 3. Project plugins (./.loki/plugins/)
        if _env_enabled("LOKI_ENABLE_PROJECT_PLUGINS"):
            ...
            logger.debug(
                "Project plugins disabled (set LOKI_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with loki and must
            ...
            # all ~20 of them added several seconds to every `loki`
            # invocation — including plain `loki chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform Loki ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `loki plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``loki plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`loki plugins
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `loki plugins list`.
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``loki_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``loki_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="loki-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``loki tools`` TUI so plugin-provided toolsets appear
