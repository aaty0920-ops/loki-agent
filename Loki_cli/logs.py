...
Usage examples::
    ...
    loki logs                    # last 50 lines of agent.log
    loki logs -f                 # follow agent.log in real time
    loki logs errors             # last 50 lines of errors.log
    loki logs gateway -n 100    # last 100 lines of gateway.log
    loki logs gui -f            # follow gui.log (dashboard/pty/ws)
    loki logs desktop -f        # follow desktop.log (Electron app boot/backend)
    loki logs --level WARNING    # only WARNING+ lines
    loki logs --session abc123   # filter by session ID substring
    loki logs --component tools  # only tool-related lines
    loki logs --since 1h         # lines from the last hour
    loki logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    if not log_path.exists():
        ...
        print(f"(Logs are created when Loki runs — try 'loki chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_loki_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_loki_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
