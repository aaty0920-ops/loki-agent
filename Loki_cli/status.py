...
Shows the status of all Loki Agent components.
"""
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``loki config``'s
...
def show_status(args):
    ...
    print(color("│                 ⚕ Loki Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: loki portal)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: loki model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: loki auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: loki auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: loki model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from loki_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    print(color("  Run 'loki doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'loki setup' to configure", Colors.DIM))
    print()
