...
def _preserve_loki_home_path(path: str | Path) -> str:
    """Render Loki-owned paths under the configured LOKI_HOME spelling.

    Windows installs may keep ``%LOCALAPPDATA%\\loki`` as a symlink/junction to
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "loki_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("LOKI_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("LOKI_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("LOKI_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["LOKI_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["LOKI_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["LOKI_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("LOKI_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("LOKI_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("LOKI_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_name() -> str:
    ...
    Default profile: ``Loki_Gateway``
    Named profile X: ``Loki_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during loki_cli boot.
    from loki_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\loki\\gateway-service\\<task_name>.cmd``
    (or ``<LOKI_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Loki installs stay self-contained).
    """
    _assert_windows()
    from loki_cli.config import get_loki_home

    script_dir = Path(get_loki_home()) / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``LOKI_HOME`` whenever it
    ...
    source checkout only if ``LOKI_HOME`` cannot be used yet. Preserve the
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports LOKI_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``pythonw -m loki_cli.main [--profile X] gateway run``
    ...
    lines.append(f'set "LOKI_HOME={loki_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "LOKI_GATEWAY_DETACHED=1"')
    pythonw_path, venv_dir, extra_pythonpath = _resolve_detached_python(python_path)
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports loki_constants-based logic during startup.
    lines.append(f'set "VIRTUAL_ENV={_preserve_loki_home_path(venv_dir)}"')
    pythonpath_entries = [
    ...
    prog_args = [pythonw_path, "-m", "loki_cli.main"]
    if profile_arg:
...
def _build_gateway_vbs_script(
    ...
    repo_root = _preserve_loki_home_path(Path(__file__).resolve().parent.parent)
    static_pythonpath = os.pathsep.join(
        [repo_root, *[_preserve_loki_home_path(entry) for entry in extra_pythonpath]]
    )
    ...
    lines = [
        ...
        f"env.Item({_quote_vbs_string('LOKI_HOME')}) = {_quote_vbs_string(loki_home)}",
        f"env.Item({_quote_vbs_string('PYTHONIOENCODING')}) = {_quote_vbs_string('utf-8')}",
        f"env.Item({_quote_vbs_string('LOKI_GATEWAY_DETACHED')}) = {_quote_vbs_string('1')}",
        f"env.Item({_quote_vbs_string('VIRTUAL_ENV')}) = {_quote_vbs_string(_preserve_loki_home_path(venv_dir))}",
...
def _write_task_script() -> Path:
    ...
    python_path = _preserve_loki_home_path(get_python_path())
    working_dir = _stable_gateway_working_dir(PROJECT_ROOT)
    loki_home = str(Path(get_loki_home()))
    profile_arg = _profile_arg(loki_home)

    content = _build_gateway_cmd_script(python_path, working_dir, loki_home, profile_arg)
    ...
    vbs_content = _build_gateway_vbs_script(python_path, working_dir, loki_home, profile_arg)
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Loki builds and failed
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    project_root = _preserve_loki_home_path(PROJECT_ROOT)
    working_dir = _stable_gateway_working_dir(PROJECT_ROOT)
    loki_home = str(Path(get_loki_home()))
    profile_arg = _profile_arg(loki_home)
    ...
    env_overlay = {
        ...
        "VIRTUAL_ENV": _preserve_loki_home_path(venv_dir),
    }
    _prepend_pythonpath(
        env_overlay,
        [project_root, *[_preserve_loki_home_path(entry) for entry in extra_pythonpath]]
        if extra_pythonpath
        else [project_root],
    )
...
def windowless_gateway_restart_spec(
    ...
    ``loki_cli`` package without the venv launcher's site config.

    Returns ``(new_argv, working_dir, env_overlay)``.  ``new_argv``
    preserves every argument after the interpreter (``-m loki_cli.main
    ...
    try:
        loki_home = str(Path(get_loki_home()).resolve())
    except Exception:
        loki_home = ""
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m loki_cli.main gateway run``
    ...
    log_dir = Path(get_loki_home()) / "logs"
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("LOKI_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("LOKI_GATEWAY_INSTALL_START_ON_LOGIN")
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `loki -p <profile> gateway install` must be safe.
    ...
    from loki_cli.gateway import find_gateway_pids, _profile_arg
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from loki_cli.config import get_loki_home
        print(f"    type {Path(get_loki_home())}\\logs\\gateway.log")
        print(f"    type {Path(get_loki_home())}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from loki_cli.config import get_loki_home

    loki_home = Path(get_loki_home())
    print()
    print("Next steps:")
    print("  loki gateway status                      # Check status")
    print(f"  type {loki_home}\\logs\\gateway.log       # View logs")
