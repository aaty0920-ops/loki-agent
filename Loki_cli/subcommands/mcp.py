...
Extracted from ``loki_cli/main.py:main()`` (god-file Phase 2 follow-up).
...
def build_mcp_parser(subparsers, *, cmd_mcp: Callable) -> None:
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Loki as an MCP server",
        description=(
            "Manage MCP server connections and run Loki as an MCP server.\n\n"
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Loki as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `loki mcp add ...` to fall through to
    # interactive chat.
    ...
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `loki mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
    ...
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `loki mcp install n8n`)",
    )
