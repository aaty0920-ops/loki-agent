...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``loki login`` was removed in favor of ``loki auth`` / ``loki model``
    (the runtime handler in ``loki_cli/auth.py::login_command`` just prints a
    deprecation message and exits).  The subparser is kept registered so that
    old scripts/aliases invoking ``loki login [--flags]`` still receive the
    ...
    - The subparser is registered WITHOUT a ``help=`` kwarg so the row is
      omitted from ``loki --help`` (argparse only lists subcommands that
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ``loki login --provider anthropic`` reaches the deprecation handler and
      gets pointed at ``loki model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `loki auth` to manage credentials, "
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored — see `loki model`",
    )
    login_parser.add_argument(
