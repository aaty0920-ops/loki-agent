...
``loki_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `loki dashboard` / `loki serve` cmdlines and SIGTERM them directly —
    # the same path `loki update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Loki web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Loki web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    # Backward-compat shim: older Loki desktop app shells (<= 0.15.x) spawn the
    # backend as `loki dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Loki couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Loki Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        ...
        description=(
            "Run the Loki backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # `loki dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.loki/.env.
    # Nested subparser so bare `loki dashboard` keeps launching the server
