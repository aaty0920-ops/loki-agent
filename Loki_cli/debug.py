...
Currently supports:
    loki debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.loki/logs/*.log`` are not leaked into
...
def _pending_file() -> Path:
    """Path to ``~/.loki/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``loki debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``loki debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "loki-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``loki debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.loki/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``loki debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "loki-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``loki dump`` is run
        internally.
...
_NOUS_BUNDLE_FORMAT = "loki-debug-share/1"
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``loki debug share`` (CLI) and the
...
def run_debug_share(args):
    ...
    print(f"To delete now:  loki debug delete <url>")
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``loki debug share --nous``: upload the bundle to Nous-S3.
    ...
    suggests ``--local``.
    """
    from loki_cli.diagnostics_upload import share_to_nous
    ...
    if redact:
        logger.info(
            "loki debug share --nous: applied force-mode redaction before upload"
        )
...
def run_debug_delete(args):
    ...
    if not urls:
        ...
        print("  Deletes paste.rs pastes uploaded by 'loki debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``loki debug`` call.
    ...
    # best-effort — any failure is swallowed so ``loki debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: loki debug <command>")
        print()
        print("Commands:")
