...
- ``loki_cli.providers``        -- canonical provider identity + overlays
- ``loki_cli.model_normalize``  -- per-provider name formatting
...
_LOKI_MODEL_WARNING = (
    "Nous Research Loki 3 & 4 models are NOT agentic and are not designed "
    "for use with Loki Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Loki 3 / Loki 4 chat families.
# The previous substring check (`"loki" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``loki-brain:qwen3-14b-ctx16k`` that just
# happen to carry "loki" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Loki-3-Llama-3.1-70B, loki-4-405b, openrouter/loki3:70b
# Negative examples it must NOT match:
#   loki-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_LOKI_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])loki[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_loki_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Loki 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_LOKI_NON_AGENTIC_RE.search(model_name))


def _check_loki_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Loki 3/4 chat model."""
    if is_nous_loki_non_agentic(model_name):
        return _LOKI_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``loki config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from loki_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'loki model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'loki doctor' — config issues detected:"
        ...
        from loki_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from loki_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from loki_cli.providers import resolve_user_provider as _ruser
    ...
    # loki_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    loki_warn = _check_loki_model_warning(new_model)
    if loki_warn:
        warnings.append(loki_warn)
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from loki_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from loki_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by loki provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://loki-agent.nousresearch.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Loki release. Falls back to the in-repo
    ...
    if "lmstudio" not in curated and (
        ...
        from loki_cli.models import fetch_lmstudio_models
    ...
    for loki_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(loki_id)
        if (
            _alias_target
            and _alias_target != loki_id
        ...
        # section 2 (LOKI_OVERLAYS) with proper auth store checking.
        ...
        # /model picker sees the SAME list `loki model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(loki_id)
        if not model_ids:
            model_ids = curated.get(loki_id, [])
            if loki_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(loki_id, model_ids)
        total = len(model_ids)
        if loki_id in _UNCAPPED_PICKER_PROVIDERS:
    ...
    # --- 2. Check Loki-only providers (nous, openai-codex, copilot, opencode-go) ---
    from loki_cli.providers import LOKI_OVERLAYS
    from loki_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Loki provider ID.
    # LOKI_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Loki IDs ("copilot").
    _mdev_to_loki = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in LOKI_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Loki slug — e.g. "github-copilot" → "copilot"
        loki_slug = _mdev_to_loki.get(pid, pid)
        if loki_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(loki_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, loki_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or loki_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds and loki_slug == "anthropic":
            try:
                ...
                if (loki_creds and loki_creds.get("accessToken")) or \
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                _ids = cached_provider_model_ids(loki_slug)
                model_ids = _ids if _ids else (curated.get(loki_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(loki_slug, []) or curated.get(pid, [])
        elif loki_slug == "nous":
            # Nous serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `loki model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from loki_cli.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(loki_slug, []) or curated.get(pid, [])
                if loki_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(loki_slug, model_ids)
        total = len(model_ids)
        if loki_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": loki_slug,
            "name": get_label(loki_slug),
            "is_current": loki_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "loki",
        ...
        seen_slugs.add(loki_slug.lower())
        _record_builtin_endpoint(loki_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or LOKI_OVERLAYS (keeps /model in sync
    # with `loki model`).
    try:
        from loki_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Loki's canonical write key (matches
            ...
            # Loki writes ``models:`` as a dict keyed by model id
            # (see loki_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Loki's own writer uses; a hyphen variant is accepted
            ...
            # active model. Loki's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, loki_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`loki_cli.models.fetch_openrouter_models`, which filters the
