...
`loki backup` creates a zip archive of the entire ~/.loki/ directory
(excluding the loki-agent repo and transient files).

`loki import` restores from a backup zip, overlaying onto the current
LOKI_HOME root.
"""
...
from loki_constants import get_default_loki_root, get_loki_home, display_loki_home
...
# ``loki-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/loki-agent/``
...
# LOKI_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    ...
    # Python dependency trees (plugin / MCP-server venvs under LOKI_HOME) —
...
# File names that ``loki import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE LOKI_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside LOKI_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    ...
    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``loki-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/loki-agent/`` — must be preserved.
        if part == "loki-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside LOKI_HOME.
...
def run_backup(args) -> None:
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"loki-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"loki-backup-{stamp}.zip"
    ...
    for dirpath, dirnames, filenames in os.walk(loki_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(loki_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``loki-agent`` is only pruned at the root level; nested dirs
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(loki_root)
    ...
    # outside LOKI_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_loki_home()}."
        )
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    ...
    # Look for telltale files that a loki home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Loki backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.loki/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a loki dir name
        if prefix in {".loki", "loki"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_loki_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (loki_root / "config.yaml").exists()
        has_env = (loki_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Loki configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under LOKI_HOME.
            ...
            try:
                target.resolve().relative_to(loki_root.resolve())
            except ValueError:
        ...
        if profiles_dir.is_dir():
            try:
                ...
                # loki_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: loki profile list  (after installing loki)")

        # Guidance
        print()
        if not (loki_root / "loki-agent").is_dir():
            print("Note: The loki-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: loki update")
...
# Critical state files to include in quick snapshots (relative to LOKI_HOME).
...
# — `loki update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $LOKI_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside LOKI_HOME and the entry is silently
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(loki_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside LOKI_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    Args:
        ...
        loki_home: Override for the Loki home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(loki_home), keep=keep)
...
def run_quick_backup(args) -> None:
    ...
    if snap_id:
        ...
        print(f"  {len(snaps)} snapshot(s) stored in {display_loki_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, loki_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``loki_root`` to ``out_path``.
...
def _pre_update_backup_dir(loki_home: Optional[Path] = None) -> Path:
    home = loki_home or get_loki_home()
...
def create_pre_update_backup(
    ...
    result = _write_full_zip_backup(out_path, loki_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `loki claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of LOKI_HOME under ``backups/`` before a
    ``loki claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``loki import <archive>``.  Writes to
    ``<LOKI_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `loki import` and the
