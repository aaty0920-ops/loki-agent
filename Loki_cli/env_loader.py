...
# load_loki_dotenv() calls (user env + project env, gateway hot-reload,
# tests) don't spam the same warning multiple times.
_WARNED_KEYS: set[str] = set()

# Map of env-var name → source label ("bitwarden", etc.) for credentials
# that were injected by an external secret source during load_loki_dotenv().
# Used by setup / `loki model` flows to label detected credentials so
# users understand WHERE a key came from when their .env doesn't contain it
# directly (otherwise the "credentials detected ✓" line looks identical to
# the .env case and they don't know Bitwarden is wired up).
_SECRET_SOURCES: dict[str, str] = {}

# LOKI_HOME paths we've already pulled external secrets for during this
# process.  ``load_loki_dotenv()`` is called at module-import time from
# several hot modules (cli.py, loki_cli/main.py, run_agent.py,
...
def get_secret_source(env_var: str) -> str | None:
    ...
    during the current process's ``load_loki_dotenv()`` call.  Returns
...
def reset_secret_source_cache() -> None:
    """Forget which LOKI_HOME paths have already had external secrets applied.
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    We delegate to ``loki_cli.config._sanitize_env_lines`` which
    already knows all valid Loki env-var names and can split
...
def load_loki_dotenv(
    *,
    ...
    """Load Loki environment files with user config taking precedence.

    Behavior:
    - `~/.loki/.env` overrides stale shell-exported values when present.
...
def _apply_managed_env() -> None:
    ...
    Managed scope is machine-global (independent of LOKI_HOME / profile). v1
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    locate the access token) but BEFORE the rest of Loki reads
    ``os.environ`` for credentials.  Any failure here is logged and
    swallowed — external secret sources must never block startup.

    Idempotent within a process: subsequent calls for the same
    ``home_path`` are no-ops.  ``load_loki_dotenv()`` runs at import
    time from several hot modules (cli.py, loki_cli/main.py,
    run_agent.py, trajectory_compressor.py, ...), so without this guard
    the Bitwarden status line would print 3-5x per CLI startup.  Use
    ``reset_secret_source_cache()`` if you need to force a re-pull
    (tests, future ``loki secrets bitwarden sync`` from a long-running
    process).
    """
    ...
    if result.applied:
        ...
        # edited .env (see #6843).
        _sanitize_loaded_credentials()
        # Remember where these came from so the setup / `loki model`
