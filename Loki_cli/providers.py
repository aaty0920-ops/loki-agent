...
2. **Loki overlays** — transport type, auth patterns, aggregator flags,
...
# -- Loki overlay ----------------------------------------------------------
# Loki-specific metadata that models.dev doesn't provide.
...
LOKI_OVERLAYS: Dict[str, LokiOverlay] = {
    ...
    "openai-codex": LokiOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": LokiOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": LokiOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": LokiOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="LOKI_QWEN_BASE_URL",
    ),
    "lmstudio": LokiOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": LokiOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": LokiOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": LokiOverlay(
    ...
    "minimax-oauth": LokiOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": LokiOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": LokiOverlay(
        transport="openai_chat",
    ...
    "alibaba-coding-plan": LokiOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "opencode": LokiOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": LokiOverlay(
        transport="openai_chat",
        is_aggregator=True,
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Loki overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Loki overlay
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + loki extra
    ...
    if overlay is not None:
        # Loki-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in LOKI_OVERLAYS
    if provider == "bedrock":
