...
the tool registry or launching Loki (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``loki-cli`` to the non-existent ``loki``. ``resolve_toolset('loki')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``loki-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``loki`` instead of ``loki-cli``),
