...
Outputs a compact, plain-text summary of the user's Loki setup
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.loki/.env.
    ...
    exports. ``loki debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.loki_build_sha`` by the Dockerfile's
    ``LOKI_GIT_SHA`` build-arg (see ``loki_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``loki dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def run_dump(args):
    ...
    lines.append(f"loki_home:      {display_loki_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.loki/.env: a managed
        ...
        # A credential added via `loki auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `loki auth list` shows it (#42130).
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(loki_home)}")
    lines.append(f"  skills:             {_count_skills(loki_home)}")
