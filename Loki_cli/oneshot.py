...
configured for "cli" in `loki tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (LOKI_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `loki chat`:
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to LOKI_INFERENCE_MODEL
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``loki -z`` / oneshot mode.

    Oneshot bypasses ``LokiCLI._init_agent()``, so it must wire the SQLite
...
def _run_agent(
    ...
    # Imports are local so they don't run when loki is invoked for
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("LOKI_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    agent = AIAgent(
        ...
        #                LOKI_INTERACTIVE which we never set
        #   - shell-hook approval → auto-approved via LOKI_ACCEPT_HOOKS=1
        #                (set above); also falls back to deny on non-tty
        #   - dangerous-command approval → bypassed via LOKI_YOLO_MODE=1
