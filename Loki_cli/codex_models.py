...
DEFAULT_CODEX_MODELS: List[str] = [
    ...
    # in loki_cli/models.py — only the openai-codex (OAuth) provider
    ...
    # Loki uses. Removed here. If OpenAI re-enables them on Codex backend,
...
def _read_cache_models(codex_home: Path) -> List[str]:
    ...
    if isinstance(entries, list):
        for item in entries:
            ...
            # public OpenAI API, while Loki openai-codex talks to the same
