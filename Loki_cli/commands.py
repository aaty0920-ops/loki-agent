"""Slash command definitions and autocomplete for the Loki CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("busy", "Control what Enter does while Loki is working", "Configuration",
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`loki_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/loki`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. Loki ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Loki
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``loki skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``loki skills list`` and
...
# unrelated command landed. These claim their slots right after /loki,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /loki <command>).
...
# native slot, the alias spelling stays reachable via /loki reset).
...
# ``/loki <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/loki" decision, not a silent clamp.
#   - credits: the billing/top-up surface; reached via /loki credits on Slack.
#   - billing: the terminal-billing surface (buy/auto-reload/limit); /loki billing.
#   - moa: high-cost slash mode, available through /loki moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /loki debug on Slack.
_SLACK_VIA_LOKI_ONLY = frozenset({"credits", "billing", "moa", "debug"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/loki <verb>`` subcommand.
    ...
    can still reach them via ``/loki <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/loki`` is always reserved as the first entry so the
    legacy ``/loki <subcommand>`` form keeps working for anything that
    ...
    # Reserve /loki as the catch-all top-level command.
    entries.append(("loki", "Talk to Loki or run a subcommand", "[subcommand] [args]"))
    seen.add("loki")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_LOKI_ONLY:
            # Intentionally Slack-via-/loki only (see _SLACK_VIA_LOKI_ONLY).
            return
    ...
    # everything except /loki, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://loki-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /loki handler.

    Maps both canonical names and aliases so /loki bg do stuff works
    the same as /loki background do stuff.

    Plugin-registered slash commands are included so ``/loki <plugin-cmd>``
