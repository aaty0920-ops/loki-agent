...
# display_loki_home is imported lazily at call sites to avoid ImportError
# when loki_constants is cached from a pre-update version during `loki update`.
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "loki CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``loki gateway status`` from falsely counting the ``loki`` CLI that
...
def _scan_gateway_pids(
    ...
    # scan (e.g. ``loki gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        # the command doesn't advertise *some other* profile. LOKI_HOME
        ...
        # LOKI_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "loki_home=" in command_lc
            and f"loki_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``loki update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Loki profile are returned.
    """
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m loki_cli.main gateway
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose LOKI_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``loki update`` users
    # with no running gateway until they re-invoke ``loki gateway``
    ...
    if sys.platform == "win32":
        try:
            from loki_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / LOKI_HOME) the windowless
        # base interpreter needs to import loki_cli.  Empty on POSIX, where
...
def _sync_loki_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``LOKI_HOME``.

    Under ``sudo``, ``LOKI_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_loki_home` falls back to ``/root/.loki`` — the wrong
    profile. The unit file pins ``LOKI_HOME`` for the actual gateway
    ...
    unit_home = env.get("LOKI_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("LOKI_HOME", "").strip()
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}loki gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}loki gateway restart{scope_flag}"
    )
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``loki gateway status`` output so users with
...
def _reap_unsupervised_gateway_orphans() -> bool:
    ...
    in-process under a ``gateway restart`` argv (loki_cli/gateway.py restart
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (LOKI_HOME-scoped).
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Loki Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``loki gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``LOKI_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("LOKI_GATEWAY_DETACHED", "").strip().lower()
...
def _profile_suffix() -> str:
    """Derive a service-name suffix from the current LOKI_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (LOKI_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    ...
    default = get_default_loki_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary LOKI_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(loki_home: str | None = None, default_root: str | Path | None = None) -> str:
    """Return ``--profile <name>`` only when LOKI_HOME is a named profile.

    For ``~/.loki/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        loki_home: Optional explicit LOKI_HOME path. Defaults to the current
            ``get_loki_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
        default_root: Optional Loki root to compare against. Used when
            generating a system service for another user from a sudo/root
            process, where ``Path.home()`` and ``get_default_loki_root()``
...
def _profile_arg_for_target_user(loki_home: str, target_home_dir: str) -> str:
    """Return the profile arg for a system service running as another user."""
    target_root = Path(target_home_dir) / ".loki"
    try:
        Path(loki_home).resolve().relative_to(target_root.resolve())
        return _profile_arg(loki_home, default_root=target_root)
    except ValueError:
        return _profile_arg(loki_home)


def get_service_name() -> str:
    """Derive a systemd service name scoped to this LOKI_HOME.

    Default ``~/.loki`` returns ``loki-gateway`` (backward compatible).
    Profile ``~/.loki/profiles/coder`` returns ``loki-gateway-coder``.
    Any other LOKI_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Loki installs that predate the
# loki-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (loki-gateway-*.service) and unrelated third-party
# "loki" units are never matched.
...
def _find_legacy_loki_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Loki gateway units.

    Detects unit files installed by older Loki versions that used a
    different service name (e.g. ``loki.service`` before the rename to
    ``loki-gateway.service``). When both a legacy unit and the current
    ``loki-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``loki-gateway-coder.service`` and unrelated third-party
      ``loki-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``loki.service`` running an unrelated
...
def print_legacy_unit_warning() -> None:
    """Warn about legacy Loki gateway unit files if any are installed.
...
def remove_legacy_loki_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Loki gateway unit files.

    Iterates over whatever ``_find_legacy_loki_units()`` returns — which is
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Loki often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.loki`` → ``ai.loki.gateway.plist`` (backward compatible).
    Profile ``~/.loki/profiles/coder`` → ``ai.loki.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.loki/loki-agent  -> /home/alice/.loki/loki-agent
      /opt/loki                 -> /opt/loki  (kept as-is)
...
def _loki_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current LOKI_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_loki_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.loki                    → /home/alice/.loki
      /root/.loki/profiles/coder     → /home/alice/.loki/profiles/coder
      /opt/custom-loki               → /opt/custom-loki  (kept as-is)
    """
    current_loki = get_loki_home().resolve()
    current_default = (Path.home() / ".loki").resolve()
    target_default = Path(target_home_dir) / ".loki"

    # Default ~/.loki → remap to target user's default
    if current_loki == current_default:
        return str(target_default)

    # Profile or subdir of ~/.loki → preserve the relative structure
    try:
        relative = current_loki.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.loki) — keep as-is
        return str(current_loki)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    loki_node = loki_home / "node" / "bin"
    if _is_dir(loki_node):
        candidates.append(str(loki_node))
    loki_nm = loki_home / "node_modules" / ".bin"
    if _is_dir(loki_nm):
        candidates.append(str(loki_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m loki_cli.main``, so module
    ...
    dir, or a clone that ``loki update`` later relocates/removes — the path
    ...
    ``LOKI_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if LOKI_HOME
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        profile_arg = _profile_arg_for_target_user(loki_home, home_dir)
        # Remap all paths that may resolve under the calling user's home
        # (e.g. /root/) to the target user's home so the service can
        # actually access them.
        python_path = _remap_path_for_user(python_path, home_dir)
        # Anchor cwd to the target user's LOKI_HOME (stable, always exists)
...
User={username}
Group={group_name}
ExecStart={python_path} -m loki_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir LOKI_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved LOKI_HOME
    ...
    exported a throwaway ``LOKI_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``loki gateway restart``
    with ``LOKI_HOME=/tmp/loki-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="LOKI_HOME=..."`` lines and launchd
    ``<key>LOKI_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile
    ...
    candidates += re.findall(
        r"<key>LOKI_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    ...
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: LOKI_HOME resolves to a "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only LOKI_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp LOKI_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../loki_test`` path into
    # ``Environment="LOKI_HOME=..."``. Writing that to the developer's
    ...
    if not system and (
        ...
        or '/loki_test"' in new_unit
        or "/loki_test/" in new_unit
    ):
        return False

    # Structural variant of the same belt: refuse to bake ANY temp-dir
    # LOKI_HOME into the unit (manual E2E homes like /tmp/loki-e2e-NNN
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Loki install"
    )
    return True
...
def systemd_install(
    ...
    # Offer to remove legacy units (loki.service from pre-rename installs)
    # before installing the new loki-gateway.service. If both remain, they
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}loki gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}loki gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}loki gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}loki gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}loki gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}loki gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}loki gateway start{scope_flag}"
        )
...
# process invocation (each ``loki gateway start/stop/status`` call).
...
#      background process (the `nohup loki gateway run` workaround). See #23387.
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m loki_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active LOKI_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "loki_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup loki gateway run --replace`
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(
        f"  Try manually: nohup loki gateway run --replace "
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if (
        ...
        # to ~/.loki/logs/launchd-reload.log, which the health watchdog
        # can tail to detect a persistent orphan. See loki-restart
        # rootcause handoff (2026-06-26 incident).
        reload_log_path = get_loki_home() / "logs" / "launchd-reload.log"
        try:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Loki install"
    )
    return True
...
def launchd_stop():
    ...
    # `loki gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    # Loki PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the Loki PID file).
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``LOKI_S6_SUPERVISED_CHILD``.
    """
    if os.environ.get("INVOCATION_ID"):
        return True
    if os.environ.get("LOKI_S6_SUPERVISED_CHILD"):
        return True
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``loki gateway run`` is always a misconfiguration, so we
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``loki gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``loki gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other LOKI_HOME roots.
    """
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("LOKI_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Loki gateway as root inside the official Docker image."
    )
    print(
    ...
    print(
        "  Set LOKI_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `loki gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set LOKI_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `loki gateway restart` which already
    ...
    # `loki gateway start/restart` — leaving the gateway vulnerable to
    ...
    # LOKI_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Channel ID where Loki delivers cron results and notifications.",
            },
            {
...
def _all_platforms() -> list[dict]:
    ...
    ``loki setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run loki under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.loki/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_loki_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: loki pairing approve <platform> <code>"
                    )
                elif is_email:
                    print_success("  Unknown email senders will be ignored.")
                else:
                    print_info(
                        "  Skipped — configure later with 'loki gateway setup'"
                    )
            continue
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Loki will open Tencent iLink QR login in this terminal.")
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `loki pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "LokiAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``loki_cli.setup`` (which
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.loki/plugins/
        ...
        if service_running:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    ...
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo loki gateway install --system"
                        )
                    print_info("  Or run in foreground:  loki gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: loki gateway run")
                print_info(
                ...
                print_info("  Run in foreground: loki gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup loki gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`loki_cli.service_manager` are caught and surfaced as
    ...
    if profile is None:
        ...
        # LOKI_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``loki gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``loki gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``loki gateway run`` are unaffected.
      2. ``LOKI_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``loki gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``LOKI_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    # `docker logs` and ${LOKI_HOME}/logs/gateways/<profile>/current,
    ...
    #
    # Prefer `sleep infinity` (matches the static main-loki service's
    # pattern in docker/s6-rc.d/main-loki/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: loki gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  loki profile create <name>     # creates the s6 service slot")
                print("  loki -p <name> gateway start   # bring it up via s6")
                print("  loki status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: loki gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: loki gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: loki gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  loki profile delete <name>     # tears down the s6 service slot")
                print("  loki -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `loki -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: loki gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_LOKI_GATEWAY") == "1":
            print_error(
                "Refusing to stop the gateway from inside the gateway process.\n"
                "This command was blocked to prevent restart loops.\n"
                "Use `loki gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_LOKI_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `loki gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: loki gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  loki gateway install")
                else:
                    print("To install as a service:")
                    print("  loki gateway install")
                    print("  sudo loki gateway install --system")
            else:
                ...
                if is_termux():
                    ...
                    print(
                        "  loki gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  loki gateway install  # Install as user service")
                    print(
        ...
        # Stop, disable, and remove legacy Loki gateway unit files from
        # pre-rename installs (e.g. loki.service). Profile units and
        # unrelated third-party services are never touched.
