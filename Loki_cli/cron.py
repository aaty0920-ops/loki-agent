...
# commands at execution time when ``_LOKI_GATEWAY=1``.
...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     Start it with: loki gateway install", Colors.DIM))
    print(color("                    sudo loki gateway install --system  # Linux servers", Colors.DIM))
    print(color("     Check status:  loki cron status", Colors.DIM))
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'loki cron create ...' or the /cron command in chat.", Colors.DIM))
        return
...
def cron_status():
    ...
    if pids:
        ...
        print("    loki gateway install    # Install as a user service")
        print("    sudo loki gateway install --system  # Linux servers: boot-time system service")
        print("    loki gateway            # Or run in foreground")
...
def cron_command(args):
    ...
    print("Usage: loki cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
