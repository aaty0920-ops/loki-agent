"""
Loki Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m loki_cli.main web          # Start on http://127.0.0.1:9119
    python -m loki_cli.main web --port 8080
"""
...
from loki_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
from loki_cli.memory_providers import (
    MemoryProvider,
    ProviderField,
    get_memory_provider,
)
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``loki gateway run`` — but the
    desktop app spawns a ``loki dashboard`` backend, not a gateway, so a cron
    ...
    real gateway on the same LOKI_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
def _warm_gateway_module() -> None:
    try:
        import loki_cli.gateway  # noqa: F401
    except Exception:
        pass
...
async def _lifespan(app: "FastAPI"):
    ...
    # Fire loki_cli.gateway import into a background thread so the event
    # loop is not blocked and LOKI_DASHBOARD_READY fires without delay.
...
# LOKI_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("LOKI_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
...
#
# This list is defined in ``loki_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from loki_cli.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Loki-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    unauthenticated-public-dashboard hole behind the June 2026 ``loki-0day``
...
class ModelAssignment(BaseModel):
    ...
    # ``loki model`` custom flow collects. Honored only on the main slot for
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Loki provider slug — fine.
    ...
    1. Vendor-name → Loki-provider mapping: when the provider string is
       not a known Loki provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    """
    from loki_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
...
# LOKI_HOME itself, at which point every one of these basenames is a live
...
# without needing to resolve them relative to LOKI_HOME.
...
def _is_sensitive_filename(name: str) -> bool:
    ...
    canonical Loki credential-store basenames (see
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    and needs no LOKI_HOME resolution, so it blocks these trees wherever
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``loki update``.
    ...
    a container — e.g. the loki-webui image sharing the Loki source tree),
    the dashboard's ``loki update`` button is the correct update path and
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's Loki root is actually /opt/data
    # (the container/hosted layout) or when LOKI_DASHBOARD_FILES_ROOT is set.
    if _default_loki_root_is_opt_data():
...
async def fs_write_text(payload: FsWriteText):
    ...
    Mirrors the local Electron ``loki:fs:writeText`` hardening: the path is
...
# gate + path hardening as /api/fs). Logic lives in ``loki_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from loki_cli import web_git as _web_git  # noqa: E402
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_loki_home() at call time
    ...
    try:
        ...
        # Windows install the first import of loki_cli.gateway blocks the
        ...
        # and which providers are registered so ``loki status`` and the
        # SPA's StatusPage can show "OAuth gate ON via Nous Research" or
        # "loopback only — no auth gate" with no extra round trips.
        auth_required = bool(getattr(app.state, "auth_required", False))
        auth_providers: list[str] = []
        try:
            from loki_cli.dashboard_auth import list_providers as _list_providers
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the loki home root).
    """
...
async def run_curator():
    ...
    try:
        proc = _spawn_loki_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from loki_cli.auth import get_nous_auth_status
    ...
    features = []
    try:
        from loki_cli.nous_subscription import get_nous_subscription_features
...
async def run_prompt_size():
    try:
        proc = _spawn_loki_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_loki_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_loki_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
# streamed to a per-action log file under ``~/.loki/logs/<action>.log`` so
...
def _spawn_loki_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``loki <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``loki_cli.main`` module so the action
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "LOKI_NONINTERACTIVE": "1"},
    }
    if sys.platform == "win32":
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["loki", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``loki gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``loki gateway restart`` children race each other on the
    ...
    return _spawn_loki_action(subcommand, "gateway-restart"), False
...
async def gateway_drain(request: Request):
    ...
    active (no ``LOKI_DASHBOARD_DRAIN_SECRET``), the route is NOT a token
...
@app.post("/api/loki/update")
async def update_loki():
    """Kick off ``loki update`` in the background."""
    if _dashboard_local_update_managed_externally():
        message = (
            "Loki updates are managed outside this dashboard in "
    ...
    try:
        proc = _spawn_loki_action(["update"], "loki-update")
    except Exception as exc:
        _log.exception("Failed to spawn loki update")
...
async def check_loki_update(force: bool = False):
    ...
    ``POST /api/loki/update`` actually runs ``loki update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed Loki version string
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.loki/config.yaml`` under ``tts.``.
    """
...
async def search_sessions(q: str = "", limit: int = 20, profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            ...
            # logs, or another Loki surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Loki supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def get_model_options(profile: Optional[str] = None, refresh: bool = False):
    ...
    try:
        ...
        # onboarding picker) sees the SAME full provider universe `loki model`
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `loki model` does so GUI onboarding lands on a
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.loki/config.yaml`` — applies to **new** sessions only.
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (loki_cli/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from loki_cli.nous_subscription import apply_nous_managed_defaults
        ...
        # endpoint, mirroring the ``loki model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from loki_cli.main import _auto_provider_name, _save_custom_provider
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``loki model`` universe). This is what lets the desktop Keys tab
    ...
    try:
        from loki_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `loki model` provider — would be
        ...
        # Vertex — a `loki model` provider — would be invisible in the desktop
...
async def get_env_vars(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # CLI `loki model` picker uses (not desktop-only prefix guesses).
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Loki from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Loki to Discord DMs, channels, and threads.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Loki in Matrix rooms and direct messages.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Loki through an IMAP/SMTP mailbox.",
        "docs_url": "https://loki-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Loki to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Loki inside Feishu / Lark.",
    ...
    "google_chat": {
        "name": "Google Chat",
        "description": "Connect Loki to Google Chat via Cloud Pub/Sub.",
        "docs_url": "https://loki-agent.nousresearch.com/docs/user-guide/messaging/google_chat",
    },
    "wecom": {
    ...
    "weixin": {
        ...
        "docs_url": "https://loki-agent.nousresearch.com/docs/user-guide/messaging/weixin/",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Loki to a QQ Bot from the QQ Open Platform.",
    ...
    "teams": {
        "docs_url": "https://loki-agent.nousresearch.com/docs/user-guide/messaging/teams",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Loki to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
    ...
    "webhook": {
        ...
        "docs_url": "https://loki-agent.nousresearch.com/docs/user-guide/messaging/webhooks/",
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in loki gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in loki gateway setup",
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.loki-agent.nousresearch.com"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Loki appears
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # all resolve against the requested profile's LOKI_HOME.
...
# the canonical ``loki auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.loki/.anthropic_oauth.json`` — Loki-managed PKCE flow (what
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_loki_dotenv()``, so the same check covers them)
    ...
    try:
        ...
        read_loki_oauth_credentials = None  # type: ignore
        _LOKI_OAUTH_FILE = None  # type: ignore

    loki_creds = None
    if read_loki_oauth_credentials:
        try:
    ...
    if loki_creds and loki_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "loki_pkce",
            "source_label": f"Loki PKCE ({_LOKI_OAUTH_FILE})",
            "token_preview": _truncate_token(loki_creds.get("accessToken")),
            "expires_at": loki_creds.get("expiresAt"),
            "has_refresh_token": bool(loki_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Loki even
    when they also have a separate Loki-managed PKCE login.
    """
    try:
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    Loki never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "loki auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "loki auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "loki auth add minimax-oauth",
    ...
    },
    {
        ...
        "docs_url": "https://loki-agent.nousresearch.com/docs/guides/xai-grok-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "loki auth add anthropic",
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Loki, so the disconnect
    ...
    user sees exactly what executes, and Loki then stops resolving the token.
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``loki model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `loki model` universe (zero-edit for new plugins).
    try:
        from loki_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"loki auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("loki_pkce", "claude_code", ...)
    ...
    sync with the `loki model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        # Anthropic clears only the Loki-managed PKCE file and auth-store entry.
        ...
        if provider_id == "anthropic":
            cleared = False
            try:
                from agent.anthropic_adapter import _LOKI_OAUTH_FILE
                if _LOKI_OAUTH_FILE.exists():
                    _LOKI_OAUTH_FILE.unlink()
                    cleared = True
            except Exception:
                pass
        ...
        try:
            from loki_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
#          → persists to ~/.loki/.anthropic_oauth.json AND credential pool
...
# Guarded so loki web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Loki file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``loki auth add anthropic``.
    """
    from agent.anthropic_adapter import _LOKI_OAUTH_FILE
    payload = {
    ...
    _LOKI_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _LOKI_OAUTH_FILE.with_name(
        f"{_LOKI_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _LOKI_OAUTH_FILE)
        try:
            _LOKI_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
...
async def _start_device_code_flow(
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("LOKI_PORTAL_BASE_URL")
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from loki_cli.auth import persist_nous_credentials
...
def _xai_device_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            # prior ``loki auth remove xai-oauth`` (mirrors auth_add_command
            # and the ``loki model`` re-login path in _login_xai_oauth).
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `loki sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `loki sessions prune`)."""
...
def _call_cron_for_profile(target_profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process LOKI_HOME at import time. The dashboard is a single
...
#
# Wraps the same config data layer the CLI uses (loki_cli.mcp_config), so
# servers managed here show up under `loki mcp list` and vice versa.  Secrets
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from loki_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    def _probe_scoped():
        ...
        # The probe touches no skills globals; it only needs the LOKI_HOME
...
async def auth_mcp_server(name: str, profile: Optional[str] = None):
    ...
    Mirrors ``loki mcp login``: wipe cached OAuth state so the probe forces
    ...
    def _run():
        ...
        # resolve LOKI_HOME via the contextvar override, which is all they need.
        with _config_profile_scope(profile), force_interactive_oauth():
            storage = LokiTokenStorage(name)
...
async def list_mcp_catalog(profile: Optional[str] = None):
    ...
    `loki mcp catalog` / `loki mcp install` read.  ``profile`` scopes
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds LOKI_HOME-derived paths in the child.
...
#
# Wraps the same JSON store the CLI uses (loki_cli.webhook); the webhook
...
# spawn the real `loki gateway <verb>` so behaviour matches the CLI exactly.
...
# `loki memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `loki memory setup` to configure a new one.",
            )
...
async def run_doctor():
    try:
        proc = _spawn_loki_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_loki_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
def _new_dashboard_backup_path() -> Path:
    ...
    return _dashboard_backup_dir() / f"loki-backup-{stamp}-{secrets.token_hex(4)}.zip"
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_loki_action(args, "backup")
    except Exception as exc:
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `loki import`. The spawned action runs with
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_loki_action(args, "import")
    except Exception as exc:
...
async def list_checkpoints():
    ...
    # Checkpoints live under <loki_home>/checkpoints/.  Surface a count +
...
async def prune_checkpoints():
    try:
        proc = _spawn_loki_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``loki`` subprocess, and
...
def _hub_action_name(verb: str, key: str) -> str:
    ...
    ``_spawn_loki_action`` tracks one process/log per name, so a shared
...
# Human-readable labels for each hub source id (matches `loki skills search`
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`loki -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the LOKI_HOME
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local LOKI_HOME override so the write lands in the target
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local LOKI_HOME override (same mechanism as
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the LOKI_HOME override. Returns the
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's LOKI_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``loki profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from LOKI_HOME).
    """
    from loki_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``loki profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the LOKI_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``loki profile describe
    <name> --auto``.
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_loki_home()`` at call
       time — the context-local override from ``set_loki_home_override``
    ...
    retargeted to the *current* ``get_loki_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a LOKI_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_loki_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_loki_home()`` at call time
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``loki tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``loki tools`` picker shows
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `loki tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``loki tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.loki/.env`` via ``save_env_value`` —
    the same store ``loki tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``loki tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``loki -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same LOKI_HOME the rest of the
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not Loki,
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``loki computer-use permissions grant`` as a background action.
...
#
# The endpoint spawns the same ``loki --tui`` binary the CLI uses, behind
...
def _resolve_chat_argv(
    ...
    Default: whatever ``loki --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``LOKI_TUI_RESUME`` env var —
    matching what ``loki_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``LOKI_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``LOKI_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `active_session_file` (when set) is forwarded as
    ``LOKI_TUI_ACTIVE_SESSION_FILE``. The TUI writes the current session id
    ...
    ``LOKI_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_loki_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``loki -p <name>`` performs. The in-process
    ``LOKI_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from loki_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    # inherits the profile LOKI_HOME set above.
...
# ---------------------------------------------------------------------------
# /api/console — safe Loki Console command WebSocket.
#
# Unlike /api/pty, this endpoint never spawns a PTY, shell, or full Loki CLI
        ...
        with _console_executor_lock:
            if _console_executor is None:
                _console_executor = concurrent.futures.ThreadPoolExecutor(
                    ...
                    thread_name_prefix="loki-console",
                )
...
def _dashboard_console_context() -> str:
    ...
    return "hosted" if _default_loki_root_is_opt_data() else "local"
            ...
            if command_id == command_generation:
                ...
                await _console_send(
                    ...
                    send_lock,
                    {
                        ...
                        "id": command_id,
                        "message": (
                            "Command timed out. Loki Console returned to the prompt."
                        ),
                        "command": line,
                    },
                )
...
# LOKI_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`loki_cli.dashboard_auth.prefix.normalise_prefix`
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/loki/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /loki`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__LOKI_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/loki``)
        ...
        ``__LOKI_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                ...
                f"window.__LOKI_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__LOKI_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__LOKI_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__LOKI_BASE_PATH__="{prefix}";'
                f"window.__LOKI_AUTH_REQUIRED__={gated_js};"
                f"</script>"
    ...
    # under ``/loki`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Loki backend. Intercept CSS asset
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.loki/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.loki/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_loki_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.loki/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as loki_cli.plugins):
    1. User plugins:    ~/.loki/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.loki/plugins/  (only if LOKI_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()
    ...
    # ``loki_cli/plugins.py`` and the documented user contract.
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.loki/plugins/``) ship with the CWD and are
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    so Electron passes ``LOKI_DESKTOP_READY_FILE`` and waits for this JSON.
...
def start_server(
    ...
    try:
        from loki_cli.nous_auth_keepalive import start_nous_auth_keepalive
    ...
    # the loki-0day MCP-persistence campaign abused unauthenticated public
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # to register (e.g. missing LOKI_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Loki Agent exposes a
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `loki dashboard register` (Nous Portal) or "
    ...
    # LOKI_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            print(f"LOKI_DASHBOARD_READY port={actual_port}", flush=True)
