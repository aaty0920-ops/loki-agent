...
_LOKI_CORE_TOOLS = [
    ...
    # read-only terminal tab (both gated on LOKI_DESKTOP via check_fn —
    ...
    # spawned as a kanban worker (LOKI_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `loki tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (LOKI_KANBAN_TASK env "
    ...
    # Coding posture (base Loki — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in loki_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Loki toolsets (CLI + messaging platforms)
    #
    ...
    # the `loki send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "loki-cron": {
        # Mirrors loki-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `loki tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as loki-cli; gated by `loki tools`",
        "tools": _LOKI_CORE_TOOLS,
        "includes": []
    },
    ...
    "loki-email": {
        "description": "Email bot toolset - interact with Loki via email (IMAP/SMTP)",
        "tools": _LOKI_CORE_TOOLS,
        "includes": []
    },
    ...
    "loki-sms": {
        "description": "SMS bot toolset - interact with Loki via SMS (Twilio)",
        "tools": _LOKI_CORE_TOOLS,
        "includes": []
    },
    ...
    "loki-webhook": {
        ...
        "tools": _LOKI_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "loki-gateway": {
        ...
        "tools": [],
        "includes": ["loki-telegram", "loki-discord", "loki-whatsapp", "loki-slack", "loki-signal", "loki-bluebubbles", "loki-homeassistant", "loki-email", "loki-sms", "loki-mattermost", "loki-matrix", "loki-dingtalk", "loki-feishu", "loki-wecom", "loki-wecom-callback", "loki-weixin", "loki-qqbot", "loki-webhook", "loki-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``loki-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_LOKI_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``loki-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (loki-<name>).
        # Gives them _LOKI_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("loki-"):
            platform_name = name[len("loki-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_LOKI_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
