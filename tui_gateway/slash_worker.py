...
# from shadowing Loki's own top-level modules.  This worker is spawned as
...
# and crash the child in a retry loop (issue #51286).  ``loki_bootstrap``
...
def _env_float(name: str, default: float) -> float:
    ...
    import time on a typo (e.g. ``LOKI_SLASH_WATCHDOG_POLL_S=2s``) and kill
...
_WATCHDOG_POLL_S = max(0.05, _env_float("LOKI_SLASH_WATCHDOG_POLL_S", 2.0))
_ORPHAN_GRACE_S = max(0.0, _env_float("LOKI_SLASH_WATCHDOG_GRACE_S", 5.0))
...
def main():
    ...
    # Start before the (hundreds-of-ms) LokiCLI build — that window is itself
