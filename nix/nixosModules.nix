# nix/nixosModules.nix — NixOS module for loki-agent
#
# Two modes:
...
{ inputs, ... }: {
  ...
  let
    ...
    # Deep-merge config type (from 0xrsydn/nix-loki-agent)
    ...
    generatedConfigFile = pkgs.writeText "loki-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "loki-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'LOKI_DOC_EOF'\n${value}\nLOKI_DOC_EOF"
        ) cfg.documents
      )
    );
  ...
  in {
    options.services.loki-agent = with lib; {
      enable = mkEnableOption "Loki Agent gateway service";
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Loki config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $LOKI_HOME/.env at activation time.
          Loki reads this file on every startup via load_loki_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $LOKI_HOME/.env
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the loki user's per-user profile
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.loki-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      (lib.mkIf cfg.addToSystemPackages {
        ...
        environment.variables.LOKI_HOME = "${cfg.stateDir}/.loki";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.loki-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.loki        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.loki/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.loki/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.loki/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.loki/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.loki/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."loki-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.loki
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.loki ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.loki ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.loki -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.loki/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.loki/$_subdir"
            chmod 2770 "${cfg.stateDir}/.loki/$_subdir"
            find "${cfg.stateDir}/.loki/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # loki-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.loki/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.loki/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.loki/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.loki/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.loki/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.loki/.managed
          chmod 0644 ${cfg.stateDir}/.loki/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.loki/.container-mode <<'LOKI_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    loki_bin=${containerDataDir}/current-package/bin/loki
    LOKI_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.loki/.container-mode
            chmod 0644 ${cfg.stateDir}/.loki/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.loki/.container-mode
            ...
            ${lib.concatStringsSep "\n" (map (user:
              ...
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.loki" ]; then
                  rm -f "${symlinkPath}"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.loki/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.loki/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.loki/auth.json
              fi
            ''}
          ''}
          ...
          # Loki reads $LOKI_HOME/.env at startup via load_loki_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.loki/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'LOKI_NIX_ENV_EOF'
    ${envFileContent}
    LOKI_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.loki/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.loki/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.loki/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
