    ...
    in {
      ...
      checks = {
        ...
        bundled-skills = pkgs.runCommand "loki-bundled-skills" { } ''
          ...
          SKILL_COUNT=$(find ${loki-agent}/share/loki-agent/skills -name "SKILL.md" | wc -l)
        ...
        bundled-locales = pkgs.runCommand "loki-bundled-locales" { } ''
          ...
          LOC_COUNT=$(find ${loki-agent}/share/loki-agent/locales -name "*.yaml" | wc -l)
          ...
          echo "=== Rendering via the wrapper override (LOKI_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          ...
          BARE_DIR=$(cd "$HOME" && ${lokiVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${lokiVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without LOKI_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        # Verify LOKI_NODE is set in wrapper and points to Node 20+
