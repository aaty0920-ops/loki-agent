...
Jobs are stored in ~/.loki/cron/jobs.json
Output is saved to ~/.loki/cron/output/{job_id}/{timestamp}.md
"""
...
# store under its own LOKI_HOME, and a profile-scoped gateway runs that
# profile's jobs under that same LOKI_HOME — so a job authored in profile
# `coder` lives in `~/.loki/profiles/coder/cron/jobs.json` and executes with
# `coder`'s `.env`, `config.yaml`, and skills. We deliberately anchor on
# `get_loki_home()` (the active profile home), NOT `get_default_loki_root()`
# (the shared root). Anchoring at the root would funnel every profile's jobs
# into one shared `jobs.json` and run them under whatever LOKI_HOME the
...
# `_get_loki_home()` / `_get_lock_paths()` resolution in cron/scheduler.py.
LOKI_DIR = get_loki_home().resolve()
CRON_DIR = LOKI_DIR / "cron"
JOBS_FILE = CRON_DIR / "jobs.json"
# Heartbeat file the in-process ticker touches on every loop iteration. The
# gateway process and the (separate) ``loki cron status`` process share it
...
# threshold in `loki cron status` (loki_cli/cron.py), so the two never
# drift apart.
...
def _jobs_lock():
    ...
    and standalone ``loki`` CLI invocations, which previously shared no lock
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    if 'T' in schedule or re.match(r'^\d{4}-\d{2}-\d{2}', schedule):
        try:
            ...
            #
            # Anchor to the CONFIGURED Loki timezone, not the server's local
            # timezone. The due-check (`get_due_jobs`) compares `next_run_at`
            # against `loki_time.now()`, which uses the configured zone. If a
            ...
            if dt.tzinfo is None:
                loki_tz = _loki_now().tzinfo
                dt = dt.replace(tzinfo=loki_tz)
            return {
                "kind": "once",
                "run_at": dt.isoformat(),
...
def _ensure_aware(dt: datetime) -> datetime:
    """Return a timezone-aware datetime in Loki configured timezone.

    Backward compatibility:
    - Older stored timestamps may be naive.
    - Naive values are interpreted as *system-local wall time* (the timezone
      `datetime.now()` used when they were created), then converted to the
      configured Loki timezone.
...
def _stored_wall_clock_is_future(stored: datetime, current: datetime) -> bool:
    ...
    Cron schedules express local wall-clock intent. If Loki/system local time
    ...
    if schedule["kind"] == "once":
        ...
        if not HAS_CRONITER:
            logger.warning(
                ...
                "reinstall loki-agent or run 'pip install croniter' in your "
                "runtime env.",
                schedule.get("expr"),
            )
            return None
...
def _atomic_write_epoch(path: Path) -> None:
    ...
    concurrent reader in another process (``loki cron status``) never sees a
...
def record_ticker_heartbeat(success: bool = False) -> None:
    ...
    `loki cron status` can tell a thread that is merely *alive and looping*
...
def _save_jobs_unlocked(jobs: List[Dict[str, Any]]):
    ...
    try:
        with os.fdopen(fd, 'w', encoding='utf-8') as f:
            json.dump({"jobs": jobs, "updated_at": _loki_now().isoformat()}, f, indent=2)
            f.flush()
            os.fsync(f.fileno())
...
def create_job(
    ...
    Args:
        ...
        script: Optional path to a script whose stdout feeds the job. With
                ...
                ~/.loki/scripts/; ``.sh`` / ``.bash`` files run via bash,
    ...
    # covered, not just `loki cron create`.
...
def trigger_job(job_id: str) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job["id"],
        {
            ...
            "paused_at": None,
            "paused_reason": None,
            "next_run_at": _loki_now().isoformat(),
        },
    )
...
def _machine_id() -> str:
    ...
    Uses ``LOKI_MACHINE_ID`` if set, else hostname + pid. The CAS correctness
...
# execution. Unlike the quick-snapshot store (`loki_cli.backup`, capped at 20)
...
def _prune_job_output(job_output_dir: Path, keep: int) -> int:
    ...
    Mirrors the quick-snapshot retention in ``loki_cli.backup._prune_quick_snapshots``:
...
def save_job_output(job_id: str, output: str):
    ...
    timestamp = _loki_now().strftime("%Y-%m-%d_%H-%M-%S")
