...
``loki gateway restart`` (or ``launchctl kickstart ai.loki.gateway``
or ``systemctl restart loki-gateway``).  When the cron fires, the
...
``loki cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``loki gateway``, ``launchctl ... loki-gateway``,
``systemctl ... loki-gateway``, ``pkill`` against the gateway) so it
...
blocks these commands at *execution* time when ``_LOKI_GATEWAY=1``, and
``loki gateway stop|restart`` refuse to self-target from inside the
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `loki gateway restart|stop` — the canonical foot-gun.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    r"(?:loki\s+gateway\s+(?:restart|stop))"
    # Branch B: launchctl ops on a loki-gateway label. macOS launchd
    # labels look like `ai.loki.gateway` / `loki-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated loki services (e.g.
    ...
    # Branch D: pkill / kill targeting the loki gateway process. Both
...
def _resolve_script_path(script_path: str) -> Path:
    ...
    under ``<LOKI_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.loki/scripts/restart.sh``) but is passed as the bare name
