...
Uses a file-based lock (~/.loki/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `loki update` reloads
# the module) fail with ModuleNotFoundError for loki_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from loki_constants import get_loki_home
from loki_cli._subprocess_compat import windows_hide_flags
from loki_cli.config import load_config, _expand_env_vars
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy loki_cli import at cron module load (matches
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``loki tools`` config for the ``cron`` platform.
...
def _get_loki_home() -> Path:
    """Resolve Loki home dynamically while preserving test monkeypatch hooks.

    Cron is per-profile by design (#4707): the in-process ticker runs inside a
    profile-scoped gateway, so resolving the active LOKI_HOME at call time
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("LOKI_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid LOKI_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within LOKI_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other Loki-managed secrets are not inherited
    (SECURITY.md §2.3), matching terminal and MCP child processes.

    Args:
        script_path: Path to the script.  Relative paths are resolved
            against LOKI_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_loki_home() / "scripts"
    ...
    # escape — scripts MUST reside within LOKI_HOME/scripts/.
    try:
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    os.environ["LOKI_CRON_SESSION"] = "1"
    ...
    # gateway message. Do not seed LOKI_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads LOKI_SESSION_PLATFORM
    #     and LOKI_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via LOKI_CRON_AUTO_DELIVER_* below.
    ...
    #     send_message gate read LOKI_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the LOKI_CRON_AUTO_DELIVER_* vars set
    # below, so clearing LOKI_SESSION_* here does not affect delivery.
    ...
    try:
        ...
        # load_loki_dotenv (not a bare load_dotenv) and reset the secret-
        # source cache first: startup already applied external secrets and
        # recorded this LOKI_HOME in _APPLIED_HOMES, so a naive reload would
        ...
        # forced re-pull off the network. load_loki_dotenv also handles the
        # utf-8/latin-1 encoding fallback internally.
        from loki_cli.env_loader import (
            load_loki_dotenv,
        ...
        # Model resolution precedence: per-job override > LOKI_MODEL env >
        ...
        try:
            # Do not inject LOKI_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        agent = AIAgent(
            ...
            # LOKI_HOME. When a workdir is configured, also inject project
        ...
        # override via LOKI_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _raw_cron_timeout = os.getenv("LOKI_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid LOKI_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
...
**Job ID:** {job_id}
**Run Time:** {_loki_now().strftime('%Y-%m-%d %H:%M:%S')}
...
def run_one_job(job: dict, *, adapters=None, loop=None, verbose: bool = False) -> bool:
    ...
    try:
        ...
        _scope_token = set_secret_scope(
            build_profile_secret_scope(_get_loki_home())
        )
        try:
...
def tick(verbose: bool = True, adapters=None, loop=None, sync: bool = True) -> int:
    ...
    try:
        ...
        # Set LOKI_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
