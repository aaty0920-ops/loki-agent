...
class TestStartLogin:
    ...
    def test_two_calls_differ(self, provider):
        a = provider.start_login(redirect_uri="https://loki.example/auth/callback")
        b = provider.start_login(redirect_uri="https://loki.example/auth/callback")
        assert (
            a.cookie_payload["loki_session_pkce"]
            != b.cookie_payload["loki_session_pkce"]
        )
    ...
    def test_allows_http_with_arbitrary_host(self, provider):
        ...
        provider.start_login(redirect_uri="http://loki.example/auth/callback")
...
class TestPluginRegister:
    ...
    def test_empty_env_does_not_shadow_config(self, patch_config, monkeypatch):
        ...
        monkeypatch.setenv("LOKI_DASHBOARD_OIDC_ISSUER", "")
        monkeypatch.setenv("LOKI_DASHBOARD_OIDC_CLIENT_ID", "")
        ctx = MagicMock()
        oidc_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_called_once()
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
    ...
    def test_non_https_issuer_skips_with_reason(self, patch_config, monkeypatch):
        patch_config(None)
        monkeypatch.setenv(
            "LOKI_DASHBOARD_OIDC_ISSUER", "http://insecure.example"
        )
        monkeypatch.setenv("LOKI_DASHBOARD_OIDC_CLIENT_ID", _CLIENT_ID)
        ctx = MagicMock()
        oidc_plugin.register(ctx)
    ...
    def test_register_log_reports_confidential_not_secret(
        ...
        monkeypatch.setenv("LOKI_DASHBOARD_OIDC_ISSUER", _ISSUER)
        monkeypatch.setenv("LOKI_DASHBOARD_OIDC_CLIENT_ID", _CLIENT_ID)
        monkeypatch.setenv("LOKI_DASHBOARD_OIDC_CLIENT_SECRET", "logme-secret")
        ctx = MagicMock()
        with caplog.at_level(logging.INFO):
            oidc_plugin.register(ctx)
