...
def test_start_spawns_subprocess_and_writes_active_pointer(tmp_path):
    ...
    assert captured_env["LOKI_MEET_URL"] == "https://meet.google.com/abc-defg-hij"
    assert captured_env["LOKI_MEET_GUEST_NAME"] == "Test Bot"
    assert captured_env["LOKI_MEET_DURATION"] == "15m"
...
def test_enqueue_say_rejects_transcribe_mode(tmp_path):
    ...
    out_dir = Path(os.environ["LOKI_HOME"]) / "workspace" / "meetings" / "abc-defg-hij"
...
def test_start_realtime_env_vars_threaded_through():
    ...
    assert captured_env["LOKI_MEET_MODE"] == "realtime"
    assert captured_env["LOKI_MEET_REALTIME_MODEL"] == "gpt-realtime"
    assert captured_env["LOKI_MEET_REALTIME_VOICE"] == "alloy"
    assert captured_env["LOKI_MEET_REALTIME_INSTRUCTIONS"] == "Be brief."
    assert captured_env["LOKI_MEET_REALTIME_KEY"] == "sk-test"
