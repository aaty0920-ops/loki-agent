...
def test_post_setup_create_remote_user_profile_can_mirror_to_openviking_store(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        _prompt_from_values({
            ...
            "Loki peer ID in OpenViking": "loki",
...
def test_post_setup_create_remote_user_can_keep_loki_only(tmp_path, monkeypatch):
    _clear_openviking_env(monkeypatch)
    loki_home = tmp_path / "loki"
    loki_home.mkdir()
    monkeypatch.setenv("LOKI_HOME", str(loki_home))
    ...
    monkeypatch.setattr(
        ...
        _prompt_from_values({
            "OpenViking server URL": "https://openviking.example",
            "OpenViking user API key": "user-secret",
            "Loki peer ID in OpenViking": "agent",
        }),
    )
    config = {"memory": {}}
...
def test_post_setup_remote_blank_api_key_cancels_without_saving(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(loki_config, "save_config", save_config)
    choices = iter([1, 0, 1])
    monkeypatch.setattr(memory_setup, "_curses_select", lambda *args, **kwargs: next(choices))
    monkeypatch.setattr(
        memory_setup,
...
def test_post_setup_user_key_path_can_route_detected_root_key_to_root_setup(tmp_path, monkeypatch):
    ...
    assert prompt_events.count("Loki peer ID in OpenViking") == 1
    env_text = (loki_home / ".env").read_text(encoding="utf-8")
...
def test_start_local_openviking_server_writes_output_to_log(tmp_path, monkeypatch):
    ...
    def fake_popen(args, **kwargs):
        ...
        assert kwargs["stdout"].name == str(loki_home / "logs" / "openviking-server.log")
    ...
    assert started is True
    assert str(loki_home / "logs" / "openviking-server.log") in message
    assert popen_calls
...
def test_https_local_endpoint_is_not_runtime_autostart_eligible(monkeypatch):
    ...
    assert warnings == [
        ...
        "OpenViking memory disabled for this Loki run. "
...
def test_runtime_openviking_waiter_warns_when_background_start_times_out(monkeypatch):
    ...
    assert warnings == [
        ...
        "within 60 seconds. OpenViking memory disabled for this Loki run."
    ]
...
def test_handle_tool_call_forget_deletes_exact_memory_file_uri():
    uri = "viking://user/peers/loki/memories/preferences/mem_abc123.md"
    provider = OpenVikingMemoryProvider()
    provider._client = MagicMock()
...
@pytest.mark.parametrize("uri", [
    ...
    "viking://agent/loki/memories/preferences/mem_abc123.md",
...
def test_viking_client_headers_include_bearer_when_api_key_set():
    ...
    assert headers["X-OpenViking-Actor-Peer"] == "loki"
    assert "X-OpenViking-Agent" not in headers
    assert "X-OpenViking-Account" not in headers
    assert "X-OpenViking-User" not in headers
...
def test_viking_client_health_sends_auth_headers(monkeypatch):
    ...
    assert captured["headers"]["X-OpenViking-Actor-Peer"] == "loki"
...
def test_sync_turn_structured_messages_include_assistant_peer_id():
    ...
    assert captured == [(
        "/api/v1/sessions/sid-structured/messages/batch",
        {
            "messages": [
                {"role": "user", "parts": [{"type": "text", "text": "u"}]},
                {"role": "assistant", "parts": [{"type": "text", "text": "Looking."}], "peer_id": "loki"},
                {
                    "role": "assistant",
                    "parts": [
                        {
...
def test_on_session_switch_does_not_block_caller_on_slow_drain():
    """Regression for loki-agent#28296 review (H1): on_session_switch must
...
def test_on_session_switch_defers_old_commit_to_finalizer_thread():
    ...
    -_DEFERRED_COMMIT_TIMEOUT drain or the commit POST. See loki-agent#28296
...
def test_on_memory_write_uses_content_write_independent_of_session_rotation():
    ...
    assert captured_payloads[0]["uri"].startswith(
        "viking://user/peers/loki/memories/preferences/mem_"
    )
        ...
        def post(self, path, payload=None, **kwargs):
            ...
            return {
                "result": {
                    "memories": [
                        {
                            "uri": "viking://user/peers/loki/memories/events/mem_1.md",
                            "score": 0.9,
        ...
        def post(self, path, payload=None, **kwargs):
            ...
            return {
                "result": {
                    "memories": [
                        {
                            "uri": "viking://user/peers/loki/memories/events/mem_2.md",
                            "score": 0.8,
        ...
        def post(self, path, payload=None, **kwargs):
            return {
                "result": {
                    "memories": [
                        {
                            "uri": "viking://user/peers/loki/memories/events/mem_3.md",
                            "score": 0.9,
                            "level": 2,
                            "category": "events",
...
def test_prefetch_reads_empty_abstract_content_within_budget(monkeypatch):
        ...
        def post(self, path, payload=None, **kwargs):
            return {
                "result": {
                    "memories": [
                        {
                            "uri": "viking://user/peers/loki/memories/one.md",
                            "score": 0.9,
                            "abstract": "",
                        },
                    ],
    ...
    assert [params["uri"] for _path, params in captured_reads] == [
        "viking://user/peers/loki/memories/one.md",
    ]
    assert (
        "content for viking://user/peers/loki/memories/one.md"
        in context
    )
...
def test_prefetch_caps_full_content_reads(monkeypatch):
        ...
        def post(self, path, payload=None, **kwargs):
            return {
                "result": {
                    "memories": [
                        {
                            "uri": f"viking://user/peers/loki/memories/events/mem_{idx}.md",
                            "score": 0.9 - (idx * 0.01),
                            "level": 2,
                            "category": "events",
    ...
    assert "full content for viking://user/peers/loki/memories/events/mem_0.md" in context
    assert "full content for viking://user/peers/loki/memories/events/mem_1.md" in context
        ...
        def post(self, path, payload=None, **kwargs):
            ...
            return {
                "result": {
                    "memories": [
                        {
                            "uri": "viking://user/peers/loki/memories/events/mem_timeout.md",
                            "score": 0.9,
                            "level": 2,
                            "category": "events",
                            "abstract": "short abstract",
