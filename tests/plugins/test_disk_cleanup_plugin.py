...
class TestIsSafePath:
    ...
    def test_accepts_tmp_loki_prefix(self, _isolate_env, tmp_path):
        dg = _load_lib()
        assert dg.is_safe_path(Path("/tmp/loki-abc/x.log")) is True
...
class TestTrackForgetQuick:
    ...
    def test_quick_does_not_descend_into_protected_top_level_dirs(
        ...
        dg = _load_lib()
        protected_empty = (
            _isolate_env / "loki-agent" / "node_modules" / "pkg" / "empty"
        )
        ...
        def guarded_iterdir(path):
            if path == _isolate_env / "loki-agent":
                raise AssertionError("quick() descended into protected loki-agent/")
