...
def _reset_loki_time_cache():
    """Reset the loki_time module cache (replacement for removed reset_cache)."""
    loki_time._cached_tz = None
    loki_time._cached_tz_name = None
    loki_time._cache_resolved = False
...
class TestLokiTimeNow:
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["LOKI_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="loki_time"):
            result = loki_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_none_for_invalid(self):
        os.environ["LOKI_TIMEZONE"] = "Not/A/Timezone"
        tz = loki_time.get_timezone()
        assert tz is None
...
class TestCronTimezone:
    ...
    def test_ensure_aware_normalizes_aware_to_loki_tz(self):
        """Already-aware datetimes should be normalized to Loki tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Loki tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Loki tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Loki is UTC.
