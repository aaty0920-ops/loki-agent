...
def test_strategy_b_rebuild_when_dedup_insufficient(tmp_path, monkeypatch):
    ...
    real_check = loki_state._db_opens_cleanly
    calls = {"n": 0}
    ...
    monkeypatch.setattr(loki_state, "_db_opens_cleanly", flaky_check)
    report = repair_state_db_schema(db_path)
    monkeypatch.undo()
...
def test_non_malformed_error_is_not_auto_repaired(tmp_path, monkeypatch):
    ...
    called = {"n": 0}
    orig = loki_state.repair_state_db_schema

    def spy(*a, **kw):
        called["n"] += 1
        return orig(*a, **kw)

    monkeypatch.setattr(loki_state, "repair_state_db_schema", spy)
    with pytest.raises(sqlite3.DatabaseError):
        SessionDB(db_path=db_path)
...
def test_fts_write_corruption_detected_by_write_probe(tmp_path):
    ...
    from loki_state import _db_opens_cleanly
