...
class TestSetupLogging:
    ...
    def test_creates_errors_log_handler(self, loki_home):
        loki_logging.setup_logging(loki_home=loki_home)
        root = logging.getLogger()
    ...
    def test_force_reinitializes(self, loki_home):
        loki_logging.setup_logging(loki_home=loki_home)
        # Force still won't add duplicate handlers because _add_rotating_handler
        # checks by resolved path.
        loki_logging.setup_logging(loki_home=loki_home, force=True)
    ...
    def test_custom_log_level(self, loki_home):
        loki_logging.setup_logging(loki_home=loki_home, log_level="DEBUG")
    ...
    def test_custom_max_size_and_backup(self, loki_home):
        loki_logging.setup_logging(
            loki_home=loki_home, max_size_mb=10, backup_count=5
        )
    ...
    def test_suppresses_noisy_loggers(self, loki_home):
        loki_logging.setup_logging(loki_home=loki_home)
    ...
    def test_warnings_appear_in_both_logs(self, loki_home):
        loki_logging.setup_logging(loki_home=loki_home)

        test_logger = logging.getLogger("test_loki_logging.warning_test")
        ...
        agent_log = loki_home / "logs" / "agent.log"
        errors_log = loki_home / "logs" / "errors.log"
        assert "this is a warning" in agent_log.read_text()
        assert "this is a warning" in errors_log.read_text()

    def test_info_not_in_errors_log(self, loki_home):
        loki_logging.setup_logging(loki_home=loki_home)

        test_logger = logging.getLogger("test_loki_logging.info_test")
    ...
    def test_reads_config_yaml(self, loki_home):
    ...
    def test_record_factory_installed(self, loki_home):
        """The custom record factory injects session_tag on all records."""
        loki_logging.setup_logging(loki_home=loki_home)
        factory = logging.getLogRecordFactory()
        assert getattr(factory, "_loki_session_injector", False), (
            "Record factory should have _loki_session_injector marker"
        )
...
class TestGatewayMode:
    ...
    def test_gateway_log_created_after_cli_init(self, loki_home):
    ...
    def test_gateway_log_created_after_cli_init_without_duplicate_handlers(self, loki_home):
    ...
    def test_agent_log_still_receives_all(self, loki_home):
...
class TestGuiMode:
    ...
    def test_gui_log_created_after_cli_init(self, loki_home):
        loki_logging.setup_logging(loki_home=loki_home, mode="cli")
        loki_logging.setup_logging(loki_home=loki_home, mode="gui")
...
class TestSessionContext:
    ...
    def test_no_session_tag_without_context(self, loki_home):
    ...
    def test_clear_session_context(self, loki_home):
        """After clearing, session tag disappears."""
        loki_logging.setup_logging(loki_home=loki_home)
        loki_logging.set_session_context("xyz789")
        loki_logging.clear_session_context()
    ...
    def test_session_context_thread_isolated(self, loki_home):
        ...
        def thread_a():
            loki_logging.set_session_context("thread_a_session")
            logging.getLogger("test.thread_a").info("from thread A")
            for h in logging.getLogger().handlers:
                h.flush()

        def thread_b():
            loki_logging.set_session_context("thread_b_session")
...
class TestRecordFactory:
    ...
    def test_tag_with_context(self):
        loki_logging.set_session_context("sess_42")
        factory = logging.getLogRecordFactory()
        record = factory("test", logging.INFO, "", 0, "msg", (), None)
    ...
    def test_works_with_any_handler(self):
        ...
        loki_logging.set_session_context("any_handler_test")
...
class TestComponentFilter:
    ...
    def test_passes_nested_matching_prefix(self):
        ...
        f = loki_logging._ComponentFilter(
            loki_logging.COMPONENT_PREFIXES["gateway"]
        )
        record = logging.LogRecord(
    ...
    def test_multiple_prefixes(self):
        f = loki_logging._ComponentFilter(("agent", "run_agent", "model_tools"))
...
class TestComponentPrefixes:
    ...
    def test_gateway_prefix(self):
        assert "gateway" in loki_logging.COMPONENT_PREFIXES
        # The gateway component captures core gateway logs, the loki_plugins
        ...
        # break this test.
        gateway_prefixes = loki_logging.COMPONENT_PREFIXES["gateway"]
        assert "gateway" in gateway_prefixes
        assert "loki_plugins" in gateway_prefixes
        assert "plugins.platforms" in gateway_prefixes

    def test_agent_prefix(self):
        prefixes = loki_logging.COMPONENT_PREFIXES["agent"]
        assert "agent" in prefixes
        assert "run_agent" in prefixes
        assert "model_tools" in prefixes

    def test_tools_prefix(self):
        assert ("tools",) == loki_logging.COMPONENT_PREFIXES["tools"]

    def test_cli_prefix(self):
        prefixes = loki_logging.COMPONENT_PREFIXES["cli"]
        assert "loki_cli" in prefixes
        assert "cli" in prefixes

    def test_cron_prefix(self):
        assert ("cron",) == loki_logging.COMPONENT_PREFIXES["cron"]

    def test_gui_prefix(self):
        prefixes = loki_logging.COMPONENT_PREFIXES["gui"]
        assert "loki_cli.web_server" in prefixes
...
class TestSetupVerboseLogging:
    ...
    def test_idempotent(self, loki_home):
        loki_logging.setup_logging(loki_home=loki_home)
        loki_logging.setup_verbose_logging()
        loki_logging.setup_verbose_logging()  # second call
...
class TestAddRotatingHandler:
    ...
    def test_log_filter_attached(self, tmp_path):
        ...
        formatter = logging.Formatter("%(message)s")
        component_filter = loki_logging._ComponentFilter(("test",))
...
class TestWindowsConcurrentLogLockTimeout:
    ...
    def test_helper_only_matches_windows_concurrent_lock_timeout(self):
        with patch.object(loki_logging.sys, "platform", "win32"):
        ...
        with patch.object(loki_logging.sys, "platform", "linux"):
            assert not loki_logging._is_windows_concurrent_log_lock_timeout(
                RuntimeError("Cannot acquire lock after 20 attempts")
            )
...
class TestReadLoggingConfig:
    ...
    def test_handles_missing_logging_section(self, loki_home):
        import yaml
        config = {"model": "test"}
        (loki_home / "config.yaml").write_text(yaml.dump(config))
