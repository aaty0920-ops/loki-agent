...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `loki cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # loki_config_mod  — print statements in the post-migration summary that
    #                      tell the user to *review* ~/.loki/config.yaml;
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "loki_config_mod"}
...
def test_rebrand_text_replaces_legacy_bot_names():
    ...
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "Loki remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "loki prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Loki was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "loki uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "Loki said to ask Loki about Loki settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.loki`` (the real Loki home), not the broken
    ``.Loki``.
    ...
    rewritten to ``~/.Loki/config.yaml`` — a path that doesn't exist —
    and the agent kept trying to read it.
    """
    mod = load_module()
    assert mod.rebrand_text("config is at ~/.openclaw/config.yaml") == \
        "config is at ~/.loki/config.yaml"
    assert mod.rebrand_text("use .openclaw directory") == "use .loki directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.loki'"
    # Sentence with both lowercase path and capitalized prose.
    assert mod.rebrand_text("openclaw config path: ~/.openclaw/") == \
        "loki config path: ~/.loki/"
