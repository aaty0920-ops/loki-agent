...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - the child process resolves loki_cli.kanban_db on its own
  ...
  - worker logs are captured to LOKI_HOME/kanban/logs/<task>.log
    ...
    def _spawn(task, workspace):
        ...
        env = {
            **os.environ,
            ...
            "HOME": home,
            "PYTHONPATH": WT,
            "LOKI_KANBAN_TASK": task.id,
            "LOKI_KANBAN_WORKSPACE": workspace,
...
def main():
    home = tempfile.mkdtemp(prefix="loki_e2e_")
    os.environ["LOKI_HOME"] = home
    os.environ["HOME"] = home
    sys.path.insert(0, WT)
    from loki_cli import kanban_db as kb

    # Point the `loki` CLI child processes will run at the worktree
    # loki_cli.main. We do this by putting a shim on PATH.
    shim_dir = os.path.join(home, "bin")
    os.makedirs(shim_dir, exist_ok=True)
    shim_path = os.path.join(shim_dir, "loki")
    with open(shim_path, "w") as f:
        f.write(f"""#!/bin/sh
exec {PY} -m loki_cli.main "$@"
""")
    ...
    print("=" * 60)
    # Scenario A workers wrote to /tmp/loki_e2e_*/worker_*.log
    import glob
