...
def test_cli_prefers_config_provider_over_stale_env_override(monkeypatch):
    ...
    shell = cli.LokiCLI(model="fireworks/minimax-m2p5", compact=True, max_turns=1)
...
def test_model_flow_nous_prints_subscription_guidance_without_mutating_explicit_tts(monkeypatch, capsys):
    monkeypatch.setattr(
        "loki_cli.nous_subscription.managed_nous_tools_enabled",
    ...
    monkeypatch.setattr(
        "loki_cli.auth.fetch_nous_models",
        lambda *args, **kwargs: ["claude-opus-4-6"],
    )
    monkeypatch.setattr("loki_cli.auth._prompt_model_selection", lambda model_ids, current_model="", pricing=None, **kw: "claude-opus-4-6")
    monkeypatch.setattr("loki_cli.auth._save_model_choice", lambda model: None)
    monkeypatch.setattr("loki_cli.auth._update_config_for_provider", lambda provider, url: None)

    loki_main._model_flow_nous(config, current_model="claude-opus-4-6")
...
def test_model_flow_nous_does_not_restore_stale_custom_api_key(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "loki_cli.models.get_curated_nous_model_ids",
        lambda: [selected_model],
    )
    monkeypatch.setattr("loki_cli.models.get_pricing_for_provider", lambda provider: {})
    monkeypatch.setattr("loki_cli.models.check_nous_free_tier", lambda **kwargs: False)
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "loki_cli.nous_subscription.prompt_enable_tool_gateway",
        lambda config: None,
    )

    loki_main._model_flow_nous(stale_config, current_model="glm-5.2")
...
def test_model_flow_openrouter_clears_stale_custom_key(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "loki_cli.main._prompt_api_key",
        lambda *args, **kwargs: ("sk-openrouter", False),
    )
    monkeypatch.setattr(
        "loki_cli.models.model_ids",
        lambda **kwargs: ["anthropic/claude-sonnet-4.6"],
    )
    monkeypatch.setattr("loki_cli.models.get_pricing_for_provider", lambda *a, **k: {})
    monkeypatch.setattr(
        "loki_cli.auth._prompt_model_selection",
    ...
    loki_main._model_flow_openrouter({}, current_model="glm-5.2")
...
def test_model_flow_anthropic_clears_stale_custom_key_and_mode(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("loki_cli.auth.get_anthropic_key", lambda: "sk-ant-api03-test")
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "loki_cli.model_setup_flows._prompt_auth_credentials_choice",
        lambda title: "use",
    )
    monkeypatch.setattr(
        "loki_cli.auth._prompt_model_selection",
    ...
    loki_main._model_flow_anthropic({}, current_model="glm-5.2")
...
def test_model_flow_nous_offers_tool_gateway_prompt_when_unconfigured(monkeypatch, capsys):
    ...
    monkeypatch.setattr("loki_cli.setup.prompt_checklist", _fake_checklist, raising=False)
...
def test_codex_provider_preserves_explicit_codex_model(monkeypatch):
    ...
    shell = cli.LokiCLI(model="gpt-5.1-codex-mini", compact=True, max_turns=1)
...
def test_codex_provider_strips_provider_prefix_from_model(monkeypatch):
    ...
    shell = cli.LokiCLI(model="openai/gpt-5.3-codex", compact=True, max_turns=1)
...
def test_cmd_model_falls_back_to_auto_on_invalid_provider(monkeypatch, capsys):
    ...
    monkeypatch.setattr("loki_cli.auth.resolve_provider", _resolve_provider)
    monkeypatch.setattr(loki_main, "_prompt_provider_choice", lambda choices, **kwargs: len(choices) - 1)
    monkeypatch.setattr("sys.stdin", type("FakeTTY", (), {"isatty": lambda self: True})())

    loki_main.cmd_model(SimpleNamespace())
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    ...
    monkeypatch.setattr("loki_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr("loki_cli.main._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
...
def test_model_flow_custom_persists_selected_api_mode(monkeypatch):
    ...
    monkeypatch.setattr(
        "loki_cli.main._save_custom_provider",
...
def test_cmd_model_forwards_nous_login_tls_options(monkeypatch):
    ...
    monkeypatch.setattr("loki_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("loki_cli.config.save_env_value", lambda key, value: None)
    monkeypatch.setattr("loki_cli.auth.resolve_provider", lambda requested, **kwargs: "nous")
    ...
    monkeypatch.setattr("loki_cli.auth._login_nous", _fake_login)
...
def test_auto_provider_name_localhost():
    from loki_cli.main import _auto_provider_name
...
def test_save_custom_provider_uses_provided_name(monkeypatch, tmp_path):
    ...
    import yaml
    from loki_cli.main import _save_custom_provider
