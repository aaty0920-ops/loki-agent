...
class TestBranchCommandCLI:
    ...
    def test_branch_rotates_loki_session_id_env_and_context(self, cli_instance, session_db):
        """Branching must update process-local session-id readers too."""
        from cli import LokiCLI
        from gateway.session_context import _UNSET, _VAR_MAP, get_session_env

        old_session_id = cli_instance.session_id
        os.environ["LOKI_SESSION_ID"] = old_session_id
        _VAR_MAP["LOKI_SESSION_ID"].set(old_session_id)

        try:
            LokiCLI._handle_branch_command(cli_instance, "/branch")

            assert cli_instance.session_id != old_session_id
            assert os.environ["LOKI_SESSION_ID"] == cli_instance.session_id
            assert get_session_env("LOKI_SESSION_ID") == cli_instance.session_id
        finally:
