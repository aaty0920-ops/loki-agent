...
class TestMaxTurnsResolution:
    ...
    def test_env_var_max_turns(self):
        ...
        cli_obj = _make_cli(env_overrides={"LOKI_MAX_ITERATIONS": "42"})
        assert cli_obj.max_turns == 42

    def test_invalid_env_var_max_turns_falls_back_to_default(self):
        """Invalid env values should not crash CLI init."""
        cli_obj = _make_cli(env_overrides={"LOKI_MAX_ITERATIONS": "not-a-number"})
...
class TestFallbackChainInit:
    def test_merges_new_and_legacy_fallback_config(self):
        cli = _make_cli(config_overrides={
            ...
            "fallback_model": {"provider": "nous", "model": "Loki-4"},
        })
        assert cli._fallback_model == [
            {"provider": "openrouter", "model": "anthropic/claude-sonnet-4.6"},
            {"provider": "nous", "model": "Loki-4"},
        ]
...
class TestHistoryDisplay:
    ...
    def test_resume_updates_loki_session_id_env_and_context(self, tmp_path):
        ...
        os.environ["LOKI_SESSION_ID"] = "current_session"
        _VAR_MAP["LOKI_SESSION_ID"].set("current_session")

        try:
            cli._handle_resume_command("/resume target_session")

            assert cli.session_id == "target_session"
            assert os.environ["LOKI_SESSION_ID"] == "target_session"
            assert get_session_env("LOKI_SESSION_ID") == "target_session"
        finally:
