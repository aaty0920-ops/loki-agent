...
Also verifies that ``loki_cli.main._launch_tui`` correctly handles exit
...
def test_managed_install_refuses_and_does_not_set_pending_relaunch(capsys):
    ...
    with (
        ...
        patch(
            ...
            return_value="Use `brew upgrade loki-agent` to update.",
        ),
    ):
        result = _call(self_)

    out = capsys.readouterr().out
    assert "brew upgrade loki-agent" in out
