...
class TestCLIStatusBar:
    ...
    def test_scrollback_box_width_returns_viewport_width(self):
        ...
        assert LokiCLI._scrollback_box_width(20) == 32
        assert LokiCLI._scrollback_box_width(32) == 32
        # Above the floor, return the actual viewport width — no cap.
        assert LokiCLI._scrollback_box_width(48) == 48
        assert LokiCLI._scrollback_box_width(80) == 80
        assert LokiCLI._scrollback_box_width(120) == 120
        assert LokiCLI._scrollback_box_width(200) == 200
...
class TestIdleSinceLastTurn:
    ...
    def test_scales_to_minutes(self):
        label = LokiCLI._format_idle_since(time.time() - 3 * 60, turn_live=False)
        assert label == "✓ 3m"
