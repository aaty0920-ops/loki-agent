"""Tests for ACP Registry metadata shipped with Loki."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "loki-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Loki Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/NousResearch/loki-agent"
    assert data["website"].startswith("https://loki-agent.nousresearch.com/")
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"loki-agent[acp]=={data['version']}"
    assert uvx["args"] == ["loki-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"loki-agent[acp]=={_pyproject_version()}"
    )
