...
class TestThreadLocalApprovalCallback:
    ...
    def test_sudo_password_cache_isolated_across_acp_sessions_on_same_pool_thread(self):
        ...
        binds LOKI_SESSION_KEY per session, so the cache scope key differs
...
class TestAcpExecAskGate:
    ...
    def test_interactive_context_var_routes_to_callback_without_env(
        ...
        tok = set_loki_interactive_context(True)
        try:
            result = check_all_command_guards(
            ...
            reset_loki_interactive_context(tok)

        assert called_with, (
            "set_loki_interactive_context(True) should route dangerous "
            "commands through the callback without LOKI_INTERACTIVE in env"
        )
        assert result["approved"] is True
