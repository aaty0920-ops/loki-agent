...
class TestQueryLocalContextLengthOllama:
    ...
    def test_ollama_num_ctx_wins_over_model_info(self):
        ...
        training max — using it would let Loki grow conversations past
        the runtime limit and Ollama would silently truncate.

        Concrete example: loki-brain:qwen3-14b-ctx32k is a Modelfile
        derived from qwen3:14b with `num_ctx 32768`, but the underlying
        GGUF reports `qwen3.context_length: 40960` (training max). If
        Loki used 40960 it would let the conversation grow past 32768
        ...
        with patch("agent.model_metadata.detect_local_server_type", return_value="ollama"), \
            ...
            result = _query_local_context_length(
                "loki-brain:qwen3-14b-ctx32k", "http://100.77.243.5:11434/v1"
            )

        assert result == 32768, (
            f"Expected num_ctx (32768) to win over model_info (40960), got {result}. "
            "If Loki uses the GGUF training max, conversations will silently truncate."
        )
...
class TestGetModelContextLengthLocalFallback:
    ...
    def test_local_endpoint_stale_cache_reconciled_from_live_probe(self):
        ...
        model = "NousResearch/Loki-3-Llama-3.1-70B"
    ...
    def test_local_endpoint_bypasses_stale_persistent_cache(self):
        """Loki-3-Llama names must not inherit the generic llama 131072 default."""
        from agent.model_metadata import get_model_context_length
