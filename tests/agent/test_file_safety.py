...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_loki_env(self):
        """Loki' own .env inside LOKI_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: loki internal .env is in ~/.loki/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.loki/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.loki/.env"))
        assert error is not None
