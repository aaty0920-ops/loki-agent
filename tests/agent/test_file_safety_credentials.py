...
Regression for https://github.com/NousResearch/loki-agent/issues/17656 —
...
def test_skills_hub_block_still_applies(fake_home):
    ...
    assert err is not None
    assert "internal Loki cache file" in err


def test_path_traversal_resolves_to_blocked(fake_home, tmp_path):
    """A path that traverses through a sibling dir back into LOKI_HOME's
    ...
    sibling.mkdir()
    traversal = sibling / ".." / "loki_home" / "auth.json"
...
def test_symlink_to_auth_json_blocked(fake_home, tmp_path):
    """A symlink pointing at LOKI_HOME/auth.json from outside the home
...
def test_read_file_tool_blocks_relative_path_under_terminal_cwd(
    ...
    ``read_file_tool`` against ``TERMINAL_CWD == LOKI_HOME`` must still
    ...
    _create(fake_home, "auth.json")
    # Force the file_tools resolver to anchor relative paths at LOKI_HOME
...
def test_dotenv_blocked(fake_home):
    """.env in LOKI_HOME holds API keys — blocked."""
    from agent.file_safety import get_read_block_error
