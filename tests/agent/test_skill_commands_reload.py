...
class TestReloadSkillsHelper:
    ...
    def test_returns_expected_keys(self, loki_home):
    ...
    def test_detects_newly_added_skill_with_description(self, loki_home):
        from agent.skill_commands import reload_skills, get_skill_commands

        # Prime the cache so subsequent diff is meaningful
        get_skill_commands()

        _write_skill(loki_home / "skills", "demo", "a demo skill")
        result = reload_skills()
    ...
    def test_description_passes_through_verbatim(self, loki_home):
        ...
        long_desc = "x" * 200
        _write_skill(loki_home / "skills", "longdesc", long_desc)

        result = reload_skills()
        assert len(result["added"]) == 1
        assert result["added"][0]["description"] == long_desc

    def test_unchanged_skills_appear_in_unchanged_list(self, loki_home):
        from agent.skill_commands import reload_skills, get_skill_commands

        _write_skill(loki_home / "skills", "alpha")
        # Prime cache
        get_skill_commands()
