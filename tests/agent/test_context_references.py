...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_loki_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("LOKI_HOME", str(tmp_path / ".loki"))

    loki_env = tmp_path / ".loki" / ".env"
    loki_env.parent.mkdir(parents=True)
    loki_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.loki/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
