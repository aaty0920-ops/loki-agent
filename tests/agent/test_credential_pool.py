...
def test_write_credential_pool_preserves_known_provider_owned_oauth_state(tmp_path, monkeypatch):
    ...
    persisted = json.loads((tmp_path / "loki" / "auth.json").read_text())["credential_pool"]["nous"][0]
...
def test_load_pool_removes_stale_file_backed_singleton_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.read_loki_oauth_credentials",
        lambda: None,
    )
    monkeypatch.setattr(
...
def test_singleton_seed_does_not_clobber_manual_oauth_entry(tmp_path, monkeypatch):
    ...
    assert {entry.source for entry in entries} == {"manual:loki_pkce", "loki_pkce"}
...
def test_load_pool_api_key_path_skips_oauth_autodiscovery(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("agent.anthropic_adapter.read_loki_oauth_credentials", _fake_pkce)
...
def test_load_pool_api_key_path_prunes_stale_oauth_entries(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("loki_cli.auth.is_provider_explicitly_configured", lambda pid: True)
    monkeypatch.setattr("agent.anthropic_adapter.read_loki_oauth_credentials", lambda: None)
...
def test_sync_codex_entry_from_auth_store_adopts_newer_tokens(tmp_path, monkeypatch):
    ...
    # Simulate `loki auth openai-codex` replacing the token pair on disk.
...
def test_codex_exhausted_entry_recovers_via_auth_store_sync(tmp_path, monkeypatch):
    ...
    rate-limit reset the user ran `loki model` to reauth, but the pool
    ...
    # Simulate `loki model` / `loki auth` refreshing the tokens.
...
def test_is_terminal_xai_oauth_refresh_error():
    from loki_cli.auth import AuthError, _is_terminal_xai_oauth_refresh_error
...
def test_is_terminal_codex_oauth_refresh_error():
    from loki_cli.auth import AuthError, _is_terminal_codex_oauth_refresh_error
