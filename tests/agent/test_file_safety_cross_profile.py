"""Tests for the cross-Loki-profile write guard in agent/file_safety.

The guard fires when a tool tries to write into another Loki profile's
...
Reference: May 2026 incident — a loki-security profile session
accidentally edited skills under both ~/.loki/profiles/loki-security/skills/
AND ~/.loki/skills/ (the default profile's skills), realizing only
...
class TestResolveActiveProfileName:
    ...
    def test_named_profile(self, fake_loki, monkeypatch):
        _set_active_home(monkeypatch, fake_loki["security_home"])
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "loki-security"
...
class TestClassifyCrossProfileTarget:
    def test_same_profile_write_returns_none(self, fake_loki, monkeypatch):
        _set_active_home(monkeypatch, fake_loki["security_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_loki["security_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is None

    def test_security_writing_default_skill(self, fake_loki, monkeypatch):
        """The exact incident from May 2026."""
        _set_active_home(monkeypatch, fake_loki["security_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_loki["default_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is not None
        assert result["active_profile"] == "loki-security"
        assert result["target_profile"] == "default"
        assert result["area"] == "skills"

    def test_default_writing_security_skill(self, fake_loki, monkeypatch):
        ...
        assert result["target_profile"] == "loki-security"

    def test_named_to_named_cross_profile(self, fake_loki, monkeypatch):
        _set_active_home(monkeypatch, fake_loki["security_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_loki["coder_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is not None
        assert result["target_profile"] == "coder"

    @pytest.mark.parametrize("area", ["skills", "plugins", "cron", "memories"])
    def test_all_profile_scoped_areas_classified(self, fake_loki, monkeypatch, area):
        _set_active_home(monkeypatch, fake_loki["security_home"])
        from agent.file_safety import classify_cross_profile_target
        target = fake_loki["default_home"] / area / "foo.txt"
        result = classify_cross_profile_target(str(target))
        assert result is not None
        assert result["area"] == area

    def test_non_loki_path_returns_none(self, fake_loki, monkeypatch, tmp_path):
        _set_active_home(monkeypatch, fake_loki["security_home"])
        from agent.file_safety import classify_cross_profile_target
        # Path outside any Loki root
        assert classify_cross_profile_target(str(tmp_path / "random.txt")) is None

    def test_loki_config_not_classified_as_cross_profile(self, fake_loki, monkeypatch):
        ...
        result = classify_cross_profile_target(
            str(fake_loki["default_home"] / "config.yaml")
        )
        assert result is None
...
class TestGetCrossProfileWarning:
    def test_in_profile_returns_none(self, fake_loki, monkeypatch):
        _set_active_home(monkeypatch, fake_loki["security_home"])
    ...
    def test_cross_profile_warning_names_both_profiles(self, fake_loki, monkeypatch):
        _set_active_home(monkeypatch, fake_loki["security_home"])
        from agent.file_safety import get_cross_profile_warning
        ...
        assert "loki-security" in warn
        # Must name the bypass kwarg.
        assert "cross_profile=True" in warn
        # Must reference the area.
        assert "skills" in warn

    def test_warning_is_defense_in_depth_not_boundary(self, fake_loki, monkeypatch):
        _set_active_home(monkeypatch, fake_loki["security_home"])
        from agent.file_safety import get_cross_profile_warning
        warn = get_cross_profile_warning(
            str(fake_loki["default_home"] / "skills" / "foo" / "SKILL.md")
