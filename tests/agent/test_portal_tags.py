...
def test_loki_client_tag_includes_current_version():
    """The client tag must reflect loki_cli.__version__ verbatim."""
    from loki_cli import __version__
    from agent.portal_tags import loki_client_tag

    assert loki_client_tag() == f"client=loki-client-v{__version__}"


def test_loki_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import loki_client_tag

    tag = loki_client_tag()
    assert tag.startswith("client=loki-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import loki_client_tag, nous_portal_tags
