...
def test_verify_on_stop_env_can_disable(clear_verify_env):
    clear_verify_env.setenv("LOKI_VERIFY_ON_STOP", "0")
...
def test_verify_on_stop_auto_off_for_each_messaging_platform(clear_verify_env, platform):
    clear_verify_env.setenv("LOKI_SESSION_PLATFORM", platform)
    assert verify_on_stop_enabled({"agent": {"verify_on_stop": "auto"}}) is False
...
def test_verify_on_stop_auto_uses_loki_platform_override(clear_verify_env):
    # LOKI_PLATFORM mirrors the sibling platform resolution and also flags a
    # messaging surface under the "auto" sentinel.
    clear_verify_env.setenv("LOKI_PLATFORM", "discord")
    assert verify_on_stop_enabled({"agent": {"verify_on_stop": "auto"}}) is False
