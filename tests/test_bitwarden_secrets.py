...
def test_install_bws_rejects_malicious_member(loki_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(loki_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(loki_home, monkeypatch):
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under loki_home/cache/."""
    home = tmp_path / ".loki"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
