...
def test_connect_honors_kanban_busy_timeout_env(kanban_home, monkeypatch):
    ...
    """
    monkeypatch.setenv("LOKI_KANBAN_BUSY_TIMEOUT_MS", "123456")
...
def test_claim_uses_env_default_ttl(kanban_home, monkeypatch):
    monkeypatch.setenv("LOKI_KANBAN_CLAIM_TTL_SECONDS", "3600")
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x", assignee="a")
...
def test_detect_crashed_workers_skips_freshly_claimed_tasks(
    ...
    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.delenv("LOKI_KANBAN_CRASH_GRACE_SECONDS", raising=False)
...
def test_detect_crashed_workers_grace_period_env_override(
    ...
    """LOKI_KANBAN_CRASH_GRACE_SECONDS env var adjusts the window."""
    import loki_cli.kanban_db as _kb

    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.setenv("LOKI_KANBAN_CRASH_GRACE_SECONDS", "5")
...
def test_resolve_crash_grace_seconds_handles_bad_env(monkeypatch):
    ...
    for bad_val in ("notanumber", "-5", ""):
        monkeypatch.setenv("LOKI_KANBAN_CRASH_GRACE_SECONDS", bad_val)
...
def test_respawn_guard_rate_limit_cooldown_zero_allows_immediately(
    ...
    monkeypatch.setenv("LOKI_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "0")
    now = 6_000_000
...
def test_resolve_rate_limit_cooldown_handles_bad_env(monkeypatch):
    ...
    for bad_val in ("notanumber", "-5", ""):
        monkeypatch.setenv(
            "LOKI_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", bad_val
        )
        assert (
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real Loki profile — preserves the
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="loki-task", assignee="daily")
...
def test_scratch_workspace_created_under_loki_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
        assert task is not None
...
def test_cleanup_workspace_removes_managed_scratch_dir(kanban_home):
    ...
    with kb.connect() as conn:
        ...
        assert ws.is_dir()
        kb.complete_task(conn, t, result="ok")
    assert not ws.exists(), "Loki-managed scratch dir should be cleaned up"
...
def test_cleanup_workspace_honors_workspaces_root_env_override(tmp_path, monkeypatch):
    """``LOKI_KANBAN_WORKSPACES_ROOT`` extends the managed-scratch set.
    ...
    monkeypatch.setenv("LOKI_KANBAN_WORKSPACES_ROOT", str(workspaces_override))
    kb.init_db()
...
class TestSharedBoardPaths:
    ...
    def test_default_install_anchors_at_home_dot_loki(
        self, tmp_path, monkeypatch
    ):
        # Standard install: LOKI_HOME == ~/.loki, no profile active.
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # LOKI_HOME and confirm equality. This is the property the
        ...
        # Worker's perspective (profile activated by `loki -p coder`).
    ...
    def test_docker_custom_loki_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
        # Docker / custom deployment: LOKI_HOME points outside ~/.loki.
        # `get_default_loki_root()` returns env_home directly when it
        # is not a `<root>/profiles/<name>` shape and not under
        # `Path.home() / ".loki"`.
        custom_root = tmp_path / "opt" / "loki"
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: LOKI_HOME=/opt/loki/profiles/coder;
        # `get_default_loki_root()` walks up to /opt/loki because
    ...
    def test_explicit_override_via_loki_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # Explicit override: LOKI_KANBAN_HOME beats every other
        # resolution rule.
        default_home = tmp_path / ".loki"
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LOKI_HOME", str(profile_home))
        monkeypatch.setenv("LOKI_KANBAN_HOME", str(override))
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("LOKI_KANBAN_HOME", "   ")
    ...
    def test_loki_kanban_db_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # LOKI_KANBAN_DB pins the file path directly and beats both
        # LOKI_KANBAN_HOME and the `get_default_loki_root()` path.
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LOKI_HOME", str(default_home))
        monkeypatch.setenv("LOKI_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("LOKI_KANBAN_DB", str(pinned_db))

        assert kb.kanban_db_path() == pinned_db
        # workspaces_root still follows LOKI_KANBAN_HOME -- the pins
        # are independent.
        assert kb.workspaces_root() == umbrella / "kanban" / "workspaces"

    def test_loki_kanban_workspaces_root_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # LOKI_KANBAN_WORKSPACES_ROOT pins the workspaces root directly.
        default_home = tmp_path / ".loki"
        default_home.mkdir()
        umbrella = tmp_path / "umbrella"
        umbrella.mkdir()
        ...
        monkeypatch.setenv("LOKI_HOME", str(default_home))
        monkeypatch.setenv("LOKI_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("LOKI_KANBAN_WORKSPACES_ROOT", str(pinned_ws))

        assert kb.workspaces_root() == pinned_ws
        # kanban_db_path still follows LOKI_KANBAN_HOME.
    ...
    def test_empty_per_path_overrides_fall_through(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LOKI_HOME", str(default_home))
        monkeypatch.setenv("LOKI_KANBAN_DB", "   ")
        monkeypatch.setenv("LOKI_KANBAN_WORKSPACES_ROOT", "")
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # The dispatcher's `_default_spawn` must inject LOKI_KANBAN_DB
        # and LOKI_KANBAN_WORKSPACES_ROOT into the worker env so the
        # worker converges on the dispatcher's paths even when the
        # `-p <profile>` flag rewrites LOKI_HOME.
        default_home = tmp_path / ".loki"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see loki_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_resolve_loki_argv_prefers_path_shim(monkeypatch):
    """When `loki` is on PATH, use the shim — preserves familiar ps output."""
    import shutil
    import loki_cli.kanban_db as kb

    monkeypatch.delenv("LOKI_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: "/usr/local/bin/loki")
    argv = kb._resolve_loki_argv()
    assert argv == ["/usr/local/bin/loki"]


def test_resolve_loki_argv_absolutizes_relative_exe_shim(monkeypatch, tmp_path):
    """A relative executable override must not remain workspace-cwd-dependent."""
    import loki_cli.kanban_db as kb

    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("LOKI_BIN", ".\\loki.exe")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_loki_argv() == [os.path.abspath(".\\loki.exe")]


def test_resolve_loki_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    bin_dir.mkdir()
    (bin_dir / "loki.CMD").write_text("@echo off\n", encoding="utf-8")
    monkeypatch.delenv("LOKI_BIN", raising=False)
    monkeypatch.setenv("PATH", str(bin_dir))
    monkeypatch.setenv("PATHEXT", ".CMD")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_loki_argv() == [sys.executable, "-m", "loki_cli.main"]


def test_resolve_loki_argv_honors_loki_bin_path_override(monkeypatch, tmp_path):
    """An explicit path-like LOKI_BIN lets service managers pin the executable."""
    import shutil
    import loki_cli.kanban_db as kb

    shim = tmp_path / "bin" / "loki"
    shim.parent.mkdir()
    shim.write_text("#!/bin/sh\n", encoding="utf-8")
    monkeypatch.setenv("LOKI_BIN", str(shim))
    monkeypatch.setattr(shutil, "which", lambda name: None)

    assert kb._resolve_loki_argv() == [str(shim)]


def test_resolve_loki_argv_loki_bin_bare_name_uses_path(monkeypatch, tmp_path):
    """Bare LOKI_BIN values keep PATH semantics instead of cwd shadowing."""
    import stat
    import loki_cli.kanban_db as kb

    cwd_loki = tmp_path / "loki"
    cwd_loki.write_text("wrong\n", encoding="utf-8")
    cwd_loki.chmod(cwd_loki.stat().st_mode | stat.S_IXUSR)
    path_loki = tmp_path / "bin" / "loki"
    path_loki.parent.mkdir()
    path_loki.write_text("right\n", encoding="utf-8")
    path_loki.chmod(path_loki.stat().st_mode | stat.S_IXUSR)
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("PATH", str(path_loki.parent))
    monkeypatch.setenv("LOKI_BIN", "loki")

    assert kb._resolve_loki_argv() == [str(path_loki)]


def test_resolve_loki_argv_loki_bin_bare_name_ignores_cwd(monkeypatch, tmp_path):
    """Bare LOKI_BIN does not accept current-directory shadow executables."""
    import sys
    import loki_cli.kanban_db as kb

    (tmp_path / "loki.exe").write_text("wrong\n", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
...
def test_resolve_loki_argv_loki_bin_bare_cmd_uses_module_fallback(monkeypatch, tmp_path):
    """A PATH-resolved LOKI_BIN batch shim is not used as worker argv[0]."""
    import sys
    import loki_cli.kanban_db as kb
...
def test_resolve_loki_argv_loki_bin_unresolved_bare_name_falls_back(monkeypatch):
    """Unresolved LOKI_BIN command names do not delegate cwd search to Popen."""
    import sys
    import loki_cli.kanban_db as kb
...
def test_write_txn_healthy_commit_no_exception(tmp_path):
    ...
    from loki_cli.kanban_db import connect, write_txn
    db = tmp_path / "test.db"
...
def test_write_txn_raises_on_truncated_file(tmp_path):
    ...
    with pytest.raises(sqlite3.DatabaseError, match="torn-extend|page count mismatch"):
        with unittest.mock.patch("loki_cli.kanban_db.os.path.getsize", side_effect=fake_getsize):
            with write_txn(conn) as c:
                c.execute(
                    "INSERT INTO tasks (id, title, assignee, status, priority, created_at) "
...
def test_write_txn_post_commit_check_fires_every_call(tmp_path):
    ...
    from loki_cli.kanban_db import connect, write_txn
    import loki_cli.kanban_db as kanban_db_module
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
    call_count = 0
...
def test_connect_sets_wal_autocheckpoint_100(tmp_path):
    ...
    from loki_cli.kanban_db import connect
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    import struct
    from loki_cli.kanban_db import connect, _check_file_length_invariant
...
def test_reap_worker_zombies_returns_count():
    ...
    with patch("loki_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("loki_cli.kanban_db._record_worker_exit"):
...
def test_reap_worker_zombies_noop_on_windows(monkeypatch):
    ...
    monkeypatch.setattr("loki_cli.kanban_db.os.name", "nt")
    with patch("loki_cli.kanban_db.os.waitpid") as mock_waitpid:
...
def test_reap_worker_zombies_noop_no_children():
    ...
    with patch("loki_cli.kanban_db.os.waitpid", side_effect=ChildProcessError):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_reap_worker_zombies_records_exit_status():
    ...
    with patch("loki_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch(
            "loki_cli.kanban_db._record_worker_exit",
...
def test_reap_worker_zombies_handles_waitpid_os_error():
    ...
    with patch("loki_cli.kanban_db.os.waitpid", side_effect=OSError("test error")):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_zombie_reaper_survives_all_boards_failing():
    ...
    for tick in range(5):
        ...
        with patch(
            "loki_cli.kanban_db.os.waitpid", side_effect=make_fake_waitpid(pids)
        ):
            with patch("loki_cli.kanban_db._record_worker_exit"):
                pids = kb.reap_worker_zombies()
...
def test_dispatch_once_still_reaps_via_extracted_fn(kanban_home):
    ...
    with patch("loki_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("loki_cli.kanban_db._record_worker_exit"):
            with patch("loki_cli.kanban_db.os.name", "posix"):
                pids = kb.reap_worker_zombies()
