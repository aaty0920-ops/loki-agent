...
def test_project_env_overrides_stale_shell_values_when_user_env_missing(tmp_path, monkeypatch):
    ...
    loaded = load_loki_dotenv(loki_home=home, project_env=project_env)
...
def test_main_import_applies_user_env_over_shell_values(tmp_path, monkeypatch):
    ...
    home.mkdir()
    (home / ".env").write_text(
        "OPENAI_BASE_URL=https://new.example/v1\nLOKI_INFERENCE_PROVIDER=custom\n",
        encoding="utf-8",
    )
    ...
    sys.modules.pop("loki_cli.main", None)
    importlib.import_module("loki_cli.main")

    assert os.getenv("OPENAI_BASE_URL") == "https://new.example/v1"
    assert os.getenv("LOKI_INFERENCE_PROVIDER") == "custom"
