...
def test_get_git_commit_falls_back_to_build_sha_when_live_git_fails(tmp_path):
    ...
    with patch("loki_cli.dump.subprocess.run", return_value=failed), \
         patch("loki_cli.build_info.get_build_sha", return_value="cafef00d"):
        commit = dump._get_git_commit(repo_dir)
...
def test_get_git_commit_falls_back_when_git_returns_empty_stdout(tmp_path):
    ...
    with patch("loki_cli.dump.subprocess.run", return_value=empty), \
         patch("loki_cli.build_info.get_build_sha", return_value="abcdef12"):
        commit = dump._get_git_commit(repo_dir)
...
def test_get_git_commit_falls_back_when_git_raises(tmp_path):
    ...
    with patch("loki_cli.dump.subprocess.run", side_effect=FileNotFoundError("git")), \
         patch("loki_cli.build_info.get_build_sha", return_value="feedface"):
        commit = dump._get_git_commit(repo_dir)
...
def test_get_git_commit_returns_unknown_when_neither_source_available(tmp_path):
    ...
    failed = MagicMock(returncode=128, stdout="")
    with patch("loki_cli.dump.subprocess.run", return_value=failed), \
         patch("loki_cli.build_info.get_build_sha", return_value=None):
        commit = dump._get_git_commit(repo_dir)
