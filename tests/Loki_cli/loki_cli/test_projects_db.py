...
def test_slug_collision_disambiguates(conn):
    pdb.create_project(conn, name="Loki Agent")
    pdb.create_project(conn, name="Loki Agent")
    slugs = sorted(p.slug for p in pdb.list_projects(conn))

    assert slugs == ["loki-agent", "loki-agent-2"]
...
def test_db_path_under_loki_home():
    # Resolves under LOKI_HOME (set by the autouse isolation fixture).
