...
def test_recovery_self_lock_guard_clears_marker_without_install(tmp_path, monkeypatch):
    # Windows self-lock: loki.exe is an ancestor of this Python process, so a
    ...
    monkeypatch.setattr(m, "_loki_exe_shims", lambda d: [shim])
...
def test_recovery_self_lock_guard_inactive_when_not_ancestor(tmp_path, monkeypatch):
    # Windows, but loki.exe is NOT in the ancestry (launched via `loki
    ...
    class FakeProc:
        ...
        def parents(self):
            # Ancestry is plain pythons / cmd — no loki.exe shim.
