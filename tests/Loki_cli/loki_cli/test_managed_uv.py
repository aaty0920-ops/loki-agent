...
class TestManagedUvPath:
    def test_posix(self, tmp_path):
        with patch("loki_cli.managed_uv.get_loki_home", return_value=tmp_path), \
             patch("loki_cli.managed_uv.platform.system", return_value="Linux"):
            from loki_cli.managed_uv import managed_uv_path
            assert managed_uv_path() == tmp_path / "bin" / "uv"

    def test_windows(self, tmp_path):
        with patch("loki_cli.managed_uv.get_loki_home", return_value=tmp_path), \
             patch("loki_cli.managed_uv.platform.system", return_value="Windows"):
            from loki_cli.managed_uv import managed_uv_path
...
class TestUpdateManagedUv:
    def test_no_uv_returns_none(self, tmp_path):
        with patch("loki_cli.managed_uv.get_loki_home", return_value=tmp_path):
            from loki_cli.managed_uv import update_managed_uv
...
class TestInstallUvInternals:
    def test_posix_sets_uv_unmanaged_install(self, tmp_path):
        ...
        with patch("loki_cli.managed_uv._install_uv_posix") as mock_posix:
            from loki_cli.managed_uv import _install_uv
            _install_uv(target)
    ...
    def test_windows_sets_uv_install_dir(self, tmp_path):
        ...
        with patch("loki_cli.managed_uv.platform.system", return_value="Windows"), \
