...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from loki_cli.debug import _upload_dpaste_com
...
class TestCaptureLogSnapshot:
    ...
    def test_reads_small_file(self, loki_home):
    ...
    def test_empty_primary_reports_file_empty(self, loki_home):
    ...
    def test_truncates_large_file(self, loki_home):
        """Files larger than max_bytes get tail-truncated."""
        from loki_cli.debug import _capture_log_snapshot

        # Write a file larger than 1KB
        big_content = "x" * 100 + "\n"
        (loki_home / "logs" / "agent.log").write_text(big_content * 200)
    ...
    def test_drops_partial_when_truncation_mid_line(self, loki_home):
    ...
    def test_unknown_log_returns_none(self, loki_home):
        from loki_cli.debug import _capture_log_snapshot
        snap = _capture_log_snapshot("nonexistent", tail_lines=10)
        assert snap.full_text is None

    def test_falls_back_to_rotated_file(self, loki_home):
        """When gateway.log doesn't exist, falls back to gateway.log.1."""
        from loki_cli.debug import _capture_log_snapshot

        logs_dir = loki_home / "logs"
    ...
    def test_prefers_primary_over_rotated(self, loki_home):
    ...
    def test_falls_back_when_primary_empty(self, loki_home):
...
class TestCollectDebugReport:
    ...
    def test_report_includes_agent_log(self, loki_home):
        from loki_cli.debug import collect_debug_report
    ...
    def test_report_includes_errors_log(self, loki_home):
        from loki_cli.debug import collect_debug_report
    ...
    def test_report_includes_gateway_log(self, loki_home):
        from loki_cli.debug import collect_debug_report
    ...
    def test_report_includes_gui_log(self, loki_home):
        from loki_cli.debug import collect_debug_report
    ...
    def test_report_includes_desktop_log(self, loki_home):
        from loki_cli.debug import collect_debug_report
...
class TestRunDebugShare:
    ...
    def test_share_survives_sweep_failure(self, loki_home, capsys):
        ...
        with patch("loki_cli.dump.run_dump"), \
             patch(
                 "loki_cli.debug._sweep_expired_pastes",
    ...
    def test_local_flag_prints_full_logs(self, loki_home, capsys):
    ...
    def test_share_keeps_report_and_full_log_on_same_snapshot(self, loki_home, capsys):
        """A mid-run rotation must not make full agent.log older than the report."""
        from loki_cli.debug import run_debug_share, collect_debug_report as real_collect_debug_report
        ...
        with patch("loki_cli.dump.run_dump"), \
             patch("loki_cli.debug.collect_debug_report", side_effect=_wrapped_collect_debug_report), \
             patch("loki_cli.debug.upload_to_pastebin", side_effect=_mock_upload):
            run_debug_share(args)
    ...
    def test_share_continues_on_log_upload_failure(self, loki_home, capsys):
    ...
    def test_share_exits_on_report_upload_failure(self, loki_home, capsys):
...
class TestScheduleAutoDelete:
    ...
    handle the DELETE requests synchronously on the next ``loki debug``
    invocation.
    """

    def test_does_not_spawn_subprocess(self, loki_home):
        ...
        import ast
        import inspect
        from loki_cli.debug import _schedule_auto_delete
    ...
    def test_skips_non_paste_rs_urls(self, loki_home):
    ...
    def test_merges_with_existing_pending(self, loki_home):
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_empty_is_noop(self, loki_home):
    ...
    def test_sweep_drops_entries_past_grace_window(self, loki_home):
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_survives_sweep_failure(self, loki_home, capsys):
        ...
        out = capsys.readouterr().out
        assert "Usage: loki debug" in out
...
class TestRunDebugDelete:
    def test_deletes_valid_url(self, capsys):
        ...
        with patch("loki_cli.debug.delete_paste", return_value=True):
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_schedules_auto_delete(self, loki_home, capsys):
        from loki_cli.debug import run_debug_share
        ...
        with patch("loki_cli.dump.run_dump"), \
             ...
             patch("loki_cli.debug._schedule_auto_delete") as mock_sched:
            run_debug_share(args)
    ...
    def test_local_no_privacy_notice(self, loki_home, capsys):
        from loki_cli.debug import run_debug_share
...
class TestBuildDebugShare:
    ...
    def test_returns_structured_urls(self, loki_home):
        from loki_cli.debug import build_debug_share, DebugShareResult
    ...
    def test_skips_missing_logs_without_failure(self, loki_home):
        ...
        with patch("loki_cli.dump.run_dump"), patch(
            "loki_cli.debug.upload_to_pastebin",
    ...
    def test_required_report_failure_raises(self, loki_home):
        from loki_cli.debug import build_debug_share
...
class TestCollectShareBundle:
    def test_returns_report_and_logs(self, loki_home):
    ...
    def test_build_debug_share_uses_collector(self, loki_home):
        # build_debug_share must produce the same report text the collector does
        # (i.e. the refactor preserved paste.rs behaviour).
        from loki_cli.debug import build_debug_share, collect_share_bundle
...
class TestBuildNousBundle:
    def test_envelope_shape_and_gzip(self, loki_home):
        ...
        from loki_cli.debug import build_nous_bundle
...
class TestRunDebugShareNous:
    ...
    def test_nous_success_prints_view_url(self, loki_home, capsys):
        from loki_cli.debug import run_debug_share
        ...
        with patch("loki_cli.dump.run_dump"), patch(
            "loki_cli.diagnostics_upload.share_to_nous", return_value=res
        ) as share:
    ...
    def test_nous_failure_suggests_local(self, loki_home, capsys):
        from loki_cli.debug import run_debug_share

        with patch("loki_cli.dump.run_dump"), patch(
            "loki_cli.diagnostics_upload.share_to_nous",
    ...
    def test_nous_does_not_touch_pastebin(self, loki_home):
        from loki_cli.debug import run_debug_share
...
class TestShareConsentGate:
    ...
    def test_aborts_on_user_decline(self, loki_home, capsys, monkeypatch):
    ...
    def test_proceeds_on_user_accept(self, loki_home, capsys, monkeypatch):
    ...
    def test_yes_flag_skips_prompt(self, loki_home, capsys, monkeypatch):
    ...
    def test_non_interactive_with_yes_succeeds(self, loki_home, capsys, monkeypatch):
    ...
    def test_nous_path_also_gated(self, loki_home, capsys, monkeypatch):
        ...
        with patch("loki_cli.dump.run_dump"), \
             patch("loki_cli.diagnostics_upload.share_to_nous") as mock_nous:
