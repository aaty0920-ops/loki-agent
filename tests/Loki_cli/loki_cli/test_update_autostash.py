...
def test_stash_local_changes_if_needed_returns_specific_stash_commit(monkeypatch, tmp_path):
    ...
    def fake_run(cmd, **kwargs):
        calls.append((cmd, kwargs))
        if cmd[-2:] == ["status", "--porcelain"]:
            return SimpleNamespace(stdout=" M loki_cli/main.py\n?? notes.txt\n", returncode=0)
...
def test_resolve_stash_selector_returns_matching_entry(monkeypatch, tmp_path):
    ...
    assert loki_main._resolve_stash_selector(["git"], tmp_path, "abc123") == "stash@{1}"
...
def test_restore_stashed_changes_prompts_before_applying(monkeypatch, tmp_path, capsys):
    ...
    restored = loki_main._restore_stashed_changes(["git"], tmp_path, "abc123", prompt_user=True)
...
def test_restore_stashed_changes_applies_without_prompt_when_disabled(monkeypatch, tmp_path, capsys):
    ...
    restored = loki_main._restore_stashed_changes(["git"], tmp_path, "abc123", prompt_user=False)
...
def test_print_stash_cleanup_guidance_with_selector(capsys):
    loki_main._print_stash_cleanup_guidance("abc123", "stash@{2}")
...
def test_restore_stashed_changes_always_resets_on_conflict(monkeypatch, tmp_path, capsys):
    ...
    Leaving conflict markers in source files makes loki unrunnable (SyntaxError).
    ...
    def fake_run(cmd, **kwargs):
        ...
        if cmd[1:3] == ["diff", "--name-only"]:
            return SimpleNamespace(stdout="loki_cli/main.py\n", stderr="", returncode=0)
    ...
    result = loki_main._restore_stashed_changes(["git"], tmp_path, "abc123", prompt_user=True)

    assert result is False
    out = capsys.readouterr().out
    assert "Conflicted files:" in out
    assert "loki_cli/main.py" in out
...
def test_restore_stashed_changes_auto_resets_non_interactive(monkeypatch, tmp_path, capsys):
    ...
    result = loki_main._restore_stashed_changes(["git"], tmp_path, "abc123", prompt_user=False)
...
def test_stash_local_changes_if_needed_raises_when_stash_ref_missing(monkeypatch, tmp_path):
    ...
    with pytest.raises(CalledProcessError):
        loki_main._stash_local_changes_if_needed(["git"], Path(tmp_path))
...
def test_discard_lockfile_churn_skips_lock_when_package_json_dirty(tmp_path):
    ...
    loki_main._discard_lockfile_churn(["git"], tmp_path)
...
def _setup_update_mocks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(loki_config, "check_config_version", lambda: (5, 5))
    monkeypatch.setattr(loki_config, "migrate_config", lambda **kw: {"env_added": [], "config_added": []})
    monkeypatch.setattr(loki_main, "_refresh_active_lazy_features", lambda: None)
...
def test_cmd_update_retries_optional_extras_individually_when_all_fails(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(loki_main, "_is_termux_env", lambda env=None: False)
    monkeypatch.setattr(loki_main, "_load_installable_optional_extras", lambda group="all": ["matrix", "mcp"])
...
def test_install_with_optional_fallback_honors_custom_group(monkeypatch):
    ...
    monkeypatch.setattr(loki_main, "_run_install_with_heartbeat", fake_run_with_heartbeat)

    loki_main._install_python_dependencies_with_optional_fallback(
        ["/usr/bin/uv", "pip"],
        group="termux-all",
    )
...
def test_install_heartbeat_prints_when_dependency_install_is_silent(monkeypatch, capsys):
    ...
    def fake_run(cmd, **kwargs):
        loki_main._time.sleep(1.2)
        return SimpleNamespace(returncode=0)

    monkeypatch.setattr(loki_main.subprocess, "run", fake_run)

    loki_main._run_install_with_heartbeat(
        ["uv", "pip", "install", "-e", "."],
...
def test_non_interactive_discard_throws_changes_away(monkeypatch, tmp_path):
    ...
    loki_main.cmd_update(SimpleNamespace(gateway=True))
...
def test_interactive_update_ignores_discard_setting(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(loki_main.sys.stdin, "isatty", lambda: True)
    monkeypatch.setattr(loki_main.sys.stdout, "isatty", lambda: True)

    loki_main.cmd_update(SimpleNamespace())  # no gateway, no --yes
...
def test_non_interactive_defaults_to_stash_when_setting_absent(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(loki_config, "load_config", lambda *a, **kw: {"model": {}})
