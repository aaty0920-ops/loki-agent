...
def test_console_parses_bare_and_loki_prefixed_commands(_isolate_loki_home):
    engine = LokiConsoleEngine()

    bare = engine.execute("config path")
    prefixed = engine.execute("loki config path")
...
def test_console_status_hides_cli_next_step_footer(
    ...
    import loki_cli.status as status_mod
    ...
    def fake_show_status(_args):
        ...
        print("\x1b[2m  Run 'loki doctor' for detailed diagnostics\x1b[0m")
        print("\x1b[2m  Run 'loki setup' to configure\x1b[0m")
        print()

    monkeypatch.setattr(status_mod, "show_status", fake_show_status)

    result = LokiConsoleEngine().execute("status")

    assert result.status == "ok"
    assert "Sessions" in result.output
    assert "Active: 3 session(s)" in result.output
    assert "loki doctor" not in result.output
    assert "loki setup" not in result.output
...
def test_console_status_hides_osc_linked_cli_next_step_footer(
    ...
    def fake_show_status(_args):
        ...
        print()
        print(osc_link("\u2500" * 60))
        print(osc_link("  Run 'loki doctor' for detailed diagnostics"))
        print(osc_link("  Run 'loki setup' to configure"))
        print()
...
def test_console_help_uses_cli_subcommand_summaries():
    help_text = LokiConsoleEngine().help_text()
    ...
    assert "Run `loki skills list`" not in help_text
    assert "Run `loki tools list`" not in help_text
...
def test_console_help_for_command_uses_cli_summary():
    help_text = LokiConsoleEngine().help_text("skills list")

    assert help_text == "skills list\nList installed skills"


def test_console_registry_covers_non_admin_cli_surface():
    registered = set(LokiConsoleEngine().commands)
...
def test_hosted_console_registry_exposes_only_hosted_safe_surface():
    engine = LokiConsoleEngine(context="hosted")
    hosted = {
...
def test_hosted_console_rejects_local_only_or_dangerous_commands(line):
    result = LokiConsoleEngine(context="hosted").execute(line)
...
def test_console_rejects_destructive_and_shell_like_commands(line):
    result = LokiConsoleEngine().execute(line)
...
def test_help_lists_supported_commands_and_not_full_cli():
    result = LokiConsoleEngine().execute("help")
...
def test_config_set_requires_confirmation_then_writes(_isolate_loki_home):
    engine = LokiConsoleEngine()
...
def test_repl_refuses_non_interactive_confirmation(_isolate_loki_home):
...
def test_main_console_subcommand_smoke(_isolate_loki_home):
    import subprocess

    result = subprocess.run(
        [sys.executable, "-m", "loki_cli.main", "console"],
    ...
    assert "Loki Console" in result.stdout
