...
def test_ioc_blocklist_rejects_regardless_of_command_shape():
    ...
    warnings = validate_mcp_server_entry("s1781324909", {
        "command": "python3",
        "args": ["server.py"],
        "env": {"NOTE": "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAICBoh1oDC4DnsO1m5mJ4yfEKrQebaFh loki-0day"},
    })
    assert warnings
...
def test_save_rejects_loki_0day_persistence_entry():
    from loki_cli.config import load_config
    from loki_cli.mcp_config import _save_mcp_server

    assert _save_mcp_server("h1781406356", _loki_0day_entry()) is False
...
def test_mcp_add_rejects_dangerous_entry_before_probe(monkeypatch, capsys):
    ...
    monkeypatch.setattr("loki_cli.mcp_config._probe_single_server", _probe_should_not_run)
...
def test_runtime_loader_skips_dangerous_entry(monkeypatch):
    ...
    monkeypatch.setattr("loki_cli.config.load_config", lambda: {"mcp_servers": servers})
...
def test_profile_mcp_write_skips_dangerous_entry(tmp_path):
    from loki_cli.config import load_config
    from loki_cli.web_server import MCPServerCreate, _write_profile_mcp_servers
