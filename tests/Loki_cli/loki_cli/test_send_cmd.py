...
def test_load_loki_env_does_not_override_existing(tmp_path, monkeypatch):
    """Existing env vars must not be clobbered by config.yaml values."""
    import os

    loki_home = tmp_path / ".loki"
    loki_home.mkdir()
    (loki_home / "config.yaml").write_text("TELEGRAM_HOME_CHANNEL: yaml_value\n")
...
def test_load_loki_env_handles_missing_files(tmp_path, monkeypatch):
