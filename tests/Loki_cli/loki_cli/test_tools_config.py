...
from loki_cli.nous_account import NousPortalAccountInfo
...
def test_get_platform_tools_x_search_auto_enabled_when_xai_oauth_present(monkeypatch):
    ...
    to also click through `loki tools` → X (Twitter) Search to flip the
    toolset on. Tool's check_fn still gates schema registration if creds
    later go missing.
    """
    monkeypatch.delenv("XAI_API_KEY", raising=False)
    monkeypatch.setattr(
        "loki_cli.tools_config._xai_credentials_present", lambda: True
    )
...
def test_get_platform_tools_x_search_off_when_no_xai_credentials(monkeypatch):
    ...
    monkeypatch.delenv("XAI_API_KEY", raising=False)
    monkeypatch.setattr(
        "loki_cli.tools_config._xai_credentials_present", lambda: False
    )
...
def test_get_platform_tools_x_search_respects_explicit_config(monkeypatch):
    """Once the user has saved an explicit toolset list via `loki tools`,
    ...
    # User explicitly opted into spotify but not x_search via `loki tools`.
    config = {"platform_toolsets": {"cli": ["loki-cli", "spotify"]}}
    enabled = _get_platform_tools(config, "cli")
...
def test_get_platform_tools_expands_composite_when_mixed_with_configurable():
    """``[loki-cli, spotify]`` (composite + configurable) must keep the full
    ``loki-cli`` toolset alongside the explicit Spotify opt-in. The
    has_explicit_config branch used to drop ``loki-cli`` on the floor,
    ...
    for ts in ("terminal", "file", "web", "browser", "memory", "delegation",
        ...
        assert ts in enabled, f"{ts} should be enabled when loki-cli is listed"
...
def test_get_platform_tools_mixed_does_not_resurrect_default_off():
    ...
    pull-in. Without this, ``loki-cli`` expansion would re-enable
    ``moa`` / ``rl`` / ``homeassistant`` for users who never opted in."""
    config = {"platform_toolsets": {"cli": ["loki-cli", "terminal"]}}
...
def test_get_platform_tools_preserves_explicit_empty_selection():
    ...
    # live in _LOKI_CORE_TOOLS but aren't user-toggleable) are still
    # auto-recovered by _get_platform_tools so saving via `loki tools`
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/NousResearch/loki-agent/issues/1247
    """
    config = {
        "platform_toolsets": {
...
def test_save_platform_tools_does_not_preserve_platform_default_toolsets():
    """Platform default toolsets (loki-cli, loki-telegram, etc.) must NOT
    ...
    ``loki tools``, but loki-cli stays in the config and re-enables
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                ...
                "delegation", "file", "loki-cli",  # <-- the culprit
    ...
    # loki-cli must NOT survive — it's a platform default, not an MCP server
    assert "loki-cli" not in saved
...
def test_save_platform_tools_does_not_preserve_loki_telegram():
    """Same bug for Telegram — loki-telegram must not be preserved."""
    config = {
        "platform_toolsets": {
            "telegram": [
                "browser", "file", "loki-telegram", "terminal", "web",
            ]
        }
    }
    ...
    assert "loki-telegram" not in saved
    assert "web" in saved
...
def test_save_platform_tools_still_preserves_mcp_with_platform_default_present():
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                "web", "terminal", "loki-cli", "my-mcp-server", "github-tools",
            ]
        }
    }
...
def test_visible_providers_include_nous_subscription_when_logged_in(monkeypatch):
    ...
    monkeypatch.setattr(
        "loki_cli.nous_subscription.get_nous_portal_account_info",
...
def test_visible_providers_force_fresh_shows_nous_subscription_after_upgrade(monkeypatch):
    ...
    monkeypatch.setattr(
        "loki_cli.tools_config.get_nous_subscription_features",
...
def test_local_browser_provider_is_saved_explicitly(monkeypatch):
    ...
    monkeypatch.setattr("loki_cli.tools_config._run_post_setup", lambda key: None)
...
def test_fresh_install_browser_default_is_free_local_not_paid_nous():
    ...
    """
    from loki_cli.tools_config import _detect_active_provider_index
...
def test_reconfigure_lists_enabled_web_without_existing_provider_config(monkeypatch):
    ...
    monkeypatch.setattr("loki_cli.tools_config._prompt_choice", fake_prompt_choice)
    monkeypatch.setattr(
        "loki_cli.tools_config._configure_tool_category_for_reconfig",
...
def test_first_install_nous_auto_configures_managed_defaults(monkeypatch):
    monkeypatch.setattr("loki_cli.nous_subscription.managed_nous_tools_enabled", lambda: True)
    config = {
        "model": {"provider": "nous"},
        "platform_toolsets": {"cli": []},
    }
...
def test_numeric_mcp_server_name_does_not_crash_sorted():
    ...
    Regression test for https://github.com/NousResearch/loki-agent/issues/6901
    """
    config = {
...
def test_computer_use_needs_configuration_when_cua_driver_post_setup_pending():
    ...
    Returning users enabling Computer Use through `loki tools` must reach the
...
def test_computer_use_respects_custom_cua_driver_command():
    """The setup gate should match runtime's LOKI_CUA_DRIVER_CMD override."""
    def fake_which(name: str):
        return "/opt/bin/custom-cua" if name == "custom-cua" else None

    with patch.dict("os.environ", {"LOKI_CUA_DRIVER_CMD": "custom-cua"}), \
...
def test_computer_use_blank_custom_driver_command_falls_back_to_default():
    ...
    with patch.dict("os.environ", {"LOKI_CUA_DRIVER_CMD": "   "}), \
         patch("shutil.which", side_effect=fake_which):
        assert _toolset_needs_configuration_prompt("computer_use", {}) is False
...
def test_save_platform_tools_clears_no_mcp_sentinel():
    """`loki tools` has no UI for no_mcp, so saving from the picker clears
...
def test_get_platform_tools_recovers_non_configurable_toolsets_from_composite():
    ...
    fake_toolsets["loki-_test_platform"] = {
    ...
    test_platforms = {
        "_test_platform": {"label": "Test", "default_toolset": "loki-_test_platform"},
    }

    with mock_patch("loki_cli.tools_config.PLATFORMS", {**PLATFORMS, **test_platforms}):
...
def test_get_platform_tools_second_pass_skips_fully_claimed_toolsets():
    ...
    be added by the second pass (prevents 'search', 'loki-acp' noise).
    """
...
def test_get_platform_tools_discord_both_off_by_default():
    """Both `discord` and `discord_admin` are opt-in via `loki tools`,
...
def test_discord_toolsets_not_available_on_other_platforms():
    ...
    from loki_cli.tools_config import _toolset_allowed_for_platform
...
def test_discord_toolsets_user_enabled_are_honored():
    """When the user opts in via `loki tools`, the toolset appears."""
...
def test_save_platform_tools_strips_restricted_toolsets():
    ...
    from loki_cli.tools_config import _save_platform_tools
    config = {}
...
def test_reconfigure_provider_syncs_use_gateway(monkeypatch, provider, config_key, expected):
    ...
    monkeypatch.setattr(
        "loki_cli.nous_subscription.ensure_nous_portal_access",
        lambda **kwargs: True,
    )
    config = {}
...
def test_reconfigure_provider_runs_post_setup_for_env_var_providers(
    ...
    called = []
    monkeypatch.setattr("loki_cli.tools_config._run_post_setup", lambda key: called.append(key))
    monkeypatch.setattr("loki_cli.tools_config.get_env_value", lambda k: None)
    monkeypatch.setattr("loki_cli.tools_config._prompt", lambda *a, **kw: "")
    monkeypatch.setattr("loki_cli.tools_config.save_env_value", lambda k, v: None)
...
def test_configure_non_managed_provider_skips_portal_gate(monkeypatch):
    ...
    monkeypatch.setattr(
        "loki_cli.nous_subscription.ensure_nous_portal_access", _boom
    )
...
def test_apply_provider_selection_web_sets_backend():
    ...
    from loki_cli.tools_config import apply_provider_selection
...
def test_vision_picker_writes_provider_and_model(tmp_path, monkeypatch):
    ...
    as ``loki model`` and writes the selection to the auxiliary config keys
    the resolver reads.
    """
    monkeypatch.setenv("LOKI_HOME", str(tmp_path))
    ...
    seq = iter([1, 0, 1])
    with patch("loki_cli.model_switch.list_authenticated_providers",
