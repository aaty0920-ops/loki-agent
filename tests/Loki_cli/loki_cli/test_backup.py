...
def _make_loki_tree(root: Path) -> None:
    ...
    (root / "loki_state.db").write_bytes(b"fake-state")
...
class TestShouldExclude:
    ...
    def test_includes_nested_loki_agent_in_skills(self):
        """skills/autonomous-ai-agents/loki-agent/ must NOT be excluded —
        only the root-level loki-agent/ repo is skipped."""
        from loki_cli.backup import _should_exclude
        assert not _should_exclude(Path("skills/autonomous-ai-agents/loki-agent/SKILL.md"))
        assert not _should_exclude(Path("skills/autonomous-ai-agents/loki-agent/sub/item.txt"))
    ...
    def test_excludes_regeneratable_dependency_and_cache_dirs(self, rel):
        """Python dep trees and tool caches under LOKI_HOME must be skipped —
...
class TestBackup:
    def test_creates_zip(self, tmp_path, monkeypatch):
        ...
        # get_default_loki_root needs this
    ...
    def test_excludes_loki_agent(self, tmp_path, monkeypatch):
        """Backup does NOT include loki-agent/ directory."""
        loki_home = tmp_path / ".loki"
        loki_home.mkdir()
        _make_loki_tree(loki_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "loki-agent" in n]
            assert agent_files == [], f"loki-agent files leaked into backup: {agent_files}"

    def test_excludes_dependency_and_cache_trees(self, tmp_path, monkeypatch):
        """A plugin venv / site-packages / pip cache under LOKI_HOME must be
        ...
        venv_pkg = loki_home / "plugins" / "heavy" / ".venv" / "lib" / "site-packages" / "dep"
        venv_pkg.mkdir(parents=True)
        (venv_pkg / "__init__.py").write_text("# dep\n")
        pip_cache = loki_home / ".cache" / "uv" / "wheels"
    ...
    def test_includes_nested_loki_agent_in_skills(self, tmp_path, monkeypatch):
        """Backup includes skills/.../loki-agent/ but NOT root loki-agent/."""
        loki_home = tmp_path / ".loki"
        loki_home.mkdir()
        _make_loki_tree(loki_home)

        # Add a nested loki-agent directory inside skills (like the real layout)
        nested = loki_home / "skills" / "autonomous-ai-agents" / "loki-agent"
        nested.mkdir(parents=True)
        (nested / "SKILL.md").write_text("# Loki Agent Skill\n")
        (nested / "sub").mkdir()
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            # Root loki-agent must be excluded
            root_agent = [n for n in names if n.startswith("loki-agent/")]
            assert root_agent == [], f"root loki-agent leaked: {root_agent}"
            # Nested skill loki-agent must be included
            assert "skills/autonomous-ai-agents/loki-agent/SKILL.md" in names
            assert "skills/autonomous-ai-agents/loki-agent/sub/item.txt" in names
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        """When no output path given, zip goes to ~/loki-backup-*.zip."""
        loki_home = tmp_path / ".loki"
        loki_home.mkdir()
        ...
        zips = list(tmp_path.glob("loki-backup-*.zip"))
        assert len(zips) == 1
...
class TestValidateBackupZip:
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only loki_state.db (old wrong name) is rejected."""
        from loki_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["loki_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into loki home."""
        loki_home = tmp_path / ".loki"
        loki_home.mkdir()
        monkeypatch.setenv("LOKI_HOME", str(loki_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (loki_home / "config.yaml").read_text() == "model:\n  provider: openrouter\n"
        assert (loki_home / ".env").read_text() == "OPENROUTER_API_KEY=sk-test\n"
        assert (loki_home / "skills" / "my-skill" / "SKILL.md").read_text() == "# My Skill\n"
        assert (loki_home / "profiles" / "coder" / "config.yaml").exists()

    def test_strips_loki_prefix(self, tmp_path, monkeypatch):
        """Import strips .loki/ prefix if all entries share it."""
        loki_home = tmp_path / ".loki"
        loki_home.mkdir()
        monkeypatch.setenv("LOKI_HOME", str(loki_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".loki/config.yaml": "model: test\n",
            ".loki/skills/a/SKILL.md": "# A\n",
        })
        ...
        assert (loki_home / "config.yaml").read_text() == "model: test\n"
        assert (loki_home / "skills" / "a" / "SKILL.md").read_text() == "# A\n"
    ...
    def test_rejects_non_loki_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a loki backup."""
        loki_home = tmp_path / ".loki"
        loki_home.mkdir()
        monkeypatch.setenv("LOKI_HOME", str(loki_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        # traversal file should NOT exist outside loki home
    ...
    def test_preserves_live_gateway_state(self, tmp_path, monkeypatch):
        ...
        (loki_home / "gateway_state.json").write_text(live_state)
        ...
        assert (loki_home / "gateway_state.json").read_text() == live_state
    ...
    def test_does_not_seed_gateway_state_when_absent(self, tmp_path, monkeypatch):
        ...
        assert (loki_home / "config.yaml").exists()
        assert not (loki_home / "gateway_state.json").exists()
    ...
    def test_confirmation_prompt_abort(self, tmp_path, monkeypatch):
        ...
        (loki_home / "config.yaml").write_text("existing: true\n")
        monkeypatch.setenv("LOKI_HOME", str(loki_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (loki_home / "config.yaml").read_text() == "existing: true\n"
    ...
    def test_force_skips_confirmation(self, tmp_path, monkeypatch):
        ...
        assert (loki_home / "config.yaml").read_text() == "model: restored\n"
...
class TestFormatSize:
    def test_bytes(self):
        from loki_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without loki markers fails validation."""
        import io
        from loki_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_loki(self):
        """Detects .loki/ prefix wrapping all entries."""
        import io
        from loki_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".loki/config.yaml", "test")
            zf.writestr(".loki/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".loki/"
...
class TestBackupEdgeCases:
    def test_nonexistent_loki_home(self, tmp_path, monkeypatch):
        """Backup exits when loki home doesn't exist."""
        fake_home = tmp_path / "nonexistent" / ".loki"
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("loki-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_skips_output_zip_inside_loki(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside loki root."""
        loki_home = tmp_path / ".loki"
        loki_home.mkdir()
        (loki_home / "config.yaml").write_text("model: test\n")
        ...
        # Output inside loki home
        out_zip = loki_home / "backup.zip"
        args = Namespace(output=str(out_zip))
...
class TestImportEdgeCases:
    ...
    def test_keyboard_interrupt_during_confirmation(self, tmp_path, monkeypatch):
        ...
        loki_home = tmp_path / ".loki"
        loki_home.mkdir()
        (loki_home / ".env").write_text("KEY=val\n")
        monkeypatch.setenv("LOKI_HOME", str(loki_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert (loki_home / "profiles" / "coder" / "config.yaml").exists()
        assert (loki_home / "profiles" / "researcher" / "config.yaml").exists()
        ...
        assert "loki -p coder" in coder_wrapper
...
class TestQuickSnapshot:
    ...
    def test_creates_snapshot(self, loki_home):
    ...
    def test_label_in_id(self, loki_home):
        from loki_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(label="before-upgrade", loki_home=loki_home)
    ...
    def test_copies_nested_files(self, loki_home):
        from loki_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(loki_home=loki_home)
        assert (loki_home / "state-snapshots" / snap_id / "cron" / "jobs.json").exists()

    def test_copies_channel_aliases(self, loki_home):
        from loki_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(loki_home=loki_home)
        copied = loki_home / "state-snapshots" / snap_id / "channel_aliases.json"
        assert copied.exists()
        assert "120363408391911677@g.us" in copied.read_text()

    def test_missing_files_skipped(self, loki_home):
        from loki_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(loki_home=loki_home)
        with open(loki_home / "state-snapshots" / snap_id / "manifest.json") as f:
            meta = json.load(f)
    ...
    def test_empty_home_returns_none(self, tmp_path):
        ...
        empty.mkdir()
        assert create_quick_snapshot(loki_home=empty) is None

    def test_list_snapshots(self, loki_home):
        from loki_cli.backup import create_quick_snapshot, list_quick_snapshots
        id1 = create_quick_snapshot(label="first", loki_home=loki_home)
        id2 = create_quick_snapshot(label="second", loki_home=loki_home)

        snaps = list_quick_snapshots(loki_home=loki_home)
        assert len(snaps) == 2
        assert snaps[0]["id"] == id2  # most recent first
        assert snaps[1]["id"] == id1

    def test_list_limit(self, loki_home):
        from loki_cli.backup import create_quick_snapshot, list_quick_snapshots
        for i in range(5):
            create_quick_snapshot(label=f"s{i}", loki_home=loki_home)
        snaps = list_quick_snapshots(limit=3, loki_home=loki_home)
        assert len(snaps) == 3

    def test_restore_config(self, loki_home):
        from loki_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(loki_home=loki_home)

        (loki_home / "config.yaml").write_text("model:\n  provider: anthropic\n")
        assert "anthropic" in (loki_home / "config.yaml").read_text()

        result = restore_quick_snapshot(snap_id, loki_home=loki_home)
        assert result is True
        assert "openrouter" in (loki_home / "config.yaml").read_text()

    def test_restore_state_db(self, loki_home):
        from loki_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(loki_home=loki_home)

        conn = sqlite3.connect(str(loki_home / "state.db"))
    ...
    def test_restore_nonexistent(self, loki_home):
        from loki_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", loki_home=loki_home) is False

    def test_auto_prune(self, loki_home):
        from loki_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
        for i in range(_QUICK_DEFAULT_KEEP + 5):
            create_quick_snapshot(label=f"snap-{i:03d}", loki_home=loki_home)
        snaps = list_quick_snapshots(limit=100, loki_home=loki_home)
        assert len(snaps) <= _QUICK_DEFAULT_KEEP

    def test_manual_prune(self, loki_home):
        from loki_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", loki_home=loki_home)
        deleted = prune_quick_snapshots(keep=3, loki_home=loki_home)
        assert deleted == 7
        assert len(list_quick_snapshots(loki_home=loki_home)) == 3
    ...
    def test_empty_pairing_dir_does_not_fail(self, loki_home):
...
# ---------------------------------------------------------------------------
    ...
    def test_restore_rejects_snapshot_id_traversal(self, loki_home):
    ...
    def test_restore_rejects_manifest_rel_traversal(self, loki_home):
        ...
        escape_dst = loki_home.parent.parent / "outside.txt"
        ...
        assert not escape_dst.exists(), (
            f"manifest rel traversal escaped LOKI_HOME: {escape_dst} exists"
        )
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_projects_db_snapshotted(self, loki_home):
        from loki_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(loki_home=loki_home)
        copy = loki_home / "state-snapshots" / snap_id / "projects.db"
        assert copy.exists()
    ...
    def test_kanban_db_snapshotted(self, loki_home):
        from loki_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(loki_home=loki_home)
        copy = loki_home / "state-snapshots" / snap_id / "kanban.db"
        assert copy.exists()
        conn = sqlite3.connect(str(copy))
    ...
    def test_restore_recreates_emptied_projects_db(self, loki_home):
        from loki_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(loki_home=loki_home)

        # Simulate the upgrade wiping the store back to an empty schema.
        conn = sqlite3.connect(str(loki_home / "projects.db"))
    ...
    def test_additional_per_profile_dbs_round_trip(self, loki_home):
        ...
        for name, (table, row) in seeded.items():
            conn = sqlite3.connect(str(loki_home / name))
            conn.execute(f"CREATE TABLE {table} (id TEXT PRIMARY KEY, data TEXT)")
            conn.execute(f"INSERT INTO {table} VALUES (?, ?)", row)
            conn.commit()
            conn.close()
    ...
    def test_board_workspaces_and_attachments_are_skipped(self, loki_home):
        ...
        snap_id = create_quick_snapshot(loki_home=loki_home)
        snap = loki_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work"
...
class TestPreUpdateBackup:
    ...
    def test_rotation_preserves_manual_files(self, loki_home):
        """Hand-dropped zips in ``backups/`` must not be touched by
        rotation — it only prunes files matching ``pre-update-*.zip``."""
        import time as _t
        from loki_cli.backup import create_pre_update_backup

        (loki_home / "backups").mkdir(exist_ok=True)
        manual = loki_home / "backups" / "my-manual.zip"
        manual.write_bytes(b"manual backup")

        for _ in range(5):
            create_pre_update_backup(loki_home=loki_home, keep=2)
            _t.sleep(1.05)
    ...
    def test_returns_none_if_root_missing(self, tmp_path):
        from loki_cli.backup import create_pre_update_backup
        assert create_pre_update_backup(loki_home=tmp_path / "does-not-exist") is None

    def test_keep_zero_does_not_delete_freshly_created_backup(self, loki_home):
        ...
        """
        from loki_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(loki_home=loki_home, keep=0)
        assert out is not None
        assert out.exists(), (
    ...
    def test_keep_negative_does_not_delete_freshly_created_backup(self, loki_home):
    ...
    def test_keep_zero_still_prunes_older_backups(self, loki_home):
        ...
        first = create_pre_update_backup(loki_home=loki_home, keep=5)
        _t.sleep(1.05)
        second = create_pre_update_backup(loki_home=loki_home, keep=5)
        _t.sleep(1.05)
        third = create_pre_update_backup(loki_home=loki_home, keep=0)
...
class TestRunPreUpdateBackup:
    ...
    def loki_home(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for loki_cli.config + loki_constants so they pick up LOKI_HOME
        for mod in list(__import__("sys").modules.keys()):
            if mod.startswith("loki_cli.config") or mod == "loki_constants":
                del __import__("sys").modules[mod]
        return root

    def test_backup_flag_creates_backup(self, loki_home, capsys):
        """--backup forces the pre-update backup for one run even when config is off."""
        from loki_cli.main import _run_pre_update_backup
        _run_pre_update_backup(Namespace(no_backup=False, backup=True))
        out = capsys.readouterr().out
        assert "Creating pre-update backup" in out
        assert "Saved:" in out
        assert "Restore:" in out
        assert "loki import" in out
        assert "Disable:" in out
        # Actual backup was created
        backups = list((loki_home / "backups").glob("pre-update-*.zip"))
        assert len(backups) == 1

    def test_default_disabled_is_silent(self, loki_home, capsys):
        """With the default (``pre_update_backup: false``), ``loki update``
        ...
        out = capsys.readouterr().out
        assert out == ""
        assert not list((loki_home / "backups").glob("pre-update-*.zip")) \
            if (loki_home / "backups").exists() else True

    def test_no_backup_flag_skips(self, loki_home, capsys):
        from loki_cli.main import _run_pre_update_backup
        ...
        # No backup written
        assert not (loki_home / "backups").exists() or not list(
            (loki_home / "backups").glob("pre-update-*.zip")
        )

    def test_config_enabled_creates_backup(self, loki_home, capsys):
        ...
        for mod in list(_sys.modules.keys()):
            if mod.startswith("loki_cli.config"):
    ...
    def test_config_disabled_is_silent(self, loki_home, capsys):
    ...
    def test_cli_flag_overrides_enabled_config(self, loki_home, capsys):
...
class TestPreMigrationBackup:
    ...
    def test_creates_backup_under_backups_dir(self, loki_home):
        ...
        assert out is not None
        assert out.exists()
        # Shares the backups/ directory with pre-update backups so `loki
        # import` and the update-backup listing both pick them up.
        assert out.parent == loki_home / "backups"
        assert out.name.startswith("pre-migration-")
        assert out.suffix == ".zip"

    def test_backup_uses_shared_exclusion_rules(self, loki_home):
        """Pre-migration backup reuses the same exclusion rules as
        ``loki backup`` / ``create_pre_update_backup`` — no drift."""
        from loki_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(loki_home=loki_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_recurse_into_prior_backups(self, loki_home):
        from loki_cli.backup import create_pre_migration_backup
        out1 = create_pre_migration_backup(loki_home=loki_home)
        assert out1 is not None
        out2 = create_pre_migration_backup(loki_home=loki_home)
        assert out2 is not None
        with zipfile.ZipFile(out2) as zf:
            names = zf.namelist()
        assert not any(n.startswith("backups/") for n in names)
    ...
    def test_rotation_keeps_only_n(self, loki_home):
        ...
        created = []
        for _ in range(7):
            out = create_pre_migration_backup(loki_home=loki_home, keep=3)
            if out is not None:
                created.append(out)
            _t.sleep(1.05)  # timestamp resolution

        remaining = sorted((loki_home / "backups").glob("pre-migration-*.zip"))
        assert len(remaining) <= 3, f"expected <=3 backups retained, got {len(remaining)}"

    def test_missing_loki_home_returns_none(self, tmp_path):
        """Fresh install with no ~/.loki yet — nothing to back up."""
        from loki_cli.backup import create_pre_migration_backup
        missing = tmp_path / "does-not-exist"
        out = create_pre_migration_backup(loki_home=missing)
        assert out is None
...
class TestRestoreCronJobsIfEmptied:
    ...
    def test_noop_when_snapshot_id_missing(self, tmp_path):
        ...
        jobs_path = loki_home / "cron" / "jobs.json"
        self._seed_jobs(jobs_path, [])
        assert restore_cron_jobs_if_emptied(None, loki_home=loki_home) is None
        assert restore_cron_jobs_if_emptied("", loki_home=loki_home) is None
...
class TestMemoryProviderExternalPaths:
    ...
    def test_backup_captures_external_paths_under_external_prefix(self, tmp_path, monkeypatch):
        ...
        # External provider state living OUTSIDE LOKI_HOME.
