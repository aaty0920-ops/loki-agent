...
def test_xai_oauth_poll_device_token_waits_until_authorized(monkeypatch):
    ...
    monkeypatch.setattr("loki_cli.auth.time.sleep", lambda _: None)
...
def test_read_xai_oauth_tokens_missing_refresh_token(tmp_path, monkeypatch):
    loki_home = tmp_path / "loki"
    _setup_loki_auth(loki_home, refresh_token="")
    monkeypatch.setenv("LOKI_HOME", str(loki_home))
...
def test_resolve_xai_runtime_credentials_honours_env_base_url(tmp_path, monkeypatch):
    ...
    _setup_loki_auth(loki_home, access_token=fresh)
    monkeypatch.setenv("LOKI_HOME", str(loki_home))
    monkeypatch.setenv("LOKI_XAI_BASE_URL", "https://custom.x.ai/v1/")
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    """
    loki_home = tmp_path / "loki"
    _seed_xai_oauth_state(loki_home, dict(_STALE_XAI_OAUTH_STATE), active_provider="nous")
    monkeypatch.setenv("LOKI_HOME", str(loki_home))
...
def test_resolve_credentials_does_not_quarantine_on_transient_refresh_failure(
    ...
    """
    loki_home = tmp_path / "loki"
    _seed_xai_oauth_state(loki_home, dict(_STALE_XAI_OAUTH_STATE))
    monkeypatch.setenv("LOKI_HOME", str(loki_home))
    ...
    monkeypatch.setattr("loki_cli.auth._refresh_xai_oauth_tokens", _transient_refresh)
...
def test_pool_entry_needs_refresh_when_jwt_within_skew(tmp_path, monkeypatch):
    ...
    from agent.credential_pool import load_pool, AUTH_TYPE_OAUTH, PooledCredential
    from loki_cli.auth import XAI_ACCESS_TOKEN_REFRESH_SKEW_SECONDS
    import uuid
...
def test_pool_refresh_marks_entry_exhausted_on_failure(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("loki_cli.auth.refresh_xai_oauth_pure", _fake_refresh_fail)
...
def test_pool_seeded_entry_sync_back_after_refresh(tmp_path, monkeypatch):
    ...
    loki_home = tmp_path / "loki"
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_loki_auth(loki_home, access_token=near_expiry, refresh_token="rt-singleton")
    monkeypatch.setenv("LOKI_HOME", str(loki_home))
...
def test_pool_refresh_adopts_singleton_tokens_when_consumed_elsewhere(tmp_path, monkeypatch):
    """Multi-process race: another Loki process refreshed the singleton
    ...
    profiles + Loki processes."""
    from agent.credential_pool import load_pool

    loki_home = tmp_path / "loki"
    in_memory_at = _jwt_with_exp(int(time.time()) + 30)  # near-expiry
    _setup_loki_auth(loki_home, access_token=in_memory_at, refresh_token="rt-stale")
    monkeypatch.setenv("LOKI_HOME", str(loki_home))
...
def test_pool_exhausted_xai_entry_recovers_after_singleton_refresh(tmp_path, monkeypatch):
    ...
    user runs ``loki model`` -> xAI Grok OAuth (or another process
    ...
    _setup_loki_auth(loki_home, access_token=stale_at, refresh_token="rt-stale")
    monkeypatch.setenv("LOKI_HOME", str(loki_home))
    ...
    # Simulate the user re-running `loki model` -> xAI Grok OAuth: the
...
def test_pool_manual_xai_entry_not_synced_from_singleton(tmp_path, monkeypatch):
    ...
    ``loki auth add xai-oauth``) own their own refresh-token lifecycle
