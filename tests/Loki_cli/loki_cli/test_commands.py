...
class TestSlackNativeSlashes:
    ...
    def test_loki_catchall_is_first(self):
        """``/loki`` must be reserved as the first slot so the legacy
        ``/loki <subcommand>`` form keeps working after we add new
        commands and hit the 50-slash cap."""
        slashes = slack_native_slashes()
        assert slashes[0][0] == "loki"
    ...
    def test_excludes_slack_reserved_commands(self):
        ...
        Users can still reach them via /loki <command>."""
        names = {n for n, _d, _h in slack_native_slashes()}
    ...
    def test_includes_aliases_as_first_class_slashes(self):
        ...
        aliases stay reachable via ``/loki <alias>``.
        """
        slashes = slack_native_slashes()
...
class TestSubcommandCompletion:
    ...
    def test_tools_enable_completes_toolset_names(self, monkeypatch):
        ...
        from loki_cli import commands as commands_mod
...
from loki_cli.commands import discord_skill_commands_by_category  # noqa: E402
...
class TestPluginCommandEnumeration:
    ...
    def test_plugin_command_appears_in_slack_subcommand_map(self, monkeypatch):
        """/loki metricas must route through the Slack subcommand map."""
        self._patch_plugin_commands(monkeypatch, {
            "metricas": {
                "handler": lambda _a: "ok",
                "description": "Metrics",
