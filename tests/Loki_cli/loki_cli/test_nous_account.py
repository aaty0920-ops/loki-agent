...
def test_valid_jwt_with_paid_access_true(monkeypatch):
    token = _jwt(
        {
            ...
            "org_id": "org_123",
            "client_id": "loki-cli",
            "product_id": "nous-loki-agent",
            "nous_client": "loki-agent",
    ...
    assert info.product_id == "nous-loki-agent"
...
def test_expired_jwt_falls_back_to_fresh_account(monkeypatch):
    ...
    monkeypatch.setattr("loki_cli.auth.get_provider_auth_state", lambda provider: _state(token))
    monkeypatch.setattr("loki_cli.auth.resolve_nous_access_token", lambda: "fresh-token")
    monkeypatch.setattr("loki_cli.nous_account._fetch_nous_account_info", lambda *a, **kw: payload)
...
def test_entitlement_message_for_inference_key_without_portal_login():
    ...
    assert "Log in with `loki model`" in message
...
def test_entitlement_message_for_unknown_entitlement_is_explicit():
    ...
    assert "Run `loki model`" in message
