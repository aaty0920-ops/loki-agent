...
def test_spotify_interactive_setup_persists_client_id(
    ...
    """The wizard writes LOKI_SPOTIFY_CLIENT_ID to .env and returns the value."""
    monkeypatch.setenv("LOKI_HOME", str(tmp_path))
    ...
    assert "LOKI_SPOTIFY_CLIENT_ID=wizard-client-123" in env_text
    # Default redirect URI should NOT be persisted.
    assert "LOKI_SPOTIFY_REDIRECT_URI" not in env_text
...
def test_spotify_interactive_setup_empty_aborts(
    ...
    env_path = tmp_path / ".env"
    if env_path.exists():
        assert "LOKI_SPOTIFY_CLIENT_ID" not in env_path.read_text()
