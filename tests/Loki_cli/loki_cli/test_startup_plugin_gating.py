...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["loki", "--tui", "chat"], "chat"),
        (["loki", "-w", "logs"], "logs"),
        (["loki", "chat", "hello world"], "chat"),
        (["loki", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["loki", "-m", "gpt5", "chat"], "chat"),
        (["loki", "--model", "gpt5", "chat", "hi"], "chat"),
        (["loki", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["loki", "-z", "hello world"], None),
        (["loki", "-z", "hello", "chat"], "chat"),
        (["loki", "--model=gpt5", "chat"], "chat"),     # inline form
        (["loki", "--", "chat"], "chat"),               # -- terminator
        (["loki", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["loki", "some-plugin-cmd"], "some-plugin-cmd"),
        (["loki", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["loki"],                          # bare → chat
        ["loki", "--help"],                # top-level help
        ["loki", "-h"],
        ["loki", "version"],               # known built-in
        ["loki", "logs"],
        ["loki", "gateway", "run"],
        ["loki", "--tui"],
        ["loki", "-w", "--tui"],
        ["loki", "chat", "hi"],
        ["loki", "help"],                  # accepted built-in-ish
        ["loki", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["loki", "meet", "join"],          # potential google_meet plugin
        ["loki", "honcho", "status"],      # potential memory plugin
        ["loki", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``loki help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"loki_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
