...
def test_dry_run_makes_no_filesystem_changes(tmp_path: Path) -> None:
    ...
    actions = reconcile_profile_gateways(
        loki_home=tmp_path, scandir=scandir, dry_run=True,
    )
...
def test_missing_profiles_root_still_registers_default_slot(
    ...
    """When $LOKI_HOME/profiles doesn't exist (fresh install), the
    ...
    so `loki gateway start` (no -p) has somewhere to land."""
    scandir = tmp_path / "run-service"; scandir.mkdir()
    actions = reconcile_profile_gateways(
        loki_home=tmp_path, scandir=scandir, dry_run=False,
    )
...
def test_default_slot_always_registered_on_empty_home(tmp_path: Path) -> None:
    """Bare LOKI_HOME with nothing under it still produces a
...
def test_default_slot_run_script_omits_profile_flag(tmp_path: Path) -> None:
    ...
    that would resolve to $LOKI_HOME/profiles/default/ instead of
    the root profile. It must call `loki gateway run` directly."""
    scandir = tmp_path / "run-service"; scandir.mkdir()
...
@pytest.mark.parametrize(
    ...
    [
        ("gateway", "run"),
        ("/init", "/opt/loki/docker/main-wrapper.sh", "gateway", "run"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("/init", "/opt/loki/docker/main-wrapper.sh", "gateway", "run", "--no-supervise"),
    ],
)
...
def test_legacy_gateway_run_env_no_supervise_does_not_seed_s6_state(
    ...
    scandir = tmp_path / "run-service"; scandir.mkdir()
    monkeypatch.setenv("LOKI_GATEWAY_NO_SUPERVISE", "1")
...
def test_default_slot_cleans_up_stale_runtime_files_at_root(
    ...
    """gateway.pid and processes.json at the LOKI_HOME root (left
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Through s6 /init + the main-wrapper that re-execs `loki`.
        ("/init", "/opt/loki/docker/main-wrapper.sh", "dashboard"),
        (
            "/init",
        ...
        # Wrapper that kept the explicit `loki` argv0.
        ("/init", "/opt/loki/docker/main-wrapper.sh", "loki", "dashboard"),
...
def test_is_dashboard_container_true_for_dashboard_argv(
    ...
    from loki_cli.container_boot import _is_dashboard_container
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("gateway", "run"),
        ("/init", "/opt/loki/docker/main-wrapper.sh", "gateway", "run"),
        ("/init", "/opt/loki/docker/main-wrapper.sh", "loki", "gateway", "run"),
        ("chat",),
...
def test_main_skips_reconcile_in_dashboard_container(
    ...
    monkeypatch.setattr(
        ...
        lambda: ("/init", "/opt/loki/docker/main-wrapper.sh", "dashboard"),
    )
...
def test_main_reconciles_in_gateway_container(
    ...
    monkeypatch.setattr(
        container_boot,
        "_read_container_argv",
        lambda: ("/init", "/opt/loki/docker/main-wrapper.sh", "gateway", "run"),
    )
...
def test_main_ignores_removed_skip_reconcile_env_var(
    ...
    """The legacy LOKI_SKIP_PROFILE_RECONCILE flag is gone: setting it on a
    ...
    monkeypatch.setenv("LOKI_HOME", str(tmp_path))
    monkeypatch.setenv("S6_PROFILE_GATEWAY_SCANDIR", str(scandir))
    monkeypatch.setenv("LOKI_SKIP_PROFILE_RECONCILE", "1")
    monkeypatch.setattr(
        container_boot,
        "_read_container_argv",
        lambda: ("/init", "/opt/loki/docker/main-wrapper.sh", "gateway", "run"),
