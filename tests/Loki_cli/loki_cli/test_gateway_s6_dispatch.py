...
def test_dispatch_returns_false_on_host(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "loki_cli.service_manager.detect_service_manager", lambda: "systemd",
    )
...
def test_dispatch_defaults_profile_to_default(
    ...
    monkeypatch.setattr(
        "loki_cli.gateway._profile_suffix", lambda: "",
    )
...
def test_dispatch_renders_gateway_not_registered_friendly(
    ...
    """`loki -p typo gateway start` should print a clear message and
    exit 1 — not dump a traceback at the user."""
    from loki_cli import gateway as gw
    from loki_cli.service_manager import GatewayNotRegisteredError
    ...
    monkeypatch.setattr(
        "loki_cli.service_manager.get_service_manager", lambda: _RaisesMissing(),
    )
    ...
    assert "loki profile create typo" in out
...
def test_dispatch_renders_s6_command_error_friendly(
    ...
    monkeypatch.setattr(
        "loki_cli.service_manager.get_service_manager", lambda: _RaisesS6Error(),
    )
...
def test_redirect_fires_inside_s6_container(
    ...
    monkeypatch.setattr("loki_cli.gateway.os.execvp", fake_execvp)
    ...
    assert "LOKI_GATEWAY_NO_SUPERVISE" in err
...
def test_block_until_terminated_installs_sigterm_handler_and_blocks(
    ...
    monkeypatch.setattr(
        "loki_cli.gateway.signal.signal",
    ...
    monkeypatch.setattr("loki_cli.gateway.signal.pause", fake_pause)
...
def test_redirect_short_circuits_supervised_child(
    ...
    running execs `loki gateway run --replace`, the
    LOKI_S6_SUPERVISED_CHILD sentinel must short-circuit the redirect
...
def test_redirect_respects_no_supervise_env(
    ...
    """`LOKI_GATEWAY_NO_SUPERVISE=1` (env var) must skip the redirect.
    ...
    monkeypatch.delenv("LOKI_S6_SUPERVISED_CHILD", raising=False)
    monkeypatch.setenv("LOKI_GATEWAY_NO_SUPERVISE", value)
...
def test_redirect_no_supervise_env_falsy_values_dont_opt_out(
    ...
    """Falsy / unrecognized values of LOKI_GATEWAY_NO_SUPERVISE must
    NOT opt out. We're strict about what counts as "yes" so a typo
    like `LOKI_GATEWAY_NO_SUPERVISE=0` doesn't silently enable the
    ...
    for falsy in ("", "0", "false", "no", "off", "garbage"):
        execvp_calls.clear()
        monkeypatch.setenv("LOKI_GATEWAY_NO_SUPERVISE", falsy)
