...
def test_managed_install_takes_precedence(tmp_path):
    """When LOKI_MANAGED is set, that takes precedence over git detection."""
    (tmp_path / ".git").mkdir()
    with patch("loki_cli.config.get_managed_system", return_value="NixOS"), \
         patch("loki_cli.config.get_loki_home", return_value=tmp_path):
        from loki_cli.config import detect_install_method
        method = detect_install_method(project_root=tmp_path)
        assert method == "nixos"
...
def test_home_docker_stamp_ignored_when_not_containerized(tmp_path):
    ...
    'docker' into the shared $LOKI_HOME. With no code-scoped stamp, a host
    ...
    with patch("loki_cli.config.get_managed_system", return_value=None), \
         patch("loki_cli.config.get_loki_home", return_value=home), \
         patch("loki_cli.config._running_in_container", return_value=False):
        from loki_cli.config import detect_install_method
        assert detect_install_method(project_root=code) == "git"
...
def test_home_docker_stamp_honored_inside_container(tmp_path):
    ...
    with patch("loki_cli.config.get_managed_system", return_value=None), \
         patch("loki_cli.config.get_loki_home", return_value=home), \
         patch("loki_cli.config._running_in_container", return_value=True):
        from loki_cli.config import detect_install_method
...
def test_home_non_docker_stamp_still_honored_for_backcompat(tmp_path):
    ...
    install that historically stamped 'git'/'pip' into $LOKI_HOME keeps
...
def test_stamp_install_method_writes_code_scoped(tmp_path):
    """stamp_install_method writes next to the code, not into $LOKI_HOME."""
    code = tmp_path / "code"
    home = tmp_path / "home"
    code.mkdir()
    home.mkdir()
...
def test_banner_warns_on_pip_install(tmp_path):
    ...
    with patch("loki_cli.config.get_loki_home", return_value=hh), \
         patch("loki_constants.get_loki_home", return_value=hh):
        buf = io.StringIO()
