...
def test_check_for_updates_invalidates_on_version_change(tmp_path, monkeypatch):
    ...
    fake_banner = tmp_path / "loki_cli" / "banner.py"
    ...
    with patch("loki_cli.banner.subprocess.run") as mock_run, \
         patch("loki_cli.banner.check_via_pypi", return_value=0) as mock_pypi:
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("LOKI_HOME", str(tmp_path))
    with patch("loki_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_official_ssh_origin_uses_https_probe(tmp_path):
    ...
    def fake_run(cmd, **kwargs):
        ...
        if cmd == ["git", "remote", "get-url", "origin"]:
            return MagicMock(returncode=0, stdout="git@github.com:NousResearch/loki-agent.git\n")
...
def test_check_via_local_git_shallow_clone_behind_reports_no_count(tmp_path):
    ...
    def fake_run(cmd, **kwargs):
        calls.append(cmd)
        if cmd == ["git", "remote", "get-url", "origin"]:
            return MagicMock(returncode=0, stdout="https://github.com/NousResearch/loki-agent.git\n")
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("LOKI_HOME", str(tmp_path))
    with patch("loki_cli.banner.subprocess.run") as mock_run:
        with patch("loki_cli.banner.check_via_pypi", return_value=0):
            result = banner.check_for_updates()
    assert result == 0
    mock_run.assert_not_called()
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when LOKI_HOME has no git repo."""
    import loki_cli.banner as banner
    ...
    # Point LOKI_HOME at a temp dir with no loki-agent/.git
    monkeypatch.setenv("LOKI_HOME", str(tmp_path))
    with patch("loki_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_docker_returns_none(tmp_path, monkeypatch):
    ...
    LOKI_REVISION (nix-only), so without a docker guard check_for_updates()
    ...
    container, and `loki update` correctly refusing to run there. The guard
    ...
    with patch("loki_cli.config.detect_install_method", return_value="docker"), \
         patch("loki_cli.banner.subprocess.run") as mock_run, \
         patch("loki_cli.banner.check_via_pypi") as mock_pypi:
        result = banner.check_for_updates()
...
def test_check_for_updates_non_docker_still_checks(tmp_path, monkeypatch):
    ...
    with patch("loki_cli.config.detect_install_method", return_value="pip"), \
         patch("loki_cli.banner.subprocess.run") as mock_run, \
         patch("loki_cli.banner.check_via_pypi", return_value=1) as mock_pypi:
        result = banner.check_for_updates()
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from loki_cli.main import _invalidate_update_cache

    # Build a fake ~/.loki with default + two named profiles
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"LOKI_HOME": str(default_home)}):
