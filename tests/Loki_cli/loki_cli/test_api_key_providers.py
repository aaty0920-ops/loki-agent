...
PROVIDER_ENV_VARS = (
    ...
    "OPENAI_BASE_URL", "LOKI_COPILOT_ACP_COMMAND", "COPILOT_CLI_PATH",
...
class TestApiKeyProviderStatus:
    ...
    def test_copilot_status_uses_gh_cli_token(self, monkeypatch):
        monkeypatch.setattr("loki_cli.copilot_auth._try_gh_cli_token", lambda: "gho_gh_cli_token")
    ...
    def test_get_auth_status_dispatches_to_external_process(self, monkeypatch):
        monkeypatch.setattr("loki_cli.auth.shutil.which", lambda command: f"/opt/bin/{command}")
...
class TestHasAnyProviderConfigured:
    ...
    def test_claude_code_creds_counted_when_loki_configured(self, monkeypatch, tmp_path):
        """Claude Code credentials should count when Loki has been explicitly configured."""
        import yaml
        from loki_cli import config as config_module
        loki_home = tmp_path / ".loki"
        loki_home.mkdir()
...
class TestZaiEndpointAutoDetect:
    ...
    def test_probe_success_returns_detected_url(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "loki_cli.auth.detect_zai_endpoint",
            lambda *a, **kw: {
                "id": "coding-global",
...
class TestKimiMoonshotModelListIsolation:
    ...
    def test_moonshot_list_excludes_coding_plan_only_models(self):
        from loki_cli.main import _PROVIDER_MODELS
...
class TestHuggingFaceModels:
    ...
    def test_model_lists_match(self):
        ...
        from loki_cli.main import _PROVIDER_MODELS as main_models
