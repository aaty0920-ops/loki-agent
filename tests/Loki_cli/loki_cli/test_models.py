...
class TestFetchOpenRouterModels:
    def test_live_fetch_recomputes_free_tags(self, monkeypatch):
        ...
        with patch("loki_cli.models.urllib.request.urlopen", return_value=_Resp()):
    ...
    def test_falls_back_to_static_snapshot_on_fetch_failure(self, monkeypatch):
        monkeypatch.setattr(_models_mod, "_openrouter_catalog_cache", None)
        with patch("loki_cli.models.urllib.request.urlopen", side_effect=OSError("boom")):
            models = fetch_openrouter_models(force_refresh=True)
...
class TestUnionWithPortalFreeRecommendations:
    ...
    def test_adds_portal_free_model_missing_from_curated(self):
        ...
        with patch(
            "loki_cli.models.fetch_nous_recommended_models",
    ...
    def test_empty_payload_returns_inputs_unchanged(self):
        ...
        with patch("loki_cli.models.fetch_nous_recommended_models", return_value={}):
            ids, p = union_with_portal_free_recommendations(curated, pricing, "")
        assert ids == curated
        assert p == pricing
...
class TestCheckNousFreeTierCache:
    ...
    @patch("loki_cli.nous_account.get_nous_portal_account_info")
...
class TestNousRecommendedModels:
    ...
    def test_fetch_caches_per_portal_url(self):
        from loki_cli.models import fetch_nous_recommended_models
    ...
    def test_paid_tier_prefers_paid_recommendation(self):
        ...
        with patch("loki_cli.models.fetch_nous_recommended_models", return_value=payload):
    ...
    def test_auto_detects_tier_when_not_supplied(self):
        ...
        with (
            patch("loki_cli.models.fetch_nous_recommended_models", return_value=payload),
            patch("loki_cli.models.check_nous_free_tier", return_value=True),
        ):
            assert get_nous_recommended_aux_model(vision=False) == "free-model"
        with (
            patch("loki_cli.models.fetch_nous_recommended_models", return_value=payload),
            patch("loki_cli.models.check_nous_free_tier", return_value=False),
        ):
    ...
    def test_tier_detection_error_defaults_to_paid(self):
        ...
        with (
            patch("loki_cli.models.fetch_nous_recommended_models", return_value=payload),
            patch("loki_cli.models.check_nous_free_tier", side_effect=RuntimeError("boom")),
        ):
            assert get_nous_recommended_aux_model(vision=False) == "paid-model"
...
class TestCodexSoftAcceptPlausibilityGate:
    ...
    def test_unrelated_name_rejected_on_openai_codex(self):
        from loki_cli.models import validate_requested_model
