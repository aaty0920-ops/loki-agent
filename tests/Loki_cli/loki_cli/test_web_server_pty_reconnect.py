...
def test_resolve_chat_argv_sets_active_session_file_env(monkeypatch):
    ...
    _argv, _cwd, env = ws._resolve_chat_argv(
        active_session_file="/tmp/loki-active-session.json"
    )

    assert env["LOKI_TUI_ACTIVE_SESSION_FILE"] == "/tmp/loki-active-session.json"
...
def test_child_eof_closes_socket_and_bridge(pty_client, monkeypatch):
    ...
    monkeypatch.setattr(
        ws, "_resolve_chat_argv", lambda **kw: (["fake-loki-tui"], None, None)
    )
