...
class TestValidateProfileName:
    ...
    @pytest.mark.parametrize("name", ["loki", "test", "tmp", "root", "sudo"])
    def test_reserved_names_rejected(self, name):
        """Reserved names collide with the Loki install itself or with
...
class TestGetProfileDir:
    ...
    def test_named_profile_returns_profiles_subdir(self, profile_env):
        ...
        assert result == tmp_path / ".loki" / "profiles" / "coder"

    def test_named_profile_matching_is_case_insensitive(self, profile_env):
        tmp_path = profile_env
        assert get_profile_dir("Coder") == tmp_path / ".loki" / "profiles" / "coder"
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.loki must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".loki"
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes loki-agent, .worktrees,
        ...
        (default_home / "loki-agent" / ".git").mkdir(parents=True)
        (default_home / "loki-agent" / "venv" / "bin").mkdir(parents=True)
        (default_home / "loki-agent" / "README.md").write_text("repo")
        ...
        assert not (profile_dir / "loki-agent").exists()
...
class TestNoSkillsOptOut:
    ...
    def test_seed_profile_skills_respects_marker(self, profile_env):
        ...
        called directly (e.g. by `loki update`'s all-profile sync loop)."""
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)
...
class TestBackfillProfileEnvs:
    ...
    def test_never_overwrites_existing_profile_env(self, profile_env):
        tmp_path = profile_env
        (tmp_path / ".loki" / ".env").write_text("KEY=root\n")
...
class TestDeleteProfile:
    ...
    def test_stops_profile_bound_backends_before_removal(self, profile_env):
        ...
        with patch("loki_cli.profiles._cleanup_gateway_service"), \
             patch("loki_cli.profiles._profile_bound_backend_pids", return_value=[4242]) as pids, \
    ...
    def test_rmtree_retries_transient_enotempty_then_succeeds(self, profile_env):
        ...
        with patch("loki_cli.profiles._cleanup_gateway_service"), \
             patch("loki_cli.profiles._profile_bound_backend_pids", return_value=[]), \
             patch("loki_cli.profiles.time.sleep"), \
             patch("loki_cli.profiles.shutil.rmtree", side_effect=flaky_rmtree):
            delete_profile("coder", yes=True)
...
class TestGetActiveProfileName:
    ...
    def test_default_loki_home_returns_default(self, profile_env):
        # LOKI_HOME points to tmp_path/.loki which is the default
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("loki")
        assert result is not None
...
class TestWrapperScript:
    ...
    def test_creates_bat_on_windows(self, profile_env, monkeypatch):
        ...
        assert "loki -p mybot" in content
        assert "%*" in content
    ...
    def test_remove_returns_false_when_absent(self, profile_env):
        from loki_cli.profiles import remove_wrapper_script
    ...
    def test_custom_alias_target_on_posix(self, profile_env, monkeypatch):
        ...
        content = wrapper.read_text()
        assert content.startswith("#!/bin/sh")
        assert "loki -p redqueen" in content
...
class TestFindAliasForProfile:
    ...
    def test_no_alias_returns_none(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from loki_cli.profiles import find_alias_for_profile
...
class TestRenameProfile:
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "loki.ssi_health": {"workspace": "loki", "enabled": True}
            }
        }))
        ...
        assert "loki.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["loki_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["loki_heimdall"]["workspace"] == "loki"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "loki.ssi_health": {"aiPeer": "ssi_health"},
                "loki_heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        cfg = json.loads(honcho_path.read_text())
        assert cfg["hosts"]["loki.ssi_health"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["loki_heimdall"]["aiPeer"] == "heimdall"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("loki-agent", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".loki_history",
        ...
        excluded_prefixes = [
            "default/loki-agent", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.loki_history",
...
class TestInternalHelpers:
    ...
    def test_profiles_root_under_home(self, profile_env):
        ...
        assert root == tmp_path / ".loki" / "profiles"

    def test_default_loki_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_loki_home()
        assert home == tmp_path / ".loki"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (LOKI_HOME outside ~/.loki), profiles go under LOKI_HOME."""
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (LOKI_HOME under ~/.loki), profiles root is still ~/.loki/profiles."""
        native = tmp_path / ".loki"
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under LOKI_HOME."""
        from loki_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LOKI_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".loki" / "profiles" / "mybot"
        assert result == expected
    ...
    def test_gateway_running_check_rejects_pid_reused_by_other_profile(self, profile_env):
        ...
        gateway green while ``loki -p <name> gateway status`` showed it
        stopped.
        ...
        tmp_path = profile_env
        coder_home = tmp_path / ".loki" / "profiles" / "coder"
    ...
    def test_gateway_running_check_detects_matching_named_profile(self, profile_env):
        ...
        with patch("gateway.status.get_running_pid", return_value=None), patch(
            ...
            "gateway.status._read_process_cmdline",
            return_value="loki -p coder gateway run --replace",
        ):
...
class TestProfilesToServe:
    ...
    def test_off_returns_only_active_default(self, profile_env):
        ...
        name, home = serve[0]
        assert name == "default"
        assert home == _get_default_loki_home()
