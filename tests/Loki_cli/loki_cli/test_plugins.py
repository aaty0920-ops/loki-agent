...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    *base* is expected to be ``<loki_home>/plugins/``; we derive
    ``<loki_home>`` from it by walking one level up.
    """
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.loki/plugins/ are discovered."""
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("LOKI_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".loki" / "plugins"
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("loki_cli.plugins._plugin_manager", mgr):
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "loki_test" / "plugins"
        # Set LOKI_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginDebugLogging:
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With LOKI_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("LOKI_PLUGINS_DEBUG", "1")
        from loki_cli import plugins as plugins_mod
...
class TestPluginContextProfileName:
    ...
    def test_works_without_cli_ref(self, tmp_path, monkeypatch):
        ...
        prof = tmp_path / ".loki" / "profiles" / "worker1"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LOKI_HOME", str(prof))
        ctx = self._ctx()
