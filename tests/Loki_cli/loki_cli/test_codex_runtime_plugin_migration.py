...
class TestMigrate:
    def test_no_servers_no_plugins_no_perms_writes_placeholder(self, tmp_path):
        report = migrate({}, codex_home=tmp_path,
                         discover_plugins=False,
                         default_permission_profile=None, expose_loki_tools=False)
    ...
    def test_no_servers_still_writes_permissions_default(self, tmp_path):
        ...
        report = migrate({}, codex_home=tmp_path, discover_plugins=False, expose_loki_tools=False)
        assert report.written
        text = (tmp_path / "config.toml").read_text()
    ...
    def test_plugin_discovery_skips_unavailable_plugins(self):
        ...
        from loki_cli.codex_runtime_plugin_migration import _query_codex_plugins
    ...
    def test_discover_plugins_false_skips_query(self, tmp_path, monkeypatch):
        ...
        migrate({"mcp_servers": {"x": {"command": "y"}}},
                codex_home=tmp_path, discover_plugins=False, expose_loki_tools=False)
    ...
    def test_dry_run_skips_plugin_query(self, tmp_path, monkeypatch):
        ...
        migrate({"mcp_servers": {"x": {"command": "y"}}},
                codex_home=tmp_path, dry_run=True, discover_plugins=True, expose_loki_tools=False)
        assert called["yes"] is False
    ...
    def test_expose_loki_tools_writes_callback_mcp_entry(self, tmp_path):
        """When expose_loki_tools=True (production default), an
        [mcp_servers.loki-tools] entry is written so codex calls back
        into Loki for browser/web/delegate_task/vision/memory tools.

        This is the fix for 'all other tools that codex doesn't provide
        should be useable by loki' — quirk #7."""
        report = migrate({}, codex_home=tmp_path,
                         discover_plugins=False,
                         default_permission_profile=None,
                         expose_loki_tools=True)
        text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.loki-tools]" in text
        assert "loki_tools_mcp_server" in text
        ...
        assert "loki-tools" in report.migrated

    def test_expose_loki_tools_disabled_skips_entry(self, tmp_path):
        """expose_loki_tools=False suppresses the callback registration."""
        migrate({}, codex_home=tmp_path,
                discover_plugins=False,
                default_permission_profile=None,
                expose_loki_tools=False)
        text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.loki-tools]" not in text
        assert "loki_tools_mcp_server" not in text

    def test_dry_run_doesnt_write(self, tmp_path):
        report = migrate({"mcp_servers": {"x": {"command": "y"}}},
                         codex_home=tmp_path, dry_run=True, expose_loki_tools=False)
    ...
    def test_idempotent_re_run_replaces_managed_block(self, tmp_path):
        # First migration
        migrate({"mcp_servers": {"a": {"command": "x"}}}, codex_home=tmp_path, expose_loki_tools=False)
        first_text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.a]" in first_text
        # Second migration with different servers
        migrate({"mcp_servers": {"b": {"command": "y"}}}, codex_home=tmp_path, expose_loki_tools=False)
    ...
    def test_managed_root_keys_stay_top_level_when_config_ends_in_table(self, tmp_path):
        """TOML has no explicit 'leave current table' syntax. If Loki appends
        ...
        migrate({}, codex_home=tmp_path, discover_plugins=False, expose_loki_tools=False)
        new_text = target.read_text()
    ...
    def test_invalid_mcp_servers_value(self, tmp_path):
        report = migrate({"mcp_servers": "notadict"}, codex_home=tmp_path, expose_loki_tools=False)
...
class TestLokiHomeLeakGuard:
    ...
    def test_tempdir_detector_recognizes_pytest_paths(self):
        assert _looks_like_test_tempdir(
            "/private/var/folders/abc/pytest-of-kshitij/pytest-137/popen-gw2/test_X/loki_test"
        )
        assert _looks_like_test_tempdir(
            "/tmp/pytest-of-user/pytest-12/test_X/loki"
        )
        assert _looks_like_test_tempdir(
            "/private/var/folders/zz/T/pytest-of-bob/pytest-1"
        )

    def test_tempdir_detector_accepts_real_loki_home(self):
        assert not _looks_like_test_tempdir("/Users/alice/.loki")
        assert not _looks_like_test_tempdir("/home/bob/.loki")
        assert not _looks_like_test_tempdir("/opt/loki")
        assert not _looks_like_test_tempdir("")

    def test_pytest_tempdir_not_burned_into_mcp_env(self, monkeypatch):
        """The headline regression: even when LOKI_HOME points at a pytest
        tempdir, _build_loki_tools_mcp_entry() must NOT propagate it."""
        monkeypatch.setenv(
            "LOKI_HOME",
            "/private/var/folders/xx/pytest-of-user/pytest-99/test_x/loki_test",
        )
        entry = _build_loki_tools_mcp_entry()
        env = entry.get("env", {})
        assert "LOKI_HOME" not in env, (
            f"pytest-tempdir LOKI_HOME leaked into codex MCP entry: "
