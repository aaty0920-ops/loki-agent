...
def test_catalog_covers_every_loki_model_provider():
    """PARITY CONTRACT: the catalog == the `loki model` universe."""
    slugs = {d.slug for d in provider_catalog()}
    for entry in CANONICAL_PROVIDERS:
        assert entry.slug in slugs, (
            f"{entry.slug} is shown in `loki model` but missing from provider_catalog()"
        )


def test_catalog_has_no_providers_outside_loki_model():
    """The catalog must not invent providers `loki model` doesn't show."""
