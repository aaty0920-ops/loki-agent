...
def test_main_top_level_tui_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(sys, "argv", ["loki", "--tui", "--toolsets", "web,terminal"])
    monkeypatch.setitem(
        sys.modules,
        "loki_cli.plugins",
...
def test_termux_fast_tui_launch_uses_light_parser(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(
        sys, "argv", ["loki", "--tui", "--toolsets", "web,terminal"]
    )
    monkeypatch.setattr(
        main_mod,
        "cmd_chat",
...
def test_termux_fast_tui_launch_skips_help(monkeypatch, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.setattr(sys, "argv", ["loki", "--tui", "--help"])
...
def test_termux_fast_cli_launch_chat_uses_light_parser(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(
        sys, "argv", ["loki", "chat", "-q", "hello", "--toolsets", "web,terminal"]
    )
    monkeypatch.setattr(
...
def test_termux_fast_cli_launch_bare_defers_agent_startup(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("LOKI_TUI", raising=False)
    monkeypatch.delenv("LOKI_DEFER_AGENT_STARTUP", raising=False)
    monkeypatch.delenv("LOKI_FAST_STARTUP_BANNER", raising=False)
    monkeypatch.setattr(sys, "argv", ["loki"])
    monkeypatch.setattr(
        main_mod, "_prepare_agent_startup", lambda args: prepared.append(args.command)
    )
    monkeypatch.setattr(
    ...
    assert os.environ["LOKI_DEFER_AGENT_STARTUP"] == "1"
    assert os.environ["LOKI_FAST_STARTUP_BANNER"] == "1"
...
def test_termux_fast_cli_launch_oneshot_uses_light_parser(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["loki", "-z", "hello", "--model", "gpt-test", "--provider", "openai"],
    )
    monkeypatch.setattr(
        main_mod, "_prepare_agent_startup", lambda args: prepared.append(args.command)
    )
...
def test_termux_fast_cli_launch_version_skips_update_check(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("LOKI_TUI", raising=False)
    monkeypatch.setattr(sys, "argv", ["loki", "version"])
    monkeypatch.setattr(
...
def test_termux_ultrafast_version_runs_before_heavy_startup(
    ...
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("LOKI_TERMUX_DISABLE_FAST_CLI", raising=False)
    monkeypatch.setattr(sys, "argv", ["loki", "--version"])

    assert main_mod._try_termux_ultrafast_version() is True

    out = capsys.readouterr().out
    assert "Loki Agent v" in out
...
def test_termux_fast_cli_launch_skips_help(monkeypatch, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("LOKI_TUI", raising=False)
    monkeypatch.setattr(sys, "argv", ["loki", "chat", "--help"])
...
def test_termux_fast_cli_launch_can_be_disabled(monkeypatch, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.setenv("LOKI_TERMUX_DISABLE_FAST_CLI", "1")
    monkeypatch.delenv("LOKI_TUI", raising=False)
    monkeypatch.setattr(sys, "argv", ["loki", "version"])
...
def test_termux_bundled_skills_stamp_controls_sync(monkeypatch, tmp_path, main_mod):
    ...
    monkeypatch.setenv("LOKI_TERMUX_FORCE_SKILLS_SYNC", "1")
    assert main_mod._termux_bundled_skills_sync_needed() is True
...
def test_main_top_level_oneshot_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(
        sys, "argv", ["loki", "-z", "hello", "--toolsets", "web,terminal"]
    )
    monkeypatch.setitem(
        sys.modules,
        "loki_cli.plugins",
...
def test_oneshot_filters_invalid_toolsets_before_redirect(monkeypatch, capsys):
    _stub_plugin_discovery(monkeypatch)
    from loki_cli.oneshot import _validate_explicit_toolsets
...
def test_oneshot_rejects_disabled_mcp_toolset(monkeypatch, capsys):
    ...
    assert valid is None
    assert error == "loki -z: --toolsets did not contain any valid toolsets.\n"
    err = capsys.readouterr().err
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="nous/loki-test", provider="nous", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["LOKI_MODEL"] == "nous/loki-test"
    assert env["LOKI_INFERENCE_MODEL"] == "nous/loki-test"
    assert env["LOKI_TUI_PROVIDER"] == "nous"
    assert env["LOKI_INFERENCE_PROVIDER"] == "nous"
...
def test_launch_tui_applies_terminal_backend_config(
    monkeypatch, main_mod, _isolate_loki_home
):
    captured = {}
    ...
    assert captured["env"]["TERMINAL_DOCKER_IMAGE"] == "example/loki-tools:latest"
...
def test_launch_tui_exit_code_42_relaunches_update(monkeypatch, main_mod):
    ...
    with patch("loki_cli.relaunch.relaunch") as mock_relaunch:
        with pytest.raises(SystemExit) as exc:
            main_mod._launch_tui()
...
def test_launch_tui_drops_stale_resume_env_without_resume_arg(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("LOKI_TUI_RESUME", "stale-missing-session")
    monkeypatch.setattr(
        main_mod,
        "_make_tui_argv",
        lambda tui_dir, tui_dev: (["node", "dist/entry.js"], Path(".")),
    ...
    assert "LOKI_TUI_RESUME" not in captured["env"]
...
def test_launch_tui_sets_resume_env_from_resume_arg(monkeypatch, main_mod):
    ...
    assert captured["env"]["LOKI_TUI_RESUME"] == "20260518_000000_goodid"
...
def test_print_tui_exit_summary_includes_resume_and_token_totals(monkeypatch, capsys):
    ...
    monkeypatch.setitem(
        sys.modules, "loki_state", types.SimpleNamespace(SessionDB=lambda: _FakeDB())
    )
    ...
    assert "loki --tui --resume 20260409_000001_abc123" in out
    assert 'loki --tui -c "demo title"' in out
...
def test_print_tui_exit_summary_prefers_actual_active_session_file(
    ...
    assert "loki --tui --resume actual_session" in out
