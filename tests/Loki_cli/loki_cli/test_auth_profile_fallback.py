...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    _write(loki_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (LOKI_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.loki as the "real home" — which is NOT equal
    # to the LOKI_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    loki_home = tmp_path / "classic"
    ...
    _write(loki_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from loki_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: LOKI_HOME is set to a custom path that is NOT under
    # ~/.loki/profiles/ — get_default_loki_root() returns LOKI_HOME
