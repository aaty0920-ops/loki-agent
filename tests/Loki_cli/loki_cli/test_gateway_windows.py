...
def test_exec_schtasks_decodes_with_replace_errors(monkeypatch):
    ...
    code, out, err = gateway_windows._exec_schtasks(["/Query", "/TN", "Loki_Gateway"])
...
def test_build_gateway_argv_uses_base_pythonw_for_uv_venv_launcher(monkeypatch, tmp_path):
    ...
    assert argv[:3] == [str(base_pythonw), "-m", "loki_cli.main"]
...
def test_write_task_script_anchors_cmd_cd_at_loki_home(monkeypatch, tmp_path):
    project = tmp_path / "project"
    loki_home = tmp_path / "loki-home"
    loki_home.mkdir()
    ...
    assert f"cd /d {gateway_windows._quote_cmd_script_arg(str(loki_home.resolve()))}" in content
...
def test_gateway_cmd_script_uses_pythonw_without_replace_or_start_churn(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda exe: (exe.replace("python.exe", "pythonw.exe"), r"C:\\Loki\\loki-agent\\venv", []),
    )

    content = gateway_windows._build_gateway_cmd_script(
        r"C:\\Loki\\loki-agent\\venv\\Scripts\\python.exe",
        r"C:\\Loki\\loki-agent",
        r"C:\\LokiHome\\profiles\\alice",
        "--profile alice",
    )
...
def test_install_scheduled_task_success_start_now_uses_direct_spawn_not_task_run(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        gateway_windows,
        "_install_scheduled_task",
        lambda task_name, script_path: (True, "Created Scheduled Task 'Loki_Gateway_alice'"),
    )
...
def test_install_access_denied_launches_elevated_install_before_startup_fallback(monkeypatch, tmp_path, capsys):
    ...
    assert "Launched elevated Loki gateway install prompt" in out
...
def test_install_startup_fallback_does_not_auto_spawn_when_gateway_stopped(monkeypatch, tmp_path, capsys):
    ...
    assert "loki --profile alice gateway start" in out
...
def test_uninstall_access_denied_prompts_before_elevating(monkeypatch, tmp_path, capsys):
    ...
    assert "UAC is Windows' admin approval prompt" in out
    assert "Launched elevated Loki gateway uninstall prompt" in out
...
def test_stop_writes_planned_stop_marker_before_killing(monkeypatch):
    ...
    monkeypatch.setattr("loki_cli.gateway.kill_gateway_processes", fake_kill)
    monkeypatch.setattr("loki_cli.gateway._get_restart_drain_timeout", lambda: 5.0)
...
def test_stop_escalates_to_force_kill_when_drain_times_out(monkeypatch):
    ...
    `loki gateway start` fails with "another instance is running".
    """
    pid = 77777
    events = []
