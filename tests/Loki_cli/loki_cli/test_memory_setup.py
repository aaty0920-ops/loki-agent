...
def test_curses_select_cancel_defaults_to_selected(monkeypatch):
    ...
    monkeypatch.setattr("loki_cli.curses_ui.curses_radiolist", fake_radiolist)
...
def test_cmd_setup_top_level_cancel_writes_nothing(monkeypatch):
    ...
    monkeypatch.setattr("loki_cli.config.load_config", load_config)
...
def test_cmd_setup_clears_interactive_picker_before_provider_post_setup(monkeypatch):
    ...
    class PostSetupProvider:
        def post_setup(self, loki_home, config):
    ...
    monkeypatch.setattr(memory_setup, "get_loki_home", lambda: "/tmp/loki-test")
