...
class TestEarlyMouseDisable:
    def test_writes_disable_sequence_when_tui_flag_in_argv(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["loki", "--tui", "-c", "abc"])
    ...
    def test_writes_disable_sequence_when_loki_tui_env_set(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["loki"])
        monkeypatch.setenv("LOKI_TUI", "1")
        monkeypatch.delenv("LOKI_TUI_NO_EARLY_DISABLE", raising=False)
    ...
    def test_skips_when_stdout_is_not_a_tty(self, monkeypatch):
        # `loki --tui … >log` or CI capture: pipe is fd 1, not a TTY. The
