...
from loki_cli.main import _cmd_update_check, cmd_update
...
def test_cmd_update_in_docker_prints_guidance_and_exits(
    ...
    """``loki update`` inside Docker → friendly message + exit 1, no git calls."""
    ...
    assert "docker pull nousresearch/loki-agent:latest" in out
...
def test_cmd_update_check_in_docker_prints_guidance_and_exits(
    ...
    """``loki update --check`` inside Docker → same message + exit 1, no fetch."""
    with pytest.raises(SystemExit) as excinfo:
...
def test_format_docker_update_message_contents():
    ...
    assert "docker pull nousresearch/loki-agent:latest" in msg
