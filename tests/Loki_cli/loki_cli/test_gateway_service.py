...
class TestSystemdServiceRefresh:
    ...
    def test_run_gateway_refreshes_outdated_unit_on_boot(self, tmp_path, monkeypatch):
        ...
        via exit-code-75 (bypassing `loki gateway restart`)."""
        unit_path = tmp_path / "loki-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
    ...
    def test_refresh_refuses_to_bake_any_tempdir_home_into_real_user_unit(
        ...
        """Structural guard: a manual E2E LOKI_HOME like
        ``/tmp/loki-e2e-41264`` carries none of the pytest markers but
        poisons the unit identically (seen live 2026-06-11 — an E2E probe ran
        ``loki gateway restart`` with a /tmp LOKI_HOME exported; the
        ...
        must refuse ANY temp-dir LOKI_HOME, not just pytest-shaped ones.
        """
        unit_path = tmp_path / "loki-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
        ...
        polluted_unit = (
            ...
            "WorkingDirectory=/tmp/loki-e2e-41264\n"
        )
        monkeypatch.setattr(
            gateway_cli,
            "generate_systemd_unit",
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_var_tmp_home(self):
        unit = '[Service]\nEnvironment="LOKI_HOME=/var/tmp/loki-x"\n'
    ...
    def test_detects_tmp_home_in_launchd_plist(self):
        plist = (
            "<dict>\n  <key>LOKI_HOME</key>\n"
            "  <string>/tmp/loki-e2e-99999</string>\n</dict>\n"
        )
        assert (
    ...
    def test_accepts_macos_real_home_plist(self):
        plist = (
            "<dict>\n  <key>LOKI_HOME</key>\n"
            "  <string>/Users/alice/.loki</string>\n</dict>\n"
        )
        assert gateway_cli._temp_home_in_service_definition(plist) is None

    def test_accepts_unit_without_loki_home(self):
    ...
    def test_tmp_prefixed_non_temp_path_is_accepted(self):
        ...
        unit = '[Service]\nEnvironment="LOKI_HOME=/tmpfs-data/.loki"\n'
        assert gateway_cli._temp_home_in_service_definition(unit) is None
...
class TestGeneratedSystemdUnits:
    ...
    def test_system_unit_includes_wsl_windows_interop_paths(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "_loki_home_for_target_user", lambda home: "/home/alice/.loki")
        monkeypatch.setenv("PATH", "/usr/local/bin:/mnt/c/WINDOWS/system32")
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
...
class TestLaunchdServiceRecovery:
    def test_get_restart_drain_timeout_prefers_env_then_config_then_default(self, monkeypatch):
        ...
        monkeypatch.setenv("LOKI_RESTART_DRAIN_TIMEOUT", "9")
        assert gateway_cli._get_restart_drain_timeout() == 9.0

        monkeypatch.setenv("LOKI_RESTART_DRAIN_TIMEOUT", "invalid")
        assert (
            gateway_cli._get_restart_drain_timeout()
            == DEFAULT_GATEWAY_RESTART_DRAIN_TIMEOUT
        )
    ...
    def test_launchd_install_repairs_outdated_plist_without_force(self, tmp_path, monkeypatch):
        ...
        # LOKI_HOME resolves under the (pytest tmp) test LOKI_HOME.
        monkeypatch.setattr(
            gateway_cli,
    ...
    def test_launchd_install_falls_back_to_detached_on_bootstrap_5(self, tmp_path, monkeypatch, capsys):
        ...
        # (which would trip on the pytest-tmp test LOKI_HOME) stays out of
        # the way — this test exercises the bootstrap-error fallback.
        monkeypatch.setattr(
            gateway_cli,
            "generate_launchd_plist",
            lambda: (
                "<plist><key>LOKI_HOME</key>"
                "<string>/Users/alice/.loki</string></plist>"
            ),
        )
    ...
    def test_launchd_fallback_exits_when_spawn_fails(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "nohup loki gateway run" in out
...
class TestGatewaySystemServiceRouting:
    ...
    def test_gateway_install_reports_termux_manual_mode(self, monkeypatch, capsys):
        ...
        assert "Run manually: loki gateway" in out
    ...
    def test_gateway_status_on_termux_shows_manual_guidance(self, monkeypatch, capsys):
        ...
        assert "nohup loki gateway" in out
...
class TestSystemUnitLokiHome:
    ...
    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: LOKI_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("LOKI_HOME", "/root/.loki/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'LOKI_HOME=/home/alice/.loki/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_loki_home(self, monkeypatch):
        # Custom LOKI_HOME not under any user's home — keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("LOKI_HOME", "/opt/loki-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'LOKI_HOME=/opt/loki-shared' in unit
...
class TestLokiHomeForTargetUser:
    ...
    def test_remaps_profile_path(self, monkeypatch):
        ...
        result = gateway_cli._loki_home_for_target_user("/home/alice")
        assert result == "/home/alice/.loki/profiles/coder"

    def test_keeps_custom_path(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("LOKI_HOME", "/opt/loki")

        result = gateway_cli._loki_home_for_target_user("/home/alice")
        assert result == "/opt/loki"
...
class TestProfileArg:
    ...
    def test_default_loki_home_returns_empty(self, tmp_path, monkeypatch):
        """Default ~/.loki should not produce a --profile flag."""
        loki_home = tmp_path / ".loki"
        loki_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LOKI_HOME", str(loki_home))
        result = gateway_cli._profile_arg(str(loki_home))
        assert result == ""

    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.loki/profiles/mybot should return '--profile mybot'."""
    ...
    def test_named_profile_under_target_user_root_returns_flag(self, tmp_path):
        ...
        target_root = tmp_path / "home" / "alice" / ".loki"
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile LOKI_HOME should return empty string."""
        custom_home = tmp_path / "custom" / "loki"
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.loki/profiles/mybot/subdir should NOT match — too deep."""
        nested = tmp_path / ".loki" / "profiles" / "mybot" / "subdir"
    ...
    def test_invalid_profile_name_returns_empty(self, tmp_path, monkeypatch):
        ...
        bad_profile = tmp_path / ".loki" / "profiles" / "My Bot!"
...
class TestRemapPathForUser:
    ...
    def test_keeps_system_path_unchanged(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "root")
        (tmp_path / "root").mkdir()
        result = gateway_cli._remap_path_for_user("/opt/loki", str(tmp_path / "alice"))
        assert result == "/opt/loki"

    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "alice")
        (tmp_path / "alice").mkdir()
        original = str(tmp_path / "alice" / ".loki" / "loki-agent")
...
class TestDockerAwareGateway:
    ...
    def test_uninstall_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'loki gateway uninstall' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_start_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'loki gateway start' inside Docker exits 0 with container guidance."""
        import pytest
...
class TestLegacyLokiUnitDetection:
    ...
    def test_detects_legacy_loki_service_in_user_scope(self, tmp_path, monkeypatch):
        ...
        assert name == "loki.service"
        assert path == legacy
        assert is_system is False
        assert gateway_cli.has_legacy_loki_units() is True

    def test_detects_legacy_loki_service_in_system_scope(self, tmp_path, monkeypatch):
        _, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        legacy = system_dir / "loki.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_ignores_profile_unit_loki_gateway_coder(self, tmp_path, monkeypatch):
        """CRITICAL: profile units must NOT be flagged as legacy.

        Teknium's concern — ``loki-gateway-coder.service`` is our standard
        ...
        for base in (user_dir, system_dir):
            (base / "loki-gateway-coder.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "loki-gateway-orcha.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "loki-gateway.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
        ...
        assert results == []
        assert gateway_cli.has_legacy_loki_units() is False

    def test_ignores_unrelated_loki_service(self, tmp_path, monkeypatch):
        """Third-party ``loki.service`` that isn't ours stays untouched.

        If a user has some other package named ``loki`` installed as a
        service, we must not flag it.
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "loki.service").write_text(
            "[Unit]\nDescription=Some Other Loki\n[Service]\n"
            "ExecStart=/opt/other-loki/bin/daemon --foreground\n",
            encoding="utf-8",
        )
    ...
    def test_returns_empty_when_no_legacy_files_exist(self, tmp_path, monkeypatch):
        ...
        assert gateway_cli._find_legacy_loki_units() == []
        assert gateway_cli.has_legacy_loki_units() is False
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        for i, execstart in enumerate(variants):
            name = f"loki.service" if i == 0 else f"loki.service"  # same name
    ...
    def test_handles_unreadable_unit_file_gracefully(self, tmp_path, monkeypatch):
        ...
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        unreadable = user_dir / "loki.service"
...
class TestRemoveLegacyLokiUnits:
    ...
    def test_dry_run_lists_without_removing(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_loki_units(
    ...
    def test_system_scope_without_root_defers_removal(self, tmp_path, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "sudo loki gateway migrate-legacy" in out
    ...
    def test_system_scope_with_root_removes(self, tmp_path, monkeypatch, capsys):
        ...
        assert any(
            c.startswith("systemctl stop loki.service") for c in cmds_joined
        )
        assert any(
            c.startswith("systemctl disable loki.service") for c in cmds_joined
        )
    ...
    def test_removes_both_scopes_with_root(self, tmp_path, monkeypatch, capsys):
        ...
        user_legacy = user_dir / "loki.service"
        system_legacy = system_dir / "loki.service"
    ...
    def test_interactive_prompt_no_skips_removal(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_loki_units(interactive=True)
...
class TestSystemScopeRemediationOutput:
    ...
    def test_restart_remediation_uses_systemctl_restart(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl restart loki-gateway" in out
    ...
    def test_stop_remediation_uses_systemctl_stop(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl stop loki-gateway" in out
...
class TestServiceWorkingDirIsStable:
    ...
    def test_stable_working_dir_uses_loki_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".loki"
        home.mkdir()
    ...
    def test_stable_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        # LOKI_HOME points somewhere that does not exist -> fall back.
        missing = tmp_path / "does-not-exist" / ".loki"
        monkeypatch.setattr(gateway_cli, "get_loki_home", lambda: missing)
