...
class TestNonInteractiveSetup:
    ...
    def test_cmd_setup_allows_noninteractive_flag_without_tty(self):
        ...
        from loki_cli.main import cmd_setup

        args = _make_setup_args(non_interactive=True)

        with (
            patch("loki_cli.setup.run_setup_wizard") as mock_run_setup,
    ...
    def test_cmd_setup_defers_no_tty_handling_to_setup_wizard(self):
        """Bare `loki setup` should reach setup.py, which prints headless guidance."""
        from loki_cli.main import cmd_setup
    ...
    def test_non_interactive_flag_skips_wizard(self, capsys):
        ...
        with (
            patch("loki_cli.setup.ensure_loki_home"),
            patch("loki_cli.setup.load_config", return_value={}),
            patch("loki_cli.setup.get_loki_home", return_value="/tmp/.loki"),
            patch("loki_cli.auth.get_active_provider", side_effect=AssertionError("wizard continued")),
    ...
    def test_main_accepts_tts_setup_section(self, monkeypatch):
        """`loki setup tts` should parse and dispatch like other setup sections."""
        ...
        monkeypatch.setattr("sys.argv", ["loki", "setup", "tts"])
