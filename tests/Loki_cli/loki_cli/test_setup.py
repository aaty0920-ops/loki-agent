...
def test_setup_gateway_skips_service_install_when_systemctl_missing(monkeypatch, capsys):
    ...
    import loki_cli.gateway as _gw_mod
...
def test_modal_setup_can_use_nous_subscription_without_modal_creds(tmp_path, monkeypatch, capsys):
    monkeypatch.setattr("loki_cli.setup.managed_nous_tools_enabled", lambda: True)
    monkeypatch.setenv("LOKI_HOME", str(tmp_path))
    config = load_config()
    ...
    monkeypatch.setattr("loki_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "loki_cli.setup.get_nous_subscription_features",
...
def test_prompt_yes_no_returns_default_when_noninteractive_env_set(monkeypatch):
    """LOKI_NONINTERACTIVE=1 (set by dashboard/desktop spawns) must make
    prompt_yes_no fall back to its default instead of reading stdin."""
    monkeypatch.setenv("LOKI_NONINTERACTIVE", "1")
...
def test_prompt_yes_no_eof_returns_default_instead_of_exiting(monkeypatch):
    ...
    monkeypatch.delenv("LOKI_NONINTERACTIVE", raising=False)
