...
class TestFastFails:
    def test_not_logged_in_exits_1_with_setup_hint(self, capsys):
        ...
        with patch.object(dr, "cmd_dashboard_register", dr.cmd_dashboard_register):
            with patch(
                "loki_cli.auth.resolve_nous_access_token", side_effect=err
...
class TestHappyPath:
    def _run(self, *, args, account_token="tok_abc", portal="https://portal.nousresearch.com",
        ...
        with patch(
            "loki_cli.auth.resolve_nous_access_token", return_value=account_token
    ...
    def test_custom_redirect_uri_is_forwarded(self, capsys):
        captured: dict = {}
        self._run(
            args=_ns(redirect_uri="https://loki.example.com/auth/callback"),
            captured=captured,
        )
        assert (
            captured["body"]["custom_redirect_uri"]
            == "https://loki.example.com/auth/callback"
        )
    ...
    def test_non_default_portal_is_persisted(self, capsys):
        ...
        assert (
            saved["LOKI_DASHBOARD_PORTAL_URL"]
...
class TestCustomPortalPersistence:
    ...
    def _run(self, *, args, portal, existing_portal):
        ...
        with patch(
            "loki_cli.auth.resolve_nous_access_token", return_value="tok"
    ...
    def test_explicit_custom_url_updates_existing_in_place(self, capsys):
        ...
        assert (
            saved["LOKI_DASHBOARD_PORTAL_URL"] == "https://new-preview.example.com"
        )
    ...
    def test_explicit_custom_url_persisted_even_when_equals_default(self, capsys):
        ...
        assert (
            saved["LOKI_DASHBOARD_PORTAL_URL"] == "https://portal.nousresearch.com"
        )
...
class TestPublicUrlPersistence:
    ...
    def test_origin_derived_from_full_callback_path(self, capsys):
        ...
        assert "/auth/callback" not in saved["LOKI_DASHBOARD_PUBLIC_URL"]

    def test_origin_preserves_port(self, capsys):
        saved = self._run(
            args=_ns(redirect_uri="https://loki.example.com:8443/auth/callback"),
            existing_public=None,
        )
        assert saved["LOKI_DASHBOARD_PUBLIC_URL"] == "https://loki.example.com:8443"
