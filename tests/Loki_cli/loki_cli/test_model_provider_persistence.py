...
class TestProviderPersistsAfterModelSave:
    ...
    def test_api_key_provider_saved_when_model_was_string(self, config_home, monkeypatch):
        ...
        from loki_cli.main import _model_flow_api_key_provider
    ...
    def test_copilot_provider_saved_when_selected(self, config_home):
        ...
        from loki_cli.main import _model_flow_copilot
        from loki_cli.config import load_config
        ...
        with patch(
            ...
            "loki_cli.main._prompt_reasoning_effort_selection",
            return_value="high",
        ), patch(
    ...
    def test_named_custom_provider_preserves_explicit_api_mode(self, config_home):
        ...
        from loki_cli.main import _model_flow_named_custom
        ...
        with patch("loki_cli.auth._save_model_choice"), \
             patch("loki_cli.auth.deactivate_provider"), \
             patch("loki_cli.models.fetch_api_models", return_value=["gpt-5.4"]), \
             patch("loki_cli.curses_ui.curses_radiolist", side_effect=OSError("no tty in test")), \
    ...
    def test_named_custom_provider_with_builtin_slug_persists_custom_prefix(
        ...
        with patch("loki_cli.auth._save_model_choice"), \
             patch("loki_cli.auth.deactivate_provider"), \
             patch("loki_cli.models.fetch_api_models", return_value=["MiniMax-M3"]), \
             patch("loki_cli.curses_ui.curses_radiolist", side_effect=OSError("no tty in test")), \
             patch("builtins.input", return_value="1"):
            _model_flow_named_custom({}, provider_info)
    ...
    def test_copilot_acp_provider_saved_when_selected(self, config_home):
        ...
        from loki_cli.main import _model_flow_copilot_acp
        from loki_cli.config import load_config

        with patch(
            "loki_cli.auth.get_external_process_provider_status",
            return_value={
            ...
            "loki_cli.auth.resolve_external_process_provider_credentials",
            return_value={
    ...
    def test_opencode_go_models_are_selectable_and_persist_normalized(self, config_home, monkeypatch):
        ...
        with patch("loki_cli.models.fetch_api_models", return_value=["opencode-go/kimi-k2.5", "opencode-go/minimax-m2.7"]), \
             patch("loki_cli.auth._prompt_model_selection", return_value="kimi-k2.5"), \
             patch("loki_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
    ...
    def test_opencode_go_same_provider_switch_recomputes_api_mode(self, config_home, monkeypatch):
        ...
        with patch("loki_cli.models.fetch_api_models", return_value=["opencode-go/kimi-k2.5", "opencode-go/minimax-m2.5"]), \
             patch("loki_cli.auth._prompt_model_selection", return_value="minimax-m2.5"), \
             patch("loki_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
...
class TestBaseUrlValidation:
    ...
    def test_invalid_base_url_rejected(self, config_home, monkeypatch, capsys):
        ...
        with patch("loki_cli.auth._prompt_model_selection", return_value="MiniMax-M2"), \
             patch("loki_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value="nano ~/.loki/.env"):
...
class TestZaiEndpointPicker:
    ...
    def test_select_global_endpoint(self, config_home, monkeypatch):
        ...
        with patch("loki_cli.main._prompt_provider_choice", return_value=0), \
    ...
    def test_select_coding_plan_global_endpoint(self, config_home, monkeypatch):
        ...
        with patch("loki_cli.main._prompt_provider_choice", return_value=2), \
             patch("loki_cli.auth._prompt_model_selection", return_value="glm-5.2"), \
             patch("loki_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
            _model_flow_api_key_provider(load_config(), "zai", "old-model")
    ...
    def test_select_china_endpoint(self, config_home, monkeypatch):
        ...
        with patch("loki_cli.main._prompt_provider_choice", return_value=1), \
             patch("loki_cli.auth._prompt_model_selection", return_value="glm-5"), \
             patch("loki_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
            _model_flow_api_key_provider(load_config(), "zai", "old-model")
    ...
    def test_select_custom_proxy_url(self, config_home, monkeypatch):
        ...
        with patch("loki_cli.main._prompt_provider_choice", return_value=custom_idx), \
             patch("loki_cli.auth._prompt_model_selection", return_value="glm-5"), \
             patch("loki_cli.auth.deactivate_provider"), \
    ...
    def test_cancel_keeps_existing_base_url(self, config_home, monkeypatch):
        ...
        with patch("loki_cli.main._prompt_provider_choice", return_value=None), \
             patch("loki_cli.auth._prompt_model_selection", return_value="glm-5"), \
             patch("loki_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
            _model_flow_api_key_provider(load_config(), "zai", "old-model")
    ...
    def test_current_endpoint_is_default_choice(self, config_home, monkeypatch):
        ...
        with patch("loki_cli.main._prompt_provider_choice", side_effect=fake_choice):
