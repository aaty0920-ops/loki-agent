...
from loki_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ssl_cert_file_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.delenv("LOKI_CA_BUNDLE", raising=False)
        result = _resolve_verify(auth_state={"tls": {}})
        assert result is True

    def test_missing_loki_ca_bundle_env_falls_back(self, monkeypatch):
        from loki_cli.auth import _resolve_verify

        monkeypatch.setenv("LOKI_CA_BUNDLE", "/nonexistent/loki-ca.pem")
...
def test_resolve_nous_runtime_credentials_env_override_wins_live_not_persisted(
    ...
    monkeypatch.setattr("loki_cli.auth._refresh_access_token", _fake_refresh_access_token)
...
def test_get_nous_auth_status_pool_opaque_key_is_not_inference_credential(tmp_path, monkeypatch):
    from loki_cli.auth import get_nous_auth_status, invalidate_nous_auth_status_cache
...
def test_get_nous_auth_status_auth_store_fallback(tmp_path, monkeypatch):
    ...
    loki_home = tmp_path / "loki"
    _setup_nous_auth(loki_home, access_token="at-123")
    monkeypatch.setenv("LOKI_HOME", str(loki_home))
    monkeypatch.setattr(
        "loki_cli.auth.resolve_nous_runtime_credentials",
        lambda **kwargs: {
            "base_url": "https://inference.example.com/v1",
...
def test_get_nous_auth_status_prefers_runtime_auth_store_over_stale_pool(tmp_path, monkeypatch):
    ...
    loki_home = tmp_path / "loki"
    _setup_nous_auth(loki_home, access_token="at-fresh")
    monkeypatch.setenv("LOKI_HOME", str(loki_home))
...
def test_get_nous_auth_status_reports_revoked_refresh_session(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("loki_cli.auth.resolve_nous_runtime_credentials", _boom)
...
def test_managed_access_token_refresh_failure_quarantines_tokens(
    ...
    loki_home = tmp_path / "loki"
    _setup_nous_auth(loki_home, refresh_token="refresh-old")
    monkeypatch.setenv("LOKI_HOME", str(loki_home))
    from agent.credential_pool import load_pool
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `loki model` → Nous Portal → Skip (keep current) after
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        ...
        import argparse
        import yaml
        from loki_cli.auth import PROVIDER_REGISTRY, _login_nous
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `loki auth add nous`
    ...
    """
    from loki_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
...
def test_persist_nous_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    ...
    must succeed (not raise "Loki is not logged into Nous Portal").
    ...
    # doesn't raise "Loki is not logged into Nous Portal".
    def _fake_refresh_access_token(*, client, portal_base_url, client_id, refresh_token):
        return {
...
def test_persist_nous_credentials_embeds_custom_label(tmp_path, monkeypatch):
    ...
    Previously `loki auth add nous --type oauth --label <name>` silently
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    assert "loki auth add nous" in message.lower()
...
def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    ...
    """
    from loki_cli.auth import _nous_shared_store_path
...
def test_shared_store_honors_env_override(tmp_path, monkeypatch):
    """LOKI_SHARED_AUTH_DIR must redirect the path."""
    from loki_cli.auth import _nous_shared_store_path, NOUS_SHARED_STORE_FILENAME

    custom_dir = tmp_path / "custom_shared"
    monkeypatch.setenv("LOKI_SHARED_AUTH_DIR", str(custom_dir))
...
def test_shared_store_read_missing_returns_none(shared_store_env):
    ...
    from loki_cli.auth import _read_shared_nous_state
...
def test_shared_store_read_malformed_returns_none(shared_store_env):
    ...
    from loki_cli.auth import _nous_shared_store_path, _read_shared_nous_state
...
def test_shared_store_write_skips_when_refresh_token_missing(shared_store_env):
    ...
    from loki_cli.auth import _nous_shared_store_path, _write_shared_nous_state
...
def test_persist_nous_credentials_mirrors_to_shared_store(
    ...
    AND the shared store, so a future profile's `loki auth add nous
...
def test_try_import_shared_returns_none_when_store_missing(shared_store_env):
    ...
    from loki_cli.auth import _try_import_shared_nous_state
...
def test_shared_store_survives_across_profile_switch(
    ...
    (different LOKI_HOME) sees the same shared state and can rehydrate
    ...
    monkeypatch.setenv("LOKI_HOME", str(profile_a))
    ...
    # Profile B: fresh LOKI_HOME, no auth yet, but the shared store
    ...
    monkeypatch.setenv("LOKI_HOME", str(profile_b))
...
class TestStalePortalBaseUrlMigration:
    ...
    def test_migrates_stale_portal_url_on_load(self, tmp_path, monkeypatch):
        from loki_cli.auth import _load_auth_store, DEFAULT_NOUS_PORTAL_URL
