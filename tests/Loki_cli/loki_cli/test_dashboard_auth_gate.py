...
def test_loopback_protected_route_accepts_session_token(client_loopback):
    ...
    r = client_loopback.get(
        "/api/sessions",
        headers={"X-Loki-Session-Token": web_server._SESSION_TOKEN},
    )
...
def test_loopback_index_injects_session_token(client_loopback):
    ...
    assert "__LOKI_SESSION_TOKEN__" in r.text
...
def test_start_server_surfaces_nous_skip_reason_when_unconfigured(monkeypatch):
    ...
    'set LOKI_DASHBOARD_OAUTH_CLIENT_ID', not 'install a plugin'."""
    from loki_cli.dashboard_auth import clear_providers
    ...
    assert "LOKI_DASHBOARD_OAUTH_CLIENT_ID" in msg
    assert "nous:" in msg
