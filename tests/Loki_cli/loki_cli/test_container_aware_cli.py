...
def test_get_container_exec_info_none_inside_container(container_env):
    ...
    with patch("loki_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_loki_dev(container_env, monkeypatch):
    """Returns None when LOKI_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("LOKI_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_loki_dev_zero(container_env, monkeypatch):
    """LOKI_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("LOKI_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        loki_home = Path(tmpdir) / ".loki"
        loki_home.mkdir()
        (loki_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("loki_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_loki_home": lambda: loki_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("LOKI_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=loki-custom\n"
        "exec_user=myuser\n"
        "loki_bin=/opt/loki/bin/loki\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "loki-custom"
    assert info["exec_user"] == "myuser"
    assert info["loki_bin"] == "/opt/loki/bin/loki"
