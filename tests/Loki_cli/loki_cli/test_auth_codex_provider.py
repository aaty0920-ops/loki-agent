...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_loki_auth(loki_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("LOKI_HOME", str(loki_home))
    ...
    monkeypatch.setattr("loki_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    loki_home = tmp_path / "loki"
    _setup_loki_auth(loki_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("LOKI_HOME", str(loki_home))
...
def test_save_codex_tokens_still_refreshes_legacy_manual_alias(tmp_path, monkeypatch):
    ...
    ``loki auth add openai-codex`` as a workaround when there was no
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr("loki_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("loki_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
