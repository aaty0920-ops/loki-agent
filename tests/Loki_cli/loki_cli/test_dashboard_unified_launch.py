...
class TestUnifiedDashboardRouting:
    ...
    def test_reexec_pins_docker_machine_root(self, main_mod, monkeypatch):
        """In the Docker layout (LOKI_HOME=/opt/data, profiles under
        /opt/data/profiles/<name>) the reroute must pin the child to the
        machine root /opt/data — NOT drop LOKI_HOME.

        Dropping it makes the child fall back to $HOME/.loki
        (= /opt/data/.loki), an empty auto-seeded home, so the dashboard
        ...
        """
        monkeypatch.setenv("LOKI_HOME", "/opt/data/profiles/oracle")
        monkeypatch.setattr(
            "loki_cli.profiles.get_active_profile_name", lambda: "oracle"
        )
        monkeypatch.setattr(main_mod, "_dashboard_listening", lambda host, port: False)
        execs = []
