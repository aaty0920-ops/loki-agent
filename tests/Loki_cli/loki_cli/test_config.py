...
class TestGetLokiHome:
    ...
    def test_env_override(self):
        with patch.dict(os.environ, {"LOKI_HOME": "/custom/path"}):
            home = get_loki_home()
...
class TestEnsureLokiHome:
    ...
    def test_preserves_legacy_template_with_user_persona(self, tmp_path):
        ...
        from loki_cli.default_soul import _LEGACY_TEMPLATE_SOULS
    ...
    def test_existing_named_profile_still_bootstraps_subdirs(self, tmp_path):
        ...
        with patch.dict(os.environ, {"LOKI_HOME": str(profile_home)}):
            ensure_loki_home()
...
class TestLoadConfigParseFailure:
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        with patch.dict(os.environ, {"LOKI_HOME": str(tmp_path)}):
            ...
            import logging
            with caplog.at_level(logging.WARNING, logger="loki_cli.config"):
                config = load_config()
            ...
            # stands out at loki startup before logging is configured)
            captured = capsys.readouterr()
            assert "loki config:" in captured.err
    ...
    def test_rewarns_after_file_edit(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"LOKI_HOME": str(tmp_path)}):
            ...
            assert "loki config:" in after_edit, "edited file should re-warn"
...
class TestRemoveEnvValue:
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"LOKI_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"LOKI_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or any ``LOKI_*`` runtime flag.
    ...
    def test_loki_integration_keys_still_writable(self, allowed_key):
        ...
        credentials following the ``LOKI_*`` convention must keep
    ...
    def test_arbitrary_user_key_still_works(self):
        ...
        denylist and outside ``LOKI_*``) keep working. The denylist
