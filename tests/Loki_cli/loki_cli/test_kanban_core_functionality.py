...
def test_per_task_max_retries_overrides_dispatcher_limit(kanban_home, all_assignees_spawnable):
    ...
    Three-tier resolution order:
      1. ``task.max_retries`` (set via ``create_task(max_retries=N)`` /
         ``loki kanban create --max-retries N``)
...
def test_list_profiles_on_disk(tmp_path, monkeypatch):
    ...
    named profiles under ~/.loki/profiles/ that contain a config.yaml."""
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.delenv("LOKI_HOME", raising=False)
    profiles = tmp_path / ".loki" / "profiles"
...
def test_list_profiles_on_disk_custom_root(tmp_path, monkeypatch):
    """list_profiles_on_disk respects a custom LOKI_HOME root."""
...
def test_parse_duration_accepts_formats():
    from loki_cli.kanban import _parse_duration
...
def test_cli_create_max_runtime_via_duration(kanban_home):
    """`loki kanban create --max-runtime 2h` should persist 7200 seconds."""
...
def test_cli_bulk_complete_with_summary_rejects(kanban_home):
    ...
    # Note: loki_cli.main doesn't propagate sub-command exit codes
    ...
    env = os.environ.copy()
    r = _run(
        [sys.executable, "-m", "loki_cli.main", "kanban",
...
def test_cli_create_on_fresh_home_auto_inits(tmp_path, monkeypatch):
    """First CLI action on an empty LOKI_HOME must not error with
    ...
    r = _sp.run(
        [_sys.executable, "-m", "loki_cli.main", "kanban",
...
def test_connect_auto_inits_fresh_db(tmp_path, monkeypatch):
    """Calling connect() on a fresh LOKI_HOME must create the
...
def test_cli_show_json_carries_runs(kanban_home):
    """loki kanban show --json must include runs[] so scripts that
...
def test_build_worker_context_renders_author_with_safe_framing(kanban_home):
    ...
    + "comment from worker" prefix so a misleading LOKI_PROFILE name
    (e.g. "loki-system", "operator") can't be misread as a system
    directive above the comment body. Defense-in-depth — see #22452."""
    conn = kb.connect()
    try:
        tid = kb.create_task(conn, title="t", assignee="worker")
        kb.add_comment(conn, tid, author="loki-system", body="some note")
        ctx = kb.build_worker_context(conn, tid)

        # No bold-author rendering anywhere in the context.
        assert "**loki-system**" not in ctx
        # Explicit provenance prefix is present.
        assert "comment from worker `loki-system` at " in ctx
...
def test_cli_create_skill_flag_repeatable(kanban_home):
    """`loki kanban create --skill a --skill b` persists the list."""
    out = run_slash(
...
def test_cli_show_renders_skills(kanban_home):
    """`loki kanban show <id>` prints a skills row when present."""
    out = run_slash(
...
def test_check_dispatcher_presence_warns_when_no_gateway(monkeypatch):
    ...
    assert "loki gateway start" in msg
...
def test_cli_create_warns_when_no_gateway(kanban_home, monkeypatch, capsys):
    ...
    assert "loki gateway start" in captured.err
...
def test_cli_create_silent_when_gateway_up(kanban_home, monkeypatch, capsys):
    ...
    assert kb_cli._cmd_create(ns) == 0
    captured = capsys.readouterr()
    assert "loki gateway start" not in captured.err
...
def test_cli_create_no_warn_on_triage(kanban_home, monkeypatch, capsys):
    ...
    assert "loki gateway start" not in err
...
def test_cli_daemon_without_force_prints_deprecation_exits_2(kanban_home, capsys):
    """`loki kanban daemon` (no --force) is a deprecation stub."""
    from loki_cli import kanban as kb_cli
    ns = argparse.Namespace(
    ...
    assert "loki gateway start" in err
...
def test_gateway_dispatcher_watcher_respects_env_override(monkeypatch):
    """LOKI_KANBAN_DISPATCH_IN_GATEWAY=0 disables without touching config."""
    import asyncio
    from gateway.run import GatewayRunner
    monkeypatch.setenv("LOKI_KANBAN_DISPATCH_IN_GATEWAY", "0")
...
def test_gateway_dispatcher_watcher_env_truthy_uses_config(monkeypatch):
    ...
    monkeypatch.setenv("LOKI_KANBAN_DISPATCH_IN_GATEWAY", "yes")
    monkeypatch.setattr(
        _cfg_mod, "load_config",
        lambda: {"kanban": {"dispatch_in_gateway": False}},
    )
