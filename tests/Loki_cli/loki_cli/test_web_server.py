...
class TestReloadEnv:
    ...
    def test_does_not_remove_unknown_vars(self, tmp_path):
        """reload_env() preserves non-Loki env vars even when absent from .env."""
        env_file = tmp_path / ".env"
        env_file.write_text("")
        with patch.dict(reload_env.__globals__, {"get_env_path": lambda: env_file}):
...
class TestWebServerEndpoints:
    ...
    def test_get_status(self):
        ...
        assert "loki_home" in data
        assert "active_sessions" in data
        assert data["can_update_loki"] is True
    ...
    def test_get_status_hides_update_capability_in_managed_runtime(self, monkeypatch):
        ...
        resp = self.client.get("/api/status")
        assert resp.status_code == 200
        assert resp.json()["can_update_loki"] is False
    ...
    def test_dashboard_update_capability_allows_git_in_container(self, monkeypatch):
        """A git checkout inside a container (e.g. bind-mounted in loki-webui)
    ...
    def test_get_memory_provider_config_returns_safe_defaults(self):
        ...
        assert fields["bank_id"]["value"] == "loki"
    ...
    def test_get_media_404_for_missing_file(self):
        ...
        missing = get_loki_home() / "images" / "nope.png"
    ...
    def test_update_loki_returns_docker_guidance_without_spawning(self, monkeypatch):
        ...
        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "docker")
        monkeypatch.setattr(web_server, "_spawn_loki_action", fail_spawn)
        web_server._ACTION_PROCS.pop("loki-update", None)
        web_server._ACTION_RESULTS.pop("loki-update", None)
        ...
        assert "docker pull nousresearch/loki-agent:latest" in data["message"]
        ...
        assert any("docker pull nousresearch/loki-agent:latest" in line for line in status_data["lines"])

    def test_update_loki_returns_managed_runtime_guidance_without_spawning(self, monkeypatch):
        import loki_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("managed runtime update guard should not spawn loki update")
    ...
    def test_update_loki_spawns_on_non_docker_install(self, monkeypatch):
        import loki_cli.web_server as web_server
        ...
        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "git")
        monkeypatch.setattr(web_server, "_spawn_loki_action", fake_spawn)
        web_server._ACTION_PROCS.pop("loki-update", None)
        web_server._ACTION_RESULTS.pop("loki-update", None)
        ...
        assert resp.status_code == 200
        assert resp.json() == {"ok": True, "pid": 12345, "name": "loki-update"}
        assert calls == [(["update"], "loki-update")]
    ...
    def test_action_status_reaps_completed_process(self, monkeypatch):
        ...
        web_server._ACTION_PROCS.pop("loki-update", None)
        web_server._ACTION_RESULTS.pop("loki-update", None)
        web_server._ACTION_PROCS["loki-update"] = proc

        resp = self.client.get("/api/actions/loki-update/status")
        assert resp.status_code == 200
        data = resp.json()
        ...
        assert "loki-update" not in web_server._ACTION_PROCS
        assert web_server._ACTION_RESULTS["loki-update"] == {
            "exit_code": 0,
            "pid": 42424,
        }
    ...
    def test_get_env_vars_marks_channel_managed_keys(self):
        from loki_cli.web_server import _channel_managed_env_keys
    ...
    def test_get_env_vars_surfaces_catalog_providers(self):
        ...
        tencent-tokenhub, copilot were configurable via `loki model` but
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        from loki_cli.web_server import (
    ...
    def test_ops_import_passes_force_flag(self, tmp_path, monkeypatch):
        ...
        `loki import` doesn't auto-abort at the overwrite prompt."""
        import loki_cli.web_server as ws
        ...
        monkeypatch.setattr(ws, "_spawn_loki_action", fake_spawn)
    ...
    def test_ops_backup_defaults_to_dashboard_downloadable_archive(self, monkeypatch):
        ...
        assert archive.parent == get_loki_home() / "backups"
        assert archive.name.startswith("loki-backup-")
        assert archive.suffix == ".zip"

    def test_ops_backup_uses_hosted_loki_home(self, tmp_path, monkeypatch):
        from pathlib import Path

        import loki_cli.web_server as ws

        hosted_home = tmp_path / "opt-data"
        monkeypatch.setenv("LOKI_HOME", str(hosted_home))
        captured = {}
    ...
    def test_ops_backup_download_streams_dashboard_backup(self, tmp_path):
        ...
        archive = backup_dir / "loki-backup-test.zip"
    ...
    def test_ops_import_upload_rejects_invalid_zip(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_loki_action", fail_spawn)
    ...
    def test_weixin_messaging_metadata_describes_personal_ilink_setup(self):
        ...
        assert weixin["docs_url"] == (
            "https://loki-agent.nousresearch.com/docs/user-guide/messaging/weixin/"
        )
    ...
    def test_teams_messaging_metadata_links_setup_guide(self):
        ...
        from loki_cli.web_server import _build_catalog_entry

        teams = _build_catalog_entry("teams")
        assert teams["docs_url"] == (
            "https://loki-agent.nousresearch.com/docs/user-guide/messaging/teams"
        )
    ...
    def test_google_chat_messaging_metadata_links_setup_guide(self):
        ...
        assert google_chat["docs_url"] == (
            "https://loki-agent.nousresearch.com/docs/user-guide/messaging/google_chat"
        )
    ...
    def test_telegram_onboarding_worker_request_uses_httpx(self, monkeypatch):
        ...
        assert kwargs["json"] == {"bot_name": "Loki Agent"}
        assert kwargs["headers"]["Accept"] == "application/json"
        assert kwargs["headers"]["Authorization"] == "Bearer poll-secret"
        assert kwargs["headers"]["Content-Type"] == "application/json"
        assert kwargs["headers"]["User-Agent"].startswith("LokiDashboard/")
    ...
    def test_telegram_onboarding_start_strips_poll_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            ...
            return {
                ...
                "suggested_username": "loki_pair123_bot",
                "deep_link": "https://t.me/newbot/LokiSetupBot/loki_pair123_bot",
                "qr_payload": "https://t.me/newbot/LokiSetupBot/loki_pair123_bot",
    ...
    def test_telegram_onboarding_ready_and_apply_never_returns_bot_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "loki_pair_ready_bot",
                    "deep_link": "https://t.me/newbot/LokiSetupBot/loki_pair_ready_bot",
                    "qr_payload": "https://t.me/newbot/LokiSetupBot/loki_pair_ready_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
            ...
            return {
                "status": "ready",
                "bot_username": "loki_pair_ready_bot",
        ...
        monkeypatch.setattr(ws, "_spawn_loki_action", fake_spawn_action)
    ...
    def test_telegram_onboarding_apply_reports_restart_failure_after_save(
        ...
        monkeypatch.setattr(ws, "_spawn_loki_action", fail_spawn_action)
    ...
    def test_telegram_onboarding_apply_reuses_inflight_gateway_restart(
        ...
        second racing ``loki gateway restart`` child (e.g. when a stale
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "loki_pair_reuse_bot",
                    "deep_link": "https://t.me/newbot/LokiSetupBot/loki_pair_reuse_bot",
                    "qr_payload": "https://t.me/newbot/LokiSetupBot/loki_pair_reuse_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            return {
                "status": "ready",
                "bot_username": "loki_pair_reuse_bot",
                "owner_user_id": 123456789,
                "token": "123456:SECRET",
            }
    ...
    def test_telegram_onboarding_apply_requires_ready_pairing(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "loki_pair_waiting_bot",
                "deep_link": "https://t.me/newbot/LokiSetupBot/loki_pair_waiting_bot",
                "qr_payload": "https://t.me/newbot/LokiSetupBot/loki_pair_waiting_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_telegram_onboarding_cancel_clears_local_session(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "loki_pair_cancel_bot",
                "deep_link": "https://t.me/newbot/LokiSetupBot/loki_pair_cancel_bot",
                "qr_payload": "https://t.me/newbot/LokiSetupBot/loki_pair_cancel_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/loki_cli/web_server.py")
        assert resp.status_code in {200, 404}
        if resp.status_code == 200:
    ...
    def test_spa_assets_are_read_as_utf8(self, monkeypatch, tmp_path):
        ...
        css_resp = spa_client.get("/assets/app.css", headers={"x-forwarded-prefix": "/loki"})
    ...
    def test_set_model_main_nous_applies_gateway_defaults(self, monkeypatch):
        ...
        import loki_cli.nous_subscription as ns
        ...
        resp = self.client.post(
            "/api/model/set",
            json={"scope": "main", "provider": "nous", "model": "loki-4"},
        )
        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is True
    ...
    def test_apply_main_model_assignment_base_url_and_context_reconcile(self):
        ...
        from loki_cli.web_server import _apply_main_model_assignment
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from loki_cli.web_server import _parse_model_ids
    ...
    def test_set_model_main_reports_stale_auxiliary_pins(self):
        ...
        cfg = load_config()
        cfg["model"] = {"provider": "nous", "default": "loki-4"}
        cfg["auxiliary"] = {
    ...
    def test_recommended_default_nous_honors_free_tier(self, monkeypatch):
        ...
        model (mirroring `loki model`), not the first curated paid entry."""
...
class TestBuildSchemaFromConfig:
    def test_produces_expected_field_count(self):
        from loki_cli.web_server import CONFIG_SCHEMA
    ...
    def test_empty_prefix_produces_correct_keys(self):
        from loki_cli.web_server import _build_schema_from_config
...
class TestNewEndpoints:
    ...
    def test_cron_blueprints_list(self):
        ...
        assert first["appUrl"].startswith("loki://")
    ...
    def test_profiles_list_includes_default(self):
        from loki_constants import get_loki_home
        get_loki_home().mkdir(parents=True, exist_ok=True)
    ...
    def test_profiles_list_falls_back_when_profile_listing_fails(self, monkeypatch):
        ...
        named = loki_home / "profiles" / "multi-agent"
    ...
    def test_profile_setup_command_uses_named_profile_wrapper(self):
        ...
        (get_loki_home() / "profiles" / "coder").mkdir(parents=True)
    ...
    def test_profile_setup_command_uses_loki_for_default_profile(self):
        from loki_constants import get_loki_home
        ...
        assert resp.status_code == 200
        assert resp.json()["command"] == "loki setup"
    ...
    def test_profiles_create_creates_wrapper_alias_when_safe(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(profiles_mod.shutil, "which", lambda name: "/opt/loki/bin/loki")
        ...
        if is_windows:
            assert lines == ["@echo off", "loki -p writer %*"]
        else:
            assert lines == ["#!/bin/sh", 'exec /opt/loki/bin/loki -p writer "$@"']
    ...
    def test_profiles_create_with_clone_from_copies_source_skills(self, monkeypatch):
        ...
        (get_loki_home() / "config.yaml").write_text(
            "model:\n  provider: openrouter\n",
            encoding="utf-8",
        )
        default_skill = get_loki_home() / "skills" / "custom" / "new-skill"
        ...
        assert resp.status_code == 200
        cloned_root = get_loki_home() / "profiles" / "cloned"
    ...
    def test_profiles_create_with_clone_from_duplicates_source(self, monkeypatch):
        ...
        source_skill = get_loki_home() / "profiles" / "source-prof" / "skills" / "custom" / "src-skill"
        ...
        assert resp.status_code == 200
        cloned_skill = (
            get_loki_home() / "profiles" / "source-prof-copy" / "skills" / "custom" / "src-skill" / "SKILL.md"
        )
        assert cloned_skill.exists()
    ...
    def test_profiles_create_clone_all_from_named_source(self, monkeypatch):
        ...
        source_dir = get_loki_home() / "profiles" / "full-src"
        ...
        assert resp.status_code == 200
        target_dir = get_loki_home() / "profiles" / "full-copy"
    ...
    def test_profiles_create_without_clone_seeds_bundled_skills(self, monkeypatch):
        ...
        assert resp.status_code == 200
        seeded_skill = get_loki_home() / "profiles" / "fresh" / "skills" / "software-development" / "plan" / "SKILL.md"
    ...
    def test_profiles_create_builder_fields_model_mcp_and_keep_skills(self, monkeypatch):
        ...
        prof_dir = get_loki_home() / "profiles" / "builder"
        token = set_loki_home_override(str(prof_dir))
        try:
            cfg = load_config()
    ...
    def test_profile_model_round_trip(self, monkeypatch):
        ...
        import yaml
        cfg_path = get_loki_home() / "profiles" / "model-prof" / "config.yaml"
    ...
    def test_skills_list_includes_disabled_skills(self, monkeypatch):
        ...
        import loki_cli.skills_config as skills_config
        import loki_cli.web_server as web_server
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from loki_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from loki_cli.web_server import _denormalize_config_from_web
...
class TestGatewayBusyReadout:
    ...
    def test_restart_drain_timeout_surfaced_and_numeric(self, monkeypatch):
        ...
        monkeypatch.setenv("LOKI_RESTART_DRAIN_TIMEOUT", "90")
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_missing_name(self):
        from loki_cli.web_server import _normalise_theme_definition
...
class TestPtyWebSocket:
    ...
    def test_resolve_chat_argv_uses_dashboard_scroll_env(self, monkeypatch):
        ...
        assert env["LOKI_TUI_DASHBOARD"] == "1"
        assert env["LOKI_TUI_INLINE"] == "1"
        assert env["LOKI_TUI_DISABLE_MOUSE"] == "1"

    def test_resolve_chat_argv_applies_terminal_backend_config(
        self, monkeypatch, _isolate_loki_home
    ):
        import loki_cli.main as main_mod

        config_path = Path(os.environ["LOKI_HOME"]) / "config.yaml"
        config_path.write_text(
            "\n".join(
                [
                    "terminal:",
                    "  backend: docker",
                    "  docker_image: example/loki-tools:latest",
                    "  docker_extra_args:",
                    "    - --network=host",
                ]
            ),
        ...
        assert env["TERMINAL_DOCKER_IMAGE"] == "example/loki-tools:latest"
    ...
    def test_streams_child_stdout_to_client(self, monkeypatch):
        ...
        with self.client.websocket_connect(self._url()) as conn:
            ...
            while time.monotonic() < deadline:
                ...
                if frame:
                    buf += frame
                if b"loki-ws-ok" in buf:
                    break
            assert b"loki-ws-ok" in buf
    ...
    def test_channel_param_propagates_sidecar_url(self, monkeypatch):
        ...
        LOKI_TUI_SIDECAR_URL env var pointing back at /api/pub on the
...
class TestDesktopCronTicker:
    ...
    def test_ticker_skipped_without_desktop(self, monkeypatch, _isolate_loki_home):
        import threading
        import cron.scheduler as sched
