...
def test_removes_symlinks_pointing_into_loki_node(fake_home):
...
def test_handles_missing_local_bin(fake_home):
    ...
    assert uninstall.remove_node_symlinks(loki_home) == []


def test_removes_dangling_symlink_into_loki_node(fake_home):
    """A link into the Loki node dir is removed even if the target file is
...
def test_only_some_links_present(fake_home):
    """Removes the Loki links that exist; ignores the ones that don't."""
    loki_home = fake_home / ".loki"
    node_bin = _make_loki_node(loki_home)
    local_bin = fake_home / ".local" / "bin"

    # Only npm and npx are Loki-managed; node is a real user binary.
