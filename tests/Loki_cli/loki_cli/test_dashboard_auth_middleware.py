...
def test_full_login_round_trip_unlocks_gated_api(gated_app):
    ...
    assert any("loki_session_at" in c for c in set_cookies)
    assert any("loki_session_rt" in c for c in set_cookies)
...
def test_logout_clears_cookies_and_redirects_to_login(gated_app):
    ...
    assert any(
        c.startswith("loki_session_at=") and "Max-Age=0" in c
        for c in set_cookies
    )
    assert any(
        c.startswith("loki_session_rt=") and "Max-Age=0" in c
        for c in set_cookies
    )
