...
class TestBangPrefixCommands:
    ...
    async def test_bang_with_bot_suffix_resolves(self, adapter):
        """``!stop@loki`` matches the get_command() ``@suffix`` stripping."""
        await adapter._handle_slack_message(self._make_event("!stop@loki"))

        msg_event = adapter.handle_message.call_args[0][0]
        assert msg_event.text.startswith("/stop@loki")
        assert msg_event.message_type == MessageType.COMMAND
...
class TestSlashCommands:
    ...
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /loki btw."""
        command = {
            "command": "/btw",
    ...
    @pytest.mark.asyncio
    async def test_legacy_loki_freeform_question(self, adapter):
        """/loki <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/loki",
...
class TestSlashEphemeralAck:
    ...
    @pytest.mark.asyncio
    async def test_legacy_loki_slash_stashes_context(self, adapter):
        """Legacy /loki <subcommand> also stashes context."""
        command = {
            "command": "/loki",
            "text": "help",
            "user_id": "U_H",
    ...
    @pytest.mark.asyncio
    async def test_freeform_loki_question_does_not_stash_context(self, adapter):
        """Free-form /loki <question> must NOT route agent reply ephemeral."""
        command = {
            "command": "/loki",
