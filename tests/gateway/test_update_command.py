...
class TestHandleUpdateCommand:
    ...
    async def test_managed_install_returns_package_manager_guidance(self, monkeypatch):
        runner = _make_runner()
        event = _make_event()
        monkeypatch.setenv("LOKI_MANAGED", "homebrew")

        # Guard: prevent any accidental fall-through from spawning a real
        # `loki update --gateway` against the CI checkout. The managed-install
        ...
        assert "brew upgrade loki-agent" in result
    ...
    @pytest.mark.asyncio
    async def test_no_loki_binary(self, tmp_path):
        """Returns error when loki is not on PATH and loki_cli is not importable."""
        runner = _make_runner()
        event = _make_event()
        ...
        assert "loki update" in result

    @pytest.mark.asyncio
    async def test_fallback_to_sys_executable(self, tmp_path):
        """Falls back to sys.executable -m loki_cli.main when loki not on PATH."""
        runner = _make_runner()
        event = _make_event()
        ...
        # The update_cmd uses sys.executable -m loki_cli.main
        joined = " ".join(call_args) if isinstance(call_args, list) else call_args
        assert "loki_cli.main" in joined or "bash" in call_args[0]

    @pytest.mark.asyncio
    async def test_resolve_loki_bin_prefers_which(self, tmp_path):
        """_resolve_loki_bin returns argv parts from shutil.which when available."""
        from gateway.run import _resolve_loki_bin

        with patch("shutil.which", return_value="/custom/path/loki"):
            result = _resolve_loki_bin()

        assert result == ["/custom/path/loki"]
    ...
    @pytest.mark.asyncio
    async def test_resolve_loki_bin_returns_none_when_both_fail(self):
        """_resolve_loki_bin returns None when both strategies fail."""
        from gateway.run import _resolve_loki_bin
    ...
    async def test_writes_pending_marker_with_thread_id(self, tmp_path):
        ...
        data = json.loads((loki_home / ".update_pending.json").read_text())
...
class TestSendUpdateNotification:
    ...
    async def test_truncates_long_output(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (loki_home / ".update_pending.json").write_text(json.dumps(pending))
        (loki_home / ".update_output.txt").write_text("x" * 5000)
        (loki_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_failure_message_when_update_fails(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (loki_home / ".update_pending.json").write_text(json.dumps(pending))
        (loki_home / ".update_output.txt").write_text("Traceback: boom")
        (loki_home / ".update_exit_code").write_text("1")
