...
@pytest.mark.asyncio
async def test_goal_verdict_done_sent_via_adapter_send(loki_home):
    ...
    with patch("loki_cli.goals.judge_goal", return_value=("done", "the feature shipped", False, None)):
...
async def test_goal_verdict_continue_enqueues_continuation(loki_home):
    ...
    with patch("loki_cli.goals.judge_goal", return_value=("continue", "still needs work", False, None)):
        await runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
...
async def test_goal_verdict_budget_exhausted_sends_pause(loki_home):
    ...
    with patch("loki_cli.goals.judge_goal", return_value=("continue", "keep going", False, None)):
        await runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
...
@pytest.mark.asyncio
async def test_goal_verdict_skipped_when_no_active_goal(loki_home):
...
@pytest.mark.asyncio
async def test_goal_verdict_survives_adapter_without_send(loki_home):
    ...
    with patch("loki_cli.goals.judge_goal", return_value=("done", "ok", False, None)):
        # must not raise
        await runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
