...
class TestStagedInactivityWarning:
    ...
    def test_warning_env_var_respected(self, monkeypatch):
        """LOKI_AGENT_TIMEOUT_WARNING env var is parsed correctly."""
        monkeypatch.setenv("LOKI_AGENT_TIMEOUT_WARNING", "600")
        _warning = float(os.getenv("LOKI_AGENT_TIMEOUT_WARNING", 900))
        assert _warning == 600.0

    def test_warning_zero_means_disabled(self, monkeypatch):
        """LOKI_AGENT_TIMEOUT_WARNING=0 disables the warning."""
        monkeypatch.setenv("LOKI_AGENT_TIMEOUT_WARNING", "0")
        _raw = float(os.getenv("LOKI_AGENT_TIMEOUT_WARNING", 900))
