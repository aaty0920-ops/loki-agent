...
def test_reload_runtime_env_keeps_env_max_iterations_when_config_omits_key(
    ...
    loki_home = tmp_path / ".loki"
    loki_home.mkdir()
    (loki_home / "config.yaml").write_text(yaml.safe_dump({"agent": {}}), encoding="utf-8")
    (loki_home / ".env").write_text("LOKI_MAX_ITERATIONS=123\n", encoding="utf-8")
    ...
    assert os.environ["LOKI_MAX_ITERATIONS"] == "123"


def test_current_max_iterations_reloads_before_reading(monkeypatch) -> None:
    monkeypatch.setenv("LOKI_MAX_ITERATIONS", "90")

    def _fake_reload() -> None:
        os.environ["LOKI_MAX_ITERATIONS"] = "200"
