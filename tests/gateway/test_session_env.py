...
def test_session_source_uses_contextvars(monkeypatch):
    ...
    assert get_session_env("LOKI_SESSION_SOURCE") == "tool"
...
def test_get_session_env_default_when_nothing_set(monkeypatch):
    ...
    assert get_session_env("LOKI_SESSION_PLATFORM") == ""
    assert get_session_env("LOKI_SESSION_PLATFORM", "fallback") == "fallback"
...
def test_session_key_set_via_contextvars(monkeypatch):
    """set_session_vars should set LOKI_SESSION_KEY via contextvars."""
    ...
    assert get_session_env("LOKI_SESSION_KEY") == "tg:-1001:17585"
...
def test_session_id_set_via_contextvars(monkeypatch):
    """set_session_vars should set LOKI_SESSION_ID via contextvars."""
    monkeypatch.setenv("LOKI_SESSION_ID", "stale-env-session")

    tokens = set_session_vars(session_id="ctx-session-456")
    assert get_session_env("LOKI_SESSION_ID") == "ctx-session-456"

    clear_session_vars(tokens)
    assert get_session_env("LOKI_SESSION_ID") == ""
