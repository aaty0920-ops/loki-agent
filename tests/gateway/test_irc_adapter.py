...
class TestIRCProtocolHelpers:
    ...
    def test_parse_numeric_reply(self):
        msg = _parse_irc_message(":server 001 loki-bot :Welcome to IRC")
        assert msg["prefix"] == "server"
        assert msg["command"] == "001"
        assert msg["params"] == ["loki-bot", "Welcome to IRC"]

    def test_parse_nick_collision(self):
        msg = _parse_irc_message(":server 433 * loki-bot :Nickname is already in use")
...
class TestIRCAdapterMessageParsing:
    ...
    async def test_handle_welcome(self, adapter):
        ...
        await adapter._handle_line(":server 001 loki :Welcome to IRC")
    ...
    async def test_handle_nick_collision(self, adapter):
        ...
        await adapter._handle_line(":server 433 * loki :Nickname in use")
        assert adapter._current_nick == "loki_"
        sent = writer.write.call_args[0][0]
        assert b"NICK loki_" in sent
    ...
    async def test_handle_dm(self, adapter):
        ...
        await adapter._handle_line(":user!u@host PRIVMSG loki :private message")
        assert len(dispatched) == 1
    ...
    async def test_ignores_own_messages(self, adapter):
        ...
        await adapter._handle_line(":loki!bot@host PRIVMSG #test :my own msg")
        assert len(dispatched) == 0
    ...
    async def test_allowed_users_case_insensitive(self, monkeypatch):
        ...
        await adapter._handle_line(":admin!u@host PRIVMSG #test :loki: hello")
        assert len(dispatched) == 1
    ...
    async def test_nick_collision_retry(self, adapter):
        ...
        await adapter._handle_line(":server 433 * loki :Nickname in use")
        assert adapter._current_nick == "loki_"
        await adapter._handle_line(":server 433 * loki_ :Nickname in use")
        assert adapter._current_nick == "loki_1"
        await adapter._handle_line(":server 433 * loki_1 :Nickname in use")
        assert adapter._current_nick == "loki_2"
...
class TestIRCStandaloneSend:
    ...
    async def test_standalone_send_strips_crlf_from_message_body(self, monkeypatch):
        ...
        assert all(line.startswith("NICK lokibot-cron") for line in nick_lines)
    ...
    async def test_standalone_send_joins_channel_before_privmsg(self, monkeypatch):
        ...
        conn = _FakeIRCConnection([
            b":server 001 lokibot-cron :Welcome",
            b":server 366 lokibot-cron #cron :End of /NAMES list.",
        ])
