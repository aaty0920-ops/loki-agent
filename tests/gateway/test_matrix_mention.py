...
class TestIsBotMentioned:
    ...
    def test_partial_localpart_no_match(self):
        # "lokibot" should not match word-boundary check for "loki"
        assert not self.adapter._is_bot_mentioned("lokibot is here")
...
class TestStripMention:
    ...
    def test_localpart_in_path_preserved(self):
        ...
        result = self.adapter._strip_mention("read /home/loki/config.yaml")
        assert result == "read /home/loki/config.yaml"
    ...
    def test_does_not_strip_bare_localpart_word(self):
        # Regression: plain words like "Loki Agent" should not be mutated.
        result = self.adapter._strip_mention("Loki Agent")
        assert result == "Loki Agent"

    def test_strip_returns_empty_for_mention_only(self):
        result = self.adapter._strip_mention("@loki:example.org")
        assert result == ""
...
async def test_require_mention_html_pill(monkeypatch):
    ...
    adapter = _make_adapter()
    formatted = '<a href="https://matrix.to/#/@loki:example.org">Loki</a> help'
    event = _make_event("Loki help", formatted_body=formatted)
...
async def test_dm_preserves_localpart_in_body(monkeypatch):
    ...
    adapter = _make_adapter()
    _set_dm(adapter)
    event = _make_event("loki help me")

    await adapter._on_room_message(event)
    adapter.handle_message.assert_awaited_once()
    msg = adapter.handle_message.await_args.args[0]
    assert msg.text == "loki help me"
...
async def test_require_mention_disabled_skips_stripping(monkeypatch):
    ...
    adapter.handle_message.assert_awaited_once()
    msg = adapter.handle_message.await_args.args[0]
    assert msg.text == "@loki:example.org help me"
...
async def test_dm_mention_thread_preserves_existing_thread(monkeypatch):
    ...
    event = _make_event("@loki:example.org help me", thread_id="$existing_thread")
...
async def test_dm_mention_thread_tracks_participation(monkeypatch):
    ...
    adapter = _make_adapter()
    _set_dm(adapter)
    event = _make_event("@loki:example.org help", event_id="$dm1")
