...
class TestMediaDeliveryPathValidation:
    ...
    def test_allows_operator_configured_extra_root(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("LOKI_MEDIA_ALLOW_DIRS", str(extra_root))
    ...
    def test_recency_trust_rejects_old_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("LOKI_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("LOKI_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("LOKI_MEDIA_TRUST_RECENT_SECONDS", "60")
    ...
    def test_recency_trust_allows_pdf_in_project_dir(self, tmp_path, monkeypatch):
        ...
        files outside ~/.loki/cache/* were silently dropped, leaving the
...
class TestMediaDeliveryDefaultMode:
    ...
    def test_denylist_blocks_loki_credentials(self, tmp_path, monkeypatch):
        """~/.loki/.env and ~/.loki/auth.json stay blocked even in
    ...
    def test_denylist_blocks_loki_config_in_active_profile(self, tmp_path, monkeypatch):
    ...
    def test_denylist_blocks_shared_loki_root_config_for_profiles(self, tmp_path, monkeypatch):
        """Profile-mode gateways must still block the shared Loki root config."""
        self._patch_roots(monkeypatch)

        fake_home = tmp_path / "home"
        profile_home = fake_home / ".loki" / "profiles" / "work"
        profile_home.mkdir(parents=True)
        loki_root = fake_home / ".loki"
        config_file = loki_root / "config.yaml"
    ...
    def test_denylist_blocks_pairing_directory_contents(self, tmp_path, monkeypatch):
        """Files under ~/.loki/pairing/ (platform pairing tokens) are
    ...
    def test_loki_cache_still_delivers_under_denied_home(self, tmp_path, monkeypatch):
        ...
        fake_home = tmp_path / "home"
        loki_dir = fake_home / ".loki"
        cache_dir = loki_dir / "cache" / "documents"
    ...
    def test_strict_mode_truthy_aliases(self, monkeypatch, tmp_path):
        """``LOKI_MEDIA_DELIVERY_STRICT=true|yes|on|1`` all enable strict mode."""
        self._patch_roots(monkeypatch)
        from gateway.platforms.base import _media_delivery_strict_mode

        for raw in ("1", "true", "TRUE", "yes", "on"):
            monkeypatch.setenv("LOKI_MEDIA_DELIVERY_STRICT", raw)
    ...
    def test_root_home_loki_env_still_blocked(self, tmp_path, monkeypatch):
        """``~/.loki/.env`` stays blocked under the $HOME exception — it is a
    ...
    def test_profile_scoped_credential_still_blocked_under_root(self, tmp_path, monkeypatch):
        ...
        denied_root = tmp_path / "root"
        loki_root = denied_root / ".loki"
        prof_dir = loki_root / "profiles" / "myprof"
...
class TestGetHumanDelay:
    def test_off_mode(self):
        with patch.dict(os.environ, {"LOKI_HUMAN_DELAY_MODE": "off"}):
            assert BasePlatformAdapter._get_human_delay() == 0.0

    def test_default_is_off(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("LOKI_HUMAN_DELAY_MODE", None)
            assert BasePlatformAdapter._get_human_delay() == 0.0

    def test_natural_mode_range(self):
        with patch.dict(os.environ, {"LOKI_HUMAN_DELAY_MODE": "natural"}):
    ...
    def test_custom_mode_uses_env_vars(self):
        env = {
            ...
            "LOKI_HUMAN_DELAY_MIN_MS": "100",
            "LOKI_HUMAN_DELAY_MAX_MS": "200",
        }
        with patch.dict(os.environ, env):
            delay = BasePlatformAdapter._get_human_delay()
