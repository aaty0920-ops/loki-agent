...
def test_every_on_disk_subpackage_is_covered_by_packages_find():
    ...
    top-level package is listed twice — bare (``loki_cli``) for the package
    itself and ``loki_cli.*`` for its subpackages — EXCEPT when someone
    forgets the wildcard. v0.15.x listed ``loki_cli`` without ``loki_cli.*``,
    so the wheel shipped ``loki_cli/*.py`` but dropped the ``dashboard_auth``
    and ``proxy`` subpackages. The dashboard then died on every install with
    ``ModuleNotFoundError: No module named 'loki_cli.dashboard_auth'``.
...
def test_bundled_plugin_manifests_ship_in_both_wheel_and_sdist():
    ...
    Plugin discovery (loki_cli/plugins.py) registers each bundled plugin by
