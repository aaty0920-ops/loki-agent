...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.loki/honcho.json
...
class TestResolveConfigPath:
    ...
    def test_falls_back_to_global_without_loki_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
    ...
    def test_from_global_config_uses_default_profile_fallback(self, tmp_path, monkeypatch):
        ...
        # via the HOME-anchored helper, not Path.home() / ".loki".
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".loki"
        profile_home = default_home / "profiles" / "work"
...
class TestResolveActiveHost:
    ...
    def test_default_returns_loki(self):
        with patch.dict(os.environ, {}, clear=True):
    ...
    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            with patch("loki_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "loki_coder"

    def test_default_profile_returns_loki(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("LOKI_HONCHO_HOST", None)
            with patch("loki_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "loki"

    def test_custom_profile_returns_loki(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("LOKI_HONCHO_HOST", None)
...
class TestProfileScopedConfig:
    ...
    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="loki")
        assert config.host == "loki"
        assert config.workspace_id == "loki"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "loki": {"aiPeer": "loki", "peerName": "alice"},
                "loki_coder": {
                    "aiPeer": "loki_coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="loki_coder", config_path=config_file,
        )
        assert config.host == "loki_coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "loki_dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="loki_dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "loki_dreamer"
    ...
    def test_from_global_config_reads_legacy_dot_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "loki.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        config = HonchoClientConfig.from_global_config(
            host="loki_dreamer",
        ...
        assert config.host == "loki_dreamer"
        assert config.peer_name == "dreamer-user"
        assert config.workspace_id == "loki_dreamer"
...
class TestObservationModeMigration:
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"loki": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_loki_config_timeout_override_used_when_config_timeout_missing(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="loki",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("loki_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_loki_request_timeout_alias_used(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="loki",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("loki_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)
