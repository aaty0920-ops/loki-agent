...
from tools.environments.local import _LOKI_PROVIDER_ENV_FORCE_PREFIX
...
def test_close_terminal_tool_gated_on_desktop(monkeypatch):
    """Hidden unless LOKI_DESKTOP is set (mirrors read_terminal gating)."""
...
class TestSpawnEnvSanitization:
    ...
    def test_spawn_via_env_uses_backend_temp_dir_for_artifacts(self, registry):
        ...
        assert "/data/data/com.termux/files/usr/tmp/loki_bg_" in bg_command
        assert ".exit" in bg_command
        assert "rc=$?;" in bg_command
        assert " > /tmp/loki_bg_" not in bg_command
        assert "cat /tmp/loki_bg_" not in bg_command
    ...
    def test_env_poller_quotes_temp_paths_with_spaces(self, registry):
        ...
        assert env.commands[0][0] == "cat '/path with spaces/loki_bg.log' 2>/dev/null"
