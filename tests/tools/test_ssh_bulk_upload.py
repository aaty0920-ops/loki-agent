...
class TestSSHBulkUpload:
    ...
    def test_tar_pipe_commands(self, mock_env, tmp_path):
        ...
        files = [(str(f1), "/home/testuser/.loki/cache/x.txt")]
        ...
        # ssh: extract from stdin at ~/.loki, preserving existing dir modes (#17767)
        ssh_str = " ".join(ssh_cmd)
        assert "ssh" in ssh_str
        assert "tar xf -" in ssh_str
        assert "--no-overwrite-dir" in ssh_str
        assert "-C /home/testuser/.loki" in ssh_str
    ...
    def test_mkdir_failure_raises(self, mock_env, tmp_path):
        ...
        f1 = tmp_path / "y.txt"
        f1.write_text("y")
        files = [(str(f1), "/home/testuser/.loki/skills/y.txt")]
    ...
    def test_tar_create_failure_raises(self, mock_env, tmp_path):
        ...
        f1 = tmp_path / "z.txt"
        f1.write_text("z")
        files = [(str(f1), "/home/testuser/.loki/skills/z.txt")]
    ...
    def test_ssh_extract_failure_raises(self, mock_env, tmp_path):
        ...
        f1 = tmp_path / "w.txt"
        f1.write_text("w")
        files = [(str(f1), "/home/testuser/.loki/skills/w.txt")]
    ...
    def test_ssh_command_uses_control_socket(self, mock_env, tmp_path):
        ...
        f1 = tmp_path / "c.txt"
        f1.write_text("c")
        files = [(str(f1), "/home/testuser/.loki/cache/c.txt")]
    ...
    def test_custom_port_and_key_in_ssh_command(self, monkeypatch, tmp_path):
        ...
        f1 = tmp_path / "d.txt"
        f1.write_text("d")
        files = [(str(f1), "/home/u/.loki/skills/d.txt")]
    ...
    def test_parent_dirs_deduplicated(self, mock_env, tmp_path):
        ...
        files = [
            (str(f1), "/home/testuser/.loki/skills/a.txt"),
            (str(f2), "/home/testuser/.loki/skills/b.txt"),
            (str(f3), "/home/testuser/.loki/credentials/c.txt"),
        ]
        ...
        assert mkdir_str.count("/home/testuser/.loki/skills") == 1
        assert "/home/testuser/.loki/credentials" in mkdir_str
    ...
    def test_tar_stdout_closed_for_sigpipe(self, mock_env, tmp_path):
        ...
        f1 = tmp_path / "s.txt"
        f1.write_text("s")
        files = [(str(f1), "/home/testuser/.loki/skills/s.txt")]
