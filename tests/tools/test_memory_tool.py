...
class TestScanMemoryContent:
    ...
    def test_loki_config_mod_blocked(self):
        result = _scan_memory_content("edit .loki/config.yaml to change settings")
        assert "Blocked" in result
        assert "loki_config_mod" in result
        result = _scan_memory_content("update .loki/SOUL.md with new personality")
        assert "Blocked" in result
        assert "loki_config_mod" in result
    ...
    def test_loki_config_mod_no_false_positives(self):
        """Merely mentioning loki config files should not trigger; only modify intent should."""
        ...
        assert _scan_memory_content("The .loki/config.yaml file contains runtime options") is None
