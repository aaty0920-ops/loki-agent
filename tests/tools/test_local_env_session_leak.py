...
def test_engaged_strips_all_session_vars_when_unset(monkeypatch):
    """The strip covers every LOKI_SESSION_* mirror, not just the key."""
    _engage()
    monkeypatch.setenv("LOKI_SESSION_KEY", "foreign-key")
    monkeypatch.setenv("LOKI_SESSION_THREAD_ID", "foreign-thread")
    monkeypatch.setenv("LOKI_SESSION_CHAT_ID", "foreign-chat")
    monkeypatch.setenv("LOKI_SESSION_USER_ID", "foreign-user")
...
def test_engaged_explicit_empty_contextvar_clears(monkeypatch):
    ...
    """
    monkeypatch.setenv("LOKI_SESSION_KEY", "foreign-after-clear")
    ...
    assert env.get("LOKI_SESSION_KEY", "") == "", (
        f"Foreign key survived an explicit clear: {env.get('LOKI_SESSION_KEY')!r}"
    )
...
def test_sanitize_subprocess_env_unengaged_preserves_fallback(monkeypatch):
    ...
    stale_base = {
        "PATH": "/usr/bin:/bin",
        "LOKI_SESSION_KEY": "cli-bg-key",
    }

    sanitized = _sanitize_subprocess_env(stale_base)

    assert sanitized.get("LOKI_SESSION_KEY") == "cli-bg-key"
...
def test_loki_subprocess_env_bound_contextvar_wins(monkeypatch):
    ...
    try:
        env = loki_subprocess_env()
        assert env.get("LOKI_SESSION_KEY") == "agent:main:discord:group:MINE:111"
    finally:
        clear_session_vars(tokens)
