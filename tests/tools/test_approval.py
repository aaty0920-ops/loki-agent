...
class TestApprovalModeParsing:
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        with mock_patch("loki_cli.config.load_config", return_value={"approvals": {"mode": False}}):
            assert _get_approval_mode() == "off"

    def test_string_off_still_maps_to_off(self):
        with mock_patch("loki_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
class TestWindowsShellDestructiveCommands:
    def test_cmd_del_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"cmd /c del /f /q C:\tmp\loki-victim\file.txt"
        )
    ...
    def test_cmd_rmdir_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"cmd.exe /k rmdir /s /q C:\tmp\loki-victim"
        )
        assert dangerous is True
        assert key is not None
        assert desc == "Windows cmd destructive delete"
    ...
    def test_powershell_remove_item_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"powershell -NoProfile -Command Remove-Item -Recurse -Force C:\tmp\loki-victim"
        )
        assert dangerous is True
        assert key is not None
    ...
    def test_pwsh_rm_alias_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"pwsh -c rm -Recurse -Force C:\tmp\loki-victim"
        )
        assert dangerous is True
        assert key is not None
        assert "delete" in desc.lower()
...
class TestTeePattern:
    ...
    def test_tee_loki_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.loki/.env")
        assert dangerous is True
        assert key is not None
    ...
    def test_tee_custom_loki_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $LOKI_HOME/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_quoted_custom_loki_home_env(self):
        dangerous, key, desc = detect_dangerous_command('echo x | tee "$LOKI_HOME/.env"')
        assert dangerous is True
        assert key is not None
...
class TestLokiConfigWriteProtection:
    ...
    def test_redirect_overwrite(self):
        dangerous, key, desc = detect_dangerous_command("echo 'approvals:' > ~/.loki/config.yaml")
        assert dangerous is True
        assert key is not None

    def test_append(self):
        dangerous, key, desc = detect_dangerous_command("echo '  mode: off' >> ~/.loki/config.yaml")
        assert dangerous is True

    def test_tee(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.loki/config.yaml")
        assert dangerous is True

    def test_cp_over_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.loki/config.yaml")
        assert dangerous is True
    ...
    def test_sed_in_place(self):
        ...
        dangerous, key, desc = detect_dangerous_command("sed -i 's/manual/off/' ~/.loki/config.yaml")
        assert dangerous is True
        assert "loki config" in desc.lower() or "in-place" in desc.lower()

    def test_sed_in_place_long_flag(self):
        dangerous, key, desc = detect_dangerous_command("sed --in-place 's/manual/off/' ~/.loki/config.yaml")
        assert dangerous is True

    def test_sed_in_place_absolute_loki_home_config(self):
    ...
    def test_sed_in_place_absolute_loki_home_env(self):
    ...
    def test_custom_loki_home(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $LOKI_HOME/config.yaml")
        assert dangerous is True
    ...
    def test_perl_in_place_config(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/approvals.mode: on/approvals.mode: off/' ~/.loki/config.yaml"
        )
        assert dangerous is True
        assert "in-place" in desc.lower() or "perl" in desc.lower()

    def test_perl_in_place_absolute_loki_home_config(self):
        config_path = get_loki_home() / "config.yaml"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_ruby_in_place_config(self):
        dangerous, key, desc = detect_dangerous_command(
            "ruby -i -pe 'gsub(/manual/, \"off\")' ~/.loki/config.yaml"
        )
        assert dangerous is True

    def test_ruby_in_place_absolute_loki_home_env(self):
        env_path = get_loki_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_perl_in_place_env(self):
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/SECRET=old/SECRET=new/' ~/.loki/.env"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_separate_flag_token(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -p -i -e 's/approvals.mode: on/approvals.mode: off/' ~/.loki/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_backup_suffix(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i.bak -pe 's/x/y/' ~/.loki/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_eval_no_inplace_safe(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -wne 'print' ~/.loki/config.yaml"
        )
        assert dangerous is False

    def test_read_is_safe(self):
        # Reading config is not a write — must not trip.
        dangerous, key, desc = detect_dangerous_command("cat ~/.loki/config.yaml")
        assert dangerous is False

    def test_normal_yaml_write_safe(self):
        # A non-Loki config.yaml in a project dir is handled by the project
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_loki_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $LOKI_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestSensitiveCopyMovePattern:
    ...
    def test_cp_to_loki_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.loki/config.yaml")
        assert dangerous is True
...
class TestWindowsAbsolutePathFolding:
    ...
    def test_windows_loki_home_config_folds(self, monkeypatch):
        # Loki home nests under the user home on Windows; it must fold before
        # the user-home rewrite eats its prefix.
        monkeypatch.setenv("HOME", r"C:\Users\tester")
        monkeypatch.setenv("LOKI_HOME", r"C:\Users\tester\.loki")
        dangerous, key, _ = detect_dangerous_command(
            r"sed -i 's/manual/off/' C:\Users\tester\.loki\config.yaml"
        )
        assert dangerous is True
        assert key is not None
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_ampersand_detected(self):
        cmd = "python -m loki_cli.main gateway run --replace &"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m loki_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "loki_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_loki_gateway_stop_detected(self):
        cmd = "loki gateway stop"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "gateway" in desc.lower()

    def test_loki_gateway_restart_with_profile_flag_detected(self):
        """A profile flag between `loki` and `gateway` must not slip past
        the guard. See the 2026-04-11 ade-profile self-kill incident."""
        cmd = "loki -p ade gateway restart"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "gateway" in desc.lower()

    def test_loki_gateway_stop_with_long_profile_flag_detected(self):
        cmd = "loki --profile ade gateway stop"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_loki_gateway_multiple_flags_detected(self):
        cmd = "loki -p cocoa --verbose gateway restart"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_loki_gateway_status_with_profile_flag_not_flagged(self):
        """Read-only subcommands stay allowed even with a profile flag."""
        cmd = "loki -p ade gateway status"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is False

    def test_loki_gateway_start_not_flagged(self):
        cmd = "loki gateway start"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is False

    def test_pkill_loki_detected(self):
        """pkill targeting loki/gateway processes must be caught."""
        cmd = 'pkill -f "cli.py --gateway"'
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc

    def test_killall_loki_detected(self):
        cmd = "killall loki"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc
...
class TestIFSWhitespaceBypass:
    ...
    def test_ifs_sed_config_dangerous(self):
        """In-place edit of the Loki security config via IFS must be caught."""
        cmd = "sed${IFS}-i ~/.loki/config.yaml"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestPgrepKillExpansion:
    ...
    def test_pkill_loki_still_detected(self):
...
class TestLaunchctlGatewayLifecycle:
    ...
    def test_launchctl_stop_loki_detected(self):
        cmd = "launchctl stop ai.loki.gateway"
        dangerous, _, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "launchd" in desc.lower() or "loki" in desc.lower()

    def test_launchctl_kickstart_loki_detected(self):
        cmd = "launchctl kickstart -k system/ai.loki.gateway"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_launchctl_bootout_loki_detected(self):
        cmd = "launchctl bootout system/ai.loki.gateway"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_launchctl_unload_loki_detected(self):
        cmd = "launchctl unload ~/Library/LaunchAgents/ai.loki.gateway.plist"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_launchctl_stop_unrelated_not_flagged(self):
        """`launchctl stop` on a non-Loki label is out of scope for the
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_fail_open_true_allows_silently_on_import_error(self):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("loki_cli.config.load_config", return_value=cfg):
