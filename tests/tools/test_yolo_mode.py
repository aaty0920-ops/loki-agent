...
class TestYoloMode:
    ...
    def test_yolo_mode_empty_string_does_not_bypass(self, monkeypatch):
        """Empty string for LOKI_YOLO_MODE should not trigger bypass."""
        monkeypatch.setenv("LOKI_YOLO_MODE", "")
        monkeypatch.setenv("LOKI_INTERACTIVE", "1")
        monkeypatch.setenv("LOKI_SESSION_KEY", "test-session")

        # Empty string is falsy in Python, so getenv("LOKI_YOLO_MODE") returns ""
