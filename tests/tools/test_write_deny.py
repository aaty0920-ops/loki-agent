...
class TestWriteDenyExactPaths:
    ...
    def test_loki_env(self):
        # ``.env`` under the active LOKI_HOME (profile-aware, not just
        # ``~/.loki``) must be write-denied. The hermetic test conftest
        # points LOKI_HOME at a tempdir — resolve via get_loki_home()
        # to match the denylist.
        from loki_constants import get_loki_home
        path = str(get_loki_home() / ".env")
        assert _is_write_denied(path) is True
