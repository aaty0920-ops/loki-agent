...
def test_memory_gate_on_no_interactive_stages(loki_home):
...
def test_memory_gate_on_then_apply(loki_home):
...
def test_load_on_disk_store_honors_configured_char_limits(loki_home, monkeypatch):
...
def test_skill_gate_off_allows_create(loki_home):
...
def test_skill_gate_on_always_stages(loki_home):
...
def test_skill_gate_on_then_apply_writes_file(loki_home):
    # SKILLS_DIR is resolved at import time, so reload the skill module under
    # this test's LOKI_HOME to exercise the real on-disk write path.
    import importlib
    import tools.skill_manager_tool as smt
    importlib.reload(smt)
    from tools import write_approval as wa
...
def test_skill_create_diff_is_full_content(loki_home):
    from tools.skill_manager_tool import skill_manage
    from tools import write_approval as wa
    _set_approval("skills", True)
...
def test_pending_store_roundtrip(loki_home):
    from tools import write_approval as wa
...
def test_handle_pending_list_empty(loki_home):
...
def test_handle_reject(loki_home):
    from loki_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_handle_mode_alias_still_works(loki_home):
...
def test_handle_approval_invalid(loki_home):
    from loki_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_handle_unknown_subcommand_returns_none(loki_home):
    from loki_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_memory_inline_callback_error_stages(loki_home, approval_callback_cleanup):
...
def test_gateway_context_stages_not_prompts(loki_home, monkeypatch):
...
def test_skills_never_prompt_inline_even_with_callback(loki_home, approval_callback_cleanup):
