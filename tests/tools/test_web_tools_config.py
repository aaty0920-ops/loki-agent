...
class TestFirecrawlClientConfig:
    ...
    def test_nous_auth_token_respects_loki_home_override(self, tmp_path):
        """Auth lookup should read from LOKI_HOME/auth.json, not ~/.loki/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".loki").mkdir(parents=True)
...
class TestCheckWebApiKey:
    ...
    def test_tool_gateway_availability_skips_refresh_for_expired_cached_token(
        ...
        monkeypatch.setattr(
            "loki_cli.auth.resolve_nous_access_token",
            _record_refresh,
        )
