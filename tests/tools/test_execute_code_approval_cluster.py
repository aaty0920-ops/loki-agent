...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad LOKI_ prefix dropped,
...
def test_env_scrub_loki_allowlist_and_secret_blocks():
    ...
    env = {
        ...
        "LOKI_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("LOKI_HOME", "LOKI_PROFILE", "LOKI_CONFIG", "LOKI_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "LOKI_BASE_URL", "LOKI_INTERACTIVE", "LOKI_KANBAN_DB",
...
def test_env_scrub_logs_dropped_loki_vars(caplog):
    """Dropping a non-allowlisted, non-secret LOKI_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset LOKI_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "LOKI_HOME": "/h",          # allowlisted → kept, not logged
        "LOKI_BASE_URL": "https://x",   # dropped → logged
        "LOKI_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "LOKI_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "LOKI_HOME" in out and "PATH" in out
    assert "LOKI_BASE_URL" not in out and "LOKI_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "LOKI_BASE_URL" in msgs and "LOKI_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "LOKI_API_KEY" not in msgs
