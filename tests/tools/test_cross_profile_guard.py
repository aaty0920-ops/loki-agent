...
class TestWriteFileCrossProfileGuard:
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_loki):
        ...
        assert "loki-security" in result["error"]
        # File untouched.
        assert target.read_text() == original

    def test_cross_profile_True_bypass(self, fake_loki):
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_blocked(self, fake_loki):
    ...
    def test_v4a_patch_extracts_path_for_guard(self, fake_loki):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_multiple_profiles(self, fake_loki, monkeypatch):
        """When the skill exists in TWO other profiles, both should be named."""
        self._make_skill_in_profile(fake_loki["root"], "everywhere-skill")
        self._make_skill_in_profile(fake_loki["coder_home"], "everywhere-skill")
