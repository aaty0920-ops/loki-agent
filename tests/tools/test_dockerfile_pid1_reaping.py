...
def test_dockerfile_entrypoint_routes_through_the_init(dockerfile_text):
    ...
    directly, the shell becomes PID 1 and will ``exec`` into loki,
    ...
    assert routes_through_init, (
        ...
        "not wired into ENTRYPOINT, loki still runs as PID 1 and zombies "
...
def test_dockerfile_installs_tui_dependencies(dockerfile_text):
    ...
    # resolve dependencies. The bundled ``loki-ink`` workspace package is
    ...
    assert "ui-tui/packages/loki-ink/" in dockerfile_text
...
def test_dockerfile_preinstalls_hindsight_memory_dependency(dockerfile_text):
    ...
    assert any("--extra hindsight" in step for step in sync_steps), (
        ...
        "into /opt/loki/.venv. It lazy-installs into the image layer (not the "
...
def test_dockerfile_materializes_local_tui_ink_package(dockerfile_text):
    ...
    assert "ui-tui/packages/loki-ink/" in dockerfile_text, (
        "Dockerfile must COPY the bundled loki-ink workspace package "
        "so ``await import('@loki/ink')`` resolves at runtime."
    )
