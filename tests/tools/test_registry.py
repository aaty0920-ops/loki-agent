...
class TestDeregisterAuthorization:
    ...
    def test_plugin_with_opt_in_can_deregister_unowned_tool(self):
        ...
        with patch.object(ToolRegistry, "_caller_module", return_value="loki_plugins.allowed"):
            reg.deregister("protected")
    ...
    def test_plugin_can_deregister_its_own_tool(self):
        ...
        reg = ToolRegistry()
        reg.register_plugin_override_policy("loki_plugins.myplug", False)
        handler = eval("lambda *a, **k: 'own'", {"__name__": "loki_plugins.myplug"})
        reg.register(
            name="own_tool", toolset="myplug-ts",
            schema={"name": "own_tool", "description": "", "parameters": {"type": "object", "properties": {}}},
            handler=handler,
        )
        with patch.object(ToolRegistry, "_caller_module", return_value="loki_plugins.myplug"):
