...
class TestModalBulkUpload:
    ...
    def test_mkdir_includes_all_parents(self, monkeypatch, tmp_path):
        ...
        files = [
            (str(src), "/root/.loki/credentials/f.txt"),
            (str(src), "/root/.loki/skills/deep/nested/f.txt"),
        ]
        ...
        cmd = exec_calls[0][2]
        assert "/root/.loki/credentials" in cmd
        assert "/root/.loki/skills/deep/nested" in cmd
    ...
    def test_single_exec_call(self, monkeypatch, tmp_path):
        ...
        for i in range(20):
            ...
            files.append((str(src), f"/root/.loki/cache/file_{i}.txt"))
