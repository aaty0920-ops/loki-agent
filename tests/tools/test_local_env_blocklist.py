...
See: https://github.com/NousResearch/loki-agent/issues/1002
See: https://github.com/NousResearch/loki-agent/issues/1264
"""
...
class TestLokiBinDirOnPath:
    ...
    def test_resolves_via_sys_executable_dir(self, monkeypatch, tmp_path):
        ...
        (venv_bin / "loki").write_text("#!/bin/sh\n")
        ...
        assert local_mod._resolve_loki_bin_dir() == str(venv_bin)
    ...
    def test_returns_none_when_unresolvable(self, monkeypatch):
        ...
        assert local_mod._resolve_loki_bin_dir() is None

    def test_prepend_adds_missing_dir_at_front(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        local_mod._LOKI_BIN_DIR = "/opt/loki/bin"
        out = local_mod._prepend_loki_bin_dir("/usr/bin:/bin")
        assert out.split(os.pathsep)[0] == "/opt/loki/bin"
    ...
    def test_prepend_is_idempotent(self, monkeypatch):
        ...
        self._reset_cache()
        local_mod._LOKI_BIN_DIR = "/opt/loki/bin"
        once = local_mod._prepend_loki_bin_dir("/usr/bin:/bin")
        twice = local_mod._prepend_loki_bin_dir(once)
        assert twice == once
        assert once.split(os.pathsep).count("/opt/loki/bin") == 1
    ...
    def test_make_run_env_injects_loki_bin_dir(self, monkeypatch):
...
class TestLokiInternalDynamicSecrets:
    ...
    def test_predicate_matches_auxiliary_api_key(self):
        from tools.environments.local import _is_loki_internal_secret
