...
class TestRegisterCredentialFiles:
    ...
    def test_string_entry(self, tmp_path):
        loki_home = tmp_path / ".loki"
        loki_home.mkdir()
        (loki_home / "secret.key").write_text("key")
...
class TestPathTraversalSecurity:
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside LOKI_HOME must still be registered."""
        loki_home = tmp_path / ".loki"
        loki_home.mkdir()
        monkeypatch.setenv("LOKI_HOME", str(loki_home))
        (loki_home / "token.json").write_text('{"token": "abc"}')
...
class TestCacheDirectoryMounts:
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.loki/cache/documents"
...
class TestMapCachePathToContainer:
    ...
    def test_custom_container_base_for_remote_home(self, tmp_path, monkeypatch):
        ...
        assert (
            map_cache_path_to_container(host_path, container_base="/home/agent/.loki")
            == "/home/agent/.loki/cache/images/remote.png"
        )

    def test_returns_none_when_outside_cache_dirs(self, tmp_path, monkeypatch):
        loki_home = tmp_path / ".loki"
        (loki_home / "cache" / "images").mkdir(parents=True)
        monkeypatch.setenv("LOKI_HOME", str(loki_home))
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(loki_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        loki_home = tmp_path / ".loki"
        ss_dir = loki_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.loki/cache/screenshots/session_abc/screen1.png"
