...
def test_manager_builds_loki_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns LokiMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _LOKI_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("LOKI_HOME", str(tmp_path))
    _set_interactive_stdin(monkeypatch)
    ...
    assert _LOKI_PROVIDER_CLS is not None
    assert isinstance(provider, _LOKI_PROVIDER_CLS)
    assert provider._loki_server_name == "srv"
