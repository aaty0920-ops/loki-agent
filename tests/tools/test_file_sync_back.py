...
class TestInferHostPath:
    ...
    def test_infer_partial_prefix_no_false_match(self, tmp_path):
        """A partial prefix like /root/.loki/sk should NOT match /root/.loki/skills/."""
        host_file = tmp_path / "host" / "skills" / "a.py"
        _write_file(host_file, b"content")
        mapping = [(str(host_file), "/root/.loki/skills/a.py")]

        mgr = _make_manager(tmp_path, file_mapping=mapping)
        # /root/.loki/skillsXtra/b.py shares prefix "skills" but the
        # directory is different — should not match /root/.loki/skills/
        result = mgr._infer_host_path(
            "/root/.loki/skillsXtra/b.py",
            file_mapping=mapping,
        )
        assert result is None
