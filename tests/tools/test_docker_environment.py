...
def test_run_command_tags_task_and_profile_labels(monkeypatch):
    ...
    returns ``"default"`` for the root Loki home."""
    monkeypatch.setattr(docker_env, "find_docker", lambda: "/usr/bin/docker")
    ...
    labels = _labels_in_run_args(_run_args_from_calls(calls))
    assert "loki-task-id=kanban-42" in labels, (
        f"loki-task-id=kanban-42 missing; got: {sorted(labels)}"
    )
    assert "loki-profile=research-bot" in labels, (
        f"loki-profile=research-bot missing; got: {sorted(labels)}"
    )
...
def test_reap_orphan_removes_stale_exited_container(monkeypatch):
    ...
    SIGKILL'd Loki processes leak containers permanently."""
...
def test_reap_orphan_spares_recently_exited_container(monkeypatch):
    ...
    container belongs to a Loki process that just finished and may be
...
def test_reap_orphan_scopes_to_profile_filter_via_label(monkeypatch):
    """The reaper must pass ``--filter label=loki-profile=<profile>`` to
    ...
    assert "label=loki-profile=research-bot" in flat, (
        f"profile filter not applied to docker ps; got args: {ps_calls[0][0]}"
    )
    assert "label=loki-agent=1" in flat, (
        f"loki-agent label filter must also be applied; got: {ps_calls[0][0]}"
    )
    assert "status=exited" in flat, (
        "must filter to exited containers only — running containers may "
        "belong to a sibling Loki process and must NEVER be reaped"
    )
...
def test_credential_mount_works_when_source_is_valid_file(monkeypatch, tmp_path):
    ...
    fake_mounts = [
        {"host_path": str(valid_file), "container_path": "/root/.loki/token.json"},
    ]
    monkeypatch.setattr(
        "tools.credential_files.get_credential_file_mounts",
        lambda: fake_mounts,
...
def test_image_uses_init_entrypoint_detects_s6_init(monkeypatch):
    ...
    monkeypatch.setattr(docker_env.subprocess, "run", _run)
    assert docker_env._image_uses_init_entrypoint("/usr/bin/docker", "loki-agent:latest") is True
...
def test_is_container_gone_matches_removal_errors(monkeypatch):
    ...
    assert env._is_container_gone(
        "Error response from daemon: No such container: loki-abc123"
    )
...
def test_execute_recovers_from_out_of_band_removal(monkeypatch):
    ...
    outputs = iter([
        {"output": "Error response from daemon: No such container: loki-x", "returncode": 1},
