...
class TestSendMessageTool:
    ...
    def test_cron_duplicate_target_is_skipped_and_explained(self):
        ...
        with patch.dict(
            os.environ,
            {
                "LOKI_CRON_AUTO_DELIVER_PLATFORM": "telegram",
                "LOKI_CRON_AUTO_DELIVER_CHAT_ID": "-1001",
            },
            clear=False,
        ), \
...
class TestSendToPlatformChunking:
    ...
    def test_slack_messages_are_formatted_before_send(self, monkeypatch):
        ...
        with _patch_slack_standalone_sender(send):
            result = asyncio.run(
                _send_to_platform(
                    ...
                    SimpleNamespace(enabled=True, token="***", extra={}),
                    "C123",
                    "**hello** from [Loki](<https://example.com>)",
                )
            )
        ...
        send.assert_awaited_once_with(
            "***",
            "C123",
            "*hello* from <https://example.com|Loki>",
            thread_ts=None,
        )
...
class TestParseTargetRefMatrix:
    ...
    def test_matrix_user_mxid_is_explicit(self):
        ...
        chat_id, thread_id, is_explicit = _parse_target_ref("matrix", "@loki:matrix.org")
        assert chat_id == "@loki:matrix.org"
        assert thread_id is None
        assert is_explicit is True
...
class TestCheckSendMessage:
    ...
    1. ``LOKI_KANBAN_TASK`` is set (worker spawned by the kanban dispatcher
       — parent gateway is by definition running, but the worker's
       ``LOKI_HOME`` may be a profile dir without a ``gateway.pid``).
    2. ``LOKI_SESSION_PLATFORM`` resolves to a non-empty, non-``local`` value
    ...
    def test_kanban_task_env_short_circuits_before_gateway_check(self, monkeypatch):
        """Honoring LOKI_KANBAN_TASK must not depend on importing or calling
        gateway.status — the worker may run with a LOKI_HOME that has no
    ...
    def test_local_platform_falls_through_to_gateway_check(self, monkeypatch):
        """``LOKI_SESSION_PLATFORM=local`` means CLI-style — must defer to
