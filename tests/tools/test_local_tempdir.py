    ...
    def test_prefers_backend_env_tmpdir_override(self, monkeypatch):
        ...
        with patch.object(LocalEnvironment, "init_session", autospec=True, return_value=None):
            env = LocalEnvironment(
                cwd=".",
                timeout=10,
                env={"TMPDIR": "/data/data/com.termux/files/home/.cache/loki-tmp/"},
            )

        assert env.get_temp_dir() == "/data/data/com.termux/files/home/.cache/loki-tmp"
        assert env._snapshot_path == (
            f"/data/data/com.termux/files/home/.cache/loki-tmp/loki-snap-{env._session_id}.sh"
        )
        assert env._cwd_file == (
            f"/data/data/com.termux/files/home/.cache/loki-tmp/loki-cwd-{env._session_id}.txt"
        )
