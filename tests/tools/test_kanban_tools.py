...
def test_kanban_tools_visible_with_env_var(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("LOKI_KANBAN_TASK", "t_fake")
    home = tmp_path / ".loki"
    home.mkdir()
    monkeypatch.setenv("LOKI_HOME", str(home))
...
def test_worker_with_kanban_toolset_still_hides_board_routing(monkeypatch, tmp_path):
    ...
    in its config, the LOKI_KANBAN_TASK env var means it's a focused
...
def test_complete_metadata_round_trips_through_show(worker_env):
    ...
    handoff = {
        "changed_files": ["loki_cli/kanban.py"],
...
def test_complete_stamps_worker_session_id_from_env(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("LOKI_SESSION_ID", "session-trusted")
...
def test_create_inherits_worker_dir_workspace(monkeypatch, worker_env):
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("LOKI_KANBAN_TASK", self_tid)
...
def test_create_no_worker_task_stays_scratch(monkeypatch, worker_env):
    """Orchestrator/CLI callers (no LOKI_KANBAN_TASK) still default to
...
def test_create_session_id_arg_overrides_env(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("LOKI_SESSION_ID", "from-env")
    from tools import kanban_tools as kt
    from loki_cli import kanban_db as kb
    out = kt._handle_create({
...
def test_kanban_guidance_not_in_normal_prompt(monkeypatch, tmp_path):
    """A normal chat session (no LOKI_KANBAN_TASK) must NOT have
...
def test_kanban_guidance_in_worker_prompt(monkeypatch, tmp_path):
    """A worker session (LOKI_KANBAN_TASK set) MUST have the full
...
def test_worker_complete_rejects_stale_run_id(worker_env, monkeypatch):
    ...
    from tools import kanban_tools as kt
    monkeypatch.setenv("LOKI_KANBAN_RUN_ID", str(run1.id))
    ...
    monkeypatch.setenv("LOKI_KANBAN_RUN_ID", str(run2.id))
...
def test_board_param_routes_heartbeat_to_alt_board(monkeypatch, tmp_path):
    ...
    use the worker-env style fixture inline (pinning LOKI_KANBAN_TASK
    to a task that exists in the alt board)."""
    home = tmp_path / ".loki"
    home.mkdir()
    monkeypatch.setenv("LOKI_HOME", str(home))
    monkeypatch.setenv("LOKI_PROFILE", "alt-worker")
    monkeypatch.delenv("LOKI_KANBAN_DB", raising=False)
    monkeypatch.delenv("LOKI_KANBAN_BOARD", raising=False)
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
def test_create_partial_session_context_no_subscribe(monkeypatch, worker_env):
    ...
    from tools import kanban_tools as kt
    monkeypatch.setenv("LOKI_SESSION_PLATFORM", "slack")
    monkeypatch.delenv("LOKI_SESSION_CHAT_ID", raising=False)
    monkeypatch.delenv("LOKI_SESSION_KEY", raising=False)
    monkeypatch.delenv("LOKI_SESSION_ID", raising=False)
