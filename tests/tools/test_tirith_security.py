...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_loki_bin(self):
        """After _INSTALL_FAILED, manual install in LOKI_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        loki_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(loki_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(loki_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._loki_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == loki_bin
            assert _tirith_mod._resolved_path == loki_bin
            mock_clear.assert_called_once()
...
class TestLokiHomeIsolation:
    ...
    def test_failure_marker_respects_loki_home(self):
        """_failure_marker_path must use LOKI_HOME, not hardcoded ~/.loki."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"LOKI_HOME": "/custom/loki"}):
            result = _failure_marker_path()
        assert result == "/custom/loki/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets LOKI_HOME; verify it's active."""
        loki_home = os.getenv("LOKI_HOME")
        assert loki_home is not None, "LOKI_HOME should be set by conftest"
        assert "loki_test" in loki_home, "Should point to test temp dir"

    def test_get_loki_home_fallback(self):
        """Without LOKI_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_loki_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove LOKI_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".loki")
            result = _get_loki_home()
    ...
    def test_mkdtemp_oserror_propagates_to_ensure_installed(self):
        ...
        with patch("tools.tirith_security.tempfile.mkdtemp",
             ...
             patch("tools.tirith_security.shutil.which",
                   return_value=None), \
             patch("tools.tirith_security._loki_bin_dir",
