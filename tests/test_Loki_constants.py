...
class TestGetDefaultLokiRoot:
    ...
    def test_loki_home_is_native(self, tmp_path, monkeypatch):
        """When LOKI_HOME = ~/.loki, returns ~/.loki."""
        native = tmp_path / ".loki"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LOKI_HOME", str(native))
        assert get_default_loki_root() == native

    def test_loki_home_is_profile(self, tmp_path, monkeypatch):
        """When LOKI_HOME is a profile under ~/.loki, returns ~/.loki."""
        native = tmp_path / ".loki"
    ...
    def test_loki_home_is_docker(self, tmp_path, monkeypatch):
        """When LOKI_HOME points outside ~/.loki (Docker), returns LOKI_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LOKI_HOME", str(docker_home))
        assert get_default_loki_root() == docker_home

    def test_loki_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any LOKI_HOME outside ~/.loki is treated as the root."""
        custom = tmp_path / "my-loki-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LOKI_HOME", str(custom))
        assert get_default_loki_root() == custom
    ...
    def test_no_loki_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        """Windows fallback still uses AppData/Local/loki without LOCALAPPDATA."""
        home = tmp_path / "Home"
        monkeypatch.delenv("LOKI_HOME", raising=False)
        ...
        assert get_default_loki_root() == home / "AppData" / "Local" / "loki"
...
class TestLokiManagedNode:
    ...
    def test_with_loki_node_path_prepends_existing_managed_dirs(self, tmp_path, monkeypatch):
        home = tmp_path / "loki"
        node_dir = home / "node"
        bin_dir = node_dir / "bin"
        node_dir.mkdir(parents=True)
        ...
        env = with_loki_node_path({"PATH": "system-node"})
...
class TestNodeToolRunnable:
    ...
    def test_broken_managed_npm_heals_instead_of_path_fallback(self, tmp_path, monkeypatch):
        ...
        assert find_loki_node_executable("npm") == str(healed_npm)
...
class TestGetLokiDir:
    ...
    def test_legacy_populated_with_subdir_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_loki_dir("platforms/matrix/store", "matrix/store")
        assert result == legacy
    ...
    def test_legacy_empty_and_new_missing_returns_new(self, tmp_path, monkeypatch):
        ...
        legacy.mkdir()
        result = get_loki_dir("cache/audio", "audio_cache")
