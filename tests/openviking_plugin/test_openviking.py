...
class TestOpenVikingTurnConversion:
            ...
            turn = [
                ...
                },
                {
                    ...
                    "content": json.dumps({
                        "results": [
                            {
                                "uri": "viking://user/loki/memories/context",
    ...
    def test_messages_to_openviking_batch_adds_assistant_peer_id_when_requested(self):
        ...
        assert batch == [
            {"role": "user", "parts": [{"type": "text", "text": "hello"}]},
            {"role": "assistant", "parts": [{"type": "text", "text": "answer"}], "peer_id": "loki"},
        ]
...
class TestOpenVikingRead:
        ...
        provider._client = FakeVikingClient(
            {
                (
                    "/api/v1/content/read",
                    (("uri", "viking://user/loki/memories/profile.md"),),
                ): {"result": "full text"},
            }
        )

        result = json.loads(provider._tool_read({"uri": "viking://user/loki/memories/profile.md", "level": "full"}))

        assert result["uri"] == "viking://user/loki/memories/profile.md"
        assert result["resolved_uri"] == "viking://user/loki/memories/profile.md"
    ...
    def test_read_deduplicates_uri_batch_and_keeps_errors_per_uri(self):
        provider = OpenVikingMemoryProvider()
        ok_uri = "viking://user/loki/memories/ok.md"
        bad_uri = "viking://user/loki/memories/bad.md"
        provider._client = FakeVikingClient(
            {
                (
                    "/api/v1/content/read",
    ...
    def test_overview_file_uri_routes_straight_to_content_read_via_stat_probe(self):
        ...
        provider = OpenVikingMemoryProvider()
        file_uri = "viking://user/loki/memories/entities/mem_abc.md"
        provider._client = FakeVikingClient(
            {
                (
                    "/api/v1/fs/stat",
    ...
    def test_overview_directory_uri_uses_stat_probe_then_overview(self):
        ...
        provider = OpenVikingMemoryProvider()
        dir_uri = "viking://user/loki/memories"
        provider._client = FakeVikingClient(
            {
                (
                    "/api/v1/fs/stat",
...
class TestOpenVikingAutoRecallPrefetch:
    def test_prefetch_e2e_sends_limit_and_reads_l2_content(self, monkeypatch):
                    ...
                    if payload.get("context_type") == "memory":
                        self._send_json({
                            "result": {
                                "memories": [
                                    {
                                        "uri": "viking://user/peers/loki/memories/e2e-full.md",
                                        "score": 0.9,
                                        "level": 2,
                                        "category": "events",
        ...
        assert records["reads"] == ["viking://user/peers/loki/memories/e2e-full.md"]
        ...
        assert all(headers.get("x-openviking-actor-peer") == "loki" for headers in normalized_headers)
        ...
        responses = {
            (
                ...
                "result": {
                    "memories": [
                        {
                            "uri": "viking://user/peers/loki/memories/caroline.md",
                            "score": 0.9,
                            "level": 1,
                            "category": "profile",
        ...
        responses = {
            ...
            (
                ...
                "result": {
                    "memories": [
                        {
                            "uri": "viking://user/peers/loki/memories/melanie-race.md",
                            "score": 0.9,
                            "level": 1,
                            "category": "events",
        ...
        responses = {
            ("/api/v1/search/search", "memory", "What should we recall?", "session-test"): {
                "result": {
                    "memories": [
                        {
                            "uri": "viking://user/peers/loki/memories/keep.md",
                            "score": 0.22,
                            "level": 1,
                        ...
                        {
                            "uri": "viking://user/peers/loki/memories/drop.md",
                            "score": 0.12,
                            "level": 1,
                            "category": "preferences",
        ...
        responses = {
            ("/api/v1/search/search", "memory", "What should we recall?", "session-test"): {
                "result": {
                    "memories": [
                        {
                            "uri": "viking://user/peers/loki/memories/too-large.md",
                            "score": 0.9,
                            "level": 1,
                            "category": "memory",
                        ...
                        {
                            "uri": "viking://user/peers/loki/memories/small.md",
                            "score": 0.8,
                            "level": 1,
                            "category": "memory",
        ...
        responses = {
            ("/api/v1/search/search", "memory", "What should we recall?", "session-test"): {
                "result": {
                    "memories": [
                        {
                            "uri": "viking://user/peers/loki/memories/full.md",
                            "score": 0.9,
                            "level": 2,
                            "category": "events",
            ...
            ("/api/v1/content/read", "viking://user/peers/loki/memories/full.md"): {
