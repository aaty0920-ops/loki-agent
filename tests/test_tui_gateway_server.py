...
def test_voice_toggle_tts_branch_also_carries_record_key(monkeypatch):
    ...
    monkeypatch.setenv("LOKI_VOICE", "1")
    monkeypatch.delenv("LOKI_VOICE_TTS", raising=False)
...
def test_session_resume_profile_uses_profile_db_cwd(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("loki_state.SessionDB", lambda db_path=None: profile_db)
...
def test_resolve_model_strips_config_model(monkeypatch):
    ...
    monkeypatch.setattr(
        server, "_load_cfg", lambda: {"model": {"default": " nous/loki-test "}}
    )

    assert server._resolve_model() == "nous/loki-test"
...
def test_startup_runtime_uses_tui_provider_env(monkeypatch):
    monkeypatch.setenv("LOKI_MODEL", "nous/loki-test")
    monkeypatch.setenv("LOKI_TUI_PROVIDER", "nous")
    monkeypatch.delenv("LOKI_INFERENCE_PROVIDER", raising=False)

    assert server._resolve_startup_runtime() == ("nous/loki-test", "nous")
...
def test_startup_runtime_does_not_treat_inference_provider_as_explicit(monkeypatch):
    ...
    monkeypatch.setenv("LOKI_INFERENCE_PROVIDER", "nous")
    monkeypatch.setattr(
        "loki_cli.models.detect_static_provider_for_model",
        lambda model, provider: None,
    )

    assert server._resolve_startup_runtime() == ("nous/loki-test", None)
...
def test_setup_status_reports_provider_config(monkeypatch):
    monkeypatch.setattr("loki_cli.main._has_any_provider_configured", lambda: False)
...
def test_setup_runtime_check_rejects_empty_runtime_key(monkeypatch):
    monkeypatch.setattr("loki_cli.main._has_any_provider_configured", lambda: True)
    monkeypatch.setattr(
        "loki_cli.runtime_provider.resolve_runtime_provider",
...
def test_session_compress_syncs_session_key_after_rotation(monkeypatch):
    ...
    continuation session — mirrors LokiCLI._manual_compress's
...
def test_file_attach_uploads_remote_file_into_session_workspace(monkeypatch, tmp_path):
    ...
    try:
        ...
        stored = workspace / ".loki" / "desktop-attachments" / "report.txt"
        assert resp["result"]["attached"] is True
        assert resp["result"]["uploaded"] is True
        assert resp["result"]["path"] == str(stored)
        assert resp["result"]["ref_text"] == "@file:.loki/desktop-attachments/report.txt"
...
def test_file_attach_copies_gateway_visible_file_outside_workspace(monkeypatch, tmp_path):
    ...
    try:
        ...
        stored = workspace / ".loki" / "desktop-attachments" / "outside.txt"
        assert resp["result"]["attached"] is True
        assert resp["result"]["uploaded"] is True
        assert resp["result"]["ref_text"] == "@file:.loki/desktop-attachments/outside.txt"
...
def test_file_attach_quotes_ref_with_spaces(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert resp["result"]["attached"] is True
        assert resp["result"]["ref_text"] == "@file:`.loki/desktop-attachments/my exam schedule.csv`"
    finally:
        server._sessions.pop("sid", None)
...
def test_complete_slash_surfaces_completer_error(monkeypatch):
    with patch(
        "loki_cli.commands.SlashCommandCompleter",
...
def test_browser_manage_connect_default_local_reports_launch_hint(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        ...
        with (
            patch(
                "loki_cli.browser_connect.try_launch_chrome_debug", return_value=False
            ),
            patch(
...
def test_browser_manage_connect_default_local_retries_after_launch(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        with patch(
            "loki_cli.browser_connect.try_launch_chrome_debug", return_value=True
        ):
            resp = server.handle_request(
                {"id": "1", "method": "browser.manage", "params": {"action": "connect"}}
            )
...
def test_session_save_writes_under_loki_home_with_system_prompt(monkeypatch, tmp_path):
    ...
    agent = types.SimpleNamespace(
        ...
        _cached_system_prompt="You are Loki.",
    )
    history = [
        {"role": "user", "content": "hi"},
        {"role": "assistant", "content": "hello"},
    ...
    assert payload["system_prompt"] == "You are Loki."
...
def test_persist_model_switch_preserves_sibling_model_keys(tmp_path, monkeypatch):
    ...
    # save_config_value() resolves the config path from cli._loki_home, which
    # is captured at import time — patch it directly (set_loki_home_override
