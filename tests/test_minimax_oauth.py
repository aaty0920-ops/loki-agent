...
def test_poll_token_timeout_raises():
    ...
    import loki_cli.auth as auth_module
...
def test_refresh_updates_access_token():
    ...
    with patch("httpx.Client") as mock_client_class:
        ...
        with patch("loki_cli.auth._minimax_save_auth_state"):
            result = _refresh_minimax_oauth_state(state)
...
def test_resolve_credentials_does_not_quarantine_on_transient_refresh_failure():
    ...
    with patch("loki_cli.auth.get_provider_auth_state", return_value=stale_state), \
         patch("loki_cli.auth._refresh_minimax_oauth_state", side_effect=_transient_refresh), \
         patch("loki_cli.auth._minimax_save_auth_state", side_effect=lambda s: saved_states.append(dict(s))):
        with pytest.raises(AuthError) as exc_info:
            resolve_minimax_oauth_runtime_credentials()
...
def test_token_provider_rereads_state_each_call():
    ...
    persisted by another loki process are immediately visible."""
    from loki_cli.auth import build_minimax_oauth_token_provider
    ...
    provider = build_minimax_oauth_token_provider()
    with patch("loki_cli.auth.get_provider_auth_state", side_effect=states):
        first = provider()
        second = provider()
