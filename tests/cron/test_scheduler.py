...
class TestRunJobSessionPersistence:
    def test_run_job_passes_session_db_and_cron_platform(self, tmp_path):
        ...
        with patch("cron.scheduler._loki_home", tmp_path), \
             ...
             patch("loki_state.SessionDB", return_value=fake_db), \
             patch(
    ...
    def test_run_job_enabled_toolsets_resolves_from_platform_config_when_not_set(self, tmp_path):
        ...
        resolves them from ``loki tools`` platform config for ``cron``
        ...
        # The cron default is _LOKI_CORE_TOOLS with _DEFAULT_OFF_TOOLSETS
    ...
    def test_run_job_per_job_toolsets_win_over_platform_config(self, tmp_path):
        ...
        over the platform-level ``loki tools`` config."""
        job = {
            "id": "override-job",
            "name": "test",
            "prompt": "hello",
        ...
        # Even if the user has ``loki tools`` configured to enable web+file
        # for cron, the per-job override wins.
        extra = [patch("loki_cli.tools_config._get_platform_tools", return_value={"web", "file"})]
...
class TestRunJobConfigEnvVarExpansion:
    ...
    def test_fallback_model_env_ref_in_config_yaml_is_expanded(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text(
            ...
            "fallback_providers:\n"
            "  - provider: openrouter\n"
            "    model: ${_LOKI_TEST_CRON_FALLBACK}\n"
        )
        monkeypatch.setenv("_LOKI_TEST_CRON_FALLBACK", "gpt-4o-fallback-test")
...
class TestRunJobModelResolution:
    ...
    def test_explicit_null_model_block_in_config_does_not_overwrite_env(self, tmp_path, monkeypatch):
        """``model: null`` in config.yaml must not overwrite a resolved LOKI_MODEL.
...
class TestRunJobSkillBacked:
    ...
    def test_run_job_preserves_credential_file_passthrough_into_worker_thread(self, tmp_path):
        ...
        with patch("cron.scheduler._loki_home", tmp_path), \
             patch("cron.scheduler._resolve_origin", return_value=None), \
             patch("tools.credential_files._resolve_loki_home", return_value=tmp_path), \
             patch("loki_cli.env_loader.load_loki_dotenv"), \
             patch("loki_cli.env_loader.reset_secret_source_cache"), \
             patch("loki_state.SessionDB", return_value=fake_db), \
             patch(
