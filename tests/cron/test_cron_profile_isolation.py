...
def test_cron_execution_home_follows_active_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(scheduler, "_loki_home", None, raising=False)
    assert scheduler._get_loki_home().resolve() == profile_home.resolve()
    assert scheduler._get_loki_home().resolve() != root.resolve()


def test_cron_storage_unaffected_when_no_profile(tmp_path, monkeypatch):
    """With no profile (LOKI_HOME == root), the store is the root's cron dir
    ...
    importlib.reload(jobs)
    try:
        assert jobs.LOKI_DIR.resolve() == root.resolve()
