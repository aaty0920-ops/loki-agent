...
def test_dashboard_slot_reports_down_when_disabled(
    ...
    """Without LOKI_DASHBOARD, s6-svstat should report the dashboard
    slot as DOWN (not up-with-sleep-infinity, which would
    false-positive `loki doctor` and any other health check).
    ...
    LOKI_DASHBOARD is unset, so the slot reflects reality.
    """
    start_container(built_image, container_name, cmd="sleep 60")
    ...
    assert "down" in r.stdout, (
        f"Dashboard slot should be 'down' without LOKI_DASHBOARD; "
...
def test_dashboard_slot_reports_up_when_enabled(
    ...
    """Symmetry: with LOKI_DASHBOARD=1, s6-svstat reports the slot as up."""
    # The default dashboard host is 0.0.0.0, which now engages the
    # OAuth auth gate. Without a provider registered (no
    # LOKI_DASHBOARD_OAUTH_CLIENT_ID in this test env), start_server
    ...
    start_container(
        ...
        "LOKI_DASHBOARD_BASIC_AUTH_PASSWORD=test-dashboard-pw",
        cmd="sleep 120",
    )
...
def test_dashboard_opt_in_starts(
    ...
    """With LOKI_DASHBOARD=1, a dashboard process should be visible."""
    ...
    ok, _ = poll_container(
        container_name, "pgrep -f 'loki dashboard'", deadline_s=30.0,
    )
    assert ok, "Dashboard should be running with LOKI_DASHBOARD=1"


def test_dashboard_port_override(
    built_image: str, container_name: str,
) -> None:
    """LOKI_DASHBOARD_PORT changes the dashboard's listen port."""
...
def test_dashboard_restarts_after_crash(
    ...
    for _attempt in range(10):
        first_pid_result = docker_exec(
            container_name, "pgrep", "-f", "loki dashboard",
        )
    ...
    # Kill the dashboard. The dashboard process runs as loki, so the
    # loki user can kill it (same UID).
