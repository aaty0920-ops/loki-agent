...
def test_install_method_stamp_is_code_scoped(
    ...
    /opt/loki/.install_method (code-scoped), NOT in $LOKI_HOME."""
    start_container(built_image, container_name)

    # Code-scoped stamp must exist and say "docker"
    r = docker_exec_sh(
        container_name,
        "cat /opt/loki/.install_method",
        timeout=10,
    )
    assert r.returncode == 0, (
        f"/opt/loki/.install_method not found: {r.stderr}"
    )
    assert r.stdout.strip() == "docker", (
        f"expected 'docker' stamp, got: {r.stdout.strip()!r}"
    )

    # $LOKI_HOME must NOT have a 'docker' stamp
    r = docker_exec_sh(
        container_name,
    ...
    assert r.stdout.strip() != "docker", (
        f"$LOKI_HOME/.install_method is stamped 'docker' - stage2 must "
