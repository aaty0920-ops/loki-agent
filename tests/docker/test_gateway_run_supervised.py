...
Before the s6 migration, ``docker run nousresearch/loki-agent gateway
...
def test_gateway_run_no_supervise_flag_preserves_legacy_behavior(
    ...
    Three positive assertions confirm we took the pre-s6 path:
      ...
      * The CMD process is a python ``loki gateway run`` invocation
        (not ``sleep infinity``).
...
def test_gateway_run_no_supervise_env_var(
    ...
    if status == "running":
        assert not _svstat_wants_up(container_name, "gateway-default"), (
            "LOKI_GATEWAY_NO_SUPERVISE=1: gateway-default has "
...
def test_supervised_gateway_stdout_reaches_docker_logs(
    ...
    log file under ``${LOKI_HOME}/logs/gateways/<profile>/current``.
    ...
    a poor user experience: the iconic "Loki Gateway Starting…"
    ...
    # Poll docker logs for the banner glyph (⚕) or "Loki Gateway
    ...
    assert "⚕" in combined or "Loki Gateway Starting" in combined, (
    ...
    assert "⚕" in file_contents or "Loki Gateway Starting" in file_contents, (
