...
def test_systemctl_status_passes_through():
    ...
    r = subprocess.run(
        ["systemctl", "--user", "status", "loki-gateway", "--no-pager"],
        capture_output=True,
        text=True,
        check=False,
    )
...
def test_systemctl_show_passes_through():
    r = subprocess.run(
        ["systemctl", "--user", "show", "loki-gateway", "--no-pager"],
        capture_output=True,
        text=True,
        check=False,
    )
...
def test_systemctl_unrelated_unit_passes_through():
    """systemctl restart of a non-loki unit is allowed (we only protect loki)."""
...
def test_subprocess_pkill_with_unrelated_pattern_passes_through():
    """``pkill -f some-unrelated-pattern`` (no loki/python) is fine."""
