"""
Loki MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Loki-specific extra)

Usage:
    loki mcp serve
    loki mcp serve --verbose
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using LOKI_HOME."""
    try:
        from loki_constants import get_loki_home
        return get_loki_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("LOKI_HOME", Path.home() / ".loki")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from loki_constants import get_loki_home
        directory_file = get_loki_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("LOKI_HOME", Path.home() / ".loki")
...
class EventBridge:
    ...
    This is the Loki equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from loki_constants import get_loki_home
            db_file = get_loki_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("LOKI_HOME", Path.home() / ".loki")) / "state.db"
