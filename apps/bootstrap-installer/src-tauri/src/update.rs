...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend loki.exe (or the desktop Loki.exe
            ...
            // force-kill every Loki.exe except ourselves, then give the OS a
            ...
            emit_log(
                ...
                LogStream::Stdout,
                &format!(
                    "[handoff] Loki still holding install files ({}); force-killing stragglers…",
                    format_locked_paths(&locked)
                ),
            );
            force_kill_other_loki();
...
/// Force-kill any `loki.exe` other than this process. Windows-only; a no-op
...
/// knew its children — so we kill the whole `loki.exe` image tree via
/// taskkill, excluding our own PID.
///
/// Safe w.r.t. our own update child: this runs inside the install-lock wait,
/// which completes BEFORE we spawn `venv\Scripts\loki.exe update`. And a
/// desktop the user relaunches mid-update will NOT have spawned a backend —
/// `startLoki()` in the desktop gates local-backend startup on our
/// update-in-progress marker and parks until we finish (#50238). So the only
/// loki.exe images here are stragglers from the old desktop — exactly what
/// we want gone. (`/FI PID ne <self>` also spares this Tauri process, though it
/// isn't named loki.exe.)
fn force_kill_other_loki() {
    if !cfg!(target_os = "windows") {
        return;
    }
