...
const { canImportLokiCli, verifyLokiCli } = require('./backend-probes.cjs')
const {
...
const { buildDesktopBackendEnv, normalizeLokiHomeRoot } = require('./backend-env.cjs')
...
function resolveLokiHome() {
  if (process.env.LOKI_HOME) return normalizeLokiHomeRoot(process.env.LOKI_HOME)
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'loki-home')
  if (IS_WINDOWS) {
    ...
    if (fromRegistry) return normalizeLokiHomeRoot(fromRegistry)
  }
...
function lokiManagedNodePathEntries() {
  // NOTE: keep this ordering in sync with iter_loki_node_dirs() in
  // loki_constants.py — this Node main process cannot import the Python
  // module, so the platform-ordering rule is mirrored here.
  const root = path.join(LOKI_HOME, 'node')
...
function pathWithLokiManagedNode(...entries) {
  return [...lokiManagedNodePathEntries(), ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}
...
// electron-builder derives from the build `appId` (com.nousresearch.loki) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.loki')
}
...
const MEDIA_PROTOCOL = 'loki-media'
// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(loki-media://…)`.
...
let mainWindow = null
let lokiProcess = null
let connectionPromise = null
// Additional per-profile backends, keyed by profile name. The PRIMARY backend
// (the desktop's launch profile) stays managed by lokiProcess +
// connectionPromise + startLoki(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.LOKI_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.LOKI_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
async function waitForUpdateToFinish() {
  let marker = readLiveUpdateMarker(LOKI_HOME)
  ...
  while (marker && Date.now() < deadline) {
    ...
    marker = readLiveUpdateMarker(LOKI_HOME)
  }
  if (marker) {
...
function unwrapWindowsVenvLokiCommand(command, backendArgs) {
  ...
  if (!/^loki(?:\.exe)?$/i.test(path.basename(resolved))) return null
...
// findGitBash — locate bash.exe on Windows. Loki' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\loki\git\... — checked
  // first so users who installed via install.ps1 are detected before we
  // start probing system-wide locations.
  const localAppData = process.env.LOCALAPPDATA || ''
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'loki', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'loki', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// LOKI_DESKTOP_LOKI_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.LOKI_DESKTOP_LOKI_ROOT && path.resolve(process.env.LOKI_DESKTOP_LOKI_ROOT),
...
// LOKI_HOME/loki-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_loki_home). That binary owns
// ALL repo mutation — running `loki update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'loki-setup.exe' : 'loki-setup'
  const candidate = path.join(LOKI_HOME, name)
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  if (lokiProcess && Number.isInteger(lokiProcess.pid)) pids.push(lokiProcess.pid)
  ...
  if (lokiProcess && !lokiProcess.killed) {
    try {
      lokiProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  while (Date.now() < deadlineMs) {
    ...
    const stragglers = []
    if (lokiProcess && Number.isInteger(lokiProcess.pid)) stragglers.push(lokiProcess.pid)
    for (const entry of backendPool.values()) {
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri loki-setup. Unlike Windows
      ...
      // whole update itself: `loki update` (backend) + `loki desktop
    ...
    if (!updater) {
      ...
      // loki-setup.exe into LOKI_HOME). They DO have a working `loki`
      ...
      // checkout they're on — bare `loki update` defaults to main and would
      ...
      try {
        ...
        if (head.code === 0 && current && current !== 'HEAD') {
          ...
          if (branch !== 'main') command = `loki update --branch ${branch}`
        }
      } catch {
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  // not just the `loki.exe` console-script shim. That shim is generated at the
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvLoki) || fileExists(path.join(updateRoot, '.loki-bootstrap-complete'))
...
// Resolve the loki CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `loki` on PATH.
function resolveLokiCliBinary(updateRoot) {
  const venvLoki = path.join(updateRoot, 'venv', 'bin', 'loki')
  if (fileExists(venvLoki)) return venvLoki
  return findOnPath('loki') || null
}
...
// macOS/Linux in-app update: backend (`loki update`) + OS-aware GUI rebuild
// (`loki desktop --build-only`), then atomically swap the running .app bundle
// with the freshly built one and relaunch. Degrades to "backend updated,
// restart to load the new GUI" if the swap can't be performed.
async function applyUpdatesPosixInApp() {
  const updateRoot = resolveUpdateRoot()
  const loki = resolveLokiCliBinary(updateRoot)
  if (!loki) {
  ...
  // Put the Loki-managed Node and the venv on PATH so `loki desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\loki\node, not node\bin.
  const env = {
    LOKI_HOME,
    PATH: pathWithLokiManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `loki update` reaps stale `loki serve` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads LOKI_DESKTOP_CHILD_PID and excludes
  ...
  if (lokiProcess && Number.isInteger(lokiProcess.pid)) {
    desktopChildPids.push(lokiProcess.pid)
  }
  for (const entry of backendPool.values()) {
  ...
  if (desktopChildPids.length) {
    env.LOKI_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Loki (git + dependencies)…', percent: 10 })
  const updated = await runStreamedUpdate(loki, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'loki update failed.', error: updated.error || 'update-failed' })
    return { ok: false, error: 'loki update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(loki, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Loki to retry.',
      error: rebuilt.error || 'rebuild-failed'
    })
    return { ok: false, backendUpdated: true, error: 'desktop rebuild failed' }
  }

  // Linux in-app update terminal state (#45205). `loki desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Loki…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `loki-desktop-update-${Date.now()}.sh`)
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Loki to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the Loki desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Loki to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Loki.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Loki.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Loki to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.loki-update-new"; then
    rm -rf "$DST.loki-update-old" 2>/dev/null || true
    mv "$DST" "$DST.loki-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.loki-update-new" "$DST"
    rm -rf "$DST.loki-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Loki to load the new version.',
      percent: 100
    })
...
function resolveLokiBackend(backendArgs) {
  // 1. Explicit override -- LOKI_DESKTOP_LOKI_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.LOKI_DESKTOP_LOKI_ROOT && path.resolve(process.env.LOKI_DESKTOP_LOKI_ROOT)
  ...
  // 3. Bootstrap-complete ACTIVE_LOKI_ROOT -- the canonical install at
  ...
  //    to spawning loki. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `loki update` from the CLI.
  ...
  if (process.env.LOKI_DESKTOP_IGNORE_EXISTING !== '1') {
    ...
    const lokiOverride = process.env.LOKI_DESKTOP_LOKI
    ...
    if (lokiCommand) {
      ...
      if (verifyLokiCli(lokiCommand, { shell: shellForProbe })) {
        return (
          unwrapWindowsVenvLokiCommand(lokiCommand, backendArgs) || {
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Loki install was cancelled.')
  ...
  if (!fileExists(venvPython)) {
    ...
    // isLokiSourceRoot() and the bootstrap writes the marker only after
...
async function waitForLoki(baseUrl, token) {
  ...
  throw new Error(`Loki backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
const ZOOM_STORAGE_KEY = 'loki:desktop:zoomLevel'
...
const OAUTH_SESSION_PARTITION = 'persist:loki-remote-oauth'
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (loki #37247), and the
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.LOKI_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.LOKI_DESKTOP_REMOTE_TOKEN
  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
...
async function teardownPrimaryBackendAndWait() {
  // Capture the reference before resetLokiConnection() nulls lokiProcess.
...
// profile to startLoki() (the window backend: boot UI, bootstrap, remote
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForLoki(remote.baseUrl, remote.token)
    return {
      ...remote,
      profile,
  ...
  child.once('exit', (code, signal) => {
    ...
    if (!ready) {
      rejectStart?.(
        new Error(`Loki backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
...
async function startLoki() {
  ...
  connectionPromise = (async () => {
    ...
    lokiProcess.once('exit', (code, signal) => {
      ...
      if (!backendReady) {
        const message = `Loki backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
    ...
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(lokiProcess, { readyFile }),
      backendStartFailed
    ])
    if (readyFile) {
      fs.unlink(readyFile, () => {})
    ...
    backendStartFailure = null
    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      // The exit/error handlers null lokiProcess when the child dies.
      childAlive: () => lokiProcess !== null && lokiProcess.exitCode === null && !lokiProcess.killed,
      rememberLog
    })
    updateBootProgress({
      phase: 'backend.ready',
...
ipcMain.handle('loki:connection', async (_event, profile) => ensureBackend(profile))
...
ipcMain.handle('loki:connection:revalidate', async () => {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetLokiConnection only
    // nulls connectionPromise for a remote (no child to SIGTERM).
    rememberLog('Cached remote Loki backend failed liveness probe; dropping stale connection.')
    resetLokiConnection()
...
ipcMain.handle('loki:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
...
ipcMain.handle('loki:window:openNewSession', async () => {
...
ipcMain.handle('loki:bootstrap:repair', async () => {
  ...
  // startLoki() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('loki:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('loki:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('loki:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
  return true
...
ipcMain.handle('loki:selectPaths', async (_event, options = {}) => {
...
function terminalShellCommand() {
  ...
  const override = (process.env.LOKI_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
ipcMain.handle('loki:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart Loki.')
  }
...
function _extractDeepLink(argv) {
  ...
  return argv.find(a => typeof a === 'string' && a.startsWith(`${LOKI_PROTOCOL}://`)) || null
}
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(LOKI_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
