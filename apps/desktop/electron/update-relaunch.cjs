 ...
 * Background
 * ----------
 * After `loki update` + `loki desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything LOKI_DESKTOP_* is preserved
// plus LOKI_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['LOKI_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['LOKI_DESKTOP_']
