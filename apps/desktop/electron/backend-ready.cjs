...
const _READY_RE = /^LOKI_DASHBOARD_READY port=(\d+)/m
...
/**
 * Watch a child process's stdout for the `LOKI_DASHBOARD_READY port=<N>`
...
function waitForDashboardPort(child, timeoutMs = resolvePortAnnounceTimeoutMs()) {
  return new Promise((resolve, reject) => {
    ...
    function onExit(code, signal) {
      cleanup()
      reject(new Error(`Loki backend: exited before port announcement (${signal || code})`))
    }
