...
function encryptDesktopSecret(value, safeStorageApi) {
  ...
  if (!encryptionAvailable) {
    throw new Error(
      'Secure token storage is unavailable, so Loki Desktop cannot save remote gateway tokens. ' +
        'Set LOKI_DESKTOP_REMOTE_URL and LOKI_DESKTOP_REMOTE_TOKEN in your environment, or enable OS keychain access and try again.'
    )
  }
