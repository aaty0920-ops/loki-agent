...
test('desktop background child processes opt into hidden Windows consoles', () => {
  ...
  requireHiddenChildOptions(source, /lokiProcess = spawn\(\s*backend\.command,\s*backend\.args/)
  requireHiddenChildOptions(source, /spawn\(\s*py,\s*\['-m', 'loki_cli\.main', 'uninstall', '--gui-summary'\]/)

  assert.match(source, /function unwrapWindowsVenvLokiCommand\(command, backendArgs\)/)
  assert.match(source, /function getVenvSitePackagesEntries\(venvRoot\)/)
  assert.match(source, /path\.join\(venvRoot, 'Lib', 'site-packages'\)/)
  assert.match(source, /args: \['-m', 'loki_cli\.main', \.\.\.backendArgs\]/)
})
...
test('desktop backend launches console python so child consoles are inherited, not pythonw', () => {
  ...
  // LOKI_DASHBOARD_READY stdout line — no ready-file side channel is set.
...
test('desktop backend teardown tree-kills Windows backend descendants', () => {
  ...
  const resetIndex = source.indexOf('function resetLokiConnection()')
  assert.notEqual(resetIndex, -1, 'missing resetLokiConnection')
  const resetSnippet = source.slice(resetIndex, resetIndex + 300)
  assert.match(resetSnippet, /stopBackendChild\(lokiProcess\)/)
  assert.doesNotMatch(resetSnippet, /lokiProcess\.kill\('SIGTERM'\)/)
  ...
  assert.match(quitSnippet, /stopBackendChild\(lokiProcess\)/)
  assert.doesNotMatch(quitSnippet, /lokiProcess\.kill\('SIGTERM'\)/)
})
