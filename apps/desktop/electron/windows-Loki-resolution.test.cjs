...
// Regression guards for Windows `loki` resolution in main.cjs.
//
...
//      Git-Bash `loki` shim shadowed the real loki.cmd/loki.exe; the
...
//      --repair by checking ONLY venv\Scripts\loki.exe (the console-script
...
test('findOnPath tries PATHEXT extensions before the bare (empty) name on Windows', () => {
  ...
  assert.doesNotMatch(
    source,
    /\['', \.\.\.\(process\.env\.PATHEXT/,
    'empty-extension-first order regressed: an extensionless shim can shadow loki.cmd/.exe'
  )
})
...
test('Windows bootstrap recovery chooses --update when any real-install signal is present', () => {
  ...
  // The old too-narrow check (only venv\Scripts\loki.exe) must not return.
  assert.doesNotMatch(
    source,
    /updaterArgs = fileExists\(venvLoki\) \?/,
    'recovery regressed to gating only on the loki.exe shim, which forces destructive --repair'
  )
})
