...
function downloadInstallScript(commit, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/NousResearch/loki-agent/${commit}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(lokiHome, installStamp.commit)
  try {
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, lokiHome } = {}) {
  return new Promise((resolve, reject) => {
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, lokiHome } = {}) {
  return new Promise((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, lokiHome }) {
...
async function fetchManifest({ scriptPath, installerKind, emit, lokiHome, activeRoot, installStamp }) {
  const isPosix = installerKind === 'posix'
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, lokiHome })]
...
async function runStage({ scriptPath, installerKind, stage, emit, lokiHome, activeRoot, abortSignal, installStamp }) {
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, lokiHome })
      ]
