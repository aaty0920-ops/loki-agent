...
export async function openSessionInNewWindow(sessionId: string, opts?: { watch?: boolean }): Promise<void> {
  ...
  await openWindow(() => window.lokiDesktop.openSessionWindow(sessionId, opts), 'Could not open chat in a new window')
}

// Open a fresh compact window on the new-session draft.
export async function openNewSessionInNewWindow(): Promise<void> {
  if (!canOpenSessionWindow() || typeof window.lokiDesktop.openNewSessionWindow !== 'function') {
    return
  }

  await openWindow(() => window.lokiDesktop.openNewSessionWindow(), 'Could not open new session window')
}
