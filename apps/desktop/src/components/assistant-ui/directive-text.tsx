...
const LOKI_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
...
const ICON_PATHS: Record<LokiRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as LokiRefType] ?? ICON_FALLBACK
...
const LOKI_DIRECTIVE_RE = new RegExp(
  ...
  const matches = [
    ...
    ...Array.from(text.matchAll(LOKI_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as LokiRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: LokiRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
