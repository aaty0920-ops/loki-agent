...
import { getSessionMessages, type SessionMessage, triggerCronJob } from '../loki'
...
export function DesktopController() {
  ...
  useEffect(() => {
    window.lokiDesktop?.setPreviewShortcutActive?.(Boolean(chatOpen && (filePreviewTarget || previewTarget)))
  }, [chatOpen, filePreviewTarget, previewTarget])

  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.lokiDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  // loki:// deep links (e.g. a docs "Send to App" button for an automation blueprint).
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.lokiDesktop?.onClosePreviewRequested?.(closeActiveRightRailTab)
  ...
  const { refreshLokiConfig, sttEnabled, voiceMaxRecordingSeconds } = useLokiConfig({
    activeSessionIdRef,
