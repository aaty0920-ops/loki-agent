...
export function GatewaySettings() {
  ...
  const save = async (apply: boolean) => {
    ...
    try {
      const next = apply
        ? await window.lokiDesktop.applyConnectionConfig(payload())
        : await window.lokiDesktop.saveConnectionConfig(payload())
  ...
  const signIn = async () => {
    ...
    try {
      ...
      if (result.connected) {
        const refreshed = await window.lokiDesktop.getConnectionConfig(scope)
        setState(refreshed)
  ...
  const signOut = async () => {
    ...
    try {
      await window.lokiDesktop.oauthLogoutConnectionConfig(trimmedUrl || undefined)
      const refreshed = await window.lokiDesktop.getConnectionConfig(scope)
      setState(refreshed)
