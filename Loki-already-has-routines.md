# Loki Agent Has Had "Routines" Since March
...
Loki equivalent — works today:
```bash
loki webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Loki implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Loki Agent |
|---|---|---|
...
## Things Loki Does That Routines Can't
...
Loki has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Loki Agent is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
loki gateway setup    # enable webhooks
loki webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation blueprints gallery: [loki-agent.nousresearch.com/docs/reference/automation-blueprints-catalog](https://loki-agent.nousresearch.com/docs/reference/automation-blueprints-catalog)

Documentation: [loki-agent.nousresearch.com](https://loki-agent.nousresearch.com)

GitHub: [github.com/NousResearch/loki-agent](https://github.com/NousResearch/loki-agent)

---

*Loki Agent is built by [Nous Research](https://nousresearch.com). Open source, model-agnostic, runs on your infrastructure.*
