...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to LOKI_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/loki), this function
    mirrors the bridge source to a writable LOKI_HOME location and returns that
    ...
    # Try LOKI_HOME location first
    loki_home = get_loki_home()
    loki_home_bridge = loki_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to LOKI_HOME if needed
    if loki_home_bridge.exists():
        return loki_home_bridge

    # Mirror the bridge source to LOKI_HOME
    try:
