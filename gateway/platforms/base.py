...
# Audio file extensions Loki recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Loki' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Loki-created Telegram private-chat
...
def _resolve_cache_dir(constant_name: str, new_subpath: str, old_name: str) -> Path:
    """Resolve fresh via get_loki_dir (active profile), unless a test has
    monkeypatched the constant away from its import-time default."""
    fresh = get_loki_dir(new_subpath, old_name)
        ...
        for attempt in range(retries + 1):
            try:
                async with client.stream(
                    ...
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; LokiAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                ) as response:
...
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _LOKI_HOME / "image_cache",
    _LOKI_HOME / "audio_cache",
    _LOKI_HOME / "video_cache",
    _LOKI_HOME / "document_cache",
    _LOKI_HOME / "browser_screenshots",
    ...
    _LOKI_HOME / "cache" / "images",
    _LOKI_HOME / "cache" / "audio",
    _LOKI_HOME / "cache" / "videos",
    _LOKI_HOME / "cache" / "documents",
    _LOKI_HOME / "cache" / "screenshots",
)
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared Loki root.

    Profile gateways write generated artifacts to
    ``<root>/profiles/<name>/cache/{images,audio,...}``. The static safe-roots
    list only covers the *active* LOKI_HOME's cache, so a gateway running at
    the root (e.g. ``LOKI_HOME=/opt/data``) while the model emits a
    ...
    is consulted (which otherwise wins when LOKI_HOME is symlinked under a
    denied prefix and $HOME is not that prefix). See issue #31733.
    """
    roots: List[Path] = []
    profiles_dir = _LOKI_ROOT / "profiles"
    try:
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Loki profile and shared Loki root both contain control
    ...
    # LOKI_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.loki
    ...
    for loki_root in (_LOKI_HOME, _LOKI_ROOT):
        for rel in _ROOT_CREDENTIAL_FILES:
            denied.append(loki_root / rel)
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Loki
    secrets (``~/.loki/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``LOKI_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Loki-managed cache, under an operator-allowlisted root
    (``LOKI_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # credential/secret stores under the Loki root (~/.loki/.env,
    ...
    # ``MEDIA:~/.loki/google_token.json``) remain rejected.
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``LOKI_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    # Loki commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("LOKI_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "LOKI_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "LOKI_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Loki sends
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        host filesystem path that would leak the Loki home layout.
        """
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the Loki home layout.
        """
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.loki/media/generated/stale.png"}
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          LOKI_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          LOKI_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          LOKI_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        ...
        try:
            min_ms = int(os.getenv("LOKI_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("LOKI_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _loki_run_generation is set on
