...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-Loki-Session-Id header; opt-in long-term memory scoping via X-Loki-Session-Key header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id; X-Loki-Session-Key supported)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists loki-agent and any configured model_routes aliases
- GET  /v1/capabilities            — machine-readable API capabilities for external UIs
- GET  /api/sessions               — list client-visible Loki sessions
- POST /api/sessions               — create an empty Loki session
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to loki-agent
...
def _loki_version() -> str:
    """Return the loki-agent version string, or "dev" if it can't be resolved.

    Tries the installed package metadata first (authoritative for a pip/uv
    install), then the in-tree ``loki_cli.__version__`` (covers editable /
...
class ResponseStore:
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        # gracefully on NFS/SMB/FUSE-mounted LOKI_HOME (same filesystem
        # issue addressed for state.db/kanban.db — see
        # loki_state._WAL_INCOMPAT_MARKERS).
...
def _derive_chat_session_id(
    ...
    the same Loki session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through loki-agent's AIAgent.
    """
    ...
    def _parse_session_key_header(
        ...
        """Extract and validate the ``X-Loki-Session-Key`` header.

        The session key is a stable per-channel identifier that scopes
        long-term memory (e.g. Honcho sessions) across transcripts.  It
        is independent of ``X-Loki-Session-Id``: callers may send
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``loki sessions list``
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the loki-api-server default.

        ``gateway_session_key`` is a stable per-channel identifier supplied
        by the client (via ``X-Loki-Session-Key``).  Unlike ``session_id``
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response(
            {"status": "ok", "platform": "loki-agent", "version": _loki_version()}
        )
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — list loki-agent and any configured model_routes aliases."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every Loki version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions — create an empty Loki session row."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        headers = {"X-Loki-Session-Id": effective_session_id or session_id}
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # stable per-channel identifier via X-Loki-Session-Key.  This
        # is independent of X-Loki-Session-Id: the key persists across
        ...
        # Allow caller to continue an existing session by passing X-Loki-Session-Id.
        ...
        if provided_session_id:
            ...
            # conversation map to the same Loki session.  The first user
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``loki.tool.progress`` with ``status: running``.
        ...
        response_headers = {
            "X-Loki-Session-Id": result.get("session_id", session_id),
        }
        if gateway_session_key:
        ...
        # truncation via finish_reason="length" + Loki-specific extras.
        response_data = {
            "id": completion_id,
    ...
    async def _write_sse_chat_completion(
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: loki.tool.progress`` SSE event so
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = {"X-Loki-Session-Id": session_id}
        if gateway_session_key:
            response_headers["X-Loki-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
        ...
        *route* is an optional ``model_routes`` entry (resolved from the
        ...
        def _run():
            ...
            try:
                ...
                # (e.g. X-Loki-Session-Id header) can track compression-
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = (
            {"X-Loki-Session-Key": gateway_session_key} if gateway_session_key else {}
        )
        return web.json_response(
    ...
    async def _handle_run_approval(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "loki.run.approval_response",
            "run_id": run_id,
            "choice": choice,
            "resolved": resolved,
        })
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Store the adapter after native routes are registered. Local Loki-Relay
            ...
            # that is the exact surface the loki-0day campaign abused to write
            # ~/.loki/config.yaml and plant persistence. Sandboxing (Docker /
