...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``loki setup`` display).
    ...
    # built-ins).  Used by ``loki gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `loki`
        # invocation -- including plain `loki chat`, which never touches any
        ...
        # `loki setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``loki setup``/``gateway status``, channel
