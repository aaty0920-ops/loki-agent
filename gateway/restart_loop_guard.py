...
Defenses 1 and 2 (the ``_LOKI_GATEWAY`` guard on ``loki gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.loki.gateway")``,
...
State lives in ``<LOKI_HOME>/gateway/restart_loop.json`` so it is
