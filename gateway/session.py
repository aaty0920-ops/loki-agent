...
# ``LOKI_AUTO_CONTINUE_FRESHNESS`` at startup.
...
# ``sessions_dir / f"{session_id}.json"`` in loki_state, request-dump
...
def _discord_tools_loaded() -> bool:
    ...
    Two conditions must hold:
      1. The `discord` or `discord_admin` toolset is enabled for the
         Discord platform via `loki tools` (opt-in, default OFF).
...
def build_session_context_prompt(
    ...
    *,
    ...
    if context.source.platform == Platform.SLACK:
        ...
        # `discord` / `discord_admin` via `loki tools` AND the bot
    ...
    lines.append(
        f"- `\"local\"` → Save to local files only ({display_loki_home()}/cron/output/)"
    )
    ...
    def _save(self) -> None:
        ...
        data = {
            "_README": (
                ...
                "~/.loki/state.db and are shown by `loki sessions list` and "
    ...
    def get_or_create_session(
        ...
        with self._lock:
            ...
            if session_key in self._entries and not force_new:
                ...
                # (loki_state.find_latest_gateway_session_for_peer) selects
