...
class GatewayKanbanWatchersMixin:
    ...
    async def _kanban_notifier_watcher(self, interval: float = 5.0) -> None:
        ...
        **Multi-board:** iterates every board discovered on disk per
        ...
        if env_override in {"0", "false", "no", "off"}:
            logger.info("kanban notifier: disabled via LOKI_KANBAN_DISPATCH_IN_GATEWAY env")
            return
        try:
            cfg = _load_config()
        ...
        while self._running:
            try:
                def _collect():
                    ...
                    # LOKI_KANBAN_DB pins the board path; without this guard
    ...
    async def _kanban_dispatcher_watcher(self) -> None:
        ...
        no separate `loki kanban daemon` process needed. When false, the
        ...
        # watcher here. Honours LOKI_KANBAN_DISPATCH_IN_GATEWAY env var
        ...
        def _ready_nonempty() -> bool:
            ...
            task on ANY board whose assignee maps to a real Loki profile
            ...
            PATH, missing venv, credential loss for a real Loki profile).
            """
            try:
                boards = _kb.list_boards(include_archived=False)
            except Exception:
