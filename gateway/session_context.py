...
(``LOKI_SESSION_PLATFORM``, ``LOKI_SESSION_CHAT_ID``, etc.) with
...
messages arrive at the same time the old code did:
    ...
    os.environ["LOKI_SESSION_THREAD_ID"] = str(context.source.thread_id)
...
``os.getenv("LOKI_SESSION_*", ...)`` calls.  Existing tool code only
needs to replace the import + call site:

    # before
    import os
    platform = os.getenv("LOKI_SESSION_PLATFORM", "")
...
_SESSION_PLATFORM: ContextVar = ContextVar("LOKI_SESSION_PLATFORM", default=_UNSET)
_SESSION_SOURCE: ContextVar = ContextVar("LOKI_SESSION_SOURCE", default=_UNSET)
_SESSION_CHAT_ID: ContextVar = ContextVar("LOKI_SESSION_CHAT_ID", default=_UNSET)
_SESSION_CHAT_NAME: ContextVar = ContextVar("LOKI_SESSION_CHAT_NAME", default=_UNSET)
_SESSION_THREAD_ID: ContextVar = ContextVar("LOKI_SESSION_THREAD_ID", default=_UNSET)
_SESSION_USER_ID: ContextVar = ContextVar("LOKI_SESSION_USER_ID", default=_UNSET)
_SESSION_USER_NAME: ContextVar = ContextVar("LOKI_SESSION_USER_NAME", default=_UNSET)
_SESSION_KEY: ContextVar = ContextVar("LOKI_SESSION_KEY", default=_UNSET)
_SESSION_ID: ContextVar = ContextVar("LOKI_SESSION_ID", default=_UNSET)
# ID of the message that triggered the current turn. Used as a reply anchor
# so background-process notifications stay inside the originating Telegram
# private-chat topic (those lanes route only with thread id + reply anchor).
_SESSION_MESSAGE_ID: ContextVar = ContextVar("LOKI_SESSION_MESSAGE_ID", default=_UNSET)

_SESSION_PROFILE: ContextVar = ContextVar("LOKI_SESSION_PROFILE", default=_UNSET)
...
_SESSION_ASYNC_DELIVERY: ContextVar = ContextVar("LOKI_SESSION_ASYNC_DELIVERY", default=_UNSET)

# Cron auto-delivery vars — set per-job in run_job() so concurrent jobs
# don't clobber each other's delivery targets.
_CRON_AUTO_DELIVER_PLATFORM: ContextVar = ContextVar("LOKI_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
_CRON_AUTO_DELIVER_CHAT_ID: ContextVar = ContextVar("LOKI_CRON_AUTO_DELIVER_CHAT_ID", default=_UNSET)
_CRON_AUTO_DELIVER_THREAD_ID: ContextVar = ContextVar("LOKI_CRON_AUTO_DELIVER_THREAD_ID", default=_UNSET)

_VAR_MAP = {
    "LOKI_SESSION_PLATFORM": _SESSION_PLATFORM,
    "LOKI_SESSION_SOURCE": _SESSION_SOURCE,
    "LOKI_SESSION_CHAT_ID": _SESSION_CHAT_ID,
    "LOKI_SESSION_CHAT_NAME": _SESSION_CHAT_NAME,
    "LOKI_SESSION_THREAD_ID": _SESSION_THREAD_ID,
    "LOKI_SESSION_USER_ID": _SESSION_USER_ID,
    "LOKI_SESSION_USER_NAME": _SESSION_USER_NAME,
    "LOKI_SESSION_KEY": _SESSION_KEY,
    "LOKI_SESSION_ID": _SESSION_ID,
    "LOKI_SESSION_MESSAGE_ID": _SESSION_MESSAGE_ID,
    "LOKI_SESSION_PROFILE": _SESSION_PROFILE,
    "LOKI_CRON_AUTO_DELIVER_PLATFORM": _CRON_AUTO_DELIVER_PLATFORM,
    "LOKI_CRON_AUTO_DELIVER_CHAT_ID": _CRON_AUTO_DELIVER_CHAT_ID,
    "LOKI_CRON_AUTO_DELIVER_THREAD_ID": _CRON_AUTO_DELIVER_THREAD_ID,
}


def set_current_session_id(session_id: str) -> None:
    """Synchronize ``LOKI_SESSION_ID`` across ContextVar and ``os.environ``.
    ...
    ``get_session_env("LOKI_SESSION_ID")`` with an ``os.environ`` fallback,
...
def reset_session_vars() -> None:
    ...
    *A's* ``LOKI_SESSION_*`` identity via the subprocess-env bridge.  The
    ...
    ``LOKI_SESSION_*`` env var read via :func:`get_session_env`), so it is
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session context variable by its legacy ``LOKI_SESSION_*`` name.

    Drop-in replacement for ``os.getenv("LOKI_SESSION_*", default)``.
