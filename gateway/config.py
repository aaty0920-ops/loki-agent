...
class GatewayConfig:
    ...
    # gateway behaves exactly as before — single LOKI_HOME, no profile stamping.
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.loki/config.yaml (primary user-facing config)
    3. ~/.loki/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_loki_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through loki_cli.config.load_config, so without this a managed
            ...
            if not isinstance(streaming_cfg, dict):
                ...
                # ``loki config set gateway.streaming.*``
