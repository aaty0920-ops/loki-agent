...
The PID file lives at ``{LOKI_HOME}/gateway.pid``.  LOKI_HOME defaults to
``~/.loki`` but can be overridden via the environment variable.  This means
separate LOKI_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting LOKI_HOME."""
    home = get_loki_home()
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Loki gateway lifecycle subcommand from a command line.
    ...
    test also matched ``loki_cli.main gateway status`` and even unrelated
    ...
    (``"C:\\Program Files\\...\\loki-gateway.exe"``) survive, and strips
    ``--profile``/``-p`` selectors from anywhere in argv -- Loki's
    ...
    for token in tokens:
        ...
        if basename in ("loki-gateway", "loki-gateway.exe"):
            return "run"

    joined = " ".join(tokens)
    has_gateway_entry = (
        "loki_cli.main" in joined
        or "loki_cli/main.py" in joined
        or any(t.rsplit("/", 1)[-1] in ("loki", "loki.exe") for t in tokens)
    )
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `loki gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating Loki-owned runtime records
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Loki gateway."""
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a LOKI_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.loki`` or ``$LOKI_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    Mirrors ``loki_cli.gateway._matches_current_profile`` so the dashboard's
    ...
    explicit ``LOKI_HOME=<path>``) on its argv; the default/root gateway runs
    ...
    if profile_name is not None and profile_name != "default":
        ...
        return (
            ...
            or f"loki_home={home_lc}" in command_lc
        )
    ...
    # a non-matching explicit LOKI_HOME= on the argv. LOKI_HOME is usually
    ...
    if "--profile " in command_lc or " -p " in command_lc:
        return False
    if "loki_home=" in command_lc and f"loki_home={home_lc}" not in command_lc:
        return False
    return True
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `loki status` show per-profile
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``LOKI_HOME`` in-process.  Defaults to
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    LOKI_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different LOKI_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``loki.service`` + ``loki-
# gateway.service``).
#
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # running under a different LOKI_HOME. When two profile gateway
    # services share the same default ~/.loki (LOKI_HOME not set
    ...
    # field is absent in markers written by older Loki versions; treat
    ...
    if replacer_home is not None and replacer_home != str(get_loki_home()):
        return False
    ...
    # ``loki gateway stop`` on Windows would be misclassified as an
...
def write_takeover_marker(target_pid: int) -> bool:
    ...
    try:
        ...
        record = {
            ...
            "replacer_loki_home": str(get_loki_home()),
