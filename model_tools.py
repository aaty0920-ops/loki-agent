...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        ...
        # async tools resolve get_loki_home() under the active profile.
...
#   - cli.py, loki_cli/*      -> inline on startup (no event loop)
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if os.environ.get("LOKI_KANBAN_TASK") and "kanban" not in effective_enabled_toolsets:
            # Dispatcher-spawned workers are scoped by LOKI_KANBAN_TASK and
    ...
    # This ensures that even if a composite toolset (like loki-cli)
    # is enabled, any tools belonging to a disabled toolset are strictly
    # stripped out. See issue #17309.
    if disabled_toolsets:
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                if toolset_name.startswith("loki-"):
                    # Platform bundles (loki-*) include _LOKI_CORE_TOOLS, so
    ...
    # window). Core Loki tools (toolsets._LOKI_CORE_TOOLS) are NEVER
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from loki_cli.config import load_config as _load
        cfg = _load() or {}
...
def handle_function_call(
    ...
    try:
        ...
        if not skip_pre_tool_call_hook:
            ...
            try:
                from loki_cli.plugins import get_pre_tool_call_block_message
