"""Windows UTF-8 bootstrap for Loki entry points.
...
should be imported at the very top of every Loki entry point
(``loki``, ``loki-agent``, ``loki-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    ...
    Loki ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Loki source root
...
def activate_durable_lazy_target() -> None:
    ...
    (``LOKI_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
...
# Apply on import — entry points just need ``import loki_bootstrap``
# (or ``from loki_bootstrap import apply_windows_utf8_bootstrap``) at
