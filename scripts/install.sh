...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/loki-agent.git"
REPO_URL_HTTPS="https://github.com/NousResearch/loki-agent.git"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   Installs optional deps + runs loki setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://loki-agent.nousresearch.com/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
# string clears that floor; anything below it is replaced with the Loki-
...
check_node() {
    ...
    if [ -x "$LOKI_HOME/node/bin/node" ] && node_satisfies_build "$("$LOKI_HOME/node/bin/node" --version)"; then
    ...
    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing Loki-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    local installed_ver
    installed_ver=$("$LOKI_HOME/node/bin/node" --version 2>/dev/null)
...
setup_path() {
    ...
    if [ ! -x "$LOKI_BIN" ]; then
        log_warn "loki entry point not found at $LOKI_BIN"
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        run_with_timeout "$NODE_DEPS_TIMEOUT" npm install --silent || {
            log_warn "TUI npm install failed or timed out (loki --tui may not work)"
        }
...
print_success() {
    ...
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'loki' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'loki' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'loki' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    ...
    local ab_bin="$LOKI_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Loki for pip install"
    ...
    LOKI_CMD="$(command -v loki 2>/dev/null || echo "")"
    if [ -n "$LOKI_CMD" ]; then
        log_info "Running loki setup..."
        "$LOKI_CMD" setup
    else
        log_warn "loki command not found on PATH"
        log_info "Try: python -m loki_cli.main setup"
    fi
}
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the Loki-managed Node to PATH, so
    ...
    # trips Gatekeeper's tamper check ("Loki is damaged and can't be opened").
