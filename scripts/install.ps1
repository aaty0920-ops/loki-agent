...
$RepoUrlSsh = "git@github.com:NousResearch/loki-agent.git"
$RepoUrlHttps = "https://github.com/NousResearch/loki-agent.git"
$PythonVersion = "3.11"
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Loki-Setup.exe
...
function Test-Node {
    ...
    try {
        ...
        if ($zipName) {
            ...
            if ($extractedDir) {
                if (Test-Path "$LokiHome\node") { Remove-Item -Recurse -Force "$LokiHome\node" }
                Move-Item $extractedDir.FullName "$LokiHome\node"
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/NousResearch/loki-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/NousResearch/loki-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/NousResearch/loki-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
...
function Set-PathVariable {
    Write-Info "Setting up loki command..."
    
    if ($NoVenv) {
        $lokiBin = "$InstallDir"
    } else {
        $lokiBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so loki is globally available
    # On Windows, the loki.exe in venv\Scripts\ has the venv Python baked in
    ...
    Write-Success "loki command ready"
}
...
function Install-Desktop {
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Loki-managed
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://loki-agent.nousresearch.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
