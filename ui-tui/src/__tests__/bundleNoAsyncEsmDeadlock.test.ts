 ...
 *
 * Root cause: re-exporting `ink-text-input` from `@loki/ink`'s
 * entry-exports drags the upstream `ink` package into the bundle. That
 * `ink` graph and our in-tree `@loki/ink` graph reference each other
...
function bundleIsFresh(): boolean {
  ...
  try {
    ...
    const sourceMtime = statSync(
      resolve(uiTuiRoot, 'packages/loki-ink/src/entry-exports.ts'),
    ).mtimeMs
...
describe('TUI bundle (issue #31227)', () => {
  ...
  it('does not bundle the upstream ink package or ink-text-input', () => {
    ...
    // that #31227 was about. The in-tree fork at @loki/ink replaces
    // all of `ink`; nothing in ui-tui imports `TextInput` from
    // `@loki/ink` so the re-export is unused dead weight.
  ...
  it('has the @loki/ink entry-exports module compiled to sync init', () => {
