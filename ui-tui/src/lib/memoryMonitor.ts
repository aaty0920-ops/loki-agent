...
// Deferred @loki/ink import: loading `@loki/ink` at module top-level
...
async function _ensureEvictInkCaches(): Promise<(level: 'all' | 'half') => unknown> {
  ...
  _evictInkCachesPromise ??= import('@loki/ink')
    .then(mod => {
...
export function startMemoryMonitor({
  ...
  const tick = async () => {
    ...
    // Deferred import keeps `@loki/ink` off the cold-start critical path;
