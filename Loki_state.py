#!/usr/bin/env python3
"""
SQLite State Store for Loki Agent.
...
DEFAULT_DB_PATH = get_loki_home() / "state.db"
...
# loki_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``loki_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see loki_cli/kanban_db.py) would
...
class SessionDB:
    ...
    # With multiple loki processes (gateway + CLI sessions + worktree agents)
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            # ``loki_state._set_last_init_error(None)`` explicitly.
    ...
    def search_sessions_by_id(
        ...
        a session id from logs, CLI output, or another Loki surface and jump
    ...
    def delete_session_if_empty(
        ...
        and ``loki sessions list`` output. (Pattern ported from
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Loki, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Loki session.

        A Loki session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Loki session is already bound to any Telegram DM topic.
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``LOKI_DISABLE_FTS_TRIGRAM`` or not yet created), so
