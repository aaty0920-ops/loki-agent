...
_UNSET = object()
_LOKI_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_LOKI_HOME_OVERRIDE", default=_UNSET
)
...
def get_loki_home() -> Path:
    """Return the Loki home directory (default: platform-native path).

    Reads LOKI_HOME env var, falls back to the platform-native default.
    This is the single source of truth — all other copies should import this.

    When ``LOKI_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``LOKI_HOME`` explicitly (see the systemd
    template in ``loki_cli/gateway.py`` and the kanban dispatcher in
    ``loki_cli/kanban_db.py``).  See https://github.com/NousResearch/loki-agent/issues/18594.
    """
    ...
    return _get_platform_default_loki_home()


def get_default_loki_root() -> Path:
    """Return the root Loki directory for profile-level operations.

    In standard deployments this is the platform-native Loki home
    (``~/.loki`` on POSIX, ``%LOCALAPPDATA%\\loki`` on native Windows).

    In Docker or custom deployments where ``LOKI_HOME`` points outside
    ``~/.loki`` (e.g. ``/opt/data``), returns ``LOKI_HOME`` directly
    — that IS the root.

    In profile mode where ``LOKI_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.loki/profiles/coder``) and Docker
    ...
    try:
        ...
        # LOKI_HOME is under ~/.loki (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — LOKI_HOME itself is the root
    return env_path
...
def _get_packaged_data_dir(name: str) -> Path | None:
    ...
    Used to discover bundled skills/optional-skills when Loki is installed
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``LOKI_OPTIONAL_SKILLS``.
    """
    override = os.getenv("LOKI_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_loki_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``LOKI_OPTIONAL_MCPS``.
    """
    override = os.getenv("LOKI_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_loki_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``LOKI_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Wheel-installed ``<sysconfig data>/skills`` (pip install path)
        3. Caller-supplied ``default`` (typically the source-checkout path)
        4. ``<LOKI_HOME>/skills`` last-resort
    """
    override = os.getenv("LOKI_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
...
def get_loki_dir(new_subpath: str, old_name: str) -> Path:
    """Resolve a Loki subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to LOKI_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to LOKI_HOME (e.g. ``"image_cache"``).
...
def iter_loki_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Loki-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\loki\\node``. POSIX installs use
    ``$LOKI_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_loki_home()
    dirs = [root / "node"]
    bin_dir = root / "node" / "bin"
    # NOTE: keep this ordering in sync with lokiManagedNodePathEntries() in
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Loki-managed Node trees live under ``$LOKI_HOME/node`` (or a profile's
    ``LOKI_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_loki_node_executable``
    used to trust file presence alone, so ``loki update`` would pick that
...
def _heal_managed_node_windows() -> bool:
    ...
    match = re.search(
        rf"node-v{_LOKI_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def heal_loki_managed_node() -> bool:
    """Redownload Loki-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not loki_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "LOKI_HOME": str(get_loki_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def find_loki_node_executable(command: str) -> str | None:
    """Return a Loki-managed Node/npm executable path, healing broken trees."""
    names = _candidate_node_command_names(command)
    broken_present = False
    for directory in iter_loki_node_dirs():
        for name in names:
            candidate = directory / name
            if candidate.is_file() and (
                sys.platform == "win32" or os.access(candidate, os.X_OK)
                ...
                if node_tool_runnable(resolved):
                    return resolved
                broken_present = True
    if broken_present and heal_loki_managed_node():
        for directory in iter_loki_node_dirs():
            for name in names:
                candidate = directory / name
                if candidate.is_file() and (
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Loki-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Loki-managed installs.

    This is for Loki-owned subprocesses that should not be broken by a bad,
...
def with_loki_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    ...
    managed = [str(path) for path in iter_loki_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``loki update`` — leaving a **dangling symlink** that ``which`` still
...
def display_loki_home() -> str:
    """Return a user-friendly display string for the current LOKI_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.loki``
        profile:  ``~/.loki/profiles/coder``
        custom:   ``/opt/loki-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.loki``.  For code that needs a real ``Path``, use
    :func:`get_loki_home` instead.
    """
    home = get_loki_home()
    try:
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``LOKI_HOME`` or other path
    env vars resolve to an unexpected location.

    See https://github.com/NousResearch/loki-agent/issues/25821.
    """
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("LOKI_REAL_HOME") or os.getenv("LOKI_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Loki profile HOME.

    ``LOKI_HOME`` scopes Loki state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={LOKI_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{LOKI_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{LOKI_HOME}/home`` when it exists, preserving the
...
def is_container() -> bool:
    ...
    See: NousResearch/loki-agent#47111
    """
...
def get_config_path() -> Path:
    ...
    Replaces the ``get_loki_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, loki_logging.py, loki_time.py, etc.).
    """
