import { buildLokiWebSocketUrl } from "@loki/shared";
...
export async function fetchJSON<T>(
  ...
  if (res.status === 401) {
    ...
    if (!window.__LOKI_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      ...
      try {
        alreadyReloaded =
          sessionStorage.getItem("loki.tokenReloadAttempted") === "1";
      } catch {
        /* SSR / privacy mode — fall through to throw */
      }
      if (!alreadyReloaded) {
        try {
          sessionStorage.setItem("loki.tokenReloadAttempted", "1");
        } catch {
  ...
  if (res.ok) {
    ...
    try {
      sessionStorage.removeItem("loki.tokenReloadAttempted");
    } catch {
      /* SSR / privacy mode — ignore */
    }
  }
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__LOKI_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the Loki dashboard server");
}
