...
# Loki' tools.terminal.security_mode → Codex's permissions profile id.
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Loki/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Loki session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _LOKI_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("LOKI_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="loki",
            client_title="Loki Agent",
            client_version=_get_loki_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Loki' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Loki' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own loki-tools callback we
            # auto-accept — the user already approved Loki' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Loki approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Loki returns 'once', 'session', 'always', or 'deny'.
