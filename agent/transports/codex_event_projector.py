"""Projects codex app-server events into Loki' messages list.

The translator that lets Loki' memory/skill review keep working under the
...
Loki' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape Loki
...
class CodexEventProjector:
    ...
    items but Loki stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way Loki already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — Loki' messages store text only.
        text_parts: list[str] = []
