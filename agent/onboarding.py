...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Loki starts and finds ``~/.openclaw/``.

    Points users at ``loki claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``loki claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Loki, run "
        "`loki claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `loki claw cleanup` (renames it to ~/.openclaw.pre-migration — "
