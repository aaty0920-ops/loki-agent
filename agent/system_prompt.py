...
upstream prefix cache warm.  See ``loki-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Loki never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want LOKI_HOME persona while keeping
    ...
    # Pointer to the loki-agent skill + docs for user questions about Loki itself.
    stable_parts.append(LOKI_AGENT_HELP_GUIDANCE)
    ...
    # LOKI_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Loki, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Loki profile the agent is running
    # under so it doesn't conflate ~/.loki/skills/ (default profile) with
    # ~/.loki/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            ...
            "under ~/.loki/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            ...
            f"and writes ~/.loki/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.loki/skills/, ~/.loki/plugins/, "
            f"~/.loki/cron/, ~/.loki/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Loki never rebuilds or reinjects parts of it
