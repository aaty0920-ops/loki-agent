...
``auth.json``.  They intentionally have no dependency on ``loki_cli.auth`` so
...
# Sources Loki owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Loki-owned OAuth/device-code state)
