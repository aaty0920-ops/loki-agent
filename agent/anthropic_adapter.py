"""Anthropic Messages API adapter for Loki Agent.

Translates between Loki's internal OpenAI-style message format and
...
# Loki effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to loki's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to loki's outer conversation
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Loki notices an expired token, Claude Code may have already rotated it.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Loki historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``loki models``) never mutates ``~/.loki/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Loki)
      2. CLAUDE_CODE_OAUTH_TOKEN env var
      3. Claude Code credentials (~/.claude.json or ~/.claude/.credentials.json)
         — with automatic refresh if expired and a refresh token is available
      4. Anthropic credential_pool OAuth entry (~/.loki/auth.json)
    ...
    # 1. Loki-managed OAuth/setup token env var
    ...
    # 4. Loki credential_pool OAuth entry.
    resolved_pool_token = _resolve_anthropic_pool_token()
    if resolved_pool_token:
        return resolved_pool_token

    # 5. Regular API key, or a legacy OAuth token saved in ANTHROPIC_API_KEY.
    # This remains as a compatibility fallback for pre-migration Loki configs.
...
# ── Loki-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.loki/.anthropic_oauth.json (our own file).
...
_LOKI_OAUTH_FILE = get_loki_home() / ".anthropic_oauth.json"
...
def run_loki_oauth_login_pure() -> Optional[Dict[str, Any]]:
    ...
    print()
    print("Authorize Loki with your Claude Pro/Max subscription.")
    print()
...
def read_loki_oauth_credentials() -> Optional[Dict[str, Any]]:
    ...
    if _LOKI_OAUTH_FILE.exists():
        try:
            data = json.loads(_LOKI_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read Loki OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See loki-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(result):
        ...
        # replaying it verbatim.  See loki-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See loki-agent#13848 (Kimi) and
    loki-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Loki Agent", "Claude Code")
                text = text.replace("Loki agent", "Claude Code")
                text = text.replace("loki-agent", "claude-code")
        ...
        #      a) bare Loki-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Loki surfaces in its CLI. We
