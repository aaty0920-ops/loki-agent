...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `loki backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - loki_home (str): The active LOKI_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.loki``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "loki").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'loki memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], loki_home: str) -> None:
        ...
        Called by 'loki memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``loki_home`` is the active LOKI_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE LOKI_HOME.

        ``loki backup`` only walks LOKI_HOME, so any provider state kept
        ...
        of the archive, and ``loki import`` restores them to their original
