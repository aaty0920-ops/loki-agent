...
def compress_context(
    ...
    *,
    ...
    # long-lived ``loki_state.SessionDB`` class still bound to the
        ...
        try:
            ...
            if getattr(agent, "_last_compression_lock_error_sid", None) != _lock_sid:
                ...
                logger.warning(
                    ...
                    "process (or `loki update`) to resync.",
    ...
    try:
        ...
        if agent._session_db:
            try:
                ...
                if in_place:
                    ...
                    # from ``loki_logging._session_context`` (set once per turn in
                    ...
                    try:
                        agent._session_db.create_session(
                            ...
                            source=agent.platform or os.environ.get("LOKI_SESSION_SOURCE", "cli"),
                            model=agent.model,
        ...
        # engines (e.g. loki-lcm) use boundary_reason="compression" to preserve
        # DAG lineage / checkpoint per-session state across the boundary instead of
        # re-initializing fresh. See loki-lcm#68. Built-in ContextCompressor
    ...
    def _shrink_data_url(url: str) -> tuple:
        ...
        try:
            ...
            tmp = tempfile.NamedTemporaryFile(
                prefix="loki_shrink_", suffix=suffix, delete=False,
            )
            try:
                tmp.write(raw)
                tmp.close()
