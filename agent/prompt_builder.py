...
from loki_constants import get_loki_home, get_skills_dir, is_wsl
...
_LOKI_MD_NAMES = (".loki.md", "LOKI.md")


def _find_loki_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.loki.md`` or ``LOKI.md``.
    ...
    # could pick up a .loki.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _LOKI_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are Loki Agent, an intelligent AI assistant created by Nous Research. "
...
LOKI_AGENT_HELP_GUIDANCE = (
    "You run on Loki Agent (by Nous Research). When the user needs help with "
    "Loki itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://loki-agent.nousresearch.com/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `loki-agent` skill with skill_view(name='loki-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.loki/kanban.db`. Your task id is in "
    "`$LOKI_KANBAN_TASK`; your workspace is `$LOKI_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $LOKI_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $LOKI_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${LOKI_KANBAN_BRANCH:-wt/$LOKI_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$LOKI_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`loki profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `loki kanban <verb>` for board operations. Use "
...
#
# The loki-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to loki-agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "run `loki computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Loki "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Loki terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "webui": (
        "You are in the Loki WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Loki itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Loki runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Loki itself is running. The host OS, home, and cwd "
                f"of the Loki process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Loki "
    ...
    # Loki desktop GUI — any agent running under the desktop app should know
    # it. LOKI_DESKTOP marks the backend powering the chat; LOKI_DESKTOP_TERMINAL
    # marks a loki launched in the embedded terminal pane. Both set by main.cjs.
    _truthy = ("1", "true", "yes")
    _in_desktop = (os.getenv("LOKI_DESKTOP") or "").strip().lower() in _truthy
    _in_desktop_term = (os.getenv("LOKI_DESKTOP_TERMINAL") or "").strip().lower() in _truthy
    if _in_desktop or _in_desktop_term:
        _desktop_hint = "Runtime surface: you're running inside the Loki desktop GUI app."
        if _in_desktop_term:
            _desktop_hint += (
    ...
    # Embedder-supplied environment description. Lets a host that wraps Loki
    ...
    extra = (os.getenv("LOKI_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.loki/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Loki Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `loki-agent` skill "
            "first. It has the actual commands (e.g. `loki config set …`, `loki tools`, "
            "`loki setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: loki setup, loki setup tools, loki setup terminal, loki status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from LOKI_HOME and return its content, or None.
...
def _load_loki_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.loki.md / LOKI.md — walk to git root."""
    loki_md_path = _find_loki_md(cwd_path)
    if not loki_md_path:
        return ""
    try:
        content = loki_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = loki_md_path.name
        try:
            rel = str(loki_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".loki.md", context_length=context_length,
            read_path=str(loki_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", loki_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .loki.md / LOKI.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from LOKI_HOME is independent and always included when present.
    ...
    project_context = (
        _load_loki_md(cwd_path, context_length)
    ...
    # SOUL.md from LOKI_HOME only — skip when already loaded as identity
    if not skip_soul:
