...
Storage
-------
Bundles live in ``~/.loki/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``LOKI_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``loki bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under LOKI_HOME.

    Honors ``LOKI_BUNDLES_DIR`` for tests; falls back to
    ``<LOKI_HOME>/skill-bundles``.
    """
    override = os.environ.get("LOKI_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_loki_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `loki bundles` CLI subcommand.
# ---------------------------------------------------------------------------
