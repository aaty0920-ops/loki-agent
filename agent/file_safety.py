...
def _loki_home_path() -> Path:
    """Resolve the active LOKI_HOME (profile-aware) without circular imports."""
    try:
        from loki_constants import get_loki_home  # local import to avoid cycles
...
def _loki_root_path() -> Path:
    """Resolve the Loki root dir (always the parent of any profile, never per-profile)."""
    try:
        from loki_constants import get_default_loki_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    loki_home = _loki_home_path()
    loki_root = _loki_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(loki_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(loki_root / ".anthropic_oauth.json"),
...
def get_safe_write_roots() -> set[str]:
    """Return resolved LOKI_WRITE_SAFE_ROOT paths. Supports multiple directories
    separated by ``os.pathsep`` (``:`` on Unix, ``;`` on Windows).
    E.g., ``/opt/data:/var/www/html`` on Unix, ``C:\\data;D:\\www`` on Windows."""
    env = os.getenv("LOKI_WRITE_SAFE_ROOT", "")
    if not env:
        return set()
    roots: set[str] = set()
...
def is_write_denied(path: str) -> bool:
    ...
    loki_dirs = []
    for base in (_loki_home_path(), _loki_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in loki_dirs:
                loki_dirs.append(real)
        except Exception:
            continue

    for base_real in loki_dirs:
        try:
...
def get_read_block_error(path: str) -> Optional[str]:
    ...
    Three categories are blocked:
      ...
      * Internal Loki cache files under ``LOKI_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under LOKI_HOME and the global Loki
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.loki/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active LOKI_HOME (profile-aware) AND the global
    # Loki root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (LOKI_HOME points at
    ...
    for hd in loki_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Loki cache file "
    ...
    for hd in loki_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Loki credential store "
    ...
    for hd in loki_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Loki MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Loki MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Loki read (see
...
#
# Loki profiles are separate LOKI_HOME dirs under
...
#
# Reference: May 2026 incident where a loki-security profile session
# edited skills under both ``~/.loki/profiles/loki-security/skills/``
# AND ``~/.loki/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under LOKI_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from LOKI_HOME.
    ...
    """
    try:
        home_real = _loki_home_path().resolve()
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Loki entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Loki profile {info['target_profile']!r}, but the "
...
# ``…/sandboxes/<backend>/<task>/home/.loki/…`` segment and warns
# regardless of which Loki profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.loki/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.loki`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.loki/…`` and returns the
    index where the inner Loki-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .loki / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".loki":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Loki state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.loki``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.loki`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Loki resolver to
    succeed, so it works correctly even when called from contexts where
    LOKI_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Loki process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real LOKI_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.loki/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.loki/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host LOKI_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Loki state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Loki "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real LOKI_HOME. Use the "
