...
Loki keeps ``api_mode='chat_completions'`` for the ``gemini`` provider so the
...
Why this exists
---------------
Google's OpenAI-compatible endpoint has been brittle for Loki's multi-turn
...
def probe_gemini_tier(
    ...
    *,
    ...
    - ``"free"``    -- key is on the free tier (unusable with Loki)
...
_FREE_TIER_GUIDANCE = (
    ...
    "gemini-2.5-flash). Loki typically makes 3-10 API calls per user turn, "
...
def build_gemini_request(
    *,
    ...
    if max_tokens is not None:
        ...
        # finishReason=MAX_TOKENS, truncating tool calls mid-stream (Loki
...
class GeminiNativeClient:
        ...
        **_: Any,
        ...
        if not (api_key or "").strip():
            raise RuntimeError(
                ...
                "Set GOOGLE_API_KEY or GEMINI_API_KEY in your environment / ~/.loki/.env "
                "(get one at https://aistudio.google.com/app/apikey), or run `loki setup` "
    ...
    def _headers(self) -> Dict[str, str]:
        headers = {
            ...
            "User-Agent": "loki-agent (gemini-native)",
        }
