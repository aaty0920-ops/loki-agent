...
``kind: backend``) or ``~/.loki/plugins/web/<name>/`` (user, opt-in via
``plugins.enabled``).
...
class WebSearchProvider(abc.ABC):
    ...
    def is_available(self) -> bool:
        ...
        runs at tool-registration time and on every ``loki tools`` paint.
        """
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        Used by ``loki_cli/tools_config.py`` to inject this provider as a
