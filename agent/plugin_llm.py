...
Plugins built on Loki Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Loki
...
:class:`~loki_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Loki' main config
...
handles every provider, fallback chain, and per-task override Loki
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`loki_cli.plugins.PluginContext`
