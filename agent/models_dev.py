...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.loki/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Loki ↔ models.dev
# ---------------------------------------------------------------------------
...
# Reverse mapping: models.dev → Loki (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_loki_home() / "models_dev_cache.json"
...
def fetch_models_dev(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    When ``force_refresh=True`` (used by ``loki config refresh``, the
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a Loki provider ID to its models dict from models.dev.
...
# Gemma models whose TPM quotas are too small for normal Loki agent traffic.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a Loki provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve Loki ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Loki or models.dev provider ID.  Tries exact match then
