...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``LOKI_KANBAN_TASK`` / ``LOKI_KANBAN_BOARD`` in the
        ...
        # The Loki Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``loki_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``LOKI_PLATFORM`` or
            ``LOKI_SESSION_PLATFORM`` env vars.  Returns the global
...
# which becomes the dominant cost of ``loki`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.loki/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``loki`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against LOKI_HOME, not cwd
        if not p.is_absolute():
            p = (loki_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.loki/skills/`` first, then external.
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Loki can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    loki = metadata.get("loki") or {}
    if not isinstance(loki, dict):
        loki = {}
    return {
        "fallback_for_toolsets": loki.get("fallback_for_toolsets", []),
        "requires_toolsets": loki.get("requires_toolsets", []),
        "fallback_for_tools": loki.get("fallback_for_tools", []),
        "requires_tools": loki.get("requires_tools", []),
    }
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Loki metadata, VCS, virtualenv/dependency, cache, and skill
