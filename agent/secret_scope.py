...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the LOKI_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    ...
    "LOKI_MAX_ITERATIONS", "LOKI_MAX_TOKENS", "LOKI_API_TIMEOUT",
    "LOKI_REDACT_SECRETS", "LOKI_NOUS_TIMEOUT_SECONDS",
    "_LOKI_GATEWAY",
    # OS / interpreter
...
def build_profile_secret_scope(loki_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(loki_home) / ".env")
