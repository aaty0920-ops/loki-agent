...
def _write_through_provider_state_to_global_root(
    ...
    ``loki_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``loki model`` / ``loki auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Loki process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``loki`` invocation that defaults to the active provider
        (e.g. setup wizard, ``loki auth status``) would land on
        ...
        try:
            with _auth_store_lock():
                ...
                # loki_cli.auth._save_xai_oauth_tokens (#43589); the pool
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sequence below must run atomically across Loki processes: otherwise
        ...
        if self.provider == "openai-codex":
            refresh_timeout_seconds = auth_mod.env_float(
                "LOKI_CODEX_REFRESH_TIMEOUT_SECONDS", 20
            )
            lock_timeout = max(
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("loki_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # refresh_token — single-use tokens consumed by another Loki
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("loki_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/nous — another Loki process
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Loki profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `loki model` / `loki auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `loki model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``loki auth add``.  Singleton-
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `loki auth remove <provider> <N>` is stable across all source types.
    try:
        from loki_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Loki PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `loki setup` ("Claude
        ...
        # Claude Code CLI, loki_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.loki/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `loki setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"loki_pkce", "claude_code"}
            ]
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_loki_oauth_credentials
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `loki auth add nous --label <name>`).  Fall back to the
        ...
        # Loki auth store or env vars, so resolve them here.
        ...
        # MiniMax OAuth tokens live in ~/.loki/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `loki auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `loki auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Loki auth store.  Without this gate the removal is instantly
        ...
        # Loki owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``loki model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``loki auth list`` reflects the logged-in state and so the pool
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.loki/.env over os.environ — the user's config file is the
    # authoritative source for Loki credentials. Stale env vars from parent
    ...
    # Honour user suppression — `loki auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.loki/.env.
    ...
    try:
        from loki_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    if provider == "openrouter":
        # Prefer ~/.loki/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `loki auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and Loki
