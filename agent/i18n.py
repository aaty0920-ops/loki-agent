"""Lightweight internationalization (i18n) for Loki static user-facing messages.

Scope (thin slice, by design): only the highest-impact static strings shown
to the user by Loki itself -- approval prompts, a handful of gateway slash
...
Language resolution order:
    ...
    2. ``LOKI_LANGUAGE`` environment variable (for tests / quick override)
...
def _locales_dir() -> Path:
    ...
    1. ``LOKI_BUNDLED_LOCALES`` env var -- set by the Nix wrapper (or any
...
def _load_catalog(lang: str) -> dict[str, str]:
    ...
    try:
        import yaml  # PyYAML is already a loki dependency
...
def reset_language_cache() -> None:
    ...
    Call after :func:`loki_cli.config.save_config` if a running process
