...
appropriate.  All installs go to a Loki-owned bin staging dir,
``<LOKI_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``loki lsp
  status``.
...
# ``<LOKI_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # `loki lsp status` reports the host's presence.
...
def loki_lsp_bin_dir() -> Path:
    ...
    if home is None:
        home = os.path.join(os.path.expanduser("~"), ".loki")
    p = Path(home) / "lsp" / "bin"
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a loki-owned target dir.
    ...
    for script_dir in script_dirs:
        for bin_path in _native_binary_candidates(script_dir / bin_name):
            if bin_path.exists():
                link = loki_lsp_bin_dir() / bin_path.name
                if not link.exists():
                    try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``loki lsp status`` CLI to give users a quick
