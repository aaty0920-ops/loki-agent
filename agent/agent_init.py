...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .loki.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / LOKI_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.loki/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from loki_cli.config import load_config as _load_pc_cfg
    ...
    # report cumulative micros spent.  Surfaced behind LOKI_DEV_CREDITS.
    ...
    # both live under ~/.loki/logs/.  Idempotent, so gateway mode
    ...
    if agent.verbose_logging:
        ...
        # on the fact that loki_logging.setup_logging() does not
        ...
        # handler level inside loki_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        try:
            from loki_cli.config import load_config as _load_br_cfg
    ...
    # worker (kanban_show tool is present iff LOKI_KANBAN_TASK is set).
    ...
    try:
        ...
        os.environ["LOKI_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.loki/sessions/ alongside gateway sessions
    loki_home = get_loki_home()
    agent.logs_dir = loki_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.loki/sessions/session_{sid}.json)
    ...
    try:
        from loki_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from loki_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "loki"
                    except Exception:
                        pass
    ...
    # Lets an enterprise admin append to or replace Loki' built-in
    ...
    # Nous Loki 3/4 are chat models, not tool-call-tuned. The interactive
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            ...
            if _loki_warn:
                _user_msg = (
                    "⚠ Nous Research Loki 3 & 4 models are NOT agentic — they "
