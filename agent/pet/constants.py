...
row taxonomy is inferred from the atlas shape so Loki can render both legacy
...
# rows (1536x1872), while older Loki/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Loki' activity state names. They are not always identical to the
...
# Legacy Loki/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Loki activity names -> accepted row-name aliases in descending
