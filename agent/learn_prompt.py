...
that instructs the live agent to:
  ...
  1. Gather the sources the user named, using the tools it already has
     ...
     material).
  2. Author a single ``SKILL.md`` via ``skill_manage`` that follows the Loki
     skill-authoring standards (description <=60 chars, the modern section
     order, Loki-tool framing, no invented commands).
...
# (HARDLINE)" and the loki-agent-dev new-skill salvage reference. Embedded in
