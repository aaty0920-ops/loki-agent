...
This adapter lets Loki treat the GitHub Copilot ACP server as a chat-style
backend. Each request starts a short-lived ACP session, sends the formatted
conversation as a single prompt, collects text chunks, and converts the result
back into the minimal shape Loki expects from an OpenAI client.
"""
...
def _resolve_args() -> list[str]:
    raw = os.getenv("LOKI_COPILOT_ACP_ARGS", "").strip()
    if not raw:
        return ["--acp", "--stdio"]
...
def _resolve_home_dir() -> str:
    ...
    # subprocess with no HOME; callers can set LOKI_HOME explicitly if they
...
def _format_messages_as_prompt(
    ...
    if model:
        sections.append(f"Loki requested model hint: {model}")
        ...
        def _request(method: str, params: dict[str, Any], *, text_parts: list[str] | None = None, reasoning_parts: list[str] | None = None) -> Any:
            ...
            if proc.poll() is not None and stderr_text:
                if _is_gh_copilot_deprecation_message(stderr_text):
                    raise RuntimeError(
                        "Loki ACP mode requires the NEW GitHub Copilot CLI "
                        ...
                        "directly with a Copilot subscription token) via `loki setup`.\n\n"
                        f"Original error:\n{stderr_text}"
                    )
        ...
        *,
        ...
        if method == "session/request_permission":
            ...
            response = _jsonrpc_error(
                message_id,
                -32601,
                f"ACP client method '{method}' is not supported by Loki yet.",
            )
