...
MoA-enabled; the normal Loki agent loop still owns tool calling and turn
...
def _reference_messages(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    conversation flow, but flatten it into clean user/assistant *text* turns:
      ...
      - system prompt: dropped (8K of Loki boilerplate, not advisory signal).
...
def _preset_temperature(preset: dict[str, Any], key: str) -> float | None:
    ...
    single-model Loki agent (which never sends temperature unless
