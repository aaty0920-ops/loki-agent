...
Every Loki request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Loki Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=loki-agent",
        "client=loki-client-v<__version__>",
    ]

The version is sourced live from ``loki_cli.__version__`` so it auto-aligns
...
``loki_cli.__version__`` is the canonical source of truth.
"""
...
def _loki_version() -> str:
    """Return the current Loki release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``loki_cli`` cannot be imported (should
...
def loki_client_tag() -> str:
    ...
    Format: ``client=loki-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=loki-client-v{_loki_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=loki-agent", loki_client_tag()]
