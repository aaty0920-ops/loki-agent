...
as ``kind: backend``) or ``~/.loki/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``loki tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``loki tools`` picker.
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        model knows when ``image_url`` is honored. Used by ``loki tools``
...
def _images_cache_dir() -> Path:
    """Return ``$LOKI_HOME/cache/images/``, creating parents as needed."""
    from loki_constants import get_loki_home

    path = get_loki_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$LOKI_HOME/cache/images/``.
...
def save_url_image(
    ...
    *,
    ...
    """Download an image URL and write it under ``$LOKI_HOME/cache/images/``.
