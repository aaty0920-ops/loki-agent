...
- **skills** → the skill name (e.g. ``"debugging-loki-desktop"``)
...
shared by the CLI (``loki journey delete|edit``), the TUI ``/journey`` overlay
(gateway RPCs), and the desktop GUI (REST). Deleting a skill *archives* it
(recoverable via ``loki curator restore``); deleting a memory rewrites its
