"""Coding-context awareness — base Loki, every interactive surface.

When the user runs Loki inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), Loki shifts into a **coding posture**. This module is the
...
# in _LOKI_CORE_TOOLS, so they're present on every surface this fires on.
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        selection (``--toolsets``, ``LOKI_TUI_TOOLSETS``, …); they never
