...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    loki_home = get_loki_home().resolve()

    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES}
    blocked_exact.add(loki_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(loki_home / rel for rel in _SENSITIVE_LOKI_DIRS)
    ...
    # message text into reference expansion, so `@file:~/.loki/auth.json` from a
    ...
    try:
        ...
        if get_read_block_error(str(path)) is not None:
            raise ValueError(
                "path is a sensitive credential or internal Loki path and cannot be attached"
            )
    except ValueError:
        raise
    except Exception:
