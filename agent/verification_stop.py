...
def _session_is_messaging_surface() -> bool:
    ...
    ``LOKI_SESSION_PLATFORM``; the CLI and TUI set ``LOKI_SESSION_SOURCE``
    (e.g. ``cli``, ``tui``) instead. Both are consulted via the session-context
    helper (with an ``os.environ`` fallback), alongside the ``LOKI_PLATFORM``
    ...
    try:
        ...
        platform = os.getenv("LOKI_PLATFORM", "") or os.environ.get(
...
def verify_on_stop_enabled(config: dict[str, Any] | None = None) -> bool:
    ...
    Precedence: an explicit ``LOKI_VERIFY_ON_STOP`` env var wins, then an
