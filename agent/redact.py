...
# `export LOKI_REDACT_SECRETS=false`) cannot disable redaction
# mid-session.  ON by default — secure default per issue #17691. Users who
# need raw credential values in tool output (e.g. working on the redactor
# itself) can opt out via `security.redact_secrets: false` in config.yaml
# (bridged to this env var in loki_cli/main.py, gateway/run.py, and
# cli.py) or `LOKI_REDACT_SECRETS=false` in ~/.loki/.env. An opt-out
...
def mask_secret(
    ...
    *,
    ...
    Canonical helper for display-time redaction across Loki — used by
    ``loki config``, ``loki status``, ``loki dump``, and anywhere
...
def redact_sensitive_text(
    ...
    *,
    ...
    for URLs, ``"eyJ" in text`` for JWTs). On a typical loki log line
