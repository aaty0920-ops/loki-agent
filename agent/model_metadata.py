...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. Loki also honours LOKI_CA_BUNDLE (its own convention)
...
def _get_model_metadata_cache_path() -> Path:
    ...
    return get_loki_home() / "cache" / "openrouter_model_metadata.json"
...
# Minimum context length required to run Loki Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent Loki from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    # (often 8K) makes it unusable for Loki' system prompt, but mapping
...
def _maybe_cache_local_context_length(
    ...
    """Persist a locally probed context length only when it meets Loki minimum.
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Loki grow conversations past the runtime limit and Ollama
...
def get_model_context_length(
    ...
    # ``Loki-3-Llama-3.1-70B`` substring-match ``llama`` (131072) even when
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Loki sends to providers:
