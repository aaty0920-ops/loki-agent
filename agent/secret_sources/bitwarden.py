...
Loki pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.loki/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<loki_home>/bin/bws`` on
  first use.  Loki pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.loki/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``loki`` invocations don't
  hammer the API.
* Failures NEVER block Loki startup.  Missing binary, no network,
...
# In-process cache so repeated load_loki_dotenv() calls (CLI startup,
# gateway hot-reload, test suites) don't re-fetch from BSM.
_CacheKey = Tuple[str, str, str]  # (access_token_fingerprint, project_id, server_url)
_CACHE: Dict[_CacheKey, "_CachedFetch"] = {}

# Disk-persisted cache so back-to-back CLI invocations (e.g. `loki chat -q ...`
...
# <loki_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.loki/.env (which
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the disk cache path under loki_home/cache/.

    `home_path` is what `load_loki_dotenv()` already resolved; falling back
    to `$LOKI_HOME` / `~/.loki` keeps direct callers working too.
    """
    if home_path is None:
        home_path = Path(os.getenv("LOKI_HOME", Path.home() / ".loki"))
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<loki_home>/bin/bws``  (our managed copy — preferred)
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``loki secrets bitwarden setup``
...
def fetch_bitwarden_secrets(
    *,
    ...
    ``<loki_home>/cache/bws_cache.json`` (for back-to-back CLI invocations).
...
# ---------------------------------------------------------------------------
# Public entry point — called from loki_cli.env_loader
# ---------------------------------------------------------------------------
