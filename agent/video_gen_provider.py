...
as ``kind: backend``) or ``~/.loki/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``loki tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$LOKI_HOME/cache/videos/``, creating parents as needed."""
    from loki_constants import get_loki_home

    path = get_loki_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$LOKI_HOME/cache/videos/``.
