...
RUN set -eu; \
    ...
    # 'Loki WebUI' catalog). The image has moved to s6-overlay /init\
...
# lock on the @loki/ink entry, tripped the TUI launcher's
...
ENTRYPOINT [ "/init", "/opt/loki/docker/main-wrapper.sh" ]
CMD [ ]
