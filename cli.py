#!/usr/bin/env python3
"""
Loki Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the Loki Agent, inspired by Claude Code.
...
os.environ["LOKI_QUIET"] = "1"  # Our own modules
...
from loki_cli.banner import _format_context_length, format_banner_version_label
...
def _resolve_prefill_messages_file(config: Dict[str, Any]) -> str:
    ...
    """
    env_path = os.getenv("LOKI_PREFILL_MESSAGES_FILE", "").strip()
    if env_path:
        return env_path
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.loki/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If LOKI_IGNORE_USER_CONFIG=1 is set (via ``loki chat --ignore-user-config``),
    the user config at ``~/.loki/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({LOKI_HOME}/config.yaml)
    user_config_path = _loki_home / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("LOKI_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "display": {
            "compact": False,
            "resume_display": "full",
            # Recap tuning for /resume — see loki_cli/config.py DEFAULT_CONFIG.
            "resume_exchanges": 10,
        ...
        try:
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and LokiCLI.__init__ checks "default" first.
    ...
    # loki_cli.config._load_config_impl (which has its own managed merge), so
    ...
    # `loki config`/`doctor`/guards (which use load_config) honor it. The
    ...
    # Normalize config key: the new config system (loki_cli/config.py) and all
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && loki` to control it.
    ...
    # UNLESS we're inside a gateway process (detected by _LOKI_GATEWAY marker)
...
# Initialize centralized logging early — agent.log + errors.log in ~/.loki/logs/.
...
try:
    from loki_cli.skin_engine import init_skin_from_config
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``loki --help`` paths inside the chat command flow).  See
...
from loki_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
def _prepare_deferred_agent_startup() -> None:
    ...
    _accept_hooks = os.environ.get("LOKI_ACCEPT_HOOKS", "").lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _arm_exit_watchdog(timeout_s: float | None = None) -> None:
    ...
    Tune with ``LOKI_EXIT_WATCHDOG_S`` (seconds); ``0`` disables.
    """
    if timeout_s is None:
        try:
            timeout_s = float(os.getenv("LOKI_EXIT_WATCHDOG_S", "30"))
...
def _resolve_worktree_base(repo_root: str) -> tuple:
    ...
    (the ``~/.loki/loki-agent`` clone is updated only by ``loki update``,
...
def _setup_worktree(repo_root: str = None, sync_base: bool = True) -> Optional[Dict[str, str]]:
    ...
    try:
        subprocess.run(
            ["git", "worktree", "lock", "--reason", f"loki pid={os.getpid()}", str(wt_path)],
...
def _worktree_lock_is_live(repo_root: str, worktree_path: str, timeout: int = 10):
    ...
    ``loki -w`` locks each worktree with reason ``loki pid=<pid>`` so a
    concurrent loki process' startup prune leaves an in-use worktree alone.
    ...
    - ``"dead"``  — locked but the owning pid is gone, or the reason isn't a
                    parseable loki lock (safe to unlock + reap).
    ...
    for line in result.stdout.splitlines():
        if line.startswith("worktree "):
            ...
            if not m:
                # Locked by something we don't recognize as a loki session
                # (or lock reason unavailable). Treat as dead — a foreign lock
                # on a loki -w worktree is almost certainly a leftover, and
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`loki_cli.config.load_config` (the authoritative loader that
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`loki_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Lock handling (orthogonal to age): ``loki -w`` locks each worktree with
    reason ``loki pid=<pid>`` so a concurrent loki process leaves an in-use
    ...
    *dead*-locked one (owning pid gone — a crashed session) is unlocked first
    ...
    Also prunes orphaned ``loki/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        ...
        # loki process means the worktree is actively in use — never touch
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``loki/loki-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``loki -w`` sessions and PR review
...
#
# Detection priority:
#   1. LOKI_LIGHT / LOKI_TUI_LIGHT env (true/false) — explicit override
#   2. LOKI_TUI_THEME=light|dark — explicit theme
#   3. LOKI_TUI_BACKGROUND=#RRGGBB — explicit bg hint
#   4. COLORFGBG env (set by xterm/Konsole/urxvt) — bg slot 7/15 = light
#   5. OSC 11 query (\x1b]11;?\x1b\\) — ask the terminal directly
#   6. Default: assume dark (matches the legacy Loki assumption)
#
...
def _detect_light_mode() -> bool:
    ...
    try:
        ...
        if theme == "dark":
            ...
            return result
        # 3. Explicit bg hex
        bg_hint = os.environ.get("LOKI_TUI_BACKGROUND") or ""
...
def _strip_markdown_syntax(text: str) -> str:
    ...
    # but in Loki output it's common to display cron schedules verbatim.
    #
...
def _apply_bracketed_paste_timeout_patch() -> None:
    ...
    ``_loki_bp_timeout_patched`` sentinel on the module.
    """
    try:
...
class ChatConsole:
    ...
    def status(self, *_args, **_kwargs):
        ...
        ``LokiCLI._busy_command()``.
        """
        yield self

# ASCII Art - LOKI-AGENT logo (full width, single line - requires ~95 char terminal)
LOKI_AGENT_LOGO = """[bold #FFD700]██╗  ██╗███████╗██████╗ ███╗   ███╗███████╗███████╗       █████╗  ██████╗ ███████╗███╗   ██╗████████╗[/]
[bold #FFD700]██║  ██║██╔════╝██╔══██╗████╗ ████║██╔════╝██╔════╝      ██╔══██╗██╔════╝ ██╔════╝████╗  ██║╚══██╔══╝[/]
[#FFBF00]███████║█████╗  ██████╔╝██╔████╔██║█████╗  ███████╗█████╗███████║██║  ███╗█████╗  ██╔██╗ ██║   ██║[/]
[#FFBF00]██╔══██║██╔══╝  ██╔══██╗██║╚██╔╝██║██╔══╝  ╚════██║╚════╝██╔══██║██║   ██║██╔══╝  ██║╚██╗██║   ██║[/]
[#CD7F32]██║  ██║███████╗██║  ██║██║ ╚═╝ ██║███████╗███████║      ██║  ██║╚██████╔╝███████╗██║ ╚████║   ██║[/]
...
# ASCII Art - Loki Caduceus (compact, fits in left panel)
LOKI_CADUCEUS = """[#CD7F32]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣀⡀⠀⣀⣀⠀⢀⣀⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
[#CD7F32]⠀⠀⠀⠀⠀⠀⢀⣠⣴⣾⣿⣿⣇⠸⣿⣿⠇⣸⣿⣿⣷⣦⣄⡀⠀⠀⠀⠀⠀⠀[/]
[#FFBF00]⠀⢀⣠⣴⣶⠿⠋⣩⡿⣿⡿⠻⣿⡇⢠⡄⢸⣿⠟⢿⣿⢿⣍⠙⠿⣶⣦⣄⡀⠀[/]
[#FFBF00]⠀⠀⠉⠉⠁⠶⠟⠋⠀⠉⠀⢀⣈⣁⡈⢁⣈⣁⡀⠀⠉⠀⠙⠻⠶⠈⠉⠉⠀⠀[/]
[#FFD700]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣴⣿⡿⠛⢁⡈⠛⢿⣿⣦⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = "⚕ NOUS LOKI - AI Agent Framework"
        tiny_line = "⚕ NOUS LOKI"
    else:
        agent_name = _skin.get_branding("agent_name", "Loki Agent") if _skin else "Loki Agent"
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.loki/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.loki/config.yaml on first use)
...
class LokiCLI(CLIAgentSetupMixin, CLICommandsMixin):
    """
    Interactive CLI for the Loki Agent.
    ...
    def __init__(
        ...
        # Read max_tokens from config (env var override: LOKI_MAX_TOKENS)
        ...
        self.requested_provider = (
            ...
            or os.getenv("LOKI_INFERENCE_PROVIDER")
            or "auto"
        )
        ...
        elif os.getenv("LOKI_MAX_ITERATIONS"):
            try:
                self.max_turns = int(os.getenv("LOKI_MAX_ITERATIONS", ""))
            except (TypeError, ValueError):
                self.max_turns = 90
        else:
            self.max_turns = 90
        ...
        # by `loki chat --ignore-rules` in loki_cli/main.py. When true we
        ...
        self.system_prompt = (
            os.getenv("LOKI_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        # it's shared across all Loki processes for this LOKI_HOME.
    ...
    def _pet_resolve_config(self) -> None:
        ...
        switch made via ``/pet`` or ``loki pets`` without a restart, mirroring
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``loki doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``loki doctor`` so the banner stays
        small.
        """
        try:
    ...
    def show_banner(self):
        ...
        # Warn if the configured model is a Nous Loki LLM (not agentic)
        from loki_cli.model_switch import is_nous_loki_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_nous_loki_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Nous Research Loki 3 & 4 models are NOT agentic and are not "
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.loki/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from loki_cli.clipboard import save_clipboard_image

        img_dir = get_loki_home() / "images"
    ...
    def show_help(self):
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with Loki!{_RST}")
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from loki_cli.main import _relative_time
    ...
    def _discard_session_if_empty(self, session_id: Optional[str]) -> bool:
        ...
        ``/resume`` and ``loki sessions list``. Delegates the
    ...
    def new_session(self, silent=False, title=None):
        ...
        if self._session_db and old_session_id:
            ...
            # /resume and `loki sessions list` (gemini-cli#27770 port).
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.loki/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``loki --resume <id>``
        ...
        try:
            ...
            return
        path = saved_dir / f"loki_conversation_{timestamp}.json"
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `loki model` and gateway pickers).
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in loki_cli/commands.py instead of touching every dispatch site.
        ...
        if canonical in {"quit", "exit"}:
            ...
            try:
                # Discover from disk (bundled + user), matching `loki plugins
                ...
                # catalog lives behind `loki plugins list`.
    ...
    def _is_session_yolo_active(self) -> bool:
        ...
        ``LOKI_YOLO_MODE`` into ``_YOLO_MODE_FROZEN`` before tool imports
        happen.
        """
        try:
            from tools.approval import (
    ...
    def _toggle_yolo(self):
        ...
        ``LOKI_YOLO_MODE`` here — that env var is read once at module import
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by loki_logging not
    ...
    def _print_nous_credits_block(self) -> bool:
        ...
        wall-clock-bounded inside the helper; also honors LOKI_DEV_CREDITS_FIXTURE
    ...
    def _show_credits(self):
        ...
        if not view.logged_in:
            ...
            print("  Run `loki portal` to log in, then /credits.")
            return
    ...
    def _show_billing(self, command: str = "/billing"):
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                _cprint(f"  💳 {_d('Not logged into Nous Portal.')}")
                print("  Run `loki portal` to log in, then /billing.")
            return
    ...
    def _billing_confirm_and_charge(self, state, amount):
        ...
        from loki_cli.nous_billing import (
            BillingError,
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        from loki_cli.nous_billing import BillingRateLimited
    ...
    def _billing_handle_scope_required(self, state):
        ...
        if not getattr(self, "_app", None):
            print("  Run `loki portal` and approve terminal billing, then retry.")
            return
        confirm_choices = [
        ...
        if choice != "yes":
            ...
            return
        try:
            from loki_cli.auth import step_up_nous_billing_scope
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.loki/skills/ and queue a note for the
        next user turn.
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _loki_home / "interrupt_debug.log"
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            # profile names use the standard LOKI_HOME, so no -p needed.
            try:
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Loki Agent! Type your message or /help for commands.")
        ...
        try:
            from loki_cli.model_switch import prewarm_picker_cache_async
        ...
        # after an OpenClaw→Loki migration (especially migrations done by
        ...
        def handle_ignored_terminal_sequence(event):
            ...
            install_ignored_terminal_sequences() in loki_cli.pt_input_extras
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting Loki binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        # Dynamic prompt: shows Loki symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Loki needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'Loki needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("Loki needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``LOKI_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("LOKI_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the loki process when
        ...
        try:
            ...
            if hasattr(self, '_session_db') and self._session_db and self.agent:
                ...
                # drop the empty row so /resume and `loki sessions list`
...
def _run_kanban_goal_loop_q(cli: "LokiCLI", first_response: str) -> None:
    ...
    only when ``LOKI_KANBAN_GOAL_MODE`` is set (dispatcher-spawned
    ...
    task_id = (_os.environ.get("LOKI_KANBAN_TASK") or "").strip()
    if not task_id:
        return
...
def main(
    ...
    # Default to loki-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
        ...
        # Coding posture (base Loki): with no explicit --toolsets, collapse
    ...
    # mode registers its own inside LokiCLI.run(), but `-q` runs
    ...
    # so main unwinds normally.  LOKI_SIGTERM_GRACE overrides the 1.5 s
    ...
    if query or image:
        ...
        try:
            ...
            # Kanban workers spawn with ``loki chat -q "work kanban task <id>"``;
            ...
            if quiet:
                ...
                if cli._ensure_runtime_credentials():
                    ...
                    if cli._init_agent(
                        ...
                        # machine-readable (no styled "Loki" box, no tool-gen
                        ...
                        if isinstance(result, dict) and result.get("failed"):
                            _exit_code = 1
                            if os.environ.get("LOKI_KANBAN_TASK") and result.get(
                                "failure_reason"
                ...
                # Single-query mode (`loki chat -q "…"`): skip the welcome
