...
Loki is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: Loki ships a **lot** — most merges are bug fixes to
...
- **Expand reach at the edges.** New platform adapters, channels, providers,
  ...
  (`loki tools`, `loki setup`, auto-install) rather than bolting on a raw
  env var.
...
- **E2E validation, not just green unit mocks.** For anything touching
  ...
  against a temp `LOKI_HOME`. Mocks hide integration bugs.
...
- **New `LOKI_*` env vars for non-secret config.** `.env` is for secrets
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `loki tools`
...
- **Third-party products / other people's projects integrated into the core
  ...
  **standalone plugin repo** users install into `~/.loki/plugins/` (or via a
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `loki <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `loki webhook`, `loki cron`, `loki tools`.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.loki/plugins/` or a pip package, discovered at runtime.
...
`$HOME/.loki/loki-agent/venv` (for worktrees that share a venv with the
main checkout).
...
├── toolsets.py           # Toolset definitions, _LOKI_CORE_TOOLS list
├── cli.py                # LokiCLI class — interactive CLI orchestrator (~11k LOC)
├── loki_state.py       # SessionDB — SQLite session store (FTS5 search)
├── loki_constants.py   # get_loki_home(), display_loki_home() — profile-aware paths
├── loki_logging.py     # setup_logging() — agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # Agent internals (provider adapters, memory, caching, compression, etc.)
├── loki_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
│   ├── loki-achievements/  # Gamified achievement tracking
...
**User config:** `~/.loki/config.yaml` (settings), `~/.loki/.env` (API keys only).
**Logs:** `~/.loki/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_loki_home()`.
Browse with `loki logs [--follow] [--level ...] [--session ...]`.

## TypeScript Style

Applies to TypeScript across Loki: desktop, TUI, website, and future TS packages.
...
- **Skin engine** (`loki_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `LokiCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
- Skill slash commands: `agent/skill_commands.py` scans `~/.loki/skills/`, injects as **user message** (not system prompt) to preserve prompt caching
...
- **Slack** — `slack_subcommand_map()` generates `/loki` subcommand routing
- **Autocomplete** — `COMMANDS` flat dict feeds `SlashCommandCompleter`
- **CLI help** — `COMMANDS_BY_CATEGORY` dict feeds `show_help()`

### Adding a Slash Command

1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `loki_cli/commands.py`:
```python
CommandDef("mycommand", "Description of what it does", "Session",
           aliases=("mc",), args_hint="[arg]"),
```
2. Add handler in `LokiCLI.process_command()` in `cli.py`:
```python
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `loki --tui` or `LOKI_TUI=1`.
...
### TUI in the Dashboard (`loki dashboard` → `/chat`)

The dashboard embeds the real `loki --tui` — **not** a rewrite.  See `loki_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `loki_cli/web_server.py`.

- Browser loads `web/src/pages/ChatPage.tsx`, which mounts xterm.js's `Terminal` with the WebGL renderer, `@xterm/addon-fit` for container-driven resize, and `@xterm/addon-unicode11` for modern wide-character widths.
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same ephemeral `_SESSION_TOKEN` as REST, via query param (browsers can't set `Authorization` on WS upgrade).
- The server spawns whatever `loki --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL works, native Windows does not).
- Frames: raw PTY bytes each direction; resize via `\x1b[RESIZE:<cols>;<rows>]` intercepted on the server and applied with `TIOCSWINSZ`.

**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `loki --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.

**Structured React UI around the TUI is allowed when it is not a second chat surface.** Sidebar widgets, inspectors, summaries, status panels, and similar supporting views (e.g. `ChatSidebar`, `ModelPickerDialog`, `ToolCall`) are fine when they complement the embedded TUI rather than replacing the transcript / composer / terminal. Keep their state independent of the PTY child's session and surface their failures non-destructively so the terminal pane keeps working unimpaired.

### Electron Desktop Chat App (`apps/desktop/`)

A **separate** chat surface from both the classic CLI and the dashboard's embedded TUI. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). The WebSocket/JSON-RPC transport lives in the framework-agnostic `apps/shared` package (`@loki/shared` — `JsonRpcGatewayClient` + WS URL helpers), which the web dashboard (`web/`) also consumes; **desktop has no build/runtime dependency on the dashboard frontend** — it spawns a headless `loki serve` backend server (the same gateway `dashboard` serves, minus the browser UI). `dashboard` and `serve` share `cmd_dashboard`/`start_server` but are independent surfaces — neither launches the other. The one exception is a backward-compat *fallback*: `serve` is newer, so the desktop spawn (`electron/backend-command.cjs` + `backendSupportsServe()` in `main.cjs`) detects whether the resolved runtime registers `serve` and, only when it does not (an older managed install / PATH `loki` the app hasn't updated yet), rewrites the argv to the legacy `dashboard --no-open`. Without that, a new app against an un-upgraded runtime would crash on an unknown subcommand and brick every mid-upgrade user. It does NOT embed `loki --tui` — it has its own composer, transcript, and slash-command pipeline. Route desktop bugs to the `loki-desktop-app-work` skill, not `loki-dashboard-work`.
...
- **The renderer curates via `apps/desktop/src/lib/desktop-slash-commands.ts`.** This is the load-bearing file. It holds `DESKTOP_COMMANDS` (the ~19 built-ins shown in the palette) plus block-lists for terminal-only / messaging-only / picker-owned / settings-owned / advanced commands that should NOT clutter the desktop popover.
  ...
  - `isDesktopSlashExtensionCommand(name)` — true when the command is NOT a known Loki built-in (i.e. a skill or user quick command). Both suggestion and catalog-filter paths allow extensions through so skill commands surface in the palette. (Added when fixing "skill commands missing from the desktop slash palette" — the curated allow-list was silently dropping every skill/quick command from completions even though they executed fine when typed.)
...
be core tools. For custom or local-only tools, do **not** edit Loki core.
Use the plugin route instead: create `~/.loki/plugins/<name>/plugin.yaml`
and `~/.loki/plugins/<name>/__init__.py`, then register tools with
`ctx.register_tool(...)`. Plugin toolsets are discovered automatically and can be
enabled or disabled without touching `tools/` or `toolsets.py`.

Use the built-in route below only when the user is explicitly contributing a new
core Loki tool that should ship in the base system.
...
**2. Add to `toolsets.py`** — either `_LOKI_CORE_TOOLS` (all platforms) or a new toolset. **This step is required:** auto-discovery imports the tool and registers its schema, but the tool is only *exposed to an agent* if its name appears in a toolset. `_LOKI_CORE_TOOLS` is not dead code — it's the default bundle every platform's base toolset inherits from.

Auto-discovery: any `tools/*.py` file with a top-level `registry.register()` call is imported automatically — no manual import list to maintain. Wiring into a toolset is still a deliberate, manual step.

The registry handles schema collection, dispatch, availability checking, and error wrapping. All handlers MUST return a JSON string.

**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_loki_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `LOKI_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_loki_home()` for the base directory — never `Path.home() / ".loki"`. This ensures each profile gets its own state.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `loki_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `loki_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `loki tools`, `loki setup`, most CLI subcommands | `loki_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`loki_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
loki_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.loki/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
- `default` — Classic Loki gold/kawaii (the current look)
...
Add to `_BUILTIN_SKINS` dict in `loki_cli/skin_engine.py`:
...
Users create `~/.loki/skins/<name>.yaml`:
...
Loki has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.loki/plugins/` and pip-installed entry points.

### General plugins (`loki_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.loki/plugins/`, `./.loki/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `loki` at startup so
  `loki <pluginname> <subcmd>` works with no change to `main.py`
...
`post_setup(loki_home, config)` for setup-wizard integration.

**CLI commands via `plugins/memory/<name>/cli.py`:** if a memory plugin
defines `register_cli(subparser)`, `discover_plugin_cli_commands()` finds
it at argparse setup time and wires it into `loki <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `loki --help`.

**Rule (Teknium, May 2026):** plugins MUST NOT modify core files
(`run_agent.py`, `cli.py`, `gateway/run.py`, `loki_cli/main.py`, etc.).
...
into `~/.loki/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `loki memory setup` / `post_setup()` without
...
`~/.loki/plugins/` (or via pip entry points). They register through
...
2. User: `$LOKI_HOME/plugins/model-providers/<name>/`
...
[`loki-example-plugins`](https://github.com/NousResearch/loki-example-plugins)
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `loki skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.loki.tags`, `metadata.loki.category`,
`metadata.loki.related_skills`, `metadata.loki.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.loki.*` by the loader.
...
2. **Tools referenced in SKILL.md prose must be native Loki tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Loki Agent" is the secondary collaborator. If the
   contributor's commit shows "Loki Agent" as author (because they
   used Loki to draft the skill), replace it with their actual name
...
lives in the `loki-agent-dev` skill at
...
`"messaging"`); `_LOKI_CORE_TOOLS` is the default bundle most
...
Enable/disable per platform via `loki tools` (the curses UI) or the
...
Loki returns a delegation id immediately and the result re-enters the
...
go to `~/.loki/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `loki_cli/curator.py` wires `loki curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.loki/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `loki cron <verb>`
...
- File lock at `~/.loki/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `loki kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `loki_cli/kanban.py` wires `loki kanban` with verbs
...
- **Plugin assets:** `plugins/kanban/dashboard/` (web UI) +
  `plugins/kanban/systemd/` (`loki-kanban-dispatcher.service` for
  standalone dispatcher deployment).

Isolation model:
- **Board** is the hard boundary — workers are spawned with
  `LOKI_KANBAN_BOARD` pinned in their env so they can't see other
  boards.
...
Loki-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
in config.yaml (or `LOKI_BACKGROUND_NOTIFICATIONS` env var):
...
Loki supports **profiles** — multiple fully isolated instances, each with its own
`LOKI_HOME` directory (config, API keys, memory, sessions, skills, gateway, etc.).

The core mechanism: `_apply_profile_override()` in `loki_cli/main.py` sets
`LOKI_HOME` before any module imports. All `get_loki_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_loki_home()` for all LOKI_HOME paths.** Import from `loki_constants`.
   NEVER hardcode `~/.loki` or `Path.home() / ".loki"` in code that reads/writes state.
   ```python
   # GOOD
   ...
   # BAD — breaks profiles
   config_path = Path.home() / ".loki" / "config.yaml"
   ```

2. **Use `display_loki_home()` for user-facing messages.** Import from `loki_constants`.
   This returns `~/.loki` for default or `~/.loki/profiles/<name>` for profiles.
   ```python
   # GOOD
   from loki_constants import display_loki_home
   print(f"Config saved to {display_loki_home()}/config.yaml")

   # BAD — shows wrong path for profiles
   print("Config saved to ~/.loki/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_loki_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_loki_home()`,
   not `Path.home() / ".loki"`.

4. **Tests that mock `Path.home()` must also set `LOKI_HOME`** — since code now uses
   `get_loki_home()` (reads env var), not `Path.home() / ".loki"`:
   ```python
...
6. **Profile operations are HOME-anchored, not LOKI_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".loki" / "profiles"`, NOT `get_loki_home() / "profiles"`.
   This is intentional — it lets `loki -p coder profile list` see all profiles regardless
   of which one is active.

## Known Pitfalls

### DO NOT hardcode `~/.loki` paths
Use `get_loki_home()` from `loki_constants` for code paths. Use `display_loki_home()`
for user-facing print/log messages. Hardcoding `~/.loki` breaks profiles — each profile
has its own `LOKI_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### DO NOT introduce new `simple_term_menu` usage
Existing call sites in `loki_cli/main.py` remain for legacy fallback only;
the preferred UI is curses (stdlib) because `simple_term_menu` has
ghost-duplication rendering bugs in tmux/iTerm2 with arrow keys. New
interactive menus must use `loki_cli/curses_ui.py` — see
`loki_cli/tools_config.py` for the canonical pattern.
...
with actual imports (not mocks) against a temp `LOKI_HOME`.

### Tests must not write to `~/.loki/`
The `_isolate_loki_home` autouse fixture in `tests/conftest.py` redirects `LOKI_HOME` to a temp dir. Never hardcode `~/.loki/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_loki_home()` resolve within the temp dir.
Use the pattern from `tests/loki_cli/test_profiles.py`:
```python
@pytest.fixture
...
| HOME / `~/.loki/` | Your real config+auth.json                  | Temp dir per test                         |
