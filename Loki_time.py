...
Resolution order:
  1. ``LOKI_TIMEZONE`` environment variable
  2. ``timezone`` key in ``~/.loki/config.yaml``
  3. Falls back to the server's local time (``datetime.now().astimezone()``)

Invalid timezone values log a warning and fall back safely — Loki never
...
try:
    ...
    # Python 3.8 fallback (shouldn't be needed — Loki requires 3.9+)
...
def reset_cache() -> None:
    ...
    config edit or ``LOKI_TIMEZONE`` update) to force ``get_timezone()`` /
