...
- **Search both open *and* merged PRs and issues** for your topic or error symptom — the duplicate-check in the PR template fires at review time, after you've already done the work:
  ```bash
  gh search issues --repo NousResearch/loki-agent "<your terms>"
  gh search prs --repo NousResearch/loki-agent --state all "<your terms>"
  ```
  Or use the web UI: [issues](https://github.com/NousResearch/loki-agent/issues?q=) · [PRs (all states)](https://github.com/NousResearch/loki-agent/pulls?q=is%3Apr).
...
If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `loki skills install`.
...
- Implement the relevant ABC and use the existing plugin discovery path (`~/.loki/plugins/`, project `.loki/plugins/`, or a pip entry point) — see [Build a Loki Plugin](https://loki-agent.nousresearch.com/docs/guides/build-a-loki-plugin)
...
├── cli.py                    # LokiCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (loki-cli, loki-telegram, etc.)
├── loki_state.py           # SQLite session database with FTS5 full-text search, session titles
...
├── tests/                    # Test suite
├── website/                  # Documentation site (loki-agent.nousresearch.com)
│
...
| `~/.loki/auth.json` | OAuth credentials (Nous Portal) |
...
| `~/.loki/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
...
- **Session persistence**: All conversations are stored in SQLite (`loki_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.loki/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
...
- Use [GitHub Issues](https://github.com/NousResearch/loki-agent/issues)
