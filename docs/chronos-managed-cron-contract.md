...
|---|---|---|---|
| 1 | agent → NAS (`provision`/`cancel`/`list`) | the agent's existing **Nous Portal access token** (Bearer) — for a hosted agent this is the **bootstrap-session token** NAS planted in `auth.json` (client `loki-cli-vps`), NOT an `agent:*` client token | NAS (its normal agent-token path) |
...
**dashboard app** (`loki_cli/web_server.py`) — the agent's always-reachable
