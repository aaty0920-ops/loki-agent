...
**→ [构建 Loki Plugin](/guides/build-a-loki-plugin)** — 包含完整可运行示例的分步指南。
...
| 想要添加… | 方式 | 编写指南 |
|---|---|---|
| LLM 可调用的**工具** | Python 插件 — `ctx.register_tool()` | [Build a Loki Plugin](/guides/build-a-loki-plugin) · [Adding Tools](/developer-guide/adding-tools) |
| **生命周期 hook**（LLM 前后、会话开始/结束、工具过滤） | Python 插件 — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a Loki Plugin](/guides/build-a-loki-plugin) |
| CLI / gateway 的**斜杠命令** | Python 插件 — `ctx.register_command()` | [Build a Loki Plugin](/guides/build-a-loki-plugin) · [Extending the CLI](/developer-guide/extending-the-cli) |
...
完整的处理器约定、schema 格式、hook 行为、错误处理和常见错误请参见 **[完整指南](/guides/build-a-loki-plugin)**。