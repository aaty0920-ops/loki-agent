...
参考插件（[`loki-example-plugins`](https://github.com/NousResearch/loki-example-plugins/tree/main/example-dashboard) 中的 `example-dashboard`）提供了一个向 `sessions:top` 注入横幅的实时演示——安装它可端到端了解该模式。
...
[`strike-freedom-cockpit`](https://github.com/NousResearch/loki-example-plugins/tree/main/strike-freedom-cockpit) 插件（伴随仓库 `loki-example-plugins`）是一个完整的换肤演示。它将主题 YAML 与仅插槽插件配对，在不 fork dashboard 的情况下生成驾驶舱风格的 HUD。
