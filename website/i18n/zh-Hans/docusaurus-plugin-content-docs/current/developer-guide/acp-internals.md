...
Zed ACP Registry 路径通过 `uvx --from 'loki-agent[acp]==<version>' loki-acp` 启动同一适配器，指向 `loki-agent` PyPI 发布包。
...
因此 ACP 通告并使用当前配置的 Loki provider/凭据。它还始终通告一个终端 setup 认证方法（`loki-setup`，参数 `--setup`），以便首次运行的 registry 客户端在启动正常 ACP 会话前可以打开 Loki 的交互式模型/provider 配置。
