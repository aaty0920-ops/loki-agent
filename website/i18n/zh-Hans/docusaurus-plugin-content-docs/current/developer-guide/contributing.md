...
- 构建自定义/本地工具而不修改 Loki 核心？从 [构建 Loki 插件](../guides/build-a-loki-plugin.md) 开始
...
- **Profile 安全路径**：不得硬编码 `~/.loki` — 代码路径使用 `loki_constants` 中的 `get_loki_home()`，面向用户的消息使用 `display_loki_home()`。完整规则参见 [AGENTS.md](https://github.com/NousResearch/loki-agent/blob/main/AGENTS.md#profiles-multi-instance-support)。
...
- 使用 [GitHub Issues](https://github.com/NousResearch/loki-agent/issues)
...
提交贡献即表示您同意您的贡献将以 [MIT 许可证](https://github.com/NousResearch/loki-agent/blob/main/LICENSE) 授权。