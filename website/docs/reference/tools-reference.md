...
:::tip MCP Tools
In addition to built-in tools, Loki can load tools dynamically from MCP servers. MCP tools appear with the prefix `mcp_<server>_` (e.g., `mcp_github_create_issue` for the `github` MCP server). See [MCP Integration](/user-guide/features/mcp) for configuration.
:::
...
| `read_terminal` | Read what's currently shown in the in-app terminal pane of the Loki desktop GUI (the embedded shell beside this chat). Desktop-app only. | — |
...
| Tool | Description | Requires environment |
|------|-------------|----------------------|
| `x_search` | Search X (Twitter) posts, profiles, and threads using xAI's built-in `x_search` Responses tool. Use this for current discussion, reactions, or claims on X rather than general web pages. Off by default — opt in via `loki tools` → 🐦 X (Twitter) Search. Schema is only registered when xAI credentials are configured (check_fn-gated). | XAI_API_KEY **or** xAI Grok OAuth (SuperGrok / Premium+) login |
...
Registered by the bundled `spotify` plugin. Requires an OAuth token — run `loki spotify setup` once to authorize.
