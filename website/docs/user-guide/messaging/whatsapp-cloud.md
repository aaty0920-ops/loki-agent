...
Message-template support (the workaround for outside-window sends) is not yet implemented in Loki. If you need it, please [open an issue](https://github.com/NousResearch/loki-agent/issues) — it's planned but waiting on a clear demand signal.
...
```bash
loki config set stt.provider openai
loki config set stt.use_gateway true
