...
:::tip
Not using OpenAI Codex? `loki setup --portal` configures a non-Codex backend with Claude/Gemini/etc. in one step. See [Nous Portal](/integrations/nous-portal).
:::
...
2. **Codex OAuth login.** The codex subprocess reads `~/.codex/auth.json`. Two ways to populate it:
   ...
   ```
   Loki' own `loki auth login codex` writes to `~/.loki/auth.json` — that's a separate session. **Run `codex login` separately** if you haven't.
...
- **Loki auth and codex auth are separate sessions.** You need both `codex login` AND `loki auth login codex` for the cleanest UX (the runtime uses codex's session for the LLM call). This is a deliberate design choice in Loki' `_import_codex_cli_tokens` — Loki won't share OAuth state with codex CLI to avoid clobbering each other on token refresh.
...
If you find a bug, [open an issue](https://github.com/NousResearch/loki-agent/issues) with the output of `loki logs --since 5m`. Mention `codex-runtime` in the title so it's easy to triage.
...
For implementation details, see [PR #24182](https://github.com/NousResearch/loki-agent/pull/24182) and the [Codex app-server protocol README](https://github.com/openai/codex/blob/main/codex-rs/app-server/README.md).
