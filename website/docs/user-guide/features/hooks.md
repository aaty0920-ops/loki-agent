...
Loki has three hook systems that run custom code at key lifecycle points:
...
| **[Shell hooks](#shell-hooks)** | `hooks:` block in `~/.loki/config.yaml` pointing at shell scripts | CLI + Gateway | Drop-in scripts for blocking, auto-formatting, context injection |
...
See the **[Build a Plugin guide](/guides/build-a-loki-plugin)** for the full walkthrough including tool schemas, handlers, and advanced hook patterns.
...
Declare shell-script hooks in your `cli-config.yaml` and Loki will run them as subprocesses whenever the corresponding plugin-hook event fires — in both CLI and gateway sessions. No Python plugin authoring required.
