...
Loki ships a curated catalog of MCP servers that Nous staff has reviewed
...
the loki-agent repo, so Nous has reviewed each entry before it shipped —
...
Manifests live at
[`optional-mcps/<name>/manifest.yaml`](https://github.com/NousResearch/loki-agent/tree/main/optional-mcps)
...
[`optional-mcps/`](https://github.com/NousResearch/loki-agent/tree/main/optional-mcps).
...
Most hosted MCP servers (Linear, Sentry, Atlassian, Asana, Figma, Stripe, …) require OAuth 2.1 instead of a static bearer token. Set `auth: oauth` and Loki handles discovery, dynamic client registration, PKCE, token exchange, refresh, and step-up auth via the MCP Python SDK.
...
**Remote / headless hosts.** When Loki runs on a different machine than your browser, the loopback callback can't reach your laptop. Two ways to complete the flow:
...
The MCP server reads conversation data directly from Loki's session store (`~/.loki/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same internal send engine (`tools/send_message_tool.py`) that powers cron delivery and the `loki send` CLI.
