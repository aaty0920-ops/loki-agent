...
```bash
curl -fsSL https://loki-agent.nousresearch.com/install.sh | bash -s -- --no-skills
```
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Loki to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.loki/plans/` relative to the active workspace/backend working directory.
...
- **Create locally, update in place**: New agent-created skills are written to `~/.loki/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
...
[Skills Hub](https://loki-agent.nousresearch.com/docs) page — instead of a
...
Loki supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

Loki source id: `claude-marketplace`
...
Install a single-file `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Loki fetches the URL, parses the YAML frontmatter, security-scans it, and installs.
...
If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.loki/.hub/taps.json`:
...
Taps are stored in `~/.loki/.hub/taps.json` (created on demand).
