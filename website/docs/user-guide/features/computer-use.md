...
1. Run `loki tools`, pick `🖱️  Computer Use (macOS/Windows/Linux)`.
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or Loki app). `loki computer-use doctor` will tell you which permission is missing. |
...
agent is acting here." Each Loki run declares its own cua-driver
**session id** (something like `loki-3a7b9c14d2e8`); the cursor's
...
Loki intentionally keeps its skill (`skills/computer-use/SKILL.md`)
focused on the Loki-side `computer_use` action vocabulary — the
...
- **Loki spawns its own `cua-driver mcp` child over stdio** — it does
...
- **Loki-side skill** — `skills/computer-use/SKILL.md` — teaches the
...
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects Loki (planned follow-up), this happens
