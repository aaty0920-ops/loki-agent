...
**→ [Build a Loki Plugin](/guides/build-a-loki-plugin)** — step-by-step guide with a complete working example.
...
User plugins at `~/.loki/plugins/model-providers/<name>/` and `~/.loki/plugins/memory/<name>/` override bundled plugins of the same name — last-writer-wins in `register_provider()` / `register_memory_provider()`. Drop a directory in, and it replaces the built-in without any repo edits.
...
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Loki Plugin](/guides/build-a-loki-plugin) · [Adding Tools](/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a Loki Plugin](/guides/build-a-loki-plugin) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Loki Plugin](/guides/build-a-loki-plugin) · [Extending the CLI](/developer-guide/extending-the-cli) |
...
loki plugins update my-plugin              # pull latest
...
See the **[full guide](/guides/build-a-loki-plugin)** for handler contracts, schema format, hook behavior, error handling, and common mistakes.
