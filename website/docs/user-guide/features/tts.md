...
Gemini 3.1 Flash TTS supports freeform square-bracket audio tags such as `[whispers]`, `[excitedly]`, `[very slow]`, `[laughs]`, and other expressive delivery notes. Enable `tts.gemini.audio_tags` to have Loki run a hidden rewrite pass before Gemini TTS. The rewrite inserts inline tags into the TTS script only; the visible chat reply stays unchanged.
...
Each provider has a documented per-request input-character cap. Loki truncates text before calling the provider so requests never fail with a length error:
...
**Custom local CLI fallback** — Set `LOKI_LOCAL_STT_COMMAND` if you want Loki to call a local transcription command directly. The command template supports `{input_path}`, `{output_dir}`, `{language}`, and `{model}` placeholders. Your command must write a `.txt` transcript somewhere under `{output_dir}`.
...
If your configured provider isn't available, Loki automatically falls back:
...
This complements the legacy `LOKI_LOCAL_STT_COMMAND` escape hatch — that env var still works untouched via the built-in `local_command` path. Use `stt.providers.<name>` when you want **multiple** shell-driven STT engines, a name you can pick via `stt.provider`, or anything that needs per-provider `language` / `model` / `timeout`.
...
| Only the legacy single-command escape hatch is wanted        | `LOKI_LOCAL_STT_COMMAND` env var (preserved for back-compat)  |
