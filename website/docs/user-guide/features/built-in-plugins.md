...
See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a Loki Plugin](/guides/build-a-loki-plugin) to write your own.
...
- Post-meeting artifacts (transcript, speaker-attributed notes, action items) saved under `~/.loki/cache/google_meet/<meeting_id>/`
...
**Disabling:** `loki plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.loki/cache/google_meet/` until you remove them.
...
Bundled plugins are written exactly like any other Loki plugin — see [Build a Loki Plugin](/guides/build-a-loki-plugin). The only differences are:
