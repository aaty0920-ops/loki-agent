...
See the [config reference](https://github.com/NousResearch/loki-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/loki).
...
echo "OPENVIKING_API_KEY=..." >> ~/.loki/.env
```
...
`OPENVIKING_AGENT` is Loki' peer ID in OpenViking for peer-scoped memories.
...
**Switching modes:** Re-run `loki memory setup mem0 --mode <platform|oss>` or edit `mem0.json` directly.
...
See [plugin README](https://github.com/NousResearch/loki-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
| **Requires** | `pip install supermemory` + [API key](http://app.supermemory.ai/integrations?connect=loki) |
