...
A reference plugin (`example-dashboard` in [`loki-example-plugins`](https://github.com/NousResearch/loki-example-plugins/tree/main/example-dashboard)) ships a live demo that injects a banner into `sessions:top` — install it to see the pattern end-to-end.
...
The [`strike-freedom-cockpit`](https://github.com/NousResearch/loki-example-plugins/tree/main/strike-freedom-cockpit) plugin (companion repo `loki-example-plugins`) is a complete reskin demo. It pairs a theme YAML with a slot-only plugin to produce a cockpit-style HUD without forking the dashboard.
...
```bash
git clone https://github.com/NousResearch/loki-example-plugins.git
