...
Follow the [installation instructions for Loki Desktop](../getting-started/installation.md).
...
The [manual update process](https://loki-agent.nousresearch.com/docs/getting-started/updating) also works with the GUI.
...
| `--loki-root PATH` | Override the Loki source root the app uses (sets `LOKI_DESKTOP_LOKI_ROOT`)          |
...
You can also set the backend URL without the UI via the `LOKI_DESKTOP_REMOTE_URL` environment variable before launching the app (it overrides the in-app setting); you still sign in from the Gateway settings panel.
