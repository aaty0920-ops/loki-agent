...
First-time users almost always want `loki setup --portal` — one OAuth covers a model plus the four Tool Gateway tools (search/image/TTS/browser). See [Nous Portal](/integrations/nous-portal).
:::
...
2. [Build a Loki Plugin](/guides/build-a-loki-plugin)
...
Use reinforcement learning to fine-tune model behavior with Loki Agent's RL training pipeline (powered by [Atropos](https://github.com/NousResearch/atropos)).
