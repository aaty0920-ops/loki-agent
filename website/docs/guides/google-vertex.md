...
- **`google-auth`** — installed automatically the first time you select Vertex (lazy install), or explicitly with `pip install 'loki-agent[vertex]'`.
...
Install the extra: `pip install 'loki-agent[vertex]'`. Loki also lazy-installs it the first time you select the Vertex provider.
