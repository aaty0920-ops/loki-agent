...
loki cron create "0 2 * * *" \
  "You are a project manager triaging the NousResearch/loki-agent GitHub repo.

1. Run: gh issue list --repo NousResearch/loki-agent --state open --json number,title,labels,author,createdAt --limit 30
...
loki cron create "0 9 * * 1" \
  "Scan the NousResearch/loki-agent repo for documentation drift.

1. Run: gh pr list --repo NousResearch/loki-agent --state merged --json number,title,files,mergedAt --limit 30
