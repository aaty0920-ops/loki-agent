...
:::tip
If your OpenClaw setup was multi-provider, `loki setup --portal` collapses it to one OAuth — 300+ models plus the Tool Gateway in a single login. See [Nous Portal](/integrations/nous-portal).
:::
...
| OpenClaw config path | Loki config path | Notes |
|---------------------|-------------------|-------|
...
6. **Check session policies** — run `loki config show` and verify the `session_reset` value matches your expectations.
