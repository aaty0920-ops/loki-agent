...
description: "Try NVIDIA Nemotron 3 Ultra on Nous Portal — free June 4–18 — with day 0 support in Loki Agent"
---
...
[Download the Loki Desktop installer](https://loki-agent.nousresearch.com/) for macOS or Windows, then open it. On first launch it finishes setting itself up (usually under a minute).

### 2. Connect Nous Portal

When the app opens, you'll see a "Let's get you set up" screen. Click **Nous Portal** (marked **Recommended**). Your browser opens — create a [Nous Portal](https://portal.nousresearch.com) account (or sign in), choose the **Free** plan, and authorize Loki. The app connects automatically.
...
Prefer to review first? Download [`install.sh`](https://loki-agent.nousresearch.com/install.sh), inspect it, then run it.
...
- **Don't see the model in the list?** Make sure you finished the Nous Portal connection and that you're on the **Free** plan. In the CLI, `loki portal info` confirms you're logged in and routing through Nous.
...
- **[Run Loki Agent with Nous Portal](/guides/run-loki-with-nous-portal)** — Full Portal walkthrough: models, Tool Gateway, and verification
