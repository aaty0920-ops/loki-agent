...
sidebar_position: 1
title: "Nous Portal"
description: "One subscription, 300+ frontier models, the Tool Gateway, and Nous Chat — the recommended way to run Loki Agent"
---

# Nous Portal

[Nous Portal](https://portal.nousresearch.com) is Nous Research's unified subscription gateway and **the recommended way to run Loki Agent**. One OAuth login replaces the juggling act of separate accounts, API keys, and billing relationships across every model lab, search API, image generator, and browser provider you'd otherwise need to wire up by hand.
...
That single command runs the Portal OAuth, lets you pick a Nous model, sets Nous as your inference provider in `config.yaml`, and turns on the Tool Gateway. You're ready to `loki chat` immediately after.
...
The same subscription unlocks the [Tool Gateway](/user-guide/features/tool-gateway), which routes Loki Agent's tool calls through Nous-managed infrastructure. Five backends, one login:
...
Nous Research's own **Loki 4** family (Loki-4-70B, Loki-4-405B) is available through the Portal at heavily discounted rates. These are **frontier hybrid-reasoning chat models** — strong at math, science, instruction following, schema adherence, roleplay, and long-form writing.
...
The Portal's own [model info page](https://portal.nousresearch.com/info) carries the same warning, so this isn't a Loki-side opinion — it's the official guidance from Nous Research.
...
4. Sets Nous as your inference provider in `~/.loki/config.yaml` (when you pick a model)
...
```bash
loki portal            # log in to Nous Portal + set it up (one-shot onboarding)
...
`loki portal` (with no subcommand) is the human-readable alias for `loki auth add nous --type oauth` — it logs you in, lets you pick a Nous model, sets Nous as your inference provider, and offers the Tool Gateway opt-in (identical to `loki setup --portal`, and the same Nous flow as the first-time quick setup).
...
If you already have, say, a Browserbase account and want to keep using it while routing web search and image generation through Nous, that's supported. Use `loki tools` to pick backends per tool:
...
or use `loki model` and re-select Nous Portal.

### Got a "re-authentication required" message mid-session

Your Portal refresh token was invalidated (password change, manual revoke, or session expiry). Run `loki auth add nous` and your next request will use the new credentials. Any quarantine on the old token clears automatically on successful re-login.
...
If a model is genuinely missing, [open an issue](https://github.com/NousResearch/loki-agent/issues) — we surface the Portal's catalog to Loki and gaps usually mean a routing config we can update.

### Bills not appearing on my Portal account

Check `loki portal info` first — if it shows you're using a different provider (`Model: currently openrouter` instead of `using Nous as inference provider`), your local config has drifted. Run `loki model`, pick Nous Portal, and the next request will route through your subscription.
