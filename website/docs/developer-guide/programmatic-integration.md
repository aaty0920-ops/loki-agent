...
loki acp --bootstrap      # print install snippet for an ACP-capable IDE
```
...
Every command in the Pi-mono RPC spec ([issue #360](https://github.com/NousResearch/loki-agent/issues/360)) has a TUI-gateway equivalent:
...
- **API server:** include a `model` field in the request body or set `X-Loki-Model`
