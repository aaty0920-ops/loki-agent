...
- Building a custom/local tool without modifying Loki core? Start with [Build a Loki Plugin](../guides/build-a-loki-plugin.md)
...
- **Profile-safe paths**: Never hardcode `~/.loki` — use `get_loki_home()` from `loki_constants` for code paths and `display_loki_home()` for user-facing messages. See [AGENTS.md](https://github.com/NousResearch/loki-agent/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.
...
By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/NousResearch/loki-agent/blob/main/LICENSE).
