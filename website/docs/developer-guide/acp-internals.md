...
The ACP adapter wraps Loki' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
The Zed ACP Registry path launches the same adapter through `uvx --from 'loki-agent[acp]==<version>' loki-acp`, pointed at the `loki-agent` PyPI release.
...
So ACP advertises and uses the currently configured Loki provider/credentials. It also always advertises a terminal setup auth method (`loki-setup`, args `--setup`) so first-run registry clients can open Loki' interactive model/provider configuration before starting a normal ACP session.
