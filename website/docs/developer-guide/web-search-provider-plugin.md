...
| `requires_env` | Interactive credential prompt during `loki plugins install` (see [Build a Loki Plugin](/guides/build-a-loki-plugin#gate-on-environment-variables) for the rich format) |
...
If your provider wraps a third-party SDK (like DDGS does with the `ddgs` package), don't `import` it at module top level. Use `tools.lazy_deps.ensure(...)` inside `is_available()` or `search()` — Loki will install the package on first use, gated by `security.allow_lazy_installs`. See [Build a Loki Plugin → Lazy-install](/guides/build-a-loki-plugin#lazy-install-optional-python-dependencies) for the security model.
...
`my_backend_web_package` must expose a top-level `register` function. See [Distribute via pip](/guides/build-a-loki-plugin#distribute-via-pip) in the general plugin guide for the full setup.
...
- [Build a Loki Plugin](/guides/build-a-loki-plugin) — general tools/hooks/slash commands guide
