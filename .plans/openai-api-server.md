# OpenAI-Compatible API Server for Loki Agent
...
SSE streaming. By exposing this endpoint, loki-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  loki-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.loki/.env`
2. Run `loki gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with loki-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns loki-agent as a model) |
...
Authorization: Bearer loki-api-key-here
...
But loki-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "loki-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured loki-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of loki-agent's core prompt
...
Once implemented, loki-agent works as a drop-in backend for:
