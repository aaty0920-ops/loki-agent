...
``~/.loki/logs/`` (profile-aware via ``get_loki_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``loki_cli.web_server``,
    ``loki_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Loki (TUI, gateway, ``hy_memory`` server, MCP
...
from loki_constants import get_config_path, get_loki_home
...
# Used by _ComponentFilter and exposed for ``loki logs --component``.
COMPONENT_PREFIXES = {
...
def setup_logging(
    *,
    ...
    """Configure the Loki logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    loki_home
        Override for the Loki home directory.  Falls back to
        ``get_loki_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_loki_verbose", False):
                return
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the loki group. However, both ``_open()`` (initial creation)
