"""Progressive tool disclosure ("tool search") for Loki Agent.
...
``tool_call`` — and surfaced on demand. Core Loki tools never defer.

Design constraints this module is built around (see ``openclaw-tool-search-report``
for the full rationale):

* Core tools defined in ``toolsets._LOKI_CORE_TOOLS`` are *never* deferred.
...
def is_deferrable_tool_name(name: str) -> bool:
    ...
    OR it is not in ``_LOKI_CORE_TOOLS``. Core tools are never deferred
