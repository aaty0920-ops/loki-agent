...
#
# The single-env-var ``LOKI_LOCAL_STT_COMMAND`` escape hatch is preserved
...
def _render_command_stt_template(
    ...
    def replace_match(match: "re.Match[str]") -> str:
        ...
        token = f"__LOKI_STT_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _transcribe_xai(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    if not api_key:
        return {
            "success": False,
            "transcript": "",
            "error": "No xAI credentials found. Configure xAI OAuth in `loki model` or set XAI_API_KEY",
        }
    ...
    language = str(
        ...
        or os.getenv("LOKI_LOCAL_STT_LANGUAGE")
        or DEFAULT_LOCAL_STT_LANGUAGE
    ).strip()
