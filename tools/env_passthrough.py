...
def _is_loki_provider_credential(name: str) -> bool:
    """True if ``name`` is a Loki-managed provider credential (API key,
    token, or similar) per ``_LOKI_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Loki API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    let a skill tunnel a Loki credential into the execute_code child.
    """
    try:
        ...
        return True
    # Dynamically-generated Loki-internal secrets (AUXILIARY_*_API_KEY /
    ...
    if _is_loki_internal_secret(name):
        return True
    return name in _LOKI_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Loki-managed provider credentials (from
    ``_LOKI_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Loki-managed
    ...
    Non-Loki third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Loki-managed provider credentials must not be passed
