...
selectable via ``loki tools`` → Image Generation; the active model is
...
def _agent_cache_base_for_env(env: Any) -> str | None:
    ...
    # Loki cache roots can be translated without side effects. SSH can still
...
def _build_no_backend_setup_message() -> str:
    ...
    if managed_nous_tools_enabled():
        lines.append(
            ...
            "gateway enabled (`loki setup`)"
        )
    lines.append(
        "  3. Configure a different image_gen provider via `loki tools` "
        "→ Image Generation (run `loki plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
...
def check_image_generation_requirements() -> bool:
    ...
    the historical behavior: shipping loki with a FAL key configured
    ...
    if provider is None:
        return json.dumps({
            ...
            "error": (
                ...
                f"registered that name. Run `loki plugins list` to see "
        ...
        if "image_url" in kwargs or "reference_image_urls" in kwargs:
            ...
            return json.dumps({
                ...
                "error": (
                    ...
                    f"supports editing via `loki tools` → Image Generation."
                ),
...
# model/provider via `loki tools` or `/skills`.
