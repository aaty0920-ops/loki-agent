...
def read_terminal_tool(
    ...
    if callback is None:
        return tool_error("read_terminal is only available in the Loki desktop app.")
...
def check_read_terminal_requirements() -> bool:
    """Desktop GUI only — LOKI_DESKTOP is set on the gateway the app spawns."""
    return env_var_enabled("LOKI_DESKTOP")


READ_TERMINAL_SCHEMA = {
    "name": "read_terminal",
    "description": (
        "Read what's currently shown in the in-app terminal pane of the Loki "
