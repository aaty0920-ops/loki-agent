...
This module provides the glue:
    - ``LokiTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Loki Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses LOKI_HOME so each profile gets its own OAuth tokens.
    Layout: ``LOKI_HOME/mcp-tokens/``
    """
    try:
        from loki_constants import get_loki_home
        base = Path(get_loki_home())
    except ImportError:
        base = Path(os.environ.get("LOKI_HOME", str(Path.home() / ".loki")))
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``loki mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `loki mcp login <server>` interactively to (re)authorize, "
...
# ---------------------------------------------------------------------------
# LokiTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------
...
class LokiTokenStorage:
    ...
    File layout::
        ...
        LOKI_HOME/mcp-tokens/<server_name>.json         -- tokens
        LOKI_HOME/mcp-tokens/<server_name>.client.json   -- client info
        LOKI_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Loki records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Loki.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://loki-agent.nousresearch.com/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `loki mcp login <server>` later to "
...
def build_oauth_auth(
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`loki mcp login {server_name}` interactively first to complete "
