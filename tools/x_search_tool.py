...
* ``XAI_API_KEY`` is set in ``~/.loki/.env`` or the process environment
  (paid xAI API key), OR
* The user is signed in via xAI Grok OAuth — SuperGrok subscription —
  i.e. ``loki auth add xai-oauth`` has been run and the stored refresh
  token still works.
...
from tools.xai_http import loki_xai_user_agent, resolve_xai_http_credentials
...
def check_x_search_requirements() -> bool:
    ...
    :func:`loki_cli.auth.resolve_xai_oauth_runtime_credentials` which
