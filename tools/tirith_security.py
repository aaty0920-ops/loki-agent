...
it is automatically downloaded from GitHub releases to $LOKI_HOME/bin/tirith.
...
def _get_loki_home() -> str:
    """Return the Loki home directory, respecting LOKI_HOME env var."""
    return str(get_loki_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_loki_home(), ".tirith-install-failed")
...
def _loki_bin_dir() -> str:
    """Return $LOKI_HOME/bin, creating it if needed."""
    d = os.path.join(_get_loki_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $LOKI_HOME/bin/tirith.
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $LOKI_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $LOKI_HOME/bin/tirith
    ...
    if os.path.isfile(loki_bin) and os.access(loki_bin, os.X_OK):
        _resolved_path = loki_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return loki_bin
