...
# Loki-internal env vars that should NOT leak into terminal subprocesses.
_LOKI_PROVIDER_ENV_FORCE_PREFIX = "_LOKI_FORCE_"

# Loki-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a Loki inference secret exactly
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    # Loki-managed inference credential — Claude subscription auth is not a
    # working Loki provider path.  Stripping it broke agent-spawned
...
# project's dependencies into the Loki venv path instead of the project's own
# ``.venv`` — silently clobbering the Loki environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# Loki venv stays reachable via PATH (its bin dir is first), so stripping
...
def _is_loki_internal_secret(key: str) -> bool:
    """Return True for Loki-internal secrets injected under *dynamic* names.

    ``_LOKI_PROVIDER_ENV_BLOCKLIST`` is name-based and derived from the
    ...
    This is the single source of truth for "Loki-internal dynamic secret"
    across every spawn path — the terminal ``_make_run_env`` /
    ``_sanitize_subprocess_env`` filters, the Docker passthrough filter, and the
    non-terminal :func:`loki_subprocess_env` helper all call it, so the
...
def _inject_context_loki_home(env: dict) -> None:
    """Bridge the context-local Loki home override into subprocess env."""
    try:
        from loki_constants import get_loki_home_override

        value = get_loki_home_override()
        if value:
            env["LOKI_HOME"] = value
    except Exception:
        pass
...
def _inject_session_context_env(env: dict) -> None:
    ...
    (LOKI_SESSION_*) are bridged onto the child env here.
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Loki-managed secrets from a subprocess environment."""
    try:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_LOKI_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if _is_loki_internal_secret(key):
            continue
        if key not in _LOKI_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    for key, value in (extra_env or {}).items():
        if key.startswith(_LOKI_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_LOKI_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_loki_internal_secret(real_key):
                continue
            sanitized[real_key] = value
        elif _is_loki_internal_secret(key):
            continue
        elif key not in _LOKI_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    _inject_context_loki_home(sanitized)
...
# legitimate child Loki spawns needs them, and they are the highest-value
...
# narrow subset of _LOKI_PROVIDER_ENV_BLOCKLIST; provider keys are handled by
# the conditional Tier-2 strip in loki_subprocess_env().
...
def loki_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    single source of truth (``_LOKI_PROVIDER_ENV_BLOCKLIST``).  The terminal
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child Loki spawns legitimately needs them.
    * **Tier 2 (conditional):** the rest of ``_LOKI_PROVIDER_ENV_BLOCKLIST``
    ...
    for key in _ALWAYS_STRIP_KEYS:
        env.pop(key, None)
    # Internal routing hints and Loki-internal dynamic secrets
    ...
    # legitimate use for them. See :func:`_is_loki_internal_secret`.
    for key in list(env):
        if key.startswith(_LOKI_PROVIDER_ENV_FORCE_PREFIX):
            env.pop(key, None)
        elif _is_loki_internal_secret(key):
    ...
    if not inherit_credentials:
        ...
        for key in _LOKI_PROVIDER_ENV_BLOCKLIST:
            env.pop(key, None)

    # Windows UTF-8 safety for spawned processes (#31420).
    env.setdefault("PYTHONUTF8", "1")

    _inject_context_loki_home(env)
    from loki_constants import apply_subprocess_home_env
    ...
    # copies os.environ, whose LOKI_SESSION_* mirror is a last-writer-wins
...
def _find_bash() -> str:
    ...
    #   PortableGit: %LOCALAPPDATA%\loki\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\loki\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _loki_portable_git = os.path.join(_local_appdata, "loki", "git") if _local_appdata else ""
    if _loki_portable_git:
        for candidate in (
            os.path.join(_loki_portable_git, "bin", "bash.exe"),        # PortableGit (primary)
            os.path.join(_loki_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
...
def _resolve_loki_bin_dir() -> str | None:
    """Return the directory holding the ``loki`` console-script, or None.
    ...
    loki install dir (``~/.local/bin``, the venv ``bin``/``Scripts``, pipx,
    nix) is absent from that PATH, so plugins shelling out to bare ``loki``
    via the terminal tool hit ``command not found`` (exit 127) even though
    ``loki`` works fine in the user's own interactive terminal.

    We resolve the install dir once (it never changes within a process) and
    prepend-if-missing it to the subshell PATH so bare ``loki`` resolves
    regardless of how the gateway was started.

    Resolution order (cheap, no heavy imports):
      1. ``shutil.which("loki")`` — normal PATH-installed shim.
      2. The directory of ``sys.argv[0]`` when it's an absolute path to a
         real ``loki`` executable (covers nix-store / venv wrappers).
    ...
    if candidate is None:
        ...
        if (
            os.path.isabs(argv0)
            and (base == "loki" or base.startswith("loki."))
    ...
    _LOKI_BIN_DIR = candidate
    return candidate


def _prepend_loki_bin_dir(existing_path: str) -> str:
    """Prepend the loki install dir to ``existing_path`` if it's missing.
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    Windows commands such as ``tasklist``, ``schtasks``, and ``wmic``.  Loki
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_LOKI_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_LOKI_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_loki_internal_secret(real_key):
                continue
            run_env[real_key] = v
        elif _is_loki_internal_secret(k):
            continue
        elif k not in _LOKI_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
    path_key = _path_env_key(run_env)
    if path_key is not None:
        new_path = _append_missing_sane_path_entries(run_env.get(path_key, ""))
        # Ensure the loki install dir is reachable so plugins can shell out
        # to bare ``loki`` via the terminal tool even when the gateway was
        # launched without it on PATH (systemd, service managers, cron, etc.).
        run_env[path_key] = _prepend_loki_bin_dir(new_path)

    _inject_context_loki_home(run_env)
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Loki trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``LOKI_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under LOKI_HOME.  Using
            ...
            try:
                ...
                cache_dir = get_loki_home() / "cache" / "terminal"
            except Exception:
                cache_dir = Path(tempfile.gettempdir()) / "loki_terminal"
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_loki_pgid", None)
                    if pgid is None:
                        raise
