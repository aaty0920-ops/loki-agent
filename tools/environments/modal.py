...
wrapper, while preserving Loki' persistent snapshot behavior across sessions.
"""
...
class ModalEnvironment(BaseEnvironment):
    ...
    def _modal_bulk_download(self, dest: Path) -> None:
        ...
        Modal sandboxes always run as root, so /root/.loki is hardcoded
        (consistent with iter_sync_files call on line 269).
        """
        async def _download():
            proc = await self._sandbox.exec.aio(
                "bash", "-c", "tar cf - -C / root/.loki"
            )
