...
# LOKI_DEBUG_INTERRUPT=1 to log loop entry/exit, periodic heartbeats, and
...
def get_sandbox_dir() -> Path:
    ...
    Configurable via TERMINAL_SANDBOX_DIR. Defaults to {LOKI_HOME}/sandboxes/.
    """
    custom = os.getenv("TERMINAL_SANDBOX_DIR")
    if custom:
        p = Path(custom)
    else:
        p = get_loki_home() / "sandboxes"
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all Loki backends.
    ...
    def init_session(self):
        ...
        # caused ``C:/Users/.../loki-snap-*.sh: No such file or directory``
        ...
        bootstrap = (
            ...
            f"__loki_fns=$(declare -F | awk '{{print $3}}' | grep -vE '^_[^_]') || true\n"
            f"[ -n \"$__loki_fns\" ] && declare -f $__loki_fns "
    ...
    def _wait_for_process(self, proc: ProcessHandle, timeout: int = 120) -> dict:
        ...
        # --- Debug tracing (opt-in via LOKI_DEBUG_INTERRUPT=1) -------------
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __LOKI_CWD_{session}__ marker from stdout output.
