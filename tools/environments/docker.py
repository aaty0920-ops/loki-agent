...
def _load_loki_env_vars() -> dict[str, str]:
    """Load ~/.loki/.env values without failing Docker command execution."""
    try:
...
def _get_active_profile_name() -> str:
    """Return the active Loki profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale loki-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=loki-agent=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Loki process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=loki-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a loki process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Loki
    process leaves its container behind permanently because there's no
    subsequent Loki process scheduled to reuse that exact (task, profile)
    pair.
    """
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``LOKI_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
...
#   SETUID/SETGID - the image's init drops from root to the 'loki'
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``loki-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.loki/sandboxes/). Non-persistent
        ...
        # s6-overlay images (e.g. loki-agent:latest) already use /init as PID 1
        ...
        # Labels make loki-created containers identifiable to:
        #   * the orphan reaper (`loki-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`loki-task-id`, `loki-profile`)
        #   * operators running `docker ps --filter label=loki-agent=1`
        # Values are limited to the safe character set defined by
        # _sanitize_label_value(); the active Loki profile is captured at
        ...
        label_args = [
            "--label", "loki-agent=1",
            "--label", f"loki-task-id={task_label}",
            "--label", f"loki-profile={profile_name}",
        ]
        ...
        # container shared across sessions").  If a prior Loki process
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Loki secret blocklist. Only implicit passthrough
        # keys are filtered. Also strip Loki-internal dynamic secrets
        # (AUXILIARY_*_API_KEY / _BASE_URL, GATEWAY_RELAY_* auth) that the
        # name-based blocklist doesn't cover — see _is_loki_internal_secret.
        _implicit_forward = {
            k for k in passthrough_keys if not _is_loki_internal_secret(k)
        }
        forward_keys = explicit_forward_keys | (_implicit_forward - _LOKI_PROVIDER_ENV_BLOCKLIST)
        loki_env = _load_loki_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if not value:
                value = loki_env.get(key)
            if value:
                exec_env[key] = value
    ...
    def _recreate_container(self) -> bool:
        ...
        task_label = self._labels.get("loki-task-id", "")
        profile_label = self._labels.get("loki-profile", "")
        ...
        if not self._container_id:
            ...
            try:
                ...
                new_name = f"loki-{_uuid.uuid4().hex[:8]}"
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        matches containers that happened to be named ``loki-*`` but were
        ...
        # container) but can happen if a previous Loki process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Loki exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Loki process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Loki startup. That covers the
        ...
        # outlives Loki processes, processes inside it stay alive, and
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``loki /quit``
