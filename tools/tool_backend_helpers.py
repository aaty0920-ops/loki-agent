...
def nous_tool_gateway_unavailable_message(
    ...
    *,
    ...
    try:
        ...
        pass
    return (
        f"{capability} is unavailable. Run `loki model` to refresh your "
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.loki/.env`` (via
    ``loki_cli.config.get_env_value`` when available) so tool-side
