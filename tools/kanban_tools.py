...
running under the dispatcher (env var ``LOKI_KANBAN_TASK`` set) or when
the active profile explicitly enables the ``kanban`` toolset for
orchestrator work. A normal ``loki chat`` session still sees **zero**
kanban tools in its schema unless configured.

Why tools instead of just shelling out to ``loki kanban``?

1. **Backend portability.** A worker whose terminal tool points at Docker
   / Modal / Singularity / SSH would run ``loki kanban complete …``
   inside the container, where ``loki`` isn't installed and the DB
   isn't mounted. Tools run in the agent's Python process, so they
   always reach ``~/.loki/kanban.db`` regardless of terminal backend.
...
Humans continue to use the CLI (``loki kanban …``), the dashboard
(``loki dashboard``), and the slash command (``/kanban …``) — all
...
def _check_kanban_mode() -> bool:
    ...
    1. ``LOKI_KANBAN_TASK`` is set (dispatcher-spawned worker), OR
    2. The current profile has ``kanban`` in its toolsets config
       (orchestrator profiles like techlead that route work via Kanban).

    Humans running ``loki chat`` without the kanban toolset see zero
...
def _worker_run_id(task_id: str) -> Optional[int]:
    ...
    if os.environ.get("LOKI_KANBAN_TASK") != task_id:
        return None
...
def _enforce_worker_task_ownership(tid: str) -> Optional[str]:
    ...
    A process spawned by the dispatcher has ``LOKI_KANBAN_TASK`` set
    ...
    ``LOKI_KANBAN_TASK`` in env) aren't subject to this check — their
...
def _connect(board: Optional[str] = None):
    ...
    (``LOKI_KANBAN_DB`` → ``LOKI_KANBAN_BOARD`` env → current symlink
    → ``default``). Per-tool ``board`` lets a Telegram-side agent override
    the env-pinned active board without restarting Loki.
    """
...
#   - No-op outside dispatcher-spawned worker context (no ``LOKI_KANBAN_TASK``).
...
def heartbeat_current_worker_from_env() -> bool:
    ...
    Identity comes from:
      * ``LOKI_KANBAN_TASK`` — task id (required; absence means no-op)
      * ``LOKI_KANBAN_RUN_ID`` — pins the run row so we don't heartbeat
        a stale run that may have already been reclaimed
      * ``LOKI_KANBAN_CLAIM_LOCK`` — claim lock for ``heartbeat_claim``;
    ...
    try:
        kb, conn = _connect()
        try:
            claim_lock = os.environ.get("LOKI_KANBAN_CLAIM_LOCK")
            try:
                kb.heartbeat_claim(conn, tid, claimer=claim_lock)
            except Exception:
                logger.debug("auto-heartbeat: heartbeat_claim failed", exc_info=True)
            run_id_raw = os.environ.get("LOKI_KANBAN_RUN_ID")
            run_id: Optional[int]
            try:
...
def _handle_show(args: dict, **kw) -> str:
    ...
    if not tid:
        return tool_error(
            "task_id is required (or set LOKI_KANBAN_TASK in the env)"
        )
...
def _handle_heartbeat(args: dict, **kw) -> str:
    ...
    try:
        ...
        try:
            ...
            # LOKI_KANBAN_CLAIM_LOCK in the worker env at spawn time
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    # an authoritative-looking name like ``loki-system`` and poison
...
def _handle_create(args: dict, **kw) -> str:
    ...
    tenant = args.get("tenant") or os.environ.get("LOKI_TENANT")
    # Stamp the originating session id when the agent loop runs under
    # ACP (which sets LOKI_SESSION_ID before invoking tools). NULL on
    # CLI / dashboard paths and on legacy hosts that don't set the env.
    session_id = args.get("session_id") or os.environ.get("LOKI_SESSION_ID")
    priority = args.get("priority")
    # Resolve workspace. If the caller passed one explicitly, honor it.
    # Otherwise, a dispatcher-spawned worker (LOKI_KANBAN_TASK set)
    ...
    # (kanban toolset, no LOKI_KANBAN_TASK) and CLI/dashboard callers
    ...
    try:
        ...
        try:
            ...
            if _inherit_workspace:
                _self_tid = os.environ.get("LOKI_KANBAN_TASK")
                if _self_tid:
            ...
            new_tid = kb.create_task(
                ...
                created_by=os.environ.get("LOKI_PROFILE") or "worker",
...
def _maybe_auto_subscribe(conn: Any, task_id: str) -> bool:
    ...
    toggle (see ``loki dashboard``).

    Subscription paths:

    - **Gateway** (telegram/discord/slack/etc): ``LOKI_SESSION_PLATFORM``
      and ``LOKI_SESSION_CHAT_ID`` are set in ContextVars by the
    ...
    - **TUI** (herm desktop / herm TUI): the platform/chat_id ContextVars
      ...
      ``LOKI_SESSION_KEY`` from the parent session. We subscribe with
    ...
    try:
        ...
        if not platform or not chat_id:
            ...
            # LOKI_SESSION_KEY into the subprocess env. Treat that
            # as a "tui" subscription so the TUI notification poller
            # (tui_gateway/server.py) can pick it up.
            #
            # LOKI_SESSION_ID is intentionally NOT a fallback here:
            ...
            # poller keys on LOKI_SESSION_KEY.
            session_key = (
                get_session_env("LOKI_SESSION_KEY", "")
                or os.environ.get("LOKI_SESSION_KEY", "")
            )
            if not session_key:
        ...
        notifier_profile = (
            get_session_env("LOKI_SESSION_PROFILE", "")
            or os.environ.get("LOKI_PROFILE")
        )
