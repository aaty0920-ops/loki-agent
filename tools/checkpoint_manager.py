...
-----------------------------------------------------------------------------
    ...
    ~/.loki/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/loki/<hash16>        — per-project branch tip
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Loki convention — don't recursively snapshot siblings)
    ".worktrees/",
...
def _git_env(
    ...
    The shared store is internal Loki infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``loki checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `loki checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "loki@local"], store, cfg_wd)
    _run_git(["config", "user.name", "Loki Checkpoint"], store, cfg_wd)
...
# look for ``HEAD``/``info/exclude``/``LOKI_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at LOKI_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "LOKI_WORKDIR").write_text(
...
# ---------------------------------------------------------------------------
# Public helpers for `loki checkpoints` CLI
# ---------------------------------------------------------------------------
