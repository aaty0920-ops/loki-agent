...
def _expand_tilde(path: str) -> str:
    ...
    mirrors ``loki_constants.get_subprocess_home()`` so that ``~`` resolves
...
def _get_loki_config_resolved() -> str | None:
    ...
    global _loki_config_resolved, _loki_config_resolved_loaded
    if _loki_config_resolved_loaded:
        return _loki_config_resolved
    _loki_config_resolved_loaded = True
    try:
        from loki_cli.config import get_config_path
        _loki_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _loki_config_resolved = str(Path(_expand_tilde("~/.loki/config.yaml")).resolve())
        except Exception:
            _loki_config_resolved = None
    return _loki_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent agents from modifying the Loki config file directly.
    # approvals.mode and other security settings live here; a malicious or
    # prompt-injected agent could silently disable exec approval by writing to
    # this file.
    loki_config = _get_loki_config_resolved()
    if loki_config and (resolved == loki_config or normalized == loki_config):
        return (
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Loki mirror prefix for Docker file tools."""
    try:
        from tools.terminal_tool import (
            _active_environments,
            _env_lock,
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    """Return a soft-guard warning when ``filepath`` lands in another Loki
    profile's scoped area, a host-side sandbox-mirror of authoritative profile
    state, or the Docker container's sandbox mirror of Loki state.
    ...
    * sandbox-mirror (#32049) — writes that hit the
      ``…/sandboxes/<backend>/<task>/home/.loki/…`` mirror created by a
      non-local terminal backend (Docker, Daytona, etc.), where the host
      Loki process never reads the mirror and the authoritative file is
      left untouched.
    * container-mirror (#32049 follow-up) — writes from inside a Docker
      ...
      the agent sees a plain ``/root/.loki/…`` path.

    Returns ``None`` when the write is in-scope or outside Loki scope.
    ...
    # in a session that cd'd into ``~/.loki/profiles/other/`` is
...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── Loki internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under LOKI_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == LOKI_HOME (e.g. "auth.json") still hits the
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    ``cross_profile`` opts out of the soft cross-Loki-profile guard. The
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    ``cross_profile`` opts out of the soft cross-Loki-profile guard for
...
WRITE_FILE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Loki profile's skills/plugins/cron/memories — by default these writes are blocked with a warning because they affect a different profile than the one this session is running under.",
                "default": False,
            },
        },
...
PATCH_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Loki profile's skills/plugins/cron/memories.",
                "default": False,
            },
        },
        "required": ["mode"],
