"""
Lazy dependency installer for opt-in Loki Agent backends.

Many Loki features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``loki-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``loki tools`` or the manual pip command.
...
* **Durable-target mode (immutable images).** When the deployment seals the
  agent's own venv (the Docker image sets ``LOKI_DISABLE_LAZY_INSTALLS=1``
  and makes ``/opt/loki`` read-only), setting
  ``LOKI_LAZY_INSTALL_TARGET`` redirects lazy installs to a writable
  ...
  installed package cannot brick Loki, which is what made it safe to seal
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # (LOKI_DISABLE_LAZY_INSTALLS=1) and lazy installs are redirected to the
...
def _allow_lazy_installs() -> bool:
    ...
    2. ``LOKI_DISABLE_LAZY_INSTALLS=1`` seals the *agent venv* (set by the
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``loki update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    Mirrors the strategy in ``loki_cli.tools_config._pip_install`` but
...
def active_features() -> list[str]:
    ...
    Used by ``loki update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``loki update``. Never raises; lazy-install failures
