...
from loki_constants import agent_browser_runnable, get_loki_home
...
# means a compromised transitive dependency could read every Loki secret
...
def _build_browser_env() -> dict:
    ...
    Strips Loki-managed secrets (provider keys, gateway tokens, GitHub auth,
    infra secrets) then re-adds only the browser-backend keys the worker needs.
    The ``loki_subprocess_env`` import is deferred to keep ``browser_tool``
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    loki_node_bin = str(loki_home / "node" / "bin")
    loki_node_root = str(loki_home / "node")
    loki_nm_bin = str(loki_home / "node_modules" / ".bin")
    return [loki_node_bin, loki_node_root, loki_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
# plugins drop in under ``~/.loki/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    browser plugins (``~/.loki/plugins/browser/<vendor>/``) participate
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Loki silently
...
def _run_chrome_fallback_command(
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Loki CLI where sys.stdout and sys.stderr both report
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-loki_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed loki processes — this way every clean loki exit sweeps
    ...
    # Sweep orphans from other crashed loki processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live loki processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current loki PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live loki process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning loki process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced loki PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent loki instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-loki_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live loki process.
            continue
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Loki-managed
    ...
    # node_modules binary, which disappears on the next ``loki update`` and
    ...
    # Build an extended search PATH including Loki-managed Node, macOS
    ...
    try:
        ...
        if ensure_dependency("browser"):
            candidates = [
                ...
                shutil.which("agent-browser", path=str(get_loki_home() / "node_modules" / ".bin")),
                shutil.which("agent-browser", path=str(get_loki_home() / "node" / "bin")),
                shutil.which("agent-browser", path=str(get_loki_home() / "node")),
            ]
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this loki PID as the session owner (cross-process safe
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Loki
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/nousresearch/loki-agent:latest")
                else:
