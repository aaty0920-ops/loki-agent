...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.loki",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``LOKI_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside LOKI_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    loki_home = _resolve_loki_home()

    # Reject absolute paths — they bypass the LOKI_HOME sandbox entirely.
    ...
    host_path = loki_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape LOKI_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, loki_home)
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.loki/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="loki-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching loki_constants.get_loki_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_loki_dir()`` for backward compatibility with old directory layouts.
    """
...
def map_cache_path_to_container(
    ...
    caller decides which ``container_base`` applies (Docker ``/root/.loki``,
    SSH ``<remote_home>/.loki``, etc.) and whether translation is wanted.
