...
Lets the LLM write a Python script that calls Loki tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `loki_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `loki_tools.py` with file-based RPC stubs
...
# available on every platform Loki itself runs on.
...
# match a safe prefix, the operational LOKI_ allowlist, or (on Windows) an
# OS-essential name.
#
# NB: the broad "LOKI_" prefix was deliberately removed (#27303) — it leaked
# LOKI_*-named config that lacks a secret substring (e.g. LOKI_BASE_URL,
# LOKI_KANBAN_DB, LOKI_*_WEBHOOK).  The child only needs the few
# location/profile vars in _LOKI_CHILD_ALLOWED below; LOKI_RPC_SOCKET /
# LOKI_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational LOKI_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set loki_cli treats as the
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational LOKI_* vars (_LOKI_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret LOKI_* vars dropped by the tightened allowlist (#27303). The
    # broad "LOKI_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like LOKI_KANBAN_DB / LOKI_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if k.startswith("LOKI_"):
            ...
            # allowlist — a deliberately-dropped LOKI_* var.
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Loki-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Loki-profile soft guard."""',
...
def generate_loki_tools_module(enabled_tools: List[str],
    ...
    """
    Build the source code for the loki_tools.py stub module.
...
def _execute_remote(
    ...
    The script and the generated loki_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/loki_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_loki_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/loki_tools.py", tools_src)
        ...
        env_prefix = (
            f"LOKI_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"LOKI_RPC_TOKEN={shlex.quote(rpc_token)} "
...
def execute_code(
    ...
    # --- Set up temp directory with loki_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="loki_sandbox_")
    ...
    # selector from LOKI_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"loki_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated loki_tools module.
        ...
        tools_src = generate_loki_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "loki_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  LOKI_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        # Ensure the loki-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from loki_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _loki_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _loki_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # LOKI_TIMEZONE is an internal Loki setting and must not leak
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.loki/.env')).
...
def _load_config() -> dict:
    ...
    ``loki --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (loki-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``loki tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and loki-agent's own python.
    if mode == "strict":
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Loki tools programmatically. "
        ...
        "Also available (no import needed — built into loki_tools):\n"
