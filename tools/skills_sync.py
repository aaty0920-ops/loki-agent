...
Copies bundled skills from the repo's skills/ directory into ~/.loki/skills/
...
The manifest lives at ~/.loki/skills/.bundled_manifest.
"""
...
# Marker file written by `loki profile create --no-skills` (named profiles)
# and by the installer's `--no-skills` flag (the default ~/.loki profile).
# When present in LOKI_HOME, sync_skills() is a no-op so neither the
# installer, `loki update`, nor a direct sync re-injects bundled skills.
# Delete the file to opt back in. Mirrors
# loki_cli.profiles.NO_BUNDLED_SKILLS_MARKER (kept as a literal here to
...
def _get_bundled_dir() -> Path:
    ...
    Checks LOKI_BUNDLED_SKILLS env var first (set by Nix wrapper),
...
def _read_suppressed_names() -> set:
    ...
    to reading ``~/.loki/skills/.curator_suppressed`` directly if that import
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.loki/skills/mlops/axolotl
    """
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.loki/skills/ using the manifest.

    Returns:
        dict with keys: copied (list), updated (list), skipped (int),
                        user_modified (list), cleaned (list), total_bundled (int)
    """
    # Opt-out: a profile (named or the default ~/.loki) that wrote the
    ...
    if (LOKI_HOME / NO_BUNDLED_SKILLS_MARKER).exists():
        if not quiet:
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        # (~/.loki/skills/.curator_suppressed) is written when the curator
        # archives a bundled skill with curator.prune_builtins enabled. Without
        # this skip, every `loki update` would resurrect a skill the user
                ...
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            ...
                            f"was kept. Run `loki skills reset {skill_name}` "
...
def _rmtree_writable(path: Path) -> None:
    ...
    # ``LOKI_HOME/skills/`` to prevent the catastrophic wipe of
    # ``~/.loki/`` (``.env``, ``MEMORY.md``, ``kanban.db``, custom
    ...
    # ``LOKI_HOME``. Require a strict-child relationship so both escape
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if restore and deleted_user_copy:
        ...
        message = (
            f"Cleared manifest entry for '{name}'. Future `loki update` runs "
...
def _is_tracked_user_modification(origin_hash: str, user_hash: str) -> bool:
    """Whether an on-disk skill counts as a user modification ``loki update`` keeps.
...
def list_user_modified_bundled_skills() -> List[dict]:
    """Return the bundled skills that ``loki update`` keeps because the user
    edited them locally.
...
def diff_bundled_skill(name: str) -> dict:
    ...
    edits or ``loki skills reset`` back to upstream.
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    When ``enabled`` is True, writes LOKI_HOME/.no-bundled-skills so the
    installer, ``loki update``, and any direct sync stop seeding bundled
    skills. When False, removes the marker so seeding resumes on the next
    sync. This is the on-disk-state half of ``loki skills opt-out`` /
    ...
    Returns:
        dict with keys: ok (bool), changed (bool), marker (str path),
                        message (str).
    """
    marker = LOKI_HOME / NO_BUNDLED_SKILLS_MARKER
    ...
    try:
        if enabled:
            ...
            changed = existed
            message = (
                "Opted back in. The next `loki update` (or `loki skills "
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (LOKI_HOME / NO_BUNDLED_SKILLS_MARKER).exists()
