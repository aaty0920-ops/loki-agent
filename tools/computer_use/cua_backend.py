...
etc.) surface as specific blocked checks via `loki computer-use doctor`
...
# fetches the latest release, so a `LOKI_CUA_DRIVER_VERSION` env var would
# only have *looked* like it pinned. For a reproducible version, point
# `LOKI_CUA_DRIVER_CMD` at a specific binary instead.

_CUA_DRIVER_CMD = os.environ.get("LOKI_CUA_DRIVER_CMD", "cua-driver")
...
def _cua_telemetry_disabled() -> bool:
    """True when Loki should disable cua-driver telemetry for this user.
...
def _resolve_mcp_invocation(
    ...
    *,
    ...
    Surface 8 of NousResearch/loki-agent#47072: instead of hardcoding
...
def cua_driver_binary_available() -> bool:
    """True if `cua-driver` is on $PATH or LOKI_CUA_DRIVER_CMD resolves."""
...
def cua_driver_update_nudge() -> Optional[str]:
    ...
    return (
        ...
        f"update with `loki computer-use install --upgrade`."
    )
...
def _parse_elements_from_structured(raw_elements: List[Dict[str, Any]]) -> List[UIElement]:
    """Surface 2 of NousResearch/loki-agent#47072: read the canonical
...
class _CuaDriverSession:
    ...
    def __init__(self, bridge: _AsyncBridge) -> None:
        ...
        self._started = False
        # Surface 4 of NousResearch/loki-agent#47072: per-tool
    ...
    async def _lifecycle_coro(self) -> None:
        ...
        try:
            ...
            params = StdioServerParameters(
                ...
                # sanitize Loki-managed secrets out of the child env.
...
def _extract_tool_result(mcp_result: Any) -> Dict[str, Any]:
    ...
    NousResearch/loki-agent#47072). Each entry corresponds index-for-index
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self) -> None:
        ...
        # Surface 6 of NousResearch/loki-agent#47072: per-snapshot
        ...
        #   - Gets a distinct agent-cursor color per Loki run, with
        ...
        #     concurrent Loki runs / subagents don't step on each
        #     other.
        # We mint a UUID4-based id once per CuaDriverBackend instance —
        # one Loki run = one backend = one session — and pass it as
    ...
    def start(self) -> None:
        ...
        # `computer-use` / `mcp` extras), not part of Loki' minimal core.
    ...
    def capture(self, mode: str = "som", app: Optional[str] = None) -> CaptureResult:
        ...
        Maps loki `capture(mode, app)` → cua-driver `list_windows` +
        `get_window_state` (ax/som) or `screenshot` (vision).
        """
        # Step 1: enumerate on-screen windows to find target pid/window_id.
        # Surface 3 of NousResearch/loki-agent#47072: read the canonical
        ...
        if mode == "vision":
            ...
            # Surface 2 of NousResearch/loki-agent#47072: prefer the
    ...
    def click(
        ...
        *,
        ...
        # NousResearch/loki-agent#47072). cua-driver-rs gained an explicit
