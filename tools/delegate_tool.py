...
# Must match loki_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``loki-cli`` (which bundles
...
def _build_child_agent(
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. loki-cli) so that individual
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.loki/logs/subagent-<sid>-<ts>.log``
...
def _load_config() -> dict:
    ...
    to the persistent config (loki_cli/config.py load_config()) so that
