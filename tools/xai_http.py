...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``loki tools`` repaint, tool-registration scans,
    ...
    2. ``~/.loki/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
...
def get_env_value(name: str, default=None):
    ...
    Wraps :func:`loki_cli.config.get_env_value` so tests can patch
...
def _load_config_section(section_name: str) -> Dict[str, Any]:
    """Return a top-level Loki config section as a dict, or empty."""
    try:
...
def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    ...
    Prefers Loki-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``loki_cli.config.get_env_value`` so keys
    stored in ``~/.loki/.env`` (the standard Loki location) are honored —
