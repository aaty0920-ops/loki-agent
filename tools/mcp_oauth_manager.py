...
`mcp_tool.py`, and `loki_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# LokiMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_loki_provider_class() -> Optional[type]:
    ...
    class LokiMCPOAuthProvider(OAuthClientProvider):
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`LokiTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, LokiTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._loki_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import LokiTokenStorage
                        if isinstance(storage, LokiTokenStorage):
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import LokiTokenStorage
            if not isinstance(storage, LokiTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``loki mcp reauth``.
            ...
            back to ``loki mcp reauth``.
            """
            try:
                if self._loki_preregistered:
                    return
    ...
    return LokiMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_LOKI_PROVIDER_CLS: Optional[type] = _make_loki_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`LokiMCPOAuthProvider` directly using the helpers
        ...
        if not _is_interactive() and not storage.has_cached_tokens():
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `loki mcp login "
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``loki mcp remove <name>`` and (indirectly) by
        ``loki mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
