...
persistent browser profiles.  When managed persistence is enabled, Loki
...
def get_camofox_state_dir() -> Path:
    ...
    return get_loki_home() / CAMOFOX_STATE_DIR_NAME / CAMOFOX_STATE_SUBDIR


def get_camofox_identity(task_id: Optional[str] = None) -> Dict[str, str]:
    """Return the stable Loki-managed Camofox identity for this profile.

    The user identity is profile-scoped (same Loki profile = same userId).
