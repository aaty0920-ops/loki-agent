...
Backend is selected during ``loki tools`` setup (web.backend in config.yaml).
When available, Loki can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Loki config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Loki' config/.env layer (``loki config set``, ``loki tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.loki/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``loki tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `loki tools` repaint.
        try:
