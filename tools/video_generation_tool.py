...
provider** — turn on a backend by enabling a plugin (``loki plugins
enable video_gen/<name>``) and selecting it in ``loki tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/loki-agent-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `loki tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"installed video gen backends, or `loki tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `loki tools` → "
...
# `loki tools` or `/skills`, the schema rebuilds automatically.
