...
`loki skills publish` for sharing. No new source type, no new store, no new
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.loki.blueprint`` is a mapping containing
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.loki.blueprint`` block) they can hand
    to ``loki skills publish`` to share. ``body`` is the plain-language
