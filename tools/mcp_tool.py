...
transport, discovers their tools, and registers them into the loki-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.loki/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.loki/logs/mcp-stderr.log), tagged with the
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            loki_home = os.path.expanduser(
                os.getenv(
                    "LOKI_HOME", os.path.join(os.path.expanduser("~"), ".loki")
                )
            )
            candidates = [
                os.path.join(loki_home, "node", "bin", resolved_command),
                ...
                # image (which the Loki Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Loki MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Loki gateways know how to render.
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Loki' existing approval
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (LOKI_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit LOKI_SESSION_PLATFORM, so the
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "  pip install 'loki-agent[mcp]'\n"
                "or (full install):\n"
                "  pip install 'loki-agent[all]'"
            )
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Loki' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Loki is fully
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `loki mcp login {server_name}` (or delete the tokens "
            f"file under ~/.loki/mcp-tokens/ and restart). Do NOT retry "
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local LOKI_HOME override into ``coro``.
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local LOKI_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_loki_home()
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Loki config file.
    ...
    ``os.environ`` (which includes ``~/.loki/.env`` loaded at startup).
    """
    try:
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Loki' MEDIA: tag convention and out to messaging
            ...
            # Loki' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Loki's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Loki registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``loki tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Loki registry.
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple loki
