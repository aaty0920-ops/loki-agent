...
# os.environ["LOKI_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export LOKI_INTERACTIVE.
_loki_interactive_ctx: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "loki_interactive",
    default=None,
)
...
def set_loki_interactive_context(interactive: bool) -> contextvars.Token:
    ...
    Use this instead of mutating ``os.environ["LOKI_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``LOKI_INTERACTIVE`` env var for legacy callers.
    """
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``LOKI_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _loki_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("LOKI_INTERACTIVE")
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("LOKI_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("LOKI_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set LOKI_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind LOKI_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds LOKI_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("LOKI_CRON_SESSION"):
        return False
...
# via shell expansions like $HOME or $LOKI_HOME, or by the resolved absolute
# active profile home path such as /home/loki/.loki/config.yaml. The
# resolved-absolute form is folded into the ~/.loki/ patterns at detection
...
# go stale when LOKI_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_LOKI_ENV_PATH = (
    r'(?:~\/\.loki/|'
    r'(?:\$home|\$\{home\})/\.loki/|'
    r'(?:\$loki_home|\$\{loki_home\})/)'
    r'\.env\b'
)
# ~/.loki/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _LOKI_ENV_PATH; matches the LOKI_HOME override form as
# well as ~/.loki/.
_LOKI_CONFIG_PATH = (
    r'(?:~\/\.loki/|'
    r'(?:\$home|\$\{home\})/\.loki/|'
...
DANGEROUS_PATTERNS = [
    ...
    # `loki` and `gateway` (e.g. `loki -p ade gateway restart`) so a
    ...
    # that should always require user consent, just like `loki gateway restart`
    ...
    # The name-based pattern above catches `pkill loki` but not
    # `kill -9 $(pgrep -f loki)` because the substitution is opaque
    ...
    # the `loki gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.loki.gateway`).
    # Catch the operations that stop, restart, or unload it.
    (r'\blaunchctl\s+(stop|kickstart|bootout|unload|kill|disable|remove)\b.*\b(loki|ai\.loki)\b', "stop/restart loki launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Loki file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.loki/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Loki-managed security file (~/.loki/config.yaml or
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.loki/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / LOKI_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) Loki home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\loki), so folding the user home
    # first would eat the prefix the Loki-home fold needs.
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / LOKI_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.loki``); the matched
...
def _rewrite_resolved_loki_home(command: str) -> str:
    """Rewrite the resolved absolute Loki home prefix to ``~/.loki/``.

    Resolves the active ``LOKI_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.loki/`` so the static ``_LOKI_CONFIG_PATH`` / ``_LOKI_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/loki/.loki/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$LOKI_HOME``. Matches both POSIX and Windows separators. No-op when the
    ...
    try:
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.loki")
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    try:
        ...
        if "LOKI_SPINNER_PAUSE" in os.environ:
            del os.environ["LOKI_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def is_approval_bypass_active() -> bool:
    """Return True when the user has opted out of Loki approval prompts.

    Collapses the canonical three-source bypass check used across the codebase
    into one place:
      - process-scoped ``--yolo`` / ``LOKI_YOLO_MODE`` (frozen at import time
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s — set LOKI_INTERACTIVE or LOKI_GATEWAY_SESSION to require approval.",
            description, command[:200],
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("LOKI_EXEC_ASK"):
