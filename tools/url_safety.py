...
def normalize_url_for_request(url: str) -> str:
    """Return an ASCII-safe HTTP URL for Loki-owned URL tools.
...
def _global_allow_private_urls() -> bool:
    ...
    1. ``LOKI_ALLOW_PRIVATE_URLS`` env var  (``true``/``1``/``yes``)
...
def is_safe_url(url: str) -> bool:
    ...
    ``LOKI_ALLOW_PRIVATE_URLS=true``), private-IP blocking is skipped.
