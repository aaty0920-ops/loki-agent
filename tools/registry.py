"""Central registry for all loki-agent tools.
...
# timescales. Cache results for ~30 s so env-var flips via ``loki tools``
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``loki tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def _plugin_owner_of(self, handler: Callable) -> Optional[str]:
        ...
        if isinstance(mod, str) and mod.startswith("loki_plugins."):
            return mod
        return None
    ...
    def deregister(self, name: str) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # (``loki_plugins.{name}``), not the exact module string.
                # A handler defined in ``loki_plugins.pkg.handlers`` is
                # still owned by the ``loki_plugins.pkg`` package — exact
                ...
                if (
                    caller_mod.startswith("loki_plugins.")
                    and not same_plugin
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``loki tools enable foo``)
