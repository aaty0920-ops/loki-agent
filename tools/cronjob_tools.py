"""
Cron job management tools for Loki Agent.
...
#      saying "cat ~/.loki/.env" or "rm -rf /". `_scan_cron_prompt()` runs
...
#      `loki-agent-dev` skill contains a security postmortem mentioning
#      `cat ~/.loki/.env`, which tripped `read_secrets` and silently
...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    if origin_platform and origin_chat_id:
        ...
        return {
            ...
            # send_message, which passes LOKI_SESSION_USER_ID to
...
def _local_delivery_notice(job: Dict[str, Any], user_deliver: Optional[str]) -> Optional[str]:
    ...
    ``LOKI_SESSION_PLATFORM``/``CHAT_ID`` is set for them), so a
...
def _resolve_model_override(model_obj: Optional[Dict[str, Any]]) -> tuple:
    ...
    job doesn't drift when the user later changes their default via loki model.
    ...
    if provider_name == "custom":
        try:
            from loki_cli.runtime_provider import has_named_custom_provider
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    Scripts must be relative paths that resolve within LOKI_HOME/scripts/.
    ...
    # Only relative paths within ~/.loki/scripts/ are allowed.
...
Important safety rule: cron-run sessions should not recursively schedule more cron jobs.""",
    "parameters": {
        ...
        "properties": {
            ...
            "script": {
                "type": "string",
                "description": f"Optional path to a script that runs each tick. In the default mode its stdout is injected into the agent's prompt as context (data-collection / change-detection pattern). With no_agent=True, the script IS the job and its stdout is delivered verbatim (classic watchdog pattern). Relative paths resolve under {display_loki_home()}/scripts/. ``.sh``/``.bash`` extensions run via bash, everything else via Python. On update, pass empty string to clear."
            },
            "no_agent": {
                "type": "boolean",
                "default": False,
