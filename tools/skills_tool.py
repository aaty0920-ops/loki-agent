...
# All skills live in ~/.loki/skills/ (seeded from bundled skills/ on install).
...
def _capture_required_environment_variables(
    ...
    # — the desktop app / TUI — set LOKI_INTERACTIVE and register a
    # secret-capture callback that routes to a secure secret.request overlay, so
    # they fall through and actually prompt. (LOKI_INTERACTIVE is the same flag
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_loki_home()}/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.loki/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``LOKI_SESSION_PLATFORM``.
    """
    try:
        from gateway.session_context import get_session_env
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``LOKI_PLATFORM`` environment variable
    3. ``LOKI_SESSION_PLATFORM`` from gateway session context
    """
    try:
        from loki_cli.config import load_config
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("LOKI_PLATFORM") or _get_session_platform()
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.loki/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``loki skills`` config UI). Default False
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": f"No skills found. Skills directory created at {display_loki_home()}/skills/",
                },
                ensure_ascii=False,
            )
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: loki plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
    ...
    try:
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.loki/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `loki skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        # Check metadata.loki.* first (agentskills.io convention), fall back to top-level
        loki_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            loki_meta = metadata.get("loki", {}) or {}

        tags = _parse_tags(loki_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            loki_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
