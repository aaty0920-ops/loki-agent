...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.loki/config.yaml``. Loki
...
Configuration is loaded from ~/.loki/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``loki_cli.config.get_env_value``
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.loki/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.loki/config.yaml.
...
# custom voice-cloning scripts, etc.) into Loki without any Python code
...
#           output_format: wav
#
# Loki writes the input text to a temp UTF-8 file, runs the command with
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__LOKI_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    # output_format when Loki actually needs a non-default format/override.
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Loki caches Piper voice models.

    Resolves to ``~/.loki/cache/piper-voices/`` under the active
    LOKI_HOME so voice downloads follow profile boundaries.
    """
    from loki_constants import get_loki_dir
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.loki/config.yaml (tts: section).
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run: pip install 'loki-agent[mistral]'"
                }, ensure_ascii=False)
