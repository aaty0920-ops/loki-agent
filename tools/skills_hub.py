#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Loki Skills Hub.
...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _loki_home() / "skills"
...
class OptionalSkillSource(SkillSource):
    ...
    OFFICIAL_REPO = "NousResearch/loki-agent"
...
# ---------------------------------------------------------------------------
# Loki centralized index source
# ---------------------------------------------------------------------------

LOKI_INDEX_URL = "https://loki-agent.nousresearch.com/docs/api/skills-index.json"
...
def _load_loki_index() -> Optional[dict]:
    ...
    We cache it locally for LOKI_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    loki_index_cache_file = _loki_index_cache_file()
    if loki_index_cache_file.exists():
        try:
            age = time.time() - loki_index_cache_file.stat().st_mtime
            if age < LOKI_INDEX_TTL:
                return json.loads(loki_index_cache_file.read_text())
        except (OSError, json.JSONDecodeError):
            pass
    ...
    # Cache locally
    try:
        loki_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        loki_index_cache_file.write_text(json.dumps(data))
    except OSError:
        pass
...
class LokiIndexSource(SkillSource):
    """Skill source backed by the centralized Loki Skills Index.
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "loki-index"),
            identifier=entry.get("identifier", ""),
