...
``$LOKI_HOME/workspace/meetings/<meeting-id>/``. The main loki process
...
Run standalone for debugging::
    ...
    LOKI_MEET_URL=https://meet.google.com/abc-defg-hij \\
    LOKI_MEET_OUT_DIR=/tmp/meet-debug \\
    LOKI_MEET_HEADED=1 \\
...
# Filenames the bot reads/writes in ``LOKI_MEET_OUT_DIR``.
...
# and exposes ``window.__lokiMeetDrain()`` to pull new entries. This
...
def _start_realtime_speaker(
    *,
    ...
    if platform_tag == "linux":
        ...
        sink = (bridge_info or {}).get("write_target") or "loki_meet_sink"
        try:
            proc = _sp.Popen(
                [
                    "paplay",
...
def run_bot() -> int:  # noqa: C901 — orchestration, explicit branches
    url = os.environ.get("LOKI_MEET_URL", "").strip()
    out_dir_env = os.environ.get("LOKI_MEET_OUT_DIR", "").strip()
    headed = os.environ.get("LOKI_MEET_HEADED", "").lower() in {"1", "true", "yes"}
    auth_state = os.environ.get("LOKI_MEET_AUTH_STATE", "").strip()
    guest_name = os.environ.get("LOKI_MEET_GUEST_NAME", "Loki Agent")
    duration_s = _parse_duration(os.environ.get("LOKI_MEET_DURATION", ""))
    # v2: optional realtime mode. Enabled when LOKI_MEET_MODE=realtime.
    mode = os.environ.get("LOKI_MEET_MODE", "transcribe").strip().lower()
    realtime_model = os.environ.get("LOKI_MEET_REALTIME_MODEL", "gpt-realtime")
    realtime_voice = os.environ.get("LOKI_MEET_REALTIME_VOICE", "alloy")
    realtime_instructions = os.environ.get("LOKI_MEET_REALTIME_INSTRUCTIONS", "")
    realtime_api_key = os.environ.get("LOKI_MEET_REALTIME_KEY") or os.environ.get("OPENAI_API_KEY", "")

    if not url or not _is_safe_meet_url(url):
        sys.stderr.write(
            "google_meet bot: refusing to launch — LOKI_MEET_URL must be a "
    ...
    try:
        with sync_playwright() as pw:
            ...
            lobby_deadline = time.time() + float(
                os.environ.get("LOKI_MEET_LOBBY_TIMEOUT", "300")
            )
