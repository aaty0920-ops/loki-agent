"""`loki meet node ...` subcommand tree.

Wired into the existing ``loki meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``loki meet node`` argparse object — typically
...
def node_command(args: argparse.Namespace) -> int:
    ...
    if cmd == "run":
        ...
        print(f"             loki meet node approve <name> ws://<host>:{args.port} {token}")
        try:
