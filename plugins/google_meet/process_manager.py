...
session-scoped state file under ``$LOKI_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $LOKI_HOME):
#
#   workspace/meetings/
...
def start(
    ...
    *,
    ...
    If a bot is already running for this loki install, leave it first —
    ...
    env = os.environ.copy()
    env["LOKI_MEET_URL"] = url
    env["LOKI_MEET_OUT_DIR"] = str(out)
    env["LOKI_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["LOKI_MEET_HEADED"] = "1"
    if auth_state:
        env["LOKI_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["LOKI_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if LOKI_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["LOKI_MEET_MODE"] = mode
    if realtime_model:
        env["LOKI_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["LOKI_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["LOKI_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
