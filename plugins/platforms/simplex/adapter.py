"""SimpleX Chat platform adapter (Loki plugin).
...
This adapter ships as a Loki platform plugin under
``plugins/platforms/simplex/``. The Loki plugin loader scans the
...
discoverable and ``loki setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Loki — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("LOKI_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``loki setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.loki/.env`` via
    ``loki_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "loki_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.loki/.env"
        )
        return
