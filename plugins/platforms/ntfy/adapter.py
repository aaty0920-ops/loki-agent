"""ntfy platform adapter (Loki plugin).
...
``plugins/platforms/ntfy/``. The Loki plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "loki-in"              # subscribe topic (incoming)
          publish_topic: "loki-out"     # optional — defaults to topic
...
async def _standalone_send(
    ...
    *,
    ...
    ``loki cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Loki dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `loki gateway status` without
