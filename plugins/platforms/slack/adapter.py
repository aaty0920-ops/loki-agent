...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `loki slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Loki does not currently need to route them into the agent loop.
            ...
            # /model work directly without /loki prefix). A single regex
            ...
            # manifest (see `loki slack manifest`). In Socket Mode, Slack
            ...
            from loki_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/loki$")
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Loki handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Loki session — matching the per-thread behavior channels already
    ...
    async def _handle_slack_message(self, event: dict) -> None:
        ...
        if original_text.startswith("!"):
            try:
                ...
                # forms like ``!stop@loki`` still resolve.
                ...
                },
                {
                    ...
                    "elements": [
                        ...
                        },
                        {
                            ...
                            "action_id": "loki_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "loki_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "loki_confirm_once": "once",
            "loki_confirm_always": "always",
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "loki_approve_once": "once",
            "loki_approve_session": "session",
            "loki_approve_always": "always",
            "loki_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/loki <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/loki
        ...
        if slash_name in {"loki", ""}:
            # Legacy /loki <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from loki_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/loki   ``).
        ...
        # "Running /cmd…" ack shown by handle_loki_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/loki <question>" must produce public replies so
...
# ``loki_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``loki_cli/setup.py::_setup_slack``.
    """
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under LOKI_HOME, and print
        ...
        try:
            ...
            manifest = _build_full_manifest(
                bot_name="Loki",
                bot_description="Your Loki agent on Slack",
            )
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `loki update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://loki-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``loki_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'loki-agent[slack]'",
        # Interactive setup wizard — replaces loki_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in loki_cli/gateway.py.
