"""
LINE Messaging API platform adapter for Loki Agent.
...
to Loki Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``LOKI_HOME``. PR #8398.
        """
        from aiohttp import web
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``loki status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``loki setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.loki/.env`` via ``loki_cli.config``.
    """
    print()
    ...
    try:
        from loki_cli.config import get_env_var, set_env_var
    except ImportError:
