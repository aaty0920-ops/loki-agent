"""
IRC Platform Adapter for Loki Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Loki agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "loki-bot")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: loki_, loki_1, loki_2...
...
def check_requirements() -> bool:
    ...
    # loki setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `loki gateway setup` flow for the IRC platform.

    Lazy-imports ``loki_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Loki to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #loki — comma-separate for multiple)",
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "loki-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "loki-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Loki Agent (cron)")
