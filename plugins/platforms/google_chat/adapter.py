...
# ``model_tools`` import time, so ``loki status``, ``loki chat``, etc.
...
# ``loki_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Loki is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a loki MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${LOKI_HOME}/google_chat_thread_counts.json so
        ...
        self._thread_count_store = _ThreadCountStore(
            _loki_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Loki is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("LOKI_HOME", str(_Path.home() / ".loki"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${LOKI_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Loki is thinking…' marker message.
        ...
        "Loki is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Loki is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Loki is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Loki is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Loki is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``loki setup``.

    The setup wizard at ``loki_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.loki/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. loki-chat-events) and a Pull subscription.")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``loki cron`` running as a
    separate process from ``loki gateway``).  Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'loki-agent[google_chat]'",
        ...
        # the "Loki is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Loki is thinking…' marker message "
