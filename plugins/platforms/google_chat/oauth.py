...
- Per-user tokens (keyed by sender email):
    ``${LOKI_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${LOKI_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${LOKI_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${LOKI_HOME}/google_chat_user_oauth_pending.json``
- OAuth client secret (profile-scoped — each profile registers its own):
    ``${LOKI_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's LOKI_HOME helper so the token follows the user's
# profile (e.g. tests can override via LOKI_HOME=/tmp/...).
try:
    from loki_constants import display_loki_home, get_loki_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where loki_constants isn't importable
...
def _loki_home() -> Path:
    """Resolve LOKI_HOME at call time (NOT module import).

    Tests and ``LOKI_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.loki/google_chat_user_tokens/`` trivial.
...
def _legacy_token_path() -> Path:
    return _loki_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _loki_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _loki_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _loki_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _loki_home() / "google_chat_user_client_secret.json"
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Install loki-agent[google_chat]."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("  pip install 'loki-agent[google_chat]'")
        return False
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_loki_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_loki_home()}/google_chat_user_token.json"
    )
