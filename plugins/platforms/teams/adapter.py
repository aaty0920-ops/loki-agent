"""
Microsoft Teams platform adapter for Loki Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def _on_card_action(
        ...
        choice = choice_map.get(loki_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="loki_approve",
                    data={**btn_data_base, "loki_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="loki_approve",
                    data={**btn_data_base, "loki_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="loki_approve",
                    data={**btn_data_base, "loki_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="loki_approve",
                    data={**btn_data_base, "loki_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Loki\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
