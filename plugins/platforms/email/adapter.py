"""
Email platform adapter for the Loki gateway.

Allows users to interact with Loki by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "loki-agent" "version" "{_loki_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `loki config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `loki gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
...
# loki_cli/gateway.py, and the _send_email dispatch in
