...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.loki/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._liveness_interval_seconds = env_float(
            "LOKI_DISCORD_LIVENESS_INTERVAL_SECONDS", 60.0
        )
        self._liveness_failure_threshold = env_int(
            "LOKI_DISCORD_LIVENESS_FAILURE_THRESHOLD", 3
        )
    ...
    def _handle_bot_task_done(self, task: asyncio.Task) -> None:
        ...
        marked running, the Discord websocket is dead while the Loki gateway
    ...
    async def cancel_background_tasks(self) -> None:
        ...
        disconnect budget (``LOKI_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT``, default
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Loki session")
        ...
        @tree.command(name="status", description="Show Loki session status")
        ...
        @tree.command(name="stop", description="Stop the running Loki agent")
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.loki/skills/ for new or removed skills")
        ...
        @tree.command(name="update", description="Update Loki Agent to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Loki gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a Loki session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Loki in the thread",
        ...
        # loki_cli/commands.py automatically appear as Discord slash
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from loki_cli.commands import discord_skill_commands_by_category
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Loki session in the thread
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``loki update --gateway``
...
def _component_check_auth(
    ...
    the pairing store (``loki pairing approve``).
    ...
    # so users approved via ``loki pairing approve`` can interact with
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``loki update`` prompts.
...
# is not in this process (e.g. ``loki cron`` running standalone) and no live
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    if lis is not None and not os.getenv("LOKI_DISCORD_LIVENESS_INTERVAL_SECONDS"):
        os.environ["LOKI_DISCORD_LIVENESS_INTERVAL_SECONDS"] = str(lis)
    lft = discord_cfg.get("liveness_failure_threshold")
    if lft is not None and not os.getenv("LOKI_DISCORD_LIVENESS_FAILURE_THRESHOLD"):
        os.environ["LOKI_DISCORD_LIVENESS_FAILURE_THRESHOLD"] = str(lft)
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``loki_cli.gateway.get_env_value`` at call time (not via
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'loki-agent[messaging]'",
        # Interactive setup wizard — replaces the central
        # loki_cli/setup.py::_setup_discord function.  Same shape as Teams.
