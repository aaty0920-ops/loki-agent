...
Credential storage mirrors every other Loki channel:

    * runtime SDK creds  -> ``~/.loki/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.loki/auth.json`` under
...
# registers Loki as its own client_id.
...
# Default name of the project Loki provisions for the operator.
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of loki-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.loki/auth.json`` honouring the active Loki profile."""
    try:
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.loki/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.loki/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.loki/.env`` (canonical runtime store).
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `loki photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `loki photon setup`)"
    )
