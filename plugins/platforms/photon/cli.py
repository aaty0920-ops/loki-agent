"""
``loki photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Loki
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Loki Agent')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``loki photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Loki Agent" project.
    ...
    # 3. Rotate the project secret and persist creds (runtime -> ~/.loki/.env,
    ...
    print("  Start the gateway:  loki gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `loki photon install-sidecar`'}")
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.loki/.env.
...
#
# `loki gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``loki photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `loki gateway setup` wizard."""
