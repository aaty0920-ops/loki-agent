"""
Photon Spectrum (iMessage) platform adapter for Loki Agent.
...
def check_requirements() -> bool:
    ...
    if not (_SIDECAR_DIR / "node_modules").exists():
        # spectrum-ts not installed yet — `loki photon setup` will
        # install it.  check_fn still returns False so the gateway
        # surfaces the missing-deps state in `loki setup` / status.
        return False
    return True
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use Loki defaults).
    ...
    async def _inbound_loop(self) -> None:
        ...
        headers = {"X-Loki-Sidecar-Token": self._sidecar_token}
        backoff = 1.0
    ...
    def _pid_is_sidecar(pid: int) -> bool:
        ...
        try:
            ...
            return False
        # Checkout-agnostic: any Loki checkout's sidecar entry point.
    ...
    async def _start_sidecar(self) -> None:
        if not (_SIDECAR_DIR / "node_modules").exists():
            raise RuntimeError(
                ...
                f"cd {_SIDECAR_DIR} && npm install   (or `loki photon setup`)"
            )
...
def register(ctx) -> None:
    ...
    # gateway-setup hook and the `loki photon` CLI command below.
    ...
    ctx.register_platform(
        ...
        # Surfaces Photon in `loki gateway setup` alongside every other
    ...
    # Register CLI subcommands — `loki photon ...`
