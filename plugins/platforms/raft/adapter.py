...
and injects content-free wake hints into Loki' normal gateway session pipeline.
...
class RaftAdapter(BasePlatformAdapter):
    ...
    async def _handle_wake(self, request: "web.Request") -> "web.Response":
        ...
        # Loki only verifies that wake hints are content-free.
...
def interactive_setup() -> None:
    """Interactive ``loki gateway setup`` flow for the Raft platform.

    Lazy-imports CLI helpers so the plugin stays importable in gateway runtime
    and test contexts. The flow persists ``RAFT_PROFILE`` to the Loki env
    ...
    print_info("Connect Loki to Raft as an external agent.")
