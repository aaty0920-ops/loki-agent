...
``_check_spotify_available()`` — when the user has not run ``loki auth
spotify``, the tools remain registered (so they appear in ``loki
...
The Spotify auth flow (``loki auth spotify``), CLI plumbing, and docs
