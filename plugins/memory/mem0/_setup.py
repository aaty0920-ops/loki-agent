...
def _save_mem0_json(loki_home: str, data: dict) -> None:
...
def _setup_oss(loki_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "loki-user")
    ...
    if env_writes:
        _write_env(Path(loki_home) / ".env", env_writes)
    _save_mem0_json(loki_home, {"mode": "oss", "user_id": user_id, "agent_id": "loki", "oss": oss_config})
...
_PGVECTOR_CONTAINER = "loki-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "loki"
...
def _setup_oss_interactive(loki_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], loki_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], loki_home)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'loki-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "loki-user")

    agent_id = input("  Agent ID [loki]: ").strip()
    agent_id = agent_id or "loki"
    ...
    if env_writes:
        _write_env(Path(loki_home) / ".env", env_writes)
    _save_mem0_json(loki_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(loki_home: str, config: dict) -> None:
    """Entry point called by loki memory setup framework.
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(loki_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(loki_home, config, flags)
        return
