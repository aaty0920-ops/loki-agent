...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Loki peer ID in OpenViking (default: loki)
...
_DEFAULT_AGENT = "loki"
_AGENT_PROMPT_LABEL = "Loki peer ID in OpenViking"
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("LOKI_HOME", "")).expanduser() if os.environ.get("LOKI_HOME") else Path.home() / ".loki"
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See loki-agent#28296 review.
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Loki peer ID in OpenViking, sent as the actor peer and "
    ...
    def post_setup(self, loki_home: str, config: dict) -> None:
        ...
        loki_home_path = Path(loki_home)
        env_path = loki_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            if health_state == "unreachable":
                ...
                _emit_runtime_warning(
                    f"{health_message} OpenViking memory disabled for this Loki run.",
                    warning_callback,
                )
                self._client = None
        except ImportError:
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Loki' full canonical transcript."""
        if not messages:
            return []
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See loki-agent#28296.
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/loki/.overview.md).
