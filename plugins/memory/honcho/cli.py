...
Handles: loki honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "loki" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Loki profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'loki honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'loki honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `loki update` -- no output, no exceptions.
    """
    try:
        from loki_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Loki profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $LOKI_HOME/honcho.json so each profile/instance gets
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        loki_host.get("pinUserPeer"),
        loki_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in loki_host:
        aliases_src = loki_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in loki_host:
        prefix_src = loki_host.get("runtimePeerPrefix")
...
def _gateway_platforms() -> list[str] | None:
    ...
    guarded (matching the idiom loki_cli already uses for gateway refs).
    """
    try:
...
def _apply_runtime_prefix(
    loki_host: dict, current_prefix: str, prefix_from_root: bool, label: str
) -> None:
...
def _echo_identity_mapping(loki_host: dict) -> None:
    ...
    aliases = loki_host.get("userPeerAliases")
    prefix = loki_host.get("runtimePeerPrefix")
    print("  resolved →")
    print(f"    pinUserPeer       = {bool(loki_host.get('pinUserPeer'))}")
...
def _configure_raw_identity_mapping(
    ...
    _apply_runtime_prefix(
        loki_host, current_prefix, prefix_from_root,
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Loki persistent cross-session memory.")
    ...
    loki_host = hosts.setdefault(_host_key(), {})
    ...
    if not is_local:
        ...
        if use_oauth:
            ...
            try:
                ...
                print("  Re-run 'loki honcho setup' to retry, or choose an API key instead.\n")
                return
            ...
            if not cfg.get("apiKey"):
                ...
                print("  Run 'loki honcho setup' again once you have a key.\n")
                return
    ...
    if new_peer:
        loki_host["peerName"] = new_peer

    current_ai = loki_host.get("aiPeer") or cfg.get("aiPeer", "loki")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        loki_host["aiPeer"] = new_ai

    current_workspace = loki_host.get("workspace") or cfg.get("workspace", "loki")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        loki_host["workspace"] = new_workspace

    # --- 3b. Gateway identity mapping ---
    # These keys only affect the Loki GATEWAY (Telegram/Discord/Slack/...),
    ...
    if gw_platforms is None:
        ...
        run_mapping = _prompt(
            "Running the Loki gateway (Telegram/Discord/etc.)? (y/N)",
            default="n",
    ...
    if run_mapping:
        ...
        if shape == "single":
            ...
            _scrub_identity_mapping(loki_host)
            loki_host["pinUserPeer"] = False
            if prior_aliases:
                loki_host["userPeerAliases"] = prior_aliases
            _apply_runtime_prefix(
                loki_host, current_prefix, prefix_from_root,
            ...
            if merged:
                loki_host["userPeerAliases"] = merged
            _apply_runtime_prefix(
                loki_host, current_prefix, prefix_from_root,
            ...
            _configure_raw_identity_mapping(
                loki_host, current_pin, current_aliases, current_prefix,
    ...
    # --- 4. Observation mode ---
    current_obs = loki_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        loki_host["observationMode"] = new_obs
    else:
        loki_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(loki_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        loki_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        loki_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = loki_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        loki_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = loki_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    current_dialectic = str(loki_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    current_reasoning = (
        loki_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        loki_host["dialecticReasoningLevel"] = new_reasoning
    else:
        loki_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = loki_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        loki_host["sessionStrategy"] = new_strat
    ...
    try:
        ...
        print("  Run: loki config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    loki honcho status     -- show full config")
    print("    loki honcho mode       -- change recall/observation mode")
    print("    loki honcho tokens     -- tune context and dialectic budgets")
    print("    loki honcho peer       -- update peer names")
    print("    loki honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Loki profile name (respects --target-profile override)."""
    if _profile_override:
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'loki honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'loki honcho setup' to configure.\n")
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: loki honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        loki = hosts.get(_host_key(), {})
        user = loki.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = loki.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = loki.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = loki.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Loki' identity in Honcho. Seed with 'loki honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: loki honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: loki honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = loki.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = loki.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = loki.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Loki asks Honcho's AI peer a question")
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'loki honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'loki honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    loki honcho identity --show        — show both peer representations")
        print("    loki honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    ...
    if has_key:
        ...
        print("  2. Run:  loki honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'loki honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'loki honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  loki honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'loki'")
        print("  with Honcho configured and no prior session history.")
        print("  (Loki calls migrate_memory_files() on first session init.)")
        print()
    ...
    print()
    print("  In Loki, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            for f in agent_files:
                print(f"    loki honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  loki honcho identity <path/to/SOUL.md>")
    ...
    print("    Loki:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Loki:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Loki:   Honcho observes every message and updates representations")
    ...
    print("    Loki:   per-session by default — each run gets its own session")
    print("              Map a custom name:  loki honcho map <session-name>")
    ...
    if not has_key:
        ...
        print("  2. loki honcho migrate            — re-run this walkthrough")
    else:
        print("  1. loki honcho status             — verify Honcho connection")
        print("  2. loki                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. loki honcho identity --show    — verify AI peer representation")
        print("  4. loki honcho tokens             — tune context and dialectic budgets")
        print("  5. loki honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'loki memory setup'...\n")
...
def register_cli(subparser) -> None:
    """Build the ``loki honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``loki honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to loki memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Loki Honcho",
    )
