...
Resolution order for config file:
  1. $LOKI_HOME/honcho.json  (instance-local, enables isolated Loki instances)
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Loki profile.

    Resolution order:
      1. LOKI_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``loki.<profile>``
      3. Fallback: ``"loki"`` (default profile)
    """
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $LOKI_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.loki/honcho.json          (default profile — shared host blocks live here)
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Loki system prompt
    ...
    # True when Honcho was explicitly configured for this host (hosts.loki
    ...
    def from_global_config(
        ...
        Resolution: $LOKI_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active Loki profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.loki block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. per-session strategy — Loki session_id ({timestamp}_{hex}); authoritative,
             so a generated title never remaps a live conversation
          3. Manual directory override from sessions map
          4. Loki session title (from /title command; non-per-session)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    def _build() -> "Honcho":
        ...
        # the "go run loki honcho setup" hint they used to.
        try:
        ...
        if not resolved_base_url or resolved_timeout is None:
            try:
                ...
                loki_cfg = load_config()
                honcho_cfg = loki_cfg.get("honcho", {})
