...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with loki-agent)
2. User-installed providers: ``$LOKI_HOME/plugins/<name>/``
...
def _register_synthetic_package(name: str, search_locations: List[str]) -> None:
    ...
    User-installed providers import as ``_loki_user_memory.<name>``, a
    ...
    ``ModuleNotFoundError: No module named '_loki_user_memory'`` — the
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$LOKI_HOME/plugins/<name>/``) directories.  Bundled takes
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    try:
        ...
        if module_name in sys.modules:
            ...
            if not _is_bundled:
                # cli.py imports as _loki_user_memory.<name>.cli, usually
