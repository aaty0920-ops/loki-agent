...
def _load_supermemory_config(loki_home: str) -> dict:
...
def _resolve_container_tag_for_setup(loki_home: str, *, identity: str = "default") -> str:
...
def _probe_supermemory_connection(api_key: str, loki_home: str, *, identity: str = "default") -> dict:
    ...
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(loki_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `loki memory setup`.
        # All other options are documented for $LOKI_HOME/supermemory.json
    ...
    def post_setup(self, loki_home: str, config: dict) -> None:
        ...
        if env_writes:
            _write_env_vars(Path(loki_home) / ".env", env_writes)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._turn_count = 0
        self._config = _load_supermemory_config(self._loki_home)
