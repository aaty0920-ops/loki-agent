...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: loki)
...
Or via $LOKI_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Loki can degrade gracefully instead of repeatedly trying to start
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $LOKI_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_loki_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "loki": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "loki"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Loki config."""
    profile = config.get("profile", "loki")
    return str(profile or "loki")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Loki profile name (from agent_identity)
      {workspace} — Loki workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``loki-{user}`` with no user becomes ``loki``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Loki Agent and the User"
    ...
    def save_config(self, values, loki_home):
        """Write config to $LOKI_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(loki_home) / "hindsight"
    ...
    def post_setup(self, loki_home: str, config: dict) -> None:
        ...
        self.save_config(provider_config, loki_home)
        ...
        if mode == "local_embedded":
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(loki_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "loki"), llm_provider)
                kwargs = dict(
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "loki", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "loki")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Loki Agent and the User")
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Loki created/used that client on the shared async loop,
