# Loki Achievements Implementation Spec (Detailed)
...
- persisted runtime files:
  - `~/.loki/plugins/loki-achievements/state.json` (existing unlock state)
  - `~/.loki/plugins/loki-achievements/scan_snapshot.json` (new)
  - `~/.loki/plugins/loki-achievements/scan_checkpoint.json` (new)
