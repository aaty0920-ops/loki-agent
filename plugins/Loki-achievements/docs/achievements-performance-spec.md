# Loki Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: loki-achievements plugin
...
### `GET /api/plugins/loki-achievements/achievements`
...
### `POST /api/plugins/loki-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/loki-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/loki-achievements/overview`
...
- Persist plugin data under:
  - `~/.loki/plugins/loki-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.loki/plugins/loki-achievements/scan_checkpoint.json`
...
- `~/.loki/plugins/loki-achievements/`
