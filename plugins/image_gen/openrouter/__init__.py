...
agent's existing :func:`~loki_cli.runtime_provider.resolve_runtime_provider`,
...
# image_gen.model from ``loki tools``): use exactly that model (no auto
...
class OpenRouterCompatImageProvider(ImageGenProvider):
    ...
    def _resolve_model_chain(self, explicit: Optional[str] = None) -> list[str]:
        ...
        by ``loki tools``) → the quality-first default chain.
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        headers = {
            ...
            "HTTP-Referer": "https://github.com/NousResearch/loki-agent",
            "X-Title": "Loki Agent",
        }
