"""disk_cleanup — ephemeral file cleanup for Loki Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under LOKI_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly LOKI_HOME and /tmp/loki-*
Never touches: ~/.loki/logs/ or any system directory.
"""
...
try:
    ...
    def get_loki_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("LOKI_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".loki").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$LOKI_HOME/logs/``."""
    return get_loki_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$LOKI_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under LOKI_HOME or ``/tmp/loki-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    loki_home = get_loki_home()
    try:
        path.resolve().relative_to(loki_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/loki-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("loki-"):
        return True
    return False
...
# Paths under $LOKI_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so LOKI_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        loki_home = get_loki_home()
        for parent in ("cron", "cronjobs"):
            base = loki_home / parent
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under LOKI_HOME, but never recurse into known
    # durable state trees.  Some installs place the Loki checkout, venv,
    # and desktop build under LOKI_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    loki_home = get_loki_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in loki_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    loki_home = get_loki_home()
    try:
        rel = path.resolve().relative_to(loki_home)
        ...
        # Path isn't under LOKI_HOME (e.g. /tmp/loki-*) — fall through.
        pass
