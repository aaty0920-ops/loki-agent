...
``{title, url, description, position}`` rows every other Loki web
provider produces.
...
prefers Loki-managed xAI Grok OAuth (via ``loki auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.loki/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``loki tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `loki auth` to sign in with "
