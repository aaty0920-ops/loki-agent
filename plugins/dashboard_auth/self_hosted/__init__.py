...
A standards-compliant OpenID Connect Relying Party for the ``loki dashboard``
...
:class:`~loki_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``loki_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~loki_cli.dashboard_auth.Session`.
    ...
    dashboard:
      oauth:
        ...
        self_hosted:
          ...
          # credential — prefer the env var / ~/.loki/.env over config.yaml.

    # Environment overrides (Docker/Fly secret injection)
    LOKI_DASHBOARD_OIDC_ISSUER
    LOKI_DASHBOARD_OIDC_CLIENT_ID
    LOKI_DASHBOARD_OIDC_SCOPES        # optional; defaults to "openid profile email"
    LOKI_DASHBOARD_OIDC_CLIENT_SECRET # optional; set for a confidential client
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — Loki does not call
...
def register(ctx) -> None:
    ...
    client_id are configured (via ``LOKI_DASHBOARD_OIDC_*`` env vars or the
    ...
    issuer = _resolve_setting(
        "LOKI_DASHBOARD_OIDC_ISSUER", oidc_cfg.get("issuer")
    )
    client_id = _resolve_setting(
        "LOKI_DASHBOARD_OIDC_CLIENT_ID", oidc_cfg.get("client_id")
    )
    scopes = (
        _resolve_setting("LOKI_DASHBOARD_OIDC_SCOPES", oidc_cfg.get("scopes"))
        or _DEFAULT_SCOPES
    )
    # Optional — set only for a confidential client. A credential, so the
    # canonical home is the env var / ~/.loki/.env; config.yaml is supported
    # for precedence symmetry. Empty ⇒ public client (unchanged behaviour).
    client_secret = _resolve_setting(
        "LOKI_DASHBOARD_OIDC_CLIENT_SECRET", oidc_cfg.get("client_secret")
    )
    ...
    if not issuer or not client_id:
        LAST_SKIP_REASON = (
            ...
            "(LOKI_DASHBOARD_OIDC_ISSUER + LOKI_DASHBOARD_OIDC_CLIENT_ID) "
