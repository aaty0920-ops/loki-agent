"""langfuse — Loki plugin for Langfuse observability.

Traces Loki conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Loki plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``loki plugins enable
observability/langfuse`` or ``loki tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``loki tools`` or ~/.loki/.env):
  LOKI_LANGFUSE_PUBLIC_KEY  - Langfuse project public key (pk-lf-...)
  LOKI_LANGFUSE_SECRET_KEY  - Langfuse project secret key (sk-lf-...)
  LOKI_LANGFUSE_BASE_URL    - Langfuse server URL (default: https://cloud.langfuse.com)

Optional env vars:
  LOKI_LANGFUSE_ENV         - environment tag (e.g. "production", "local")
  LOKI_LANGFUSE_RELEASE     - release/version tag
  LOKI_LANGFUSE_SAMPLE_RATE - sampling rate 0.0–1.0 (default: 1.0)
  LOKI_LANGFUSE_MAX_CHARS   - max chars per field (default: 12000)
  LOKI_LANGFUSE_DEBUG       - set to "true" for verbose logging
"""
...
def _debug_enabled() -> bool:
    return _env_bool("LOKI_LANGFUSE_DEBUG")
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Loki plugin system —
    ...
    public_key = _env("LOKI_LANGFUSE_PUBLIC_KEY") or _env("LANGFUSE_PUBLIC_KEY")
    secret_key = _env("LOKI_LANGFUSE_SECRET_KEY") or _env("LANGFUSE_SECRET_KEY")
    ...
    base_url = _env("LOKI_LANGFUSE_BASE_URL") or _env("LANGFUSE_BASE_URL") or "https://cloud.langfuse.com"
    environment = _env("LOKI_LANGFUSE_ENV") or _env("LANGFUSE_ENV")
    release = _env("LOKI_LANGFUSE_RELEASE") or _env("LANGFUSE_RELEASE")
...
def _trace_key(
    ...
    *,
    ...
    Older Loki paths only expose ``task_id``/``session_id``. Newer paths
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Loki branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Loki also has a turn-scoped
    ...
    # messages is a list only for legacy Loki branches that fired
    # pre_llm_call with API messages directly. Current Loki fires
...
def register(ctx) -> None:
    ...
    # Loki versions.  pre_api_request / post_api_request fire per API
