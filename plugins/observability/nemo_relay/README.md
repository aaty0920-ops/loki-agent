...
Optional Loki observability plugin that maps Loki observer hooks to
...
not replace Loki Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Loki emit NeMo Relay lifecycle events for
the work Loki already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Loki Agent can:

- Preserve Loki execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
loki plugins enable observability/nemo_relay
```

The `LOKI_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `LOKI_HOME` that the
agent run will use:

```bash
env LOKI_HOME=/tmp/loki-nemo-relay-test \
  loki plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`LOKI_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`LOKI_HOME` is the Loki profile/config home used by both
`loki plugins enable ...` and the later `loki chat ...` run. If unset,
Loki uses the user's default home, usually `~/.loki`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export LOKI_HOME=/tmp/loki-nemo-relay-test
loki plugins enable observability/nemo_relay
loki chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `loki` command you run is built from the
...
uv run loki plugins enable observability/nemo_relay
uv run loki chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/loki_agent-*.whl
...
The plugin can configure exporters directly from `LOKI_NEMO_RELAY_*`
...
```bash
export LOKI_NEMO_RELAY_ATOF_ENABLED=1
export LOKI_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export LOKI_NEMO_RELAY_ATIF_ENABLED=1
export LOKI_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `LOKI_NEMO_RELAY_ATOF_FILENAME`
- `LOKI_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
...
- `LOKI_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Loki at it:

```bash
export LOKI_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
agent_name = "Loki Agent"
agent_version = "local"
```

When `LOKI_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`LOKI_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`LOKI_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Loki does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Loki keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Loki routes LLM and tool
...
For the full generic Loki middleware contract, see
...
export LOKI_HOME=/tmp/loki-nemo-relay-docs/loki-home
mkdir -p "$LOKI_HOME"

cat > "$LOKI_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Loki session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export LOKI_NEMO_RELAY_ATOF_ENABLED=1
export LOKI_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/loki-nemo-relay-docs/subagent/atof
export LOKI_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export LOKI_NEMO_RELAY_ATOF_MODE=overwrite
export LOKI_NEMO_RELAY_ATIF_ENABLED=1
export LOKI_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/loki-nemo-relay-docs/subagent/atif
export LOKI_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export LOKI_NEMO_RELAY_ATIF_AGENT_NAME='Loki Agent E2E'
export LOKI_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export LOKI_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"loki.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Loki Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Loki dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/loki-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/loki-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/loki-nemo-relay-docs/workdir/beta.txt
cd /tmp/loki-nemo-relay-docs/workdir

export LOKI_NEMO_RELAY_ATOF_ENABLED=1
export LOKI_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/loki-nemo-relay-docs/parallel/atof
export LOKI_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export LOKI_NEMO_RELAY_ATOF_MODE=overwrite
export LOKI_NEMO_RELAY_ATIF_ENABLED=1
export LOKI_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/loki-nemo-relay-docs/parallel/atif
export LOKI_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export LOKI_NEMO_RELAY_ATIF_AGENT_NAME='Loki Agent E2E'
export LOKI_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Loki sessions map to `agent` scopes.
- Loki API request hooks map to `llm` scope start/end events.
- Loki tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Loki execution middleware to hand
...
```bash
export LOKI_NEMO_RELAY_PLUGINS_TOML=/tmp/loki-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Loki routes execution
...
Loki provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Loki provider adapter next_call(...)

Loki tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Loki tool dispatcher next_call(...)
```
...
middleware for a local Loki run. This path requires a NeMo Relay runtime that
...
```bash
export LOKI_HOME=/tmp/loki-middleware-test/loki-home
mkdir -p "$LOKI_HOME" /tmp/loki-middleware-test/nemo-relay
...
cat > /tmp/loki-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/loki-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/loki-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Loki Middleware E2E"
agent_version = "local"
...
export LOKI_NEMO_RELAY_PLUGINS_TOML=/tmp/loki-middleware-test/nemo-relay/plugins.toml
