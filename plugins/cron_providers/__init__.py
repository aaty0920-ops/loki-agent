...
1. Bundled providers: ``plugins/cron_providers/<name>/`` (shipped with loki-agent)
...
def _register_synthetic_package(name: str, search_locations: List[str]) -> None:
    ...
    User-installed providers import as ``_loki_user_cron.<name>``, a dotted
    ...
    ``ModuleNotFoundError: No module named '_loki_user_cron'`` — the same
...
def load_cron_scheduler(name: str) -> Optional["CronScheduler"]:  # noqa: F821
    ...
    (``$LOKI_HOME/plugins/<name>/``) directories. Bundled takes precedence
    on name collisions.
