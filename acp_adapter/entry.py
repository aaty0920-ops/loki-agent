...
Loads environment variables from ``~/.loki/.env``, configures logging
...
try:
    ...
    # shadowing Loki's own modules — ``loki acp`` can be started from any
...
def _load_env() -> None:
    ...
    if loaded:
        ...
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", loki_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="loki-acp",
        description="Run Loki Agent as an ACP stdio server.",
    )
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``loki postinstall`` and the runtime lazy installer.
