...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Loki itself is running in WSL.

    Windows ACP clients can launch ``loki acp`` inside WSL while still sending
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Loki from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["loki-acp"]):
...
class SessionManager:
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Loki runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.loki/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``LOKI_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_loki_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=loki_home / "state.db")
            return self._db_instance
        except Exception:
    ...
    def _make_agent(
        ...
        *,
        ...
        # editor/session cwd instead of the Loki daemon's process cwd.
