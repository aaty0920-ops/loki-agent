...
from tools.approval import (
    reset_loki_interactive_context,
    set_loki_interactive_context,
)
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Loki running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Loki cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
class LokiACPAgent(acp.Agent):
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Loki maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from loki_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Loki estimates ``used`` from the same
    ...
    def _provenance_meta(
        self,
        acp_session_id: str,
        current_loki_session_id: str,
        previous_loki_session_id: Optional[str] = None,
    ) -> Optional[dict]:
    ...
    async def _send_session_info_update(
        ...
        session_id: str,
        *,
        current_loki_session_id: Optional[str] = None,
        previous_loki_session_id: Optional[str] = None,
    ) -> None:
        """Send ACP native session metadata after Loki changes it.

        When the internal Loki head rotated (e.g. compression-driven session
        split during a turn), pass ``previous_loki_session_id`` so the
        attached ``_meta.loki.sessionProvenance`` flags the rotation reason.
        """
        if not self._conn:
            return
        try:
        ...
        # loki_state.py schema — only started_at/ended_at). Use "now" as
        ...
        meta = self._provenance_meta(
            session_id,
            current_loki_session_id or session_id,
            previous_loki_session_id,
        )
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["loki-acp"],
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Loki' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Loki setup/model selection out-of-band.
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Loki
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        # ACP thought panes should not receive Loki' local kawaii waiting/status
        ...
        # tools.approval (set_loki_interactive_context) rather than
        # os.environ["LOKI_INTERACTIVE"], so concurrent executor workers can't
        ...
        # callback shape — not the gateway-queue LOKI_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind LOKI_SESSION_KEY for this session so per-session caches
            ...
            interactive_token = set_loki_interactive_context(True)
            ...
            previous_session_id = os.environ.get("LOKI_SESSION_ID")
            ...
            try:
                ...
                if interactive_token is not None:
                    reset_loki_interactive_context(interactive_token)
                # Restore LOKI_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("LOKI_SESSION_ID", None)
                else:
                    os.environ["LOKI_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            # Snapshot the internal Loki DB session id before the turn so we
            ...
            # stomp on each other's ContextVar writes (LOKI_SESSION_KEY in
        ...
        # _meta.loki.sessionProvenance so ACP clients can render the boundary
        # and keep old/new ids in lineage. The ACP session_id is unchanged.
        post_turn_loki_id = getattr(state.agent, "session_id", None)
        if (
            conn
            and post_turn_loki_id
            and pre_turn_loki_id
            and post_turn_loki_id != pre_turn_loki_id
        ):
            try:
                await self._send_session_info_update(
                    session_id,
                    current_loki_session_id=post_turn_loki_id,
        ...
        # Loki' local "waiting for model response" interrupt status is metadata,
