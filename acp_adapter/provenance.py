...
This is an additive Loki extension surfaced under ACP ``_meta.loki`` so
...
compression rotates the internal Loki head, ``build_session_provenance`` lets
...
def build_session_provenance(
    ...
    *,
    ...
    Args:
        ...
        current_loki_session_id: The live internal Loki DB session id
            (``state.agent.session_id``).
        previous_loki_session_id: The internal id from before the most recent
            turn, when known. Supplied by ``prompt()`` to flag a rotation.

    Returns:
        A dict suitable for ``{"loki": {"sessionProvenance": <dict>}}`` under
        ACP ``_meta``, or ``None`` if the session can't be read.
    """
    try:
        row = db.get_session(current_loki_session_id)
    except Exception:
        return None
    if not row:
        return None
    ...
    root_id = current_loki_session_id
    compression_depth = 0
    ...
    rotated = bool(
        ...
        and previous_loki_session_id != current_loki_session_id
    )

    provenance: Dict[str, Any] = {
        "acpSessionId": acp_session_id,
        "currentLokiSessionId": current_loki_session_id,
        "rootLokiSessionId": root_id,
        "parentLokiSessionId": parent_id,
...
def session_provenance_meta(
    ...
    *,
    ...
    if prov is None:
        return None
    return {"loki": {"sessionProvenance": prov}}
