...
# Maps ACP permission option ids to Loki approval result strings.
...
def _build_permission_options(*, allow_permanent: bool) -> list[PermissionOption]:
    ...
    options = [
        ...
        PermissionOption(
            ...
            # hint while keeping Loki semantics in the option id.
            kind="allow_always",
...
def _map_outcome_to_loki(outcome: object, *, allowed_option_ids: set[str]) -> str:
    ...
    if option_id not in allowed_option_ids:
        ...
        return "deny"
    return _OPTION_ID_TO_LOKI.get(option_id, "deny")
...
def make_approval_callback(
    ...
    """
    Return a Loki-compatible approval callback that bridges to ACP.
